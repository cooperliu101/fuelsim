...
- 连续 primary 线段采用半开区间确定内部顶点的唯一归属，只有整条 primary
  ...
  不得让陈旧候选伪装成有效投影。
- 已激活机械接触的 secondary 节点若全部滑出候选窗口，必须通过
...
7. `fuelsim_m43_noncoaxial_finite_strain_moose_tests` 必须覆盖畸变四单元上先
   ...
   施加当前构形 pressure 与分量 traction。节点场、等效历史及张量 L2/峰值
   误差均小于 `0.5%`；换向过零处应力、弹性和总非弹性最大逐点相对误差按
   验证矩阵中的显式 `10%/10%/15%` qualified 门槛验收，不得增加分母下限。
   塑性和蠕变累计迹漂移必须分别输出并小于 `2e-6` 和 `2e-8`。
