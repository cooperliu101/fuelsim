...
二维热机械接触支持无摩擦、有限滑移、面到面罚函数及间隙传热。力学使用从侧节点
邻域平均约束、从侧法向场的加权积分及主侧边界附近的平滑力分配；节点力必须
保留形函数与法向的乘积积分，不能提前提出平均法向。每个二次边采用三个
节点面积区间上的 3、5、3 个等权采样点，共用从侧节点的相邻边共同构成一个约束。
热接触按主侧单元边界的投影划分积分区间，每段使用三个积分点。平均权重、法向、
投影位置、平滑范围以及热积分区间的几何导数必须进入切线，并预留全部候选耦合块。
有效热接触主侧投影区间必须连续且互不重叠，
端部按真实重叠范围截断；全部失去投影或无法唯一划分时拒绝当前状态，不得重复积分。
对应实现和验证边界见 `docs/generalized-plane-strain.md`，不得把局部切线通过
表述为全部非匹配曲边或非弹性生产例题已通过 Abaqus 对比；具体通过范围及未通过项
见 `verification/abaqus/cpeg8t/coverage.md`。
...
- 不定义项目自己的 C++ 类模板、表达式模板或标量泛型层。函数模板仅允许以下私有数学实现：
  ...
  `central_increment_gradient_impl`、`reduced_hex8_metric_impl`、`hex8_hourglass_shape_impl`、
  `reduced_hex8_thermal_hourglass_coefficients_impl`；
  `elements/src/material.cpp` 中的 `rotate_cartesian_tensor_impl`；
  `elements/src/quad8_face.cpp` 中的 `quad8_shape_impl`，仅计算固定八节点形函数及其一、二阶导数。
  标量类型仅允许 `double` 和 `adlite::Scalar`；矩阵运算固定为 3×3，
  HEX8 沙漏投影固定为八节点、四模态；张量旋转固定为六分量三维对称张量，不得扩展为通用矩阵库、材料模型模板、
...
### 当前密度与质量规则（2026-09-13 用户指定）

以下规则取代本文件下文各型号中原有的当前密度热容和当前体积热容描述；
其余导热、体热源、材料和力学构形规则继续适用。
输入 `density` 是初始温度对应的初始密度，不接受温度密度斜率。
热容使用初始密度乘参考体积的固定初始质量，比热使用当前温度，保留比热
温度导数。各型号保留原有插值、积分和节点集总方式，质量测度一律采用
参考构形，热容对位移的导数为零。有限应变当前密度为 `rho_initial/J`。
区域 `body_acceleration` 接受固定全局加速度，三维为 x/y/z，轴对称为径向/轴向，
单位 m/s²；重力残量使用位移形函数和当前密度，对当前体积积分。
装配采用严格等价的参考初始质量形式，完整利用密度与体积几何导数相消，
不修改应力内力，不增加位移惯性。检查点版本 22 拒绝旧版本，模型签名包含
区域加速度。详细规则及例题见 `docs/density-mass-and-gravity.md`。
Abaqus 2025 原生热容探测仅作为独立比较证据，不改变上述质量契约。不得引入
其整体与局部体积比混合权重、当前温度密度或可切换的 Abaqus 热容模式来追平
参考结果。已经证实的离散差异应单独记录，原应力、接触及热反力门槛继续保留；
没有通过的比较不得标记为通过。
用户随后明确授权放宽八项例题的容差；仅这些例题采用
`verification/abaqus/2025/tolerance_qualification.md` 中的分项门槛，并标记为
`qualified`（按专门约定的容差通过）。其余例题、零参考量和原始误差输出不变。
...
- `axisymmetric_1d` 使用真实 BAR2 径向网格与 `cax2t_gps`，局部自由度为
  ...
  `2*pi*R*H*dR` 参考测度，热算子与矩形 CAX4T 在相同广义平面应变约束下对应。
  热膨胀使用每个 BAR2 单元两个径向节点温度的算术平均，不跨径向单元或切片平均；
  弹性模量、泊松比及塑性、蠕变参数使用与两个径向积分点分别配对的端节点温度，
  材料空间位置仍为积分点，材料温度导数只挂回对应端节点。导热系数也采用
  配对端节点温度；温度梯度仍由形函数梯度形成。热容采用 `integral(N_i*dV)`
  节点对角权重，密度、比热与温度变化率在节点求值，保留对应节点温度导数。
  平均温度对两个温度自由度的导数各为 1/2，必须进入闭式切线；有限应变直接
  用已接受的平均热应变恢复总应变，继续保留新旧平均热应变之差。
  小应变径向、轴向应变直接计算，环向应变按参考体积平均，为
  `(ur0+ur1)/(R0+R1)`；有限应变先平均环向伸长比，再形成新旧平均伸长比的
  Hughes-Winget 中间构形增量，不得平均逐点增量。力学虚环向梯度为
  `1/((R0+R1)*mean_F_hoop_current)`；力学材料点测度为参考测度乘整体当前/参考
  体积比。有限应变导热使用增量中间构形梯度及参考权重乘整体当前/参考体积比；
  热容和体热源使用逐点真实当前测度。热算子的几何导数必须完整进入切线。
  截面轴向力按相同力学测度除当前高度
  积分，必须与端截面体内力一致。应力、虚应变及两套测度的闭式导数必须一致。
...
- ADlite 只按体单元或界面局部量播种，禁止按全局自由度播种。RZ 运动学链按
  4 个面内位移梯度分量、积分点径向
  位移和材料温度（宽度 6）播种，材料温度按各单元型号的规则取值；本构关系按 4 个应变分量加温度（宽度 5）
...
- 每个区域必须显式设置 `element`；二维轴对称只接受 `cax4t|cax4rt|cax8t|cax8rt`，
  ...
  热膨胀使用四个角点温度的算术平均值；弹性模量、泊松比以及塑性、蠕变参数
  使用与积分点配对的参考角点温度。Fuelsim 四个积分点与局部四角点顺序一致，
  材料温度导数只挂回对应角点，不再乘积分点温度形函数；有限应变的旧材料温度
  使用同一角点的已接受值。原生证据见
  `verification/abaqus/cax4t_material_temperature_probe/README.md`。
...
- C3D8RT 力学沙漏控制固定使用 Abaqus 默认总刚度系数 `0.005` 乘初始温度剪切
  ...
  Jacobian 为正。
- C3D8RT 有限应变体内力采用不同的偏应力和平均正应力虚梯度。令参考平均梯度
  为 `G0`，`Fbar=I+u^T*G0`，`Gp=G0*inverse(Fbar)`，当前平均梯度为 `Gc`，
  平均正应力为 `p=trace(stress)/3`，则节点体内力为
  `V_current*(Gp*dev(stress)^T+Gc*p)`。`Fbar` 必须具有有限正行列式；残量和
  闭式切线必须同时保留两种梯度、当前体积及应力的导数，不推广到其他型号。
  原生鉴定见 `verification/abaqus/b544_diagnosis/analyze_bulk_split.py`。
- C3D8RT 有限应变的材料增量由 `Lp=delta_u^T*G0*inverse(Fbar_mid)` 的偏量与
  真实中间构形 `Lm=delta_u^T*Gmid` 的迹组成，旋转由 `Lp` 的反对称部分计算。
  新旧热膨胀温度均采用同一组当前体积权重，旧温度因此也有位移导数，必须经
  本征应变温度导数与材料切线回接。独立原生证据限常数各向同性膨胀系数。
  热沙漏系数采用中心梯度和中心 `8*detJ`；均匀导热项及沙漏模态仍采用真实
  体积平均梯度，节点热容仍采用当前 `integral(N_i*dV)`。完整几何导数必须保留。
...
- 小应变三维 HEX20 热容使用参考构形一致矩阵，有限应变使用规定的当前二次
  几何一致矩阵。C3D8T 的热容在节点求密度、比热及温度变化率，节点权重来自
  配对 Gauss 点，不能使用角点 Jacobian。小应变使用参考权重；有限应变使用
  `reference_weight_q*V_current/V_reference`，其中当前总体积来自真实八点几何
  积分。有限应变导热也采用此整体体积比权重及当前梯度；整体体积的跨点导数
  必须进入热容和导热 Jacobian。体热源仍使用逐点当前几何测度。
  见 `verification/abaqus/b523_diagnosis/` 的非仿射探测。热容温度块保持节点对角，
  其位移块一般非零，不能把整个单元 Jacobian 称为对角矩阵。体单元均无位移惯性。
...
- `fuelsim_elements`：位于 `elements/`，独立配置、构建和测试的局部单元计算库；
  ...
  跨拓扑的接触返回映射、投影收敛判据和间隙导热律集中在私有 `src/contact_common.*`，
  使用具体类型的普通函数；曲面几何、历史转运和装配仍按表面拓扑分别实现。
...
用户要求所有椭圆度相关算例、输入、计算结果及图件仅保存在本地，不得上传 GitHub。
相关资料统一放在 `verification/abaqus/oval_open_tube/`，该目录不纳入 Git 跟踪。
不得使用强制添加或更换目录的方式绕过忽略规则，也不得再次推送含这些资料的旧提交。
本地恢复备份放在 `.local-backups/`，同样不得提交或上传。
