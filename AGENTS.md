...
- `cax8t` 和 `cax8rt` 必须使用 QUAD8 网格，局部自由度为
  ...
  NaN。二次边机械接触、热接触、摩擦和恢复节点接触应力与 CAX8T 共用实现，
  但以 B12.0—B12.14 的独立 Abaqus CAX8RT 参考按小于 0.1% 验收。
...
- 当启用 Coulomb 摩擦时，粘着切向牵引使用与法向相同的接触罚刚度；滑移
  ...
  的逐位结果。
- 轴对称机械接触允许显式 `sliding = finite`，沿用当前构形唯一节点到线段投影。
  显式正值 `slip_tolerance` 启用 Abaqus 弹性滑移规则：最大弹性滑移为该值乘
  secondary 参考边长的算术平均值，粘着刚度为 `mu*p/maximum_elastic_slip`，
  压力导数必须保留。省略或取零时保留上一条的法向罚刚度规则。
  有符号累计切向滑移独立于弹性滑移保存，只在闭合接触下累加相对运动，
  并进入状态提交、回滚和版本 18 检查点。累计滑移是输出诊断，不参与后续接触力
  或时间误差控制；时间步加倍继续检查影响摩擦响应的弹性滑移与粘滑状态。
