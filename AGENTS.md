...
- C3D8RT 力学沙漏控制固定使用 Abaqus 默认总刚度系数 `0.005` 乘初始温度剪切
  ...
  完整 32×32 Jacobian 在上述窄链组合后分别用 24 个位移自由度块和 8 个温度
  自由度块播种。位移块完整计算一次残量并同时提供残量值；温度块复用已计算的
  被动当前构形和中间构形，只计算热传导、热容和应力温度导数，不重复计算温度
  导数恒为零的体热源与机械沙漏力。禁止改为 32 自由度恒等播种或按 32 列重复
  完整残量。有限应变还必须分别保持 committed、midpoint 和 current 构形
  Jacobian 为正。
