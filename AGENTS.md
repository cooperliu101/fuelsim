...
- 所有体单元的热容使用固定初始质量。密度函数在区域初始温度、物理时间零和
  参考位置求值，当前密度的物理含义为 `rho=rho_initial/J_geo`，其中 `J_geo`
  为真实几何当前/参考体积比，不能用力学选择性体积处理后的量替代。
  装配直接使用 `rho_initial*dV_reference`，不重复乘当前体积或热膨胀密度修正。
  保留各型号原有积分点、集中或一致矩阵规则；比热在当前温度和当前时间求值，
  保留温度导数。热容对位移的导数严格为零；导热和体热源仍遵循各型号构形规则。
  初始质量不随已接受温度或重启温度重新定义；检查点格式为 20，拒绝旧物理语义。
  Abaqus 2018 原生热容参考采用不同密度规则，相关热容比较必须明确区分原生结果
  和独立质量守恒参考，不能把转换结果称为原生 Abaqus 解。
...
- `axisymmetric_1d` 使用真实 BAR2 径向网格与 `cax2t_gps`，局部自由度为
  ...
  配对端节点温度；温度梯度仍由形函数梯度形成。热容采用 `integral(N_i*dV_reference)`
  节点对角权重，初始密度和当前比热、温度变化率在节点求值，保留比热温度导数。
  ...
  热容使用参考质量，体热源使用逐点真实当前测度。导热和源项的几何导数必须完整进入切线。
  截面轴向力按相同力学测度除当前高度
...
- 每个区域必须显式设置 `element`；二维轴对称只接受 `cax4t|cax4rt|cax8t|cax8rt`，
  ...
  必须保留该角点温度的独立导数。热容采用 `integral(N_i*dV_reference)` 节点对角权重，
  初始密度、当前比热及温度变化率在节点求值。小应变热算子使用参考构形；有限应变
  热传导使用增量中间构形梯度，积分权重为参考积分权重乘单元整体当前/参考
  体积比，热容使用参考质量，体热源使用逐积分点当前构形测度。几何导数和整体体积的跨点
  导数必须进入 Jacobian。导热及源项规则由 B15.0/B15.1 独立热算子探测鉴定；热容采用前述质量守恒规则。
...
- `cax8t` 和 `cax8rt` 必须使用 QUAD8 网格，局部自由度为
  ...
  梯度和当前二次几何测度，热容使用参考二次几何和初始密度的一致矩阵，体热源使用温度
...
- 三维 HEX8 选择 `element = c3d8rt` 时采用 Abaqus C3D8RT 一点减缩积分规则，
  ...
  热容在两种应变下均使用参考构形权重和初始密度；导热与源项在有限应变使用当前
  构形的体积及平均梯度，保留相应热残量对位移的几何 Jacobian。
- C3D8RT 力学沙漏控制固定使用 Abaqus 默认总刚度系数 `0.005` 乘初始温度剪切
  ...
  当前构形和中间构形的体积、平均梯度及热沙漏量对 24 个位移
...
- `cax4t` 使用前述经原生识别的热算子规则。
  ...
  使用完整二次位移几何的当前构形；一致热容矩阵使用完整二次参考几何和初始密度；
  体热源使用八个温度角点形成的线性当前构形测度，不受位移中间节点影响。上述
  有限应变体热残量必须保留对位移的几何 Jacobian。三维 HEX8 小应变的热传导、
  体热源、表面热流和对流使用参考
  构形；有限应变的上述四类热算子使用当前构形；角点热容始终使用参考质量。三维 HEX8 有限应变热残量
  必须保留对位移的几何 Jacobian。
...
- 三维 HEX20 热容使用参考构形和初始密度的一致矩阵，保留原积分阶数。
  三维 C3D8T 保留已有配对 Gauss 点权重的节点对角规则，第 `i` 个节点热容残量为
  `w_i_reference*rho_initial_i*cp(T_i)*(T_i_new-T_i_old)/dt`，不是角点 Jacobian 权重。
  热容的温度—温度块按节点对角集总，热容的位移块为零；导热及体源仍可贡献非零
  位移块，因此不能把整个 32×32 Jacobian 称为对角矩阵。体单元均不包含位移惯性。
