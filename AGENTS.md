...
完成用户要求的修改和相应验证后，先创建本地 Git 提交，再推送到当前分支对应的
远端分支；除非用户明确要求仅保留本地修改，否则不再重复询问是否提交或推送。
只提交本次任务相关文件，保留其他任务的已有修改，不提交构建目录、结果或被忽略的
本地资料。推送使用普通推送，不得强制覆盖远端历史；若推送失败，应如实报告原因。
...
热接触使用温度角点邻域平均，每条边的两个半区分别归属两个角点，中点采样权重
各分一半；相邻边共用温度角点的贡献先合并。温度仍按角点线性插值，几何使用
二次边；采用从侧法向投影及上述 3、5、3 采样，不采用主侧各自的端点法线划分。
邻域平均间隙和两侧平均温度进入传热定律，平均温差形成热流。平均权重、法向、
投影位置、平滑范围的几何导数必须进入切线，并预留全部候选耦合块。
主侧共用端点在同一从侧切线坐标中形成统一分界；候选平滑权重总和不得超过一，
全部失去投影时拒绝当前状态，不得重复积分。
...
Abaqus 参考重算同样最多占用 4 核，可并行运行四个单核原生任务。
...
`physics = thermal` 在固定几何上只建立温度方程。区域显式选择 DCAX4、DCAX8、
DC3D8 或 DC3D20；二次型号全部节点都携带温度。一阶型号采用 Abaqus 默认节点集总
热容，二阶型号采用一致热容矩阵；均使用初始密度与参考体积的固定初始质量
及当前温度比热。该规则只适用于独立纯热
型号，不改变已有热力耦合单元的离散。纯热模式不分配机械历史，也不要求机械
残量尺度。具体实现范围及原生对比边界见 `docs/thermal.md`。端到端输入卡仍须完整显式提交。

DCAX4 纯热导热系数在与高斯点配对的角点温度求值，导数只连接到该角点；
温度梯度仍由全部节点形成。DC3D8、DCAX8、DC3D20 使用积分点插值温度求
导热系数。四种型号的温度相关材料瞬态以及二阶曲边畸变网格的验证范围见
`verification/thermal/extension.md`。该规则不改变现有热力耦合型号。

纯热 DC3D20 界面采用八节点正权重邻域平均完整二次温度与间隙，使用
`quad8_face` 中已有的 QUAD8 采样规则；跨主侧面时合并共享节点贡献，保持
两侧离散热流守恒及完整温度切线。其余纯热界面保持原积分规则。外部验收的
具体几何与边界见 `verification/thermal/contact.md`，不得推广为任意非匹配曲面。
