...
新增的[实体端部与模态中段试验](../../docs/section-solid-ends.md)采用真实 C3D20T
端部、12 个中段模态和三点六个标量约束。输入为 `*_hybrid_12.fsi`，三维参考为
`*_support_solid.fsi`。端部位移在局部消元，全部端部积分点仍参与误差比较。
这些三点支撑结果与下文整面固定根部的原有例题分别保存。
...
研究输入可以在 `[SectionModes]` 中设置 `width_lines = width_0 width_2 width_4`，
以 Exodus 节点集确定两个相连的宽度分区；五个选择器对应四个分区。这时 `count`
表示初始种子模式数，要求至少为 10，实际全长方向数由独立分区空间决定，并明确
记录在 `mode_count` 中。本轮与端部凝聚互斥。分区使用原截面共享节点保持连续，
线间耦合来自完整三维应变能。研究输入按用户选择，改用三点六个标量位移约束，
允许其余根部节点自由变形，并重算相同支撑下的三维参考。该方案的结果与下文
整端夹紧的既有验证分别记录；输入、试验方法及结论见
[多条轴向线试验](../../docs/section-width-study.md)。
