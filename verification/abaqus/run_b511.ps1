...
$JobName = "b511_hex8_c3d8t_small_norton"
...
$RunDirectory = Join-Path $env:TEMP ("fuelsim_b511_" + (Get-Date -Format "yyyyMMdd_HHmmss"))

New-Item -ItemType Directory -Force -Path $RunDirectory | Out-Null
Push-Location $RunDirectory
try {
    & abaqus job=$JobName input=$InputFile output_precision=full interactive
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.11 probe failed with exit code $LASTEXITCODE" }
    ...
    if (!(Test-Path $StatusFile) -or !(Select-String -Quiet -Path $StatusFile -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY")) {
        throw "Abaqus B5.11 probe did not complete successfully; inspect $RunDirectory"
    }
    & abaqus python $Extractor "$JobName.odb" $NodalOutput $IntegrationOutput $EnergyOutput
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.11 extraction failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
}
