...
$JobName = "b547_m58_c3d8rt_integrated"
$Work = Join-Path $env:TEMP ("fuelsim_b547_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
if ($SolveExitCode -ne 0) {
    throw "Abaqus B5.47 solve failed with exit code $SolveExitCode"
}
if (!(Test-Path "$JobName.sta") -or
    !(Select-String -Path "$JobName.sta" -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY" -Quiet)) {
    throw "Abaqus B5.47 did not report successful completion"
}
...
if ($IncrementSummary.Count -ne 21) {
    throw "Abaqus B5.47 did not produce twenty increment summary rows"
}
...
$Outputs = @(
    ...
    "${JobName}_clad_state.csv",
    "${JobName}_energy.csv"
)
& "C:\SIMULIA\Commands\abaqus.bat" `
    python extract_b547.py `
    "$JobName.odb" `
    "${JobName}_mesh.json" `
    $Outputs[0] `
    $Outputs[1] `
...
if ($LASTEXITCODE -ne 0) {
    throw "Abaqus B5.47 extraction failed with exit code $LASTEXITCODE"
}
foreach ($Output in $Outputs + @("${JobName}_timing.txt", "${JobName}_increments.tsv")) {
    if (!(Test-Path $Output)) {
        throw "Abaqus B5.47 did not create $Output"
    }
    Copy-Item $Output $SourceDirectory
}
Write-Output "Abaqus B5.47 wall time: $($Stopwatch.Elapsed.TotalSeconds) seconds"
Write-Output "Abaqus B5.47 work directory: $Work"
