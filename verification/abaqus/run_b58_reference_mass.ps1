...
try {
    ...
    & C:\SIMULIA\Commands\abaqus.bat job=$JobName input="$JobName.inp" cpus=1 output_precision=full ask_delete=OFF interactive *> "${JobName}_run.log"
    ...
    $ErrorActionPreference = "Continue"
    & C:\SIMULIA\Commands\abaqus.bat python extract_b58.py "$JobName.odb" "${JobName}_nodal.csv" "${JobName}_integration.csv" *> "${JobName}_extract.log"
