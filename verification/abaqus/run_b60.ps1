...
$JobName = "b60_fuel_plate_c3d8rt_bending"
...
& "C:\SIMULIA\Commands\abaqus.bat" `
    job=$JobName `
    input="b60_fuel_plate_c3d8rt_bending.inp" `
    cpus=1 `
...
if ($SolveExitCode -ne 0) {
    throw "Abaqus B6.0 solve failed with exit code $SolveExitCode"
}
if (!(Test-Path "$JobName.sta") -or
    !(Select-String -Path "$JobName.sta" -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY" -Quiet)) {
    throw "Abaqus B6.0 did not report successful completion"
}
...
foreach ($Output in @("${JobName}_nodal.csv", "${JobName}_timing.txt")) {
    if (!(Test-Path $Output)) { throw "Abaqus B6.0 did not create $Output" }
    Copy-Item $Output $SourceDirectory
}
Write-Output "Abaqus B6.0 wall time: $($Stopwatch.Elapsed.TotalSeconds) seconds"
Write-Output "Abaqus B6.0 work directory: $Work"
