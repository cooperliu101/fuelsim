...
pointwise error, while one low-force mechanical reaction component uses the
explicitly qualified `4%` pointwise gate and reaches `3.02624%`. The total
boundary heat-rate error is at most `0.00833932%`; nodal reaction heat flux is
reported but not gated because the two discretizations distribute that same
rate differently over constrained nodes on the distorted boundary. Abaqus
artificial energy reaches `8.85354%` of internal energy, so B5.44 remains a
qualified fixed-path result.
