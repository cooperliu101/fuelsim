...
十个原生任务分别探测小应变和有限应变下的弹性模量、泊松比、初始屈服应力、
...
former test replayed the Fuelsim finite-sliding local update in an always-sticking
diagnostic, whose three errors were `0.389820%`, `0.386772%`, and `0.400004%`.
That artificial high-friction replay is no longer part of this frictionless
production-input acceptance. Independent B4.2 and HEX8 local tests retain the
objective friction-history checks. All four constraints remain
...
The complete tracked production input is run directly; it is not generated by a test:

```bash
build/fuelsim -i verification/fuelsim/transient_hex8_finite_strain_contact_abaqus.fsi
```
...
The input deck, Exodus mesh, and complete Fuelsim cards are tracked explicitly.
The CTest runs the production executable for the full path and the two
checkpoint/restart cards, compares output with Abaqus, then runs the retained
local derivative and exact-history checks without another solve:

```bash
ctest --test-dir build -j4 --output-on-failure -R b48_hex8_multi_contact
```
...
B5.44 uses eight connected distorted C3D8RT elements with prescribed temperature,
body heat, surface flux, convection and bending traction. The active native input
uses the constant initial density 2000 kg/m³. The 2026-09-13 investigation identified
different deviatoric and volumetric virtual gradients in the native bulk force.
The production residual and its complete displacement/temperature derivative now
implement that rule. The initial-mass capacity experiment was rolled back, and
full native field comparisons have been restored. Current results and the exact
reference scope are recorded in [B5.44 validation](b544_initial_density_validation.md).

The former temperature-dependent density input and its four original CSV
files are preserved byte for byte in
[the historical density-temperature archive](b544_temperature_density_history/).
All numerical results in the remainder of this B5.44 paragraph refer to that
archived input and reference, not to the new initial-density run.
For the archived comparison, all ordinary bulk, energy,
total-boundary-heat-rate, and hourglass-energy metrics pass one percent.
Displacement reaches `0.698823%` maximum
...
so that archived B5.44 comparison remains a qualified fixed-path result.
