param(
    ...
    [string[]]$JobNames = @("b544_disconnected_weak", "b544_disconnected_weak_isothermal")
)
...
$Work = Join-Path $env:TEMP ("fuelsim_b544_disconnected_" + [guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $Work | Out-Null
foreach ($JobName in $JobNames) {
    Copy-Item (Join-Path $SourceDirectory "$JobName.inp") $Work
}
Copy-Item (Join-Path $SourceDirectory "extract_disconnected.py") $Work
Push-Location $Work
try {
    foreach ($JobName in $JobNames) {
        ...
        & "C:\SIMULIA\Commands\abaqus.bat" python extract_disconnected.py "$JobName.odb" `
        ...
        foreach ($Log in @("${JobName}_run.log", "${JobName}_extract.log")) {
            if (Test-Path $Log) { Copy-Item $Log $SourceDirectory }
        }
        @("case=$JobName", "scope=64 disconnected prescribed histories; weak hourglass; no external loads",
            ...
            "extractor_sha256=$((Get-FileHash 'extract_disconnected.py' -Algorithm SHA256).Hash)") +
