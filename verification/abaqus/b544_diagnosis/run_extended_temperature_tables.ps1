param(
    ...
    [string]$JobName = "b544_extended_temperature_tables"
)
...
$Work = Join-Path $env:TEMP ("fuelsim_b544_tables_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    @("case=$JobName", "scope=extend the original E/k/cp lines to 280 and 800 K; diagnostic only",
        "density_input=2000 kg/m3; no temperature table", "abaqus_version=3DEXPERIENCE R2018x",
        "cpus=1", "output_precision=full", "completed_utc=$([DateTime]::UtcNow.ToString('o'))",
        "work_directory=$Work", "input_sha256=$((Get-FileHash "$JobName.inp" -Algorithm SHA256).Hash)",
