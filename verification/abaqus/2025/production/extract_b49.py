...
if len(sys.argv) != 4:
    raise RuntimeError("usage: extract_b49.py <job.odb> <nodal.csv> <integration_points.csv>")
...
def integration_field(frame, prefix):
    ...
    if len(result) != 8:
        raise RuntimeError("%s has %d integration-point values, expected 8" % (prefix, len(result)))
    return result
