...
if len(sys.argv) != 6:
    raise RuntimeError(
        "usage: extract_b549.py <job.odb> <mesh.json> <temperature.csv> <displacement.csv> <material.csv>"
    )
...
if len(all_labels) != 112 or len(corner_labels) != 40 or len(all_elements) != 8 or len(secondary_contact_labels) != 8:
    raise RuntimeError("unexpected B5.49 manifest topology")

odb = openOdb(path=sys.argv[1], readOnly=True)
try:
    frame = odb.steps["PATH"].frames[-1]
    print("B5.49 final frame time=%.17g" % frame.frameValue)
    if abs(frame.frameValue - 0.4) > 1.0e-6:
        raise RuntimeError("B5.49 final frame is not at 0.4 seconds")
    ...
    print(
        "B5.49 integration rows S=%d PEEQ=%d CEEQ=%d COORD=%d IVOL=%d sample=%s"
        ...
        for label in sorted(all_elements):
            for point in range(1, 28):
                ...
                if key not in stress or key not in point_coordinates or key not in volume:
                    raise RuntimeError("missing B5.49 integration value for %s" % (key,))
