...
if len(sys.argv) != 4:
    raise RuntimeError("usage: extract_b534.py <job.odb> <nodal.csv> <integration.csv>")
...
def field(frame, prefix):
    ...
    if len(matches) != 1:
        raise RuntimeError("expected one %s field, got %s" % (prefix, sorted(frame.fieldOutputs.keys())))
...
try:
    ...
    temperature = {value.nodeLabel: data(value) for value in field(frame, "NT").values}
    displacement = {value.nodeLabel: data(value) for value in field(frame, "U").values}
    reaction_flux = {value.nodeLabel: data(value) for value in field(frame, "RFL").values}
    reaction_force = {value.nodeLabel: data(value) for value in field(frame, "RF").values}
    ...
    for node in range(1, 13):
        nodal.write(
            ...
            % ((node, temperature[node]) + tuple(displacement[node]) + (reaction_flux[node],) + tuple(reaction_force[node]))
        )
    nodal.close()

    fields = {}
    for name in ("COORD", "HFL", "IVOL", "LE", "S", "TEMP"):
        fields[name] = {
    ...
    for element in range(1, 3):
        ...
        integration.write(
            ...
            % (key + tuple(fields["COORD"][key]) + (fields["TEMP"][key],) + tuple(fields["HFL"][key])
               + tuple(fields["S"][key]) + tuple(fields["LE"][key]) + (fields["IVOL"][key],))
        )
    integration.close()
finally:
    odb.close()
