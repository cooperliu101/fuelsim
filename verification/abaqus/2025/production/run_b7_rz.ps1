...
foreach ($Case in ($Cases -join ',').Split(',')) {
    ...
    $Arguments = '/d /c start "" /b /wait /affinity 1 "' + $env:ComSpec +
        ...
        " job=$Case input=$Case.inp cpus=1 output_precision=full ask_delete=OFF interactive"
    ...
    Push-Location $Work
    try {
        & C:\SIMULIA\Commands\abq2025.bat python extract_b7_rz.py "$Case.odb" $Case
    ...
    @("case=$Case", "external_seconds=$($Watch.Elapsed.TotalSeconds)", "cpus=1", "processor_affinity=1",
