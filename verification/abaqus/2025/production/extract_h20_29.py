...
if len(sys.argv) != 8:
    raise RuntimeError(
        "usage: extract_h20_29.py <job.odb> <job.inp> <normal_x> <normal_y> "
...
if sorted(coordinates_by_label.keys()) != sorted(node_by_label.keys()):
    raise RuntimeError("H20.29 input-deck and ODB node labels differ")
...
if len(displacements) != len(nodes):
    raise RuntimeError("H20.29 displacement output does not cover every node")
...
if sorted(normal_forces.keys()) != secondary_labels:
    raise RuntimeError("H20.29 CNORMF output does not cover every secondary contact node")

output = open(sys.argv[6], "w", newline="")
...
output = open(sys.argv[7], "w", newline="")
