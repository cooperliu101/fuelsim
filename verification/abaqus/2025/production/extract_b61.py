...
if len(sys.argv) != 7:
    raise RuntimeError(
        "usage: extract_b61.py <job.odb> <mesh.json> <nodal.csv> <integration.csv> <contact.csv> <energy.csv>"
    )
...
source_element_by_label = {}
source_element = 1
for block in mesh["blocks"]:
    for element in block["elements"]:
        label = element["label"]
        if label in source_element_by_label:
            raise RuntimeError("duplicate Abaqus element label %d in B6.1 mesh manifest" % label)
        source_element_by_label[label] = source_element
        source_element += 1
if source_element != 1201:
    raise RuntimeError("B6.1 mesh manifest does not contain 1200 elements")

odb = openOdb(path=sys.argv[1], readOnly=True)
try:
    nodal = open(sys.argv[3], "w", newline="")
    integration = open(sys.argv[4], "w", newline="")
    contact = open(sys.argv[5], "w", newline="")
    energy = open(sys.argv[6], "w", newline="")
    ...
    for increment in range(1, len(step.frames)):
        ...
        for element in range(1, 1201):
            key = (element, 1)
            integration.write(
                ("%d,%.16g,%d,1," + ",".join(["%.16g"] * 40) + "\n")
                % (
                    (increment, frame.frameValue, source_element_by_label[element])
                    ...
                    + (fields["PEEQ"][key],)
                    + tuple(fields["CE"][key])
                    + (fields["CEEQ"][key], fields["IVOL"][key])
                )
            )
        energy.write(
