...
if len(sys.argv) != 7:
    raise RuntimeError(
        "usage: extract_b555.py <job.odb> <mesh.json> <temperature.csv> <displacement.csv> <material.csv> <contact.csv>"
    )
...
def boundary_labels(boundary_faces):
    result = set()
    for face in boundary_faces:
        nodes = element_records[face["element"]]["nodes"]
        result.update(nodes[index] for index in face_nodes[face["exodus_side"] - 1])
    return result


secondary_contact_labels = boundary_labels(secondary_faces)
primary_fixed_labels = boundary_labels(
    next(side_set["faces"] for side_set in mesh["side_sets"] if side_set["name"] == "PRIMARY_OUTER")
)
if (
    len(all_labels) != 112
    or len(corner_labels) != 40
    or len(all_elements) != 8
    or len(secondary_contact_labels) != 8
    or len(primary_fixed_labels) != 8
):
    raise RuntimeError("unexpected B5.55 manifest topology")

odb = openOdb(path=sys.argv[1], readOnly=True)
try:
    frame = odb.steps["PATH"].frames[-1]
    print("B5.55 final frame time=%.17g" % frame.frameValue)
    if abs(frame.frameValue - 0.4) > 1.0e-6:
        raise RuntimeError("B5.55 final frame is not at 0.4 seconds")
    temperature = nodal_values(exact_field(frame, "NT11"), corner_labels)
    displacement = nodal_values(exact_field(frame, "U"), all_labels)
    # The input prescribes all three components on PRIMARY_OUTER exactly to zero.
    # Record that mathematical boundary value instead of Abaqus solver noise of
    # order 1e-42; no magnitude-based cleanup is applied to unconstrained data.
    for label in primary_fixed_labels:
        displacement[label] = (0.0, 0.0, 0.0)
    ...
    print(
        "B5.55 integration rows S=%d PEEQ=%d CEEQ=%d COORD=%d IVOL=%d sample=%s"
        ...
        for label in sorted(all_elements):
            for point in range(1, 28):
                ...
                if key not in stress or key not in point_coordinates or key not in volume:
                    raise RuntimeError("missing B5.55 integration value for %s" % (key,))
    ...
    slip_1 = nodal_values(contact_field(frame, "CSLIP1"), secondary_contact_labels)
    slip_2 = nodal_values(contact_field(frame, "CSLIP2"), secondary_contact_labels)
    tangent_1 = nodal_values(contact_field(frame, "CTANDIR1"), secondary_contact_labels)
    tangent_2 = nodal_values(contact_field(frame, "CTANDIR2"), secondary_contact_labels)
    with open(sys.argv[6], "w", newline="") as output:
        output.write("id,x,y,z,gap,pressure,normal_x,normal_y,normal_z,shear_x,shear_y,shear_z,slip_1,slip_2,tangent_1_x,tangent_1_y,tangent_1_z,tangent_2_x,tangent_2_y,tangent_2_z\n")
        for label in sorted(secondary_contact_labels):
            output.write("%d,%.17g,%.17g,%.17g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g\n" % ((label,) + tuple(coordinates[label]) + (gap[label], pressure[label]) + tuple(normal[label]) + tuple(shear[label]) + (slip_1[label], slip_2[label]) + tuple(tangent_1[label]) + tuple(tangent_2[label])))
finally:
    odb.close()
