...
if len(sys.argv) != 3:
    raise RuntimeError("usage: extract_h20_39.py <job.odb> <result.csv>")
...
first_frame = odb.steps[step_names[0]].frames[-1]
secondary_labels = sorted(
    value.nodeLabel
    for value in field_with_prefix(first_frame, "CNORMF").values
    if value.nodeLabel in secondary_all
)
if len(secondary_labels) != 8:
    raise RuntimeError(
        "CNORMF identifies %d secondary contact nodes, expected 8"
        % len(secondary_labels)
    )
node_by_label = dict((node.label, node) for node in instance.nodes)
output = open(sys.argv[2], "w", newline="")
output.write(
    "step,id,x,y,z,normal_x,normal_y,normal_z,tangential_x,tangential_y,tangential_z,gap,pressure\n"
)
...
for step_index, step_name in enumerate(step_names, 1):
    ...
    for label in secondary_labels:
        ...
        output.write(
            "%d,%d,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g\n"
            % (
                step_index,
                label - 1,
