...
$JobName = "b513_hex8_c3d8t_small_noncoaxial"
...
$RunDirectory = Join-Path $env:TEMP ("fuelsim_b513_" + (Get-Date -Format "yyyyMMdd_HHmmss"))

New-Item -ItemType Directory -Force -Path $RunDirectory | Out-Null
Push-Location $RunDirectory
try {
    Invoke-Abaqus job=$JobName input=$InputFile output_precision=full cpus=1 interactive
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.13 probe failed with exit code $LASTEXITCODE" }
    ...
    if (!(Test-Path $StatusFile) -or !(Select-String -Quiet -Path $StatusFile -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY")) {
        throw "Abaqus B5.13 probe did not complete successfully; inspect $RunDirectory"
    }
    Invoke-Abaqus python $Extractor "$JobName.odb" $NodalOutput $IntegrationOutput $EnergyOutput
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.13 extraction failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
}
