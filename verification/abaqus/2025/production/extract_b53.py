...
if len(sys.argv) != 5:
    raise RuntimeError("usage: extract_b53.py <job.odb> <nodal.csv> <integration.csv> <surface_geometry.csv>")
...
def integration_field(frame, name, element):
    ...
    if len(result) != 8:
        raise RuntimeError("%s has %d values for element %d, expected 8" % (name, len(result), element))
    return result
...
try:
    ...
    output.close()
    points = open(sys.argv[3], "w", newline="")
