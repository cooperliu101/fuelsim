...
foreach ($Case in ($Cases -join ',').Split(',')) {
    if ($Case -notmatch '^b12[0-9]+_cax8rt_[a-z_]+$') { throw "Unexpected case: $Case" }
    $Work = Join-Path $env:TEMP ("fuelsim_cax8rt_" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $Work | Out-Null
    Copy-Item (Join-Path $PSScriptRoot "$Case.inp") $Work
    Copy-Item (Join-Path $PSScriptRoot "b12*mesh.inc") $Work -ErrorAction SilentlyContinue
