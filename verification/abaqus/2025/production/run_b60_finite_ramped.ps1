...
$JobName = "b60_fuel_plate_c3d8rt_finite_ramped_bending"
...
foreach ($InputFile in @(
    "b60_fuel_plate_c3d8rt_finite_ramped_bending.inp",
...
Invoke-Abaqus `
    job=$JobName `
    input="b60_fuel_plate_c3d8rt_finite_ramped_bending.inp" `
    `
...
if ($SolveExitCode -ne 0) {
    throw "Abaqus B6.0 ramped finite-strain solve failed with exit code $SolveExitCode"
}
if (!(Test-Path "$JobName.sta") -or
    !(Select-String -Path "$JobName.sta" -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY" -Quiet)) {
    throw "Abaqus B6.0 ramped finite-strain did not report successful completion"
}
...
Invoke-Abaqus python extract_b60_finite.py `
    "$JobName.odb" "b60_long_plate_meat_clad_c3d8rt.json" "${JobName}_nodal.csv" 10.0
if ($LASTEXITCODE -ne 0) {
    throw "Abaqus B6.0 ramped finite-strain extraction failed with exit code $LASTEXITCODE"
}
foreach ($Output in @("${JobName}_nodal.csv", "${JobName}_timing.txt")) {
    if (!(Test-Path $Output)) { throw "Abaqus B6.0 ramped finite-strain did not create $Output" }
    Copy-Item $Output $SourceDirectory
}
Write-Output "Abaqus B6.0 ramped finite-strain wall time: $($Stopwatch.Elapsed.TotalSeconds) seconds"
Write-Output "Abaqus B6.0 ramped finite-strain work directory: $Work"
