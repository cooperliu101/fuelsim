...
if len(sys.argv) != 3:
    raise RuntimeError("usage: extract_b13_pcmi.py job.odb prefix")
odb = openOdb(sys.argv[1], readOnly=True)
try:
    with open(sys.argv[2]+"_nodes.csv", "w", newline="") as nf, open(sys.argv[2]+"_points.csv", "w", newline="") as pf, open(sys.argv[2]+"_contact.csv", "w", newline="") as cf:
