...
if len(sys.argv) != 5:
    raise RuntimeError("usage: extract_b511.py <job.odb> <nodal.csv> <integration.csv> <energy.csv>")
...
try:
    ...
    energy = open(sys.argv[4], "wb")
    energy.write(
        "stage,time_s,internal_energy_j,creep_dissipation_j,external_work_j\n"
    )
    for stage in range(1, 11):
        ...
        fields = {}
        for name in ("CE", "CEEQ", "COORD", "EE", "IVOL", "S", "TEMP"):
            fields[name] = {
        ...
        energy.write(
            ...
            % (
                stage,
                time,
                history_value(step, "ALLIE"),
                history_value(step, "ALLCD"),
                history_value(step, "ALLWK"),
            )
        )
    nodal.close()
    integration.close()
