...
$JobName = "b546_m58_c3d8t_integrated"
$Work = Join-Path $env:TEMP ("fuelsim_b546_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
if ($SolveExitCode -ne 0) {
    throw "Abaqus B5.46 solve failed with exit code $SolveExitCode"
}
if (!(Test-Path "$JobName.sta") -or
    !(Select-String -Path "$JobName.sta" -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY" -Quiet)) {
    throw "Abaqus B5.46 did not report successful completion"
}
...
if ($IncrementSummary.Count -ne 21) {
    throw "Abaqus B5.46 did not produce twenty increment summary rows"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Abaqus B5.46 extraction failed with exit code $LASTEXITCODE"
}
...
Write-Output "Abaqus B5.46 wall time: $($Stopwatch.Elapsed.TotalSeconds) seconds"
Write-Output "Abaqus B5.46 work directory: $Work"
