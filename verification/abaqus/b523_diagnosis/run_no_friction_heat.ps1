...
$JobName = "b523_integrated_no_friction_heat"
$ParentDirectory = Split-Path -Parent $SourceDirectory
$Work = Join-Path $env:TEMP ("fuelsim_b523_no_friction_heat_" + [guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $Work | Out-Null
Copy-Item (Join-Path $SourceDirectory "$JobName.inp") $Work
Copy-Item (Join-Path $ParentDirectory "extract_b523.py") $Work
...
try {
    ...
    & "C:\SIMULIA\Commands\abaqus.bat" python extract_b523.py "$JobName.odb" `
    ...
    @("case=$JobName", "scope=disable conversion of frictional dissipation to heat; diagnostic only",
        "gap_heat_generation_eta=0", "gap_heat_generation_partition=0.5",
        ...
        "extractor_sha256=$((Get-FileHash 'extract_b523.py' -Algorithm SHA256).Hash)") +
