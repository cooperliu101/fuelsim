...
$JobName = "b514_hex8_c3d8t_finite_elastic"
...
$Extractor = Join-Path $SourceDirectory "extract_b514.py"
$RunDirectory = Join-Path $env:TEMP ("fuelsim_b514_" + (Get-Date -Format "yyyyMMdd_HHmmss"))

New-Item -ItemType Directory -Force -Path $RunDirectory | Out-Null
Push-Location $RunDirectory
try {
    & abaqus job=$JobName input=$InputFile output_precision=full interactive
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.14 probe failed with exit code $LASTEXITCODE" }
    ...
    if (!(Test-Path $StatusFile) -or !(Select-String -Quiet -Path $StatusFile -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY")) {
        throw "Abaqus B5.14 probe did not complete successfully; inspect $RunDirectory"
    }
    & abaqus python $Extractor "$JobName.odb" $NodalOutput $IntegrationOutput $EnergyOutput
    if ($LASTEXITCODE -ne 0) { throw "Abaqus B5.14 extraction failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
}
