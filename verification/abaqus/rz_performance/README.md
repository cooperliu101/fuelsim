...
## CAX4T 有限应变摩擦与蠕变塑性耦合（2026-09-11）

在原中等规模摩擦模型的包壳中加入 Norton 蠕变和 J2 硬化塑性。由于原 Abaqus
稳态热步骤忽略蠕变，双方同步启用瞬态热容量，并统一线性功率的区间平均加载。
末态全场通过 0.01% 门槛；全部 20 个增量的等效塑性和蠕变应变通过项目 0.5%
门槛，其中一个刚屈服积分点的相对误差为 0.0525%，绝对差为 4.45e-12，
没有达到额外的 0.01% 目标。从第 14 秒开始核实到同点、同增量的两种机制同时激活。

单处理器两次正式外部耗时平均为 Fuelsim 46.66 秒、Abaqus 58.79 秒，
Fuelsim 本次用时减少 20.64%。生产代码未修改，相关 5 项测试通过，算例未加入
CTest。完整输入、逐步精度边界和计时限制见
[CAX4T 摩擦与蠕变塑性耦合报告](medium_cax4t_finite_friction_coupled/README.md)。
