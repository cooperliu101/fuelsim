...
if len(sys.argv) != 4:
    raise RuntimeError("usage: extract_b58.py <job.odb> <nodal.csv> <integration.csv>")
...
try:
    ...
    nodal.write("time_s,node,temperature_k,ux_m,uy_m,uz_m,reaction_heat_flux_w,rfx_n,rfy_n,rfz_n\n")
    ...
    frames = [(time, odb.steps["STEP%d" % time].frames[-1]) for time in range(1, 5)]
    for time, frame in frames:
        ...
        for node in range(1, 13):
            nodal.write(
                "%.16g,%d,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g\n"
                % (
                    (time, node, temperature[node])
        ...
        for element in range(1, 3):
            for point in range(1, 9):
                key = (element, point)
                integration.write(
                    "%.16g,%d,%d,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,"
