...
$JobName = "b556_m58_c3d20t_finite_sliding_friction"
$MeshName = "b548_m58_c3d20t_integrated"
$Work = Join-Path $env:TEMP ("fuelsim_b556_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
if ($SolveExitCode -ne 0) {
    throw "Abaqus B5.56 solve failed with exit code $SolveExitCode"
}
if (!(Test-Path "$JobName.sta") -or
    !(Select-String -Path "$JobName.sta" -Pattern "THE ANALYSIS HAS COMPLETED SUCCESSFULLY" -Quiet)) {
    throw "Abaqus B5.56 did not report successful completion"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Abaqus B5.56 field extraction failed with exit code $LASTEXITCODE"
}
...
foreach ($Output in @(
    ...
    if (!(Test-Path $Output)) {
        throw "Abaqus B5.56 output is missing: $Output"
    }
    Copy-Item $Output $SourceDirectory
}
Write-Output "Abaqus B5.56 wall time: $($Stopwatch.Elapsed.TotalSeconds) seconds"
Write-Output "Abaqus B5.56 work directory: $Work"
