...
from generate_b10_cax8t_meshes import write_exodus, append_nset

ROOT = Path(__file__).resolve().parent

def write_mesh(size, fuel_nr, clad_nr):
    ...
    for name,es,ss in boundaries:
        nodes = sorted({flat[e-1][q] for e,s in zip(es,ss) for q in (s-1,s%4)})
        sets.append((name,nodes))
    stem='rz_performance_'+size
