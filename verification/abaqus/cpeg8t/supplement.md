...
用户随后指定压力传热在较小间隙范围内验证，并以首步塑性例题作为非弹性验收
依据，两项均已完成，见[当前验收记录](small-gap-acceptance.md)。本文保留原始工况结果。
...
后续按用户要求修复前两项并调整第三项续算应力容差，结果见
[修复记录](contact-repair.md)。下文保留补充验证当时的结果和门槛。
