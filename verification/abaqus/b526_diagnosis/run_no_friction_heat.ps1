...
$JobName = "b526_friction_reversal_no_friction_heat"
$ParentDirectory = Split-Path -Parent $SourceDirectory
$Work = Join-Path $env:TEMP ("fuelsim_b526_no_friction_heat_" + [guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $Work | Out-Null
Copy-Item (Join-Path $SourceDirectory "$JobName.inp") $Work
Copy-Item (Join-Path $ParentDirectory "extract_b524_b525.py") $Work
...
try {
    ...
    & "C:\SIMULIA\Commands\abaqus.bat" python extract_b524_b525.py "$JobName.odb" `
        $Outputs[0] $Outputs[1] $Outputs[2] $Outputs[3] 20 |
    ...
    @("case=$JobName", "scope=disable conversion of frictional dissipation to heat; diagnostic only",
        ...
        "extractor_sha256=$((Get-FileHash 'extract_b524_b525.py' -Algorithm SHA256).Hash)") +
