...
M4.3 对畸变 4 单元网格的全部 100 个接受步进行“每单元 QP0 值按参考单元
体积加权”的历史比较，最大极分解转角约 `25.5°`，并同时施加当前构形压力
和分量牵引。MOOSE 参考由 `selected_qp = 0` 的常数 MONOMIAL Aux 变量再经
`ElementAverageValue` 形成，并不是真正的四积分点材料体积平均。节点场和两个等效
历史量使用统一 `0.5%` 门槛；应力、弹性应变和总非弹性应变的 L2/峰值也使用
`0.5%`。换向过零处最大逐点相对误差当前为
`0.3424%/0.5511%/0.3879%`；应力和总非弹性使用 `0.5%`，仅弹性应变采用
显式 `0.6%` qualified 门槛，不增加分母下限。
塑性/蠕变迹漂移分别限制为 `2e-6/2e-8`。
