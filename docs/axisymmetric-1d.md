...
H = z_upper - z_lower
epsilon = [du_r/dr, (w_upper-w_lower)/H, u_r/r, 0]
dV = 2*pi*r*H*dr
```
...
该规则使温度引起的本征应变在单元内采用同一温度，环向机械应变仍直接计算 `u_r/r`。
不应用 CAX4T 的选择性体积处理、角点热算子或 CAX4RT 的沙漏控制。
...
三个方向没有旋转。增量与已接受的弹性、塑性、蠕变及本征应变共同构成材料
更新输入，累计材料应变不等于总伸长比的对数。当前、已接受及中间构形的
伸长比、积分点半径和体积必须为正，非法试探态进入线搜索或缩小时间步重试。
力学采用 Cauchy 应力、当前梯度和当前体积；热传导、热容与体热源也使用当前
构形。全部几何导数进入切线。本模型不表示二维局部剪切、弯曲和端部应力集中。
...
截面轴向合力为 `N = integral(sigma_zz*2*pi*r*dr)`。体单元对上下端轴向残量
分别贡献 `-N` 和 `+N`，有限应变采用当前截面面积。轴向平衡包含界面摩擦
的端点等效力，内部控制节点的平衡会同时改变相邻切片的轴向应变。
...
外部参考使用施加相同降维运动学约束的 Abaqus CAX4T；
其选择性体积处理与本单元不同，非均匀径向状态应同时检查解析解和网格收敛，
不能假定两种离散在同一粗网格上严格一致。接触算例的全部固定外体应力和
弹性应变与 Abaqus 按 0.1% 正式验收；内体因环向应变平均方式不同，仍作为
离散差异诊断，不作为全场比较通过的证据。非零验收场分别记录相对
