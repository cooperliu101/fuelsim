...
常数，完整应变迹的剩余面内修正为零。热算子也与这一约束下的矩形 CAX4T
对应。不采用 CAX4RT 的沙漏控制，也不跨单元平均材料应力或历史。
...
保持前后均温一致。平均温度对两个节点温度的导数均为 1/2；力学材料温度的
导数只挂回配对节点，导热系数及节点热容物性的温度导数也只挂回各自对应节点。
导热中的温度梯度仍同时依赖两个节点温度。
材料调用复用现有四应变分量加温度的五分量自动微分
接口，切线通过显式节点链
...
| [chain_small](../verification/fuelsim/transient_gps_chain_small.fsi) | 两条两切片轴向链，内部控制节点自由，两片径向载荷不同，解析解与 Abaqus 共同检验独立径向变形和摩擦传力 |
...
当前双切片 Abaqus 参考在公共截面保留重复节点，仅连接轴向位移，径向位移与温度独立。
修正与验收见[连接说明](../verification/abaqus/2025/gps_1d/chain_axial_connection.md)。
...
的力学平均规则已保持一致。CAX2T_GPS 使用配对径向端节点温度；沿高度温度
及材料参数恒定时，与 CAX4T 的配对角点取温一致。因此跨型号的局部投影检查同时
覆盖温度相关弹性模量、泊松比、非均匀径向温度、温度相关平均热膨胀及热物性，
比较两个增量的全部六个残量和 6×6 切线矩阵。
CAX2T_GPS 自身的材料参数解析和切线检查独立保留，生产程序的温变模量
对比见[材料温度更新记录](../verification/abaqus/gps_1d/material_temperature_update.md)。
非均匀有限应变算例现已纳入节点热反力比较，新增小应变和有限应变原生热算例
另检查节点热反力、全局储热和体热源守恒，见[热算子更新记录](../verification/abaqus/gps_1d/thermal_operator_update.md)。
当前一维结果没有体热流输出字段，因此不声称已完成生产体热流字段验收。
原接触算例保留全部内外体的
