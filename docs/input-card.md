...
`use_quadratic_time_predictor` 默认为 `false`。设为 `true` 时启用初值预测，在最近
三个已接受节点状态可用、相邻及待计算时间步的比值都位于 `[0.5, 2]` 时，使用
非等步长二次多项式外推；否则使用上述线性预测。它优先于同时启用的线性选项，
仍遵守原始残量收敛标准，预测失败后从当前已接受状态重试。启用时间步加倍误差
控制时沿用上述共同参考的线性预测，不对整步和半步使用不同的二次预测缓存。
第三个较早状态只在时间步接受后更新，是求解器的可丢弃初值缓存，不属于材料历史，
不写入检查点。重启动后先使用线性预测重新积累该缓存，故这一选项不保证重启动
前后逐位相同，必须按既定数值误差检查结果。
...
- `mumps_ordering = automatic|scotch|pord`；默认 `automatic` 使用 PORD。
  ...
  每次更新精确比较非零位置，仅在位置变化时重新进行符号分解；数值分解使用当前系数。
  完整接触候选仍保留在装配矩阵中。该组合已通过
...
- `jacobian_lag`，默认 `1`；设为大于一的整数时，在该数量的非线性迭代内复用
  已装配的 Jacobian 矩阵；
- `jacobian_lag_persists`，默认 `false`；设为 `true` 后，上述复用间隔可以跨越多次
  非线性求解。瞬态时间步长变化时强制恢复完整 Newton 更新；步长相同时可以复用
  前一步的矩阵和分解。复用期间残量保持该矩阵的行缩放，初始残量范数和物理收敛
  门槛仍按本次求解计算。已有矩阵最多尝试八次迭代，仍未满足原收敛标准时，从本次
  求解的原始初值重新组装矩阵，按完整的 `maximum_iterations` 预算求解。两次尝试
  的耗时、迭代和回调次数均累计。它不改变残量、材料积分、时间步或容差；对强非线性
  问题可能增加重试成本，应结合完整算例判断收益。检查点不保存矩阵分解，重启动后
  重新建立矩阵；启用复用时不保证与不中断运行逐位相同，应按既定数值误差验证。
...
对 C3D8T/C3D8RT 非共面有限滑移面到面接触，`contact_primary_face_<接触名>` 是节点的
法向接触代表主表面编号。它按法向积分点的当前面积乘该节点形函数，汇总每张主表面的
节点面积权重，再选择最大者；与最大权重的差不超过 `64*epsilon*最大权重` 时选择较小
编号，其中 `epsilon` 为双精度机器精度。该规则只解决辅助编号的舍入并列，不改变投影、
力学权重或验收容差。摩擦约束不能覆盖最终编号；无有效法向积分面积时不指定代表面。
该编号不是完整积分点投影归属表，其他接触离散的编号规则保持各自定义。
