...
热容中的密度由材料函数在区域 `initial_temperature`、时间零和参考位置求值，
用于固定参考网格所代表的初始质量。若使用常密度函数，直接填写初始温度下的密度；
若使用变温密度函数，其温度系数只参与初始密度求值。后续热容不再按当前温度
重新查询密度，也不随重启温度重置质量。比热仍在当前温度求值。
有限应变的当前密度满足 `rho_current=rho_initial/J_geo`；装配直接使用等价的
参考质量。详见[初始质量热容规则](initial-mass-heat-capacity.md)。
...
参与状态快照、回滚和检查点重启动；检查点格式版本 20 保存轴对称累计切向滑移及
一维广义平面应变单元的两个材料积分点历史，
固定初始质量由原始输入确定，不兼容版本 19 及更早文件。
