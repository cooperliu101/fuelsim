...
小滑移面到面算法。问题构造时先把 secondary 八节点面按参考态 primary 面所有权
递归分片，每个分片使用 3×3 积分；每个积分点固定 primary 面、自然坐标和形函数
系数。求解中不重新搜索连接关系，但会在当前构形根据固定自然坐标处的 primary
切向重新计算法向，因此能够随局部切平面转动。两侧按下式一致装配同一牵引：
