...
仍使用参考构形。pressure 的构型由边界上的 `configuration` 选择；默认推荐小应变参考构形、有限应变当前构形，
非推荐组合会提示但仍按输入执行。
...
三维笛卡尔接触使用 HEX8 的四节点面。热接触在 secondary 面的四个积分点上计算，
机械接触采用 secondary 面节点到 primary 四节点面的正交投影；每个点都预留该接触
