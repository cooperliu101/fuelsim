...
线段的唯一 NTS 投影；二维法向同时装配径向和轴向反力。机械接触在构造时为
每个 secondary 节点预留整条 primary 链的潜在稀疏耦合，残量和 Jacobian
评估时按当前构形选择距离最近的有效线段，因此节点可以跨越任意数量的链内
线段而不重建 PETSc 工作区。主面链首尾保留端点支承；内部顶点使用半开区间，
任何时刻只允许一条主段拥有该从节点。若节点滑出整条 primary 链，Newton
试探状态会作为物理域错误交给回溯线搜索；若仍无法恢复则拒绝当前载荷步或
时间步，不会静默返回零接触力。动态候选段是 committed 几何的确定性派生量，
检查点不重复保存。热接触仍使用构造时确定的投影重叠分片，不随大滑移重建。
...
当前格式 v4 还保存成功提交后控制器给出的下一名义时间步和最后一个完整接受步
的守恒/耗散摘要，因此自适应计算从检查点继续时不会重新使用输入卡的初始步长，
零步重启动结束也不会把上一接受步诊断伪装成全零。
