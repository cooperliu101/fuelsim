...
小滑移面到面算法。对于两侧均为小应变且 `mu = 0` 的线性罚接触，程序采用
...
非零摩擦或有限应变继续使用分片后的 3×3 积分路径，因为 Abaqus 的相应切向正则化
和当前构形曲面平均算子尚未完成识别。结果文件中的节点压力仍是恢复量；有符号等效
节点量只用于输出，不作为独立节点罚刚度。
...
位移三项误差最高为 `0.107700%`，法向和切向合力误差分别为 `0.000860255%` 和
