...
`(0,±0.5)` 和 `(±0.5,0)`。因此多个平面面片能够逐面改变法向，真正的二次曲面
也能在同一个面内改变法向。共享 secondary 节点会汇总相邻面的约束面积和法向。
...
`elastic_slip` 当前只允许用于两侧小应变的 HEX20 `surface_to_surface` 接触。
有限应变继续使用分片后的 3×3 积分路径。结果文件中的节点压力仍是恢复量；有符号
等效节点量只用于输出，不作为独立节点罚刚度。
...
两个切向分量同时滑动以及非零双分量弹性滑移历史的转动仍不在鉴定范围内。
