...
## 保持精度的计算优化（2026-09-16）

优化保持原有 12 个全长模态、1,188 个最终全局未知量、79,686 阶端部局部方程，
以及全部网格、材料、载荷、约束和输出。没有减少材料点或改变 1% 精度门槛。

端部线弹性方程及其凝聚矩阵具有变分对称性，但位移约束乘子使局部方程不定。
内部因子分解使用 MUMPS 的带主元对称不定分解。PETSc 的 `PCCHOLESKY` 配合
`MAT_SPD=false` 在 MUMPS 中选择 `SYM=2`，不假设正定；详见
[PETSc 的 MUMPS 实现](https://raw.githubusercontent.com/petsc/petsc/v3.24.4/src/mat/impls/aij/mpi/mumps/mumps.c)。
保留完整装配矩阵、原生材料点残量修正和原有残量验收，没有显式求逆、删除耦合项
或修改约束。已有包含不定约束块和两个非零右端项的局部测试检查恢复后的原始方程。

实体单元装配复用小应变下参考梯度和积分体积不随节点位移变化的事实。对一个材料点，
节点内力为 `r_ac = w * sum_d(sigma_cd * N_a,d)`，运动学变量 `eta` 的导数为
`dr_ac/deta = w * sum_d((dsigma_cd/deta) * N_a,d)`。六个应力分量的导数只提取一次，
再乘参考梯度形成 60 个节点内力导数，避免构造 60 个重复的自动微分残量。
宽度 10 的运动学导数链、宽度 7 的材料导数链、节点闭式链、温度导数和各材料点
历史仍按原实现计算。有限应变继续保留梯度与体积的完整自动微分链。

这项装配优化位于共享的 C3D20T 实现中，也适用于调用该实现的 C3D20RT 小应变路径，
因此完整三维模型同样受益。不能把新混合模型与旧三维计时直接作为最终加速比。
原有实体单元方向导数和残量一致性测试，以及经典局部模态的四项全域 1% 比较均已通过。
四种实体端部混合模型的完整应力误差依次为 0.102400%、0.049842%、0.206484%、0.217299%；
位移、轴向应力、弯矩和能量也全部通过原有门槛。轴向零弯矩仍按绝对误差检查。
具体结果与可执行文件、输入卡、网格和参考文件校验值见
[`results/solid_ends_speed`](../verification/section_modal/results/solid_ends_speed)。

受控计时使用同一台机器、CPU 0、单线程和 MUMPS，第一轮按三维、旧混合、新混合
的顺序运行，第二轮反序。两轮的旧、新混合结果均重新通过完整三维精度检查。
计时包含全部预处理、装配、凝聚、求解、恢复和文件输出；计时期间没有编译或其他测试。

| 模型 | 第一轮 / 秒 | 第二轮 / 秒 | 平均 / 秒 | 两轮最大内存 / GiB |
|---|---:|---:|---:|---:|
| 优化前混合模型 | 9.61 | 9.10 | 9.355 | 1.493 |
| 优化后混合模型 | 8.01 | 8.00 | 8.005 | 1.143 |
| 同一新版程序的完整三维模型 | 71.95 | 70.36 | 71.155 | 9.416 |

混合模型平均耗时缩短 14.4%，峰值内存降低 23.4%；相对同版完整三维的速度为
8.89 倍。这个性能结论限此均布载荷、固定板几何的两轮比较，其他三种载荷在这里
验证的是精度。优化前程序对应 `9b819a2`，文件校验值以 `fd8b2341` 开头；新版
程序校验值以 `8b2d55fa` 开头。完整值、各轮输入及结果证据保存在上述结果目录。
Release 构建与警告检查通过；最终 `ctest --test-dir build -j8 --output-on-failure`
为 326/326 通过，耗时 152.90 秒。五项大型三维比较继续由手动入口执行。
...
# 优化前后对比：先保存同一依赖环境编译的旧程序，仍使用完整的已提交输入卡。
python verification/section_modal/benchmark.py \
  build/fuelsim build/blackbox/hybrid_speed_benchmark --solid-ends --cpu 0 --repeats 2 \
  --baseline-executable /path/to/baseline/fuelsim
```
