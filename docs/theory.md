...
Abaqus 的 `CSLIP` 与 fuelsim 保存的累计总切向滑移不是同一个历史量。B5.39
对这两个量直接比较得到的相对 L2、相对绝对峰值和最大逐点相对误差分别为
`7.04782%`、`2.94145%` 和 `56.7689%`，因此该量只作为诊断输出，B5.39 只在
接触力、切向牵引、状态、完整场、可比能量和离散热守恒范围内鉴定。这里没有
增加分母下限，也没有通过拟合系数缩小差异。减缩积分参考另外输出 Abaqus
人工沙漏能量 `ALLAE`；物理内能按 `ALLIE - ALLAE` 比较，人工能量本身保持
独立报告。B5.38 和 B5.39 的最大人工能量分别为 `2.71136e-4 J` 和
