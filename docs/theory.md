...
有限应变 C3D8RT Jacobian 先把宽度 7 的本构切线组合到宽度 10 的运动学链，
再分别对 24 个位移自由度和 8 个温度自由度做两次块播种。位移块完整计算一次
残量并直接提供残量值；温度块复用被动当前构形和中间构形，只计算热传导、热容
和应力温度导数，不重复计算温度导数恒为零的体热源与机械沙漏力。不使用完整
32 自由度恒等播种，也不按 32 列重复完整残量。这个改动只改变导数传播工作量，
不改变残量、材料参数或固定 `0.005` 沙漏系数。6,468 自由度、20 步接触路径在
一个进程固定到逻辑处理器 0、各数值库一个线程的配对测量中，端到端时间从
`130.13 s` 降到 `119.37 s`，Jacobian 回调从 `55.4484 s` 降到
`44.2830 s`；修改前后完整自由度输出逐字节相同，相关局部方向差分和 B5.28 至
B5.45 外部对标继续通过。
