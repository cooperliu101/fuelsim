...
C3D8T 在有限应变下同样对导热和节点热容使用整体体积比权重
`w_q_reference*V_current/V_reference`，导热梯度仍取当前构形。节点热容
配对的是参考 Gauss 权重，并非角点 Jacobian。当前总体积按真实八点几何积分
求和，其跨积分点导数进入两个热算子。B5.23 的非仿射几何才区分了此规则与
逐点当前权重；此前 B5.4 的严格仿射变形不能区分两者。独立证据见
[C3D8T 热算子诊断](../verification/abaqus/b523_diagnosis/README.md)。体热源保持
原先逐点当前测度，本项鉴定不推广到尚未隔离验证的源项分配。
...
热容回退边界以[B5.44 验证报告](../verification/abaqus/b544_initial_density_validation.md)
为准；较早的约 0.699% 位移误差属于修正前结果。
