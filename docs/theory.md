...
### 5.1.1 C3D8RT 一点减缩积分 HEX8

三维 HEX8 区域选择 `element = c3d8rt` 时，可以使用小应变或有限应变。单元在
体积平均形函数梯度处计算一个均匀应变和一个体积加权材料温度；塑性、蠕变及
二者全隐式耦合只保存一个材料积分点。有限应变路径在中间构形形成平均增量位移
梯度，使用 Hughes-Winget 客观转动，在当前构形以 Cauchy 应力、平均梯度和当前
体积装配内力。运动学和本构分别使用宽度 10 和宽度 7 的窄自动微分链，再由
几何方向链组装 32×32 Jacobian，不按局部或全局 32 自由度做恒等播种。

力学沙漏控制采用 Abaqus 默认总刚度算法。四个原始沙漏模态先减去常应变投影，
形成与刚体和平移线性场正交的模态 `gamma`。每个位移方向的稳定化刚度为：

```text
K_hourglass,c = 0.005 * G_initial * metric_c * sum_m gamma_m * transpose(gamma_m)
metric_c = sqrt(2) * V * sum_a(B_average[a,c]^2) / 6
```

其中 `G_initial` 是初始温度下的剪切模量。`0.005` 是 Abaqus 默认算法规定值，
不是根据对标误差拟合的参数，也不提供输入覆盖。

有限应变沙漏项不是把上述小应变矩阵直接作用于当前位移。令 `q=gamma0^T*u`，
`Fbar=I+u^T*B0`，并令 `D` 为三个参考几何方向的沙漏度量，则总沙漏能量使用：

```text
E_hourglass = 0.5 * 0.005 * G_initial * sum_m (q_m*Fbar)*D*transpose(q_m*Fbar)
```

节点沙漏力和 Jacobian 都对这项能量完整求导，因此同时保留 `q` 和 `Fbar` 对
节点位移的导数。这个张量推前和能量导数由 B5.33 的规则、畸变、纯方向和混合
模态探针识别，没有增加可调系数。

热传导由均匀梯度项和四个正交沙漏模态组成。稳定化系数只由参考体积和由平均
梯度重建的有效几何度量计算，不含经验调整量。瞬态热容使用一致热容矩阵行和
`integral(N_i dV)` 作为节点对角权重；材料温度使用相同权重除以单元体积。畸变
单元的均匀体热源遵循 Abaqus 一点积分规则，使用中心点 `8*detJ_center` 并平均分配
到八个节点，而输出积分体积仍是完整参考体积。这两个几何量不能相互替代。

B5.28 用完整 32×32 扰动矩阵比较小应变规则、畸变和事后留出的强畸变单元；
B5.29 独立重建热容矩阵；B5.30 分离体热源、面热流和对流；B5.31 比较小应变
两单元瞬态完整场。B5.32 对规则和畸变单元重建有限应变残量及完整 32×32
Jacobian，最差 Abaqus 块误差为 `0.779172%`，生产 Jacobian 与中心差分的最差
误差为 `3.73463e-6%`。B5.33 的十二组隔离沙漏力误差均为约 `0.000959264%`。
B5.34 的两单元有限应变瞬态全场中，全部非零参考三指标误差最差为
`0.0508446%`；解析零剪切分量单独按绝对误差报告。所有规则保持固定的 Abaqus
默认 `0.005` 系数，没有按对标结果修改物理、沙漏或验收系数。

B5.35、B5.36 和 B5.37 进一步用 Abaqus C3D8RT 完整场路径检查有限应变 J2
塑性、Norton 蠕变和全隐式塑性—蠕变耦合更新。每个单元只保留一个减缩积分
材料点，并比较全部时间点的节点场、约束反力、应力、对数应变、弹性应变、
活跃非弹性历史、当前积分体积和可比能量。三项误差中的最差值分别为
`0.0352662%`、`0.0712657%` 和 `0.0749445%`，均低于 `0.1%`。材料参数、载荷
路径和固定 `0.005` 沙漏系数没有根据误差调整。

B5.38 检查有限滑移面到面热—机械接触的打开、闭合、再次打开和再次接触。
十五个时间点中的 60 个接触状态与 Abaqus 全部一致，法向接触力相对 L2 误差为
`0.000153404%`，接触热量两侧不平衡小于 `2.8e-17 W`。B5.39 检查 Coulomb
摩擦的粘着、滑动、反向和重新粘着；20 个时间点中的 80 个接触状态全部一致，
其中 76 个为粘着、4 个为滑动。位移向量、应力张量、法向接触力和切向牵引的
相对 L2 误差分别为 `0.0635675%`、`0.0244155%`、`0.00952656%` 和
`0.140503%`。这个算例只把规定法向位移缩为原路径的八分之一，使固定切向载荷
能够跨过未修改的 Coulomb 上限；材料、接触、摩擦和沙漏系数均未拟合。

Abaqus 的 `CSLIP` 是从属接触节点上的累计切向相对滑移：接触有效时按局部切向
分量逐步累加，接触打开时保持既有累计值。fuelsim 的面到面接触现在把约束点的
累计历史按 Abaqus 的约束点—从属节点对应关系恢复到节点，不再把四个约束点按
形函数再次平滑。B5.38 的滑移向量相对 L2、相对绝对峰值和最大逐点相对误差为
`0.00658777%`、`0.0106960%` 和 `0.0108406%`；B5.39 对应为 `0.0413877%`、
`0.0335531%` 和 `0.137566%`。两组非零参考均没有零参考点。滑移三项指标现在
正式参与验收；没有增加分母下限，也没有通过拟合系数缩小差异。减缩积分参考
另外输出 Abaqus 人工沙漏能量 `ALLAE`；物理内能按 `ALLIE - ALLAE` 比较，
人工能量本身保持独立报告。B5.38 和 B5.39 的最大人工能量分别为 `2.71136e-4 J` 和
`3.41002 J`，后者占相应内能的 `50.8879%`，所以不能把本组接触结果外推为
任意载荷、网格或沙漏能量占比下的一般鉴定。

B5.40 增加独立的 C3D8RT 非匹配面循环接触路径。primary 侧由两个相邻四节点面
组成，secondary 侧只有一个较小四节点面；secondary 面在接触打开时跨过 primary
共享边界，再分别与两个 primary 面闭合。45 个固定增量全部接受，其中 34 个增量
开放、11 个增量接触，最终四个接触点全部激活。接触压力的相对 L2、相对绝对峰值
和最大逐点相对误差分别为 `0.169420%`、`0.192834%` 和 `0.212430%`；累计总切向
滑移向量三项误差分别为 `0.00182397%`、`0.00279226%` 和 `0.00279980%`。两侧热
残量不平衡小于 `2.0e-15 W`。

总接触热率的相对 L2、相对绝对峰值和最大逐点相对误差分别为 `1.67864%`、
`0.594574%` 和 `13.0459%`。最大逐点误差发生在第一次闭合的较低热率状态，因此
本算例保留 `4%` 聚合门槛和显式 `20%` 逐点合格门槛。粘着与滑动的离散状态分类
一致率为 `90.5556%`；接触力和累计滑移仍完整参与验收，离散状态分类只作诊断。
接近零的位移、反力、应力、对数应变和弹性应变逐点参考量分别使用明确记录的
绝对差门槛，不增加分母下限。温度、应力张量和内能的相对 L2 误差分别为
`0.00202505%`、`0.140500%` 和 `0.284479%`。Abaqus 最大人工能量为
`0.0466314 J`，占相应内能的 `10.8015%`。材料、罚刚度、摩擦、导热和固定
`0.005` 沙漏系数均沿用既有独立设定，没有按误差拟合。
...
非共面线性 secondary 面启用摩擦时，法向力仍由上述四个逐面节点中心约束独立计算；
节点平均摩擦约束的 Coulomb 上限使用与其每个采样点一一对应的法向约束力之和除以当前
摩擦约束面积，不再把已经恢复到输出节点的法向力再次平滑。摩擦约束位置和切向基位于
父坐标 `(±0.5,±0.5)`，只有平均法向继续使用 `(±2/3,±2/3)`。切向滑移增量通过
当前 secondary 表面到参考 secondary 表面的完整二维度量映射回拉；残量传力仍使用当前
单位切向和当前面积。因而参考面剪切或翘曲时，滑移历史的度量、当前构形的牵引方向和
几何切线保持各自明确，不需要任何按算例调整的采样位置、权重或接触系数。
...
| HEX8 参数与组合独立性 | `b524.hex8_load_parameter_independence`、`b525.hex8_distorted_nearly_incompressible` | B5.24 的十三个网格、时间步、罚刚度、摩擦系数、相对弹性滑移容差和压力相关导热扫描点，以及 B5.25 的五个畸变近不可压缩弯曲扫描点，均具有独立 Abaqus C3D8T 全部时间帧节点、积分点、接触和能量基准；十八个逐工况 CTest 可并行运行，另一个轻量聚合测试读取响应摘要而不重复求解；瞬态终点缩短为原载荷路径中点 `0.2 s`，自动测试继续检查逐工况全场误差和跨工况参数响应，适用范围仍限定到跟踪的网格、参数及 `nu<=0.499` |
| HEX8 非线性转变 | `b526.hex8_nonlinear_transitions` | 六个明确登记的直接 Abaqus 全场路径分别覆盖开放—闭合—开放—再接触、粘着—正向滑移—反向滑移—再粘着，以及小应变和有限应变下的首次屈服与蠕变到塑性激活；事务测试覆盖活跃切线、失败时间步回滚、缩小时间步重试和重启动；每条互斥分支逐项验收，不要求一个人为载荷路径同时触发全部分支 |
| 三维燃料—包壳工程研究 | `b527.hex8_engineering_fuel_clad` | 四分之一圆柱独立燃料和包壳块的粗、中、细网格、半时间步及两端罚刚度共六个工况均具有独立 Abaqus 全场基准，并继续检查网格、时间步和罚刚度独立性；适用范围是指定工程模型，不是核安全软件鉴定，也不外推到未跟踪路径 |
