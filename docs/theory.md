...
| HEX8 参数与组合独立性 | `b524.hex8_load_parameter_independence`、`b525.hex8_distorted_nearly_incompressible` | B5.24 的十三个网格、时间步、罚刚度、摩擦系数、相对弹性滑移容差和压力相关导热扫描点，以及 B5.25 的五个畸变近不可压缩弯曲扫描点，均具有独立 Abaqus C3D8T 全部时间帧节点、积分点、接触和能量基准；自动测试在逐工况全场验收之外继续检查收敛和参数响应，适用范围仍限定到跟踪的网格、参数及 `nu<=0.499` |
