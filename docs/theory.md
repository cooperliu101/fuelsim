...
径向位移和按单元型号取值的材料温度（宽度 6）播种，本构关系按 4 个应变分量加温度
（宽度 5）播种，经 `adlite::compose` 挂回运动学链后，12×12 单元 Jacobian
由参考形函数梯度及对应材料温度链闭式组装。CAX4T 的材料温度链使用配对角点，
平均热膨胀另保留四节点链。边界和界面贡献仍按各自局部
自由度恒等播种。
...
所有体单元的热容使用初始质量，后向欧拉离散为：

```text
R_capacity_i = integral_V0 N_i*rho_initial*cp(T_new)*(T_new-T_old)/dt dV0
```

密度在区域初始温度和参考位置求值，比热使用当前温度。CAX2T_GPS、CAX4T、
CAX4RT 和 HEX8 使用各自规定的节点对角权重；混合阶 CAX8、HEX20 使用一致
积分。热容不随变形增加质量，其位移导数为零。位移没有惯性项，力学保持
准静态；加速度体力和完整积分规则见[密度与重力说明](density-mass-and-gravity.md)。

CAX4T 热传导采用 2×2 Gauss 梯度与对应角点温度计算的导热系数；有限应变
梯度取增量中间构形，参考积分测度乘单元当前体积与参考体积之比。C3D8T
有限应变导热使用当前梯度和同样的整体体积比权重，完整保留总体积的跨点
导数。两者体热源仍使用逐积分点当前测度。旧 Abaqus 2018 原生探测的当前
体积热容属于另一种模型，不能据此改变初始质量契约。
...
三维 C3D8T 体单元的热膨胀温度遵循已鉴定的 Abaqus 规则：每个单元只用
八个节点温度的算术平均值计算本征应变。
...
C3D8T 的导热系数、弹性、塑性和蠕变参数使用与积分点配对的角点温度，热容
物性和温度变化率使用节点温度；导热温度梯度仍由节点温度插值计算。C3D8RT
采用其专用的体积平均材料温度。三维 HEX20 保留积分点温度热膨胀离散。
轴对称 `cax4t` 和 `cax4rt` 使用不同规则：

```text
CAX4T:  T_expansion = (T0 + T1 + T2 + T3)/4
CAX4RT: T_expansion = sum(w_i*T_i), w_i = integral(N_i*dV)/integral(dV)
轴对称体积测度: dV = 2*pi*r*detJ*dξ*dη
```

CAX4T 的弹性模量、泊松比、塑性与蠕变参数使用配对角点温度；空间坐标仍是
材料积分点坐标。Fuelsim 的四个积分点与四个局部角点顺序一致，因此温度
输入为 `T_q`，对应节点导数为 `δ_qi`，不能乘积分点的温度插值形函数。
热膨胀继续使用平均温度；平均温度对各角点温度的导数均为 `1/4`。有限应变还保留已接受
角点温度及新旧平均热应变之差。原生材料取温鉴定见
[CAX4T 参数探测](../verification/abaqus/cax4t_material_temperature_probe/README.md)。

畸变单元逐节点升温的独立 Abaqus 探测分别识别出等权算术平均与上述体积
权重，不能把算术平均概括为所有一阶单元的规则。小应变与开启几何非线性
但完全约束位移的结果一致；该试验的当前构形与参考构形重合，不能单独鉴定
大变形后的平均构形或温度相关膨胀系数。完整数据和反推公式见
...
C3D8T 的八个节点热容分别采用配对参考 Gauss 权重：

```text
R_capacity_i = weight_reference_i * rho_initial_i * cp(T_i) * (T_i_new-T_i_old)/dt
```

温度—温度热容 Jacobian 为节点对角，温度—位移热容块为零。畸变参考单元
保留各点不同的权重，不使用角点 Jacobian，也不平均分配总体积。
...
具有非零几何 Jacobian。B5.4 的旧版原生探测识别了当前构形热载荷；其中
热容与当前初始质量模型存在差异，须按版本和物理模型分别报告。
...
梯度，使用 Hughes-Winget 客观转动。体内力使用当前体积和 Cauchy 应力，但偏应力
与平均正应力采用不同的虚梯度：
...
Gp = G0*inverse(Fbar)
p = trace(stress)/3
R_bulk = V_current*(Gp*dev(stress)^T + G_current*p)
```

`G0` 和 `G_current` 分别是参考及当前构形的体积平均形函数梯度。该区别由
[B5.44 原生隔离实验](../verification/abaqus/b544_diagnosis/README.md)鉴定，不能
把所有应力都乘当前梯度，也不能推广到其他型号。运动学和本构分别保持宽度
10 和 7 的自动微分链，闭式节点链同时保留两种梯度、应力和当前体积的导数。
`Fbar` 的行列式必须有限且为正。

材料增量使用 `Lp=delta_u^T*G0*inverse(Fbar_mid)` 的偏量和真实中间构形
`Lm=delta_u^T*Gmid` 的迹，即 `L=Lp+I*(trace(Lm)-trace(Lp))/3`。客观转动
由 `Lp` 的反对称部分给出。热膨胀的新旧温度均按同一组当前几何权重求均值；
不能把当前均温减去按旧构形权重得到的均温，否则保持节点温度时改变几何也会
产生虚假热膨胀。旧均温的位移导数经本征应变温度导数和材料切线回接，自动
微分宽度保持不变。原生材料识别覆盖温变弹性和常数各向同性膨胀系数，尚不能
据此宣称任意非线性膨胀系数的平均方式也已被独立鉴定。
...
热传导由均匀梯度项和四个正交沙漏模态组成。均匀项及模态仍使用真实体积平均
梯度；四个稳定化系数则使用单元中心的梯度及 `8*detJ_center`，保留原有固定
代数公式。小应变取参考中心几何，有限应变取当前中心几何，并完整保留导数。
规定历程的容量隔离与原生热矩阵分别验证这两个几何来源，不含可调参数。瞬态热容使用一致热容矩阵行和
`integral_reference(N_i dV0)` 作为节点对角质量权重；材料温度仍使用其规定的
构形体积权重，两者不再共用有限应变测度。畸变
...
生产诊断现在直接按同一个二次模态势能计算 C3D8RT 机械沙漏能量；它的位移梯度
严格等于装配到残量中的沙漏力。B5.39 对 Abaqus `ALLAE` 的相对 L2、相对绝对
峰值和最大逐点相对误差分别为 `0.0177709%`、`0.0144721%` 和 `0.0478172%`。
B5.40 的聚合误差分别为 `0.230026%` 和 `0.197179%`；一个 Abaqus 参考值只有
`2.516e-9 J` 的点以 `1e-8 J` 明确绝对差门槛验收，实际绝对差为
`1.6768e-9 J`。这里没有给相对误差分母增加下限。

### 5.1.2 C3D8RT 扩展求解路径

B5.41、B5.42 和 B5.43 分别增加小应变 J2 塑性、Norton 蠕变以及两者全隐式
耦合的 Abaqus 全场路径。每个时间点都比较八个节点场、约束反力、一个减缩
积分点的应力和应变历史以及可比能量。三组最大的正常三指标误差分别为
`4.62193e-6%`、`9.99999e-7%` 和 `4.59925e-6%`。B5.42 的理论零横向应力
最大绝对差为 `3.278255e-7 Pa`，单独报告且不增加分母下限。材料参数、载荷和
固定 `0.005` 沙漏系数均从既有路径直接继承，没有针对误差调整。

B5.44 使用八个相连的畸变 C3D8RT 单元形成弯曲主导路径，同时施加体热源、
面热流、对流和规定温度。初始密度固定为 2000 kg/m³；导热系数、比热和弹性
模量保持原先定义的温度线性函数。2026-09-13 的原生隔离实验识别了上述不同
虚梯度的体内力规则，并据此修正生产残量及切线。当前完整场误差、材料表和
历史热容回退结果见[B5.44 验证报告](../verification/abaqus/b544_initial_density_validation.md)。
当前已重新采用初始质量热容，不能沿用回退时的误差与通过状态。

B5.45 用一百个固定增量检查有限应变全隐式塑性和蠕变耦合：前二十五个增量将
轴向位移增加到 `0.03 m`，随后七十五个增量保持轴向位移并将剪切位移增加到
`1.0 m`。极分解转动达到 `26.2254` 度，塑性和蠕变都保持活跃。全部正常场量、
历史量和能量三指标小于 `0.1%`，最差值是外功最大逐点误差 `0.0738831%`。
理论零自由方向应力最大绝对值为 `0.0214132 Pa`；一个接近零的弹性应变参考点
使用 `1e-10` 明确绝对差门槛。另行试算的强反向剪切路径没有与 Abaqus 达成
一致，因此不属于 B5.45 的已验证范围，也没有为使其通过而改变系数或门槛。

有限应变 C3D8RT 残量使用普通双精度计算当前构形、中间构形、材料响应和两类
沙漏项。Jacobian 仍先用宽度 7 的本构链和宽度 10 的 Hughes-Winget 运动学链
取得材料切线，但体积、平均梯度、热沙漏量、中心体热源测度和节点热容权重对
24 个位移自由度的导数全部闭式计算，再把材料、几何和温度链闭式组装为完整
32×32 Jacobian。不再使用 24 位移或 8 温度宽恒等播种，也不按 Jacobian 列重复
完整残量。带 Jacobian 调用返回的残量与仅残量路径使用同一普通双精度结果。

在同一 6,468 自由度、20 步接触路径上，单进程固定到逻辑处理器 0，OpenMP、
OpenBLAS、MKL 和 NumExpr 各一个线程。本轮修改前提交 `6cf276c` 的外部墙钟为
`129.70 s`；普通双精度残量先降至 `77.38 s`，显式回溯线搜索再降至
`48.90 s`，闭式 Jacobian 降至 `33.73 s`，最后把 Jacobian 复用周期从 2 改为
3 后最终复测为 `30.51 s`。残量和 Jacobian 回调分别从 `42.2743 s`、`49.9390 s`
降至 `3.4467 s`、`6.8020 s`，最终执行 128 次非线性迭代、150 次残量回调和
51 次 Jacobian 回调。Abaqus 单进程外部墙钟为 `23.4891 s`，观测比为 `1.2989`。
复用周期 4 会触发缩小时间步重试并破坏固定二十步路径，因此明确不采用。以上
修改不改变残量、材料、接触、载荷、收敛门槛或固定 `0.005` 沙漏系数。
...
有限滑移时，无论表面是平面、曲面还是翘曲面，摩擦和无摩擦都使用当前构形更新的
节点中心平均约束。每个共享 secondary 节点汇集相邻二次面的局部约束采样；每个采样
先在自身当前局部法向上形成标量间隙和闭式位移梯度，再按当前节点面积合并法向与第一
切向。这样不会先平均跨面的向量间隙而引入余弦误差，法向压力、切向牵引和滑移历史也
使用相同的节点中心离散。

primary 和 secondary 面的数量及分片可以不同。每个节点中心约束的转移采样独立投影到
唯一 primary 二次面；间隙、当前法向、两个切向和压力均在刷新后的当前构形定义。离散
残量可以写成节点中心约束算子及其对偶传力：
...
两侧传力系数满足分片统一，所以作用反作用严格守恒。线性化保留投影、当前法向和当前
面积的闭式几何链；对于固定法向的闭合模式，罚切线具有
`penalty * B_gap^T * B_gap * dA` 的非负耦合形式。Q8 角点的等效节点力可以为负，
但它只是上述一致残量的投影结果，不是负的对角罚弹簧。

Coulomb 弹性切向滑移和粘滑状态按唯一 secondary 节点中心约束保存；提交、回滚和
检查点都以该实际约束为事务单位。
输出的节点压力由一致等效节点力除以一致等效节点面积恢复，只用于结果展示。
...
状态验证中按当前构形为每个节点中心约束的转移采样重新搜索完整 primary 面集合。候选投影以最小绝对间隙
...
弹性滑移按这组基分解，再在当前接触基中重构。曲面滑移增量使用已提交节点中心约束的切向基，
...
端点。NTS 在开放链首末段外保留 10% 参数长度的延伸区，并继续使用形函数
投影，不再把越过端点的投影夹持到端点。等距非相邻候选按有序
段编号确定性选择。这样内部顶点只有一个所有者，不会双计接触力。

当前状态滑出某个局部邻域时可转移到完整链上的其他线段；滑出整条 primary
链及其延伸区时，目前仍报告物理域错误，尚未鉴定和实现自然释放。候选选择由 committed 几何和当前状态确定性重建，
检查点不保存临时搜索窗口。热接触对每个积分点采用同样的完整链动态候选原则，
其中 STS 热积分点不使用延伸区，NTS 热节点与机械节点使用相同延伸范围。
两种热接触超出各自候选范围时均报告物理域错误。M5.7 已对规定网格、载荷和接触参数下同时激活 Coulomb 摩擦、机械大滑移
与热接触动态所有权的路径完成鉴定；该结果不能外推为任意组合路径。

### 8.7 轴对称 NTS 热接触

`discretization = node_to_surface` 在 secondary 温度角点计算
`Q_i = h(g_i,T_i,T_primary)*(T_i-T_primary)*A_i`，并使用 primary 温度形函数
分配等量反向热流。四节点和八节点热力耦合单元都只有角点参与热接触。
对一条当前角点连线，长度为 L，两端当前半径为 r0、r1：
...
这是线性形函数与完整轴对称表面测度的积分，不能替换为端点半径乘半边长度。
八节点单元的 secondary 边中节点不影响热面积；primary 投影仍沿完整二次曲线。
当前面积、法向、投影坐标和间隙对位移的导数进入热残量的 Jacobian。
机械节点面积继续使用相应的一次或二次位移形函数积分。详见
[B14 原生识别与验证记录](../verification/abaqus/B14_NTS.md)。
...
| HEX20 表面到面接触 | `h20.sts_finite_sliding_planar`、`h20.sts_finite_sliding_curved`、`b555.small_c3d20t_friction`、`b556.m58_c3d20t_sts_friction` | Abaqus 二次节点中心约束、曲面当前法向和切向、有限滑移搜索、摩擦历史、一致切线、部分接触、小型完整场量，以及 19,524 自由度高热流摩擦综合路径的精度和受控单核时间对比 |
...
| HEX8 参数与组合独立性 | `b524.hex8_load_parameter_independence`、`b525.hex8_distorted_nearly_incompressible` | B5.24 的十三个网格、时间步、罚刚度、摩擦系数、相对弹性滑移容差和压力相关导热扫描点，以及 B5.25 的五个畸变近不可压缩弯曲扫描点，均保留独立 Abaqus C3D8T 全部时间帧节点、积分点、接触和能量基准；这些不增加生产分支的参数扫描作为手动验证资料保留，不进入轻量 CTest；适用范围仍限定到跟踪的网格、参数及 `nu<=0.499` |
...
| 三维燃料—包壳工程研究 | `b527.hex8_engineering_fuel_clad` | 四分之一圆柱独立燃料和包壳块的中等网格作为自动全场对标；粗、细网格、半时间步及两端罚刚度工况保留为手动敏感性资料；适用范围是指定工程模型，不是核安全软件鉴定，也不外推到未跟踪路径 |
