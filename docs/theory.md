...
三维 HEX8 区域选择 `element = c3d8rt` 时，当前实现限定为小应变。单元在体积
平均形函数梯度处计算一个均匀应变和一个体积加权材料温度；塑性、蠕变及二者
全隐式耦合只保存一个材料积分点。运动学和本构仍分别使用宽度 10 和宽度 7 的
窄自动微分链。
...
B5.28 用完整 32×32 扰动矩阵比较规则、畸变和事后留出的强畸变单元；B5.29
独立重建热容矩阵；B5.30 分离体热源、面热流和对流；B5.31 比较两单元瞬态完整
节点场和单点积分量。最差已登记误差为强畸变留出单元热传导块的 `0.120803%`，
仍低于 `1%`，且没有修改任何物理、沙漏或验收系数。有限应变 C3D8RT 尚未实现，
不能从这些小应变证据外推。
