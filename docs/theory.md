...
再分别对 24 个位移自由度和 8 个温度自由度做两次块播种。当前和中间构形、
热算子以及沙漏项在每个块内只计算一次；不使用完整 32 自由度恒等播种，也不再
按 32 列重复完整残量。这个改动只改变导数传播批次，不改变残量、材料参数或
固定 `0.005` 沙漏系数。6,468 自由度接触路径的同命令一进程时间从
`452.58 s` 降到 `135.10 s`，Jacobian 回调从 `379.651 s` 降到
`54.6117 s`；相关局部方向差分和 B5.28 至 B5.45 外部对标继续通过。
