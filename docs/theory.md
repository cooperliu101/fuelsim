...
有限滑移且两侧接触边界各自严格共面时，摩擦和无摩擦也使用当前构形更新的八个
节点中心平均约束。这样平面 C3D20T 接触的法向压力、切向牵引和滑移历史采用相同
离散。任一接触边界为曲面或翘曲面时，则在当前 secondary 二次面上使用 3×3 高斯
积分，使两个切向方向保持为每个积分点的局部切平面方向。
...
Coulomb 弹性切向滑移和粘滑状态在曲面积分路径中各保存九份；平面有限滑移路径则
按八个节点中心约束保存。两条路径的提交、回滚和检查点都以实际约束为事务单位。
