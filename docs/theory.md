...
B5.44 使用八个相连的畸变 C3D8RT 单元形成弯曲主导路径，并同时施加当前构形
体热源、面热流、对流和规定温度。导热率、密度、比热和弹性模量都随温度线性
变化，函数在生成 Abaqus 参考前固定。十个增量全部接受。温度、位移、积分点
热流、应力、对数应变、弹性应变、材料温度、当前积分体积、总边界热率和沙漏
能量均通过各自 `1%` 三指标门槛；位移最大逐点误差为 `0.698823%`。机械反力
最大逐点误差为 `3.02624%`，因此使用明确记录的 `4%` 合格门槛。总边界热率
三项误差最多为 `0.00833932%`。原先节点热反力回放把 `100000 s` 时间步长误作
载荷系数，导致体热源和随载荷缩放的边界量被放大十万倍；修正为保存并回放实际
载荷系数后，节点热反力的相对 L2 和相对绝对峰值误差为 `0.004877%` 和
`0.00353322%`。最大逐点相对误差 `1.14148%` 来自 `0.302267 W` 的低热率点，
绝对差为 `0.003451 W`，按明确的 `0.01 W` 绝对门槛验收，不增加分母下限。
独立地用 Abaqus 应力、当前构形平均梯度和 B5.33 已识别的固定沙漏规则重建夹持
端反力，三项误差为 `0.237294%`、`0.251866%` 和 `2.61753%`；中心点几何及两种
参考梯度推前候选都更差，所以没有改动生产有限应变公式或任何系数。Abaqus 人工
沙漏能量最大占内能 `8.85354%`。因此 B5.44 仍是对该固定网格、载荷和材料函数的
合格证据，不是任意畸变网格或任意弯曲问题的一般鉴定。
