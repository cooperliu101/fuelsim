...
并行等价性测试固定所有数值库为单线程，并使用 PETSc `redundant` 预条件器
配合串行 LU 分解。两个进程保持 97 个接受步，最大应力和塑性应变绝对差为
...
该小网格没有并行加速，不能据此外推一般扩展性。完整首次运行、预热样本和
97/98 步路径边界见 `benchmarks/README.md`。
