...
- M4.3：50 步非共轴拉伸—剪切—反向路径达到约 `21°` 转动，逐步比较
  MOOSE 的应力、弹性应变、客观总非弹性应变及塑性/蠕变等效历史；
...
- 默认 BASIC Newton 失败后可从原始初值自动用 BT 重试；最终残量按温度、
  径向和轴向方程分场复核，可选 Backward Euler step-doubling 时间误差控制；
...
- 弹性/塑性/蠕变历史的客观旋转，以及 follower pressure 当前构形合力、
  AD Jacobian 和 MOOSE 全节点对比；
...
53 个节点的温度三项误差最大为 `0.04262%`；轴向位移三项误差最大为
`0.06704%`。径向位移的相对 L2、相对绝对峰值和最大逐点相对误差分别为
`0.08349%`、`0.11553%` 和 `0.23673%`，因此径向
位移采用全场 L2 `<0.095%`、峰值 `<0.125%`、逐点 `<0.27%` 的回归门槛。
包壳平均等效应力误差为 `0.00322%`，平均等效塑性和蠕变应变误差分别为
`0.03698%` 和 `0.02812%`。接触压力三项误差分别为 `0.04486%`、
`0.00805%` 和 `0.09203%`，总接触力误差为 `0.00065%`。

包壳 10 个单元的全部 40 个 Gauss 点也与 MOOSE 一一对齐。等效应力、
等效塑性应变和等效蠕变应变的逐点相对 L2 误差分别为 `0.01158%`、
`0.13129%` 和 `0.07111%`，相对绝对峰值误差分别为 `0.03239%`、
`0.26656%` 和 `0.18375%`，最大单点相对误差分别为 `0.03239%`、
`0.28315%` 和 `0.18375%`。收紧后的径向位移峰值和逐点门槛分别为
`<0.125%` 和 `<0.27%`，接触压力三项门槛为 `<0.11%`。积分点门槛按场
分别设置：等效应力为 `0.015%/0.04%/0.04%`，塑性应变为
...
M4.3 的 50 步非共轴路径中，应力、弹性应变、总非弹性应变、等效塑性应变
和等效蠕变应变的三项误差最大值为 `0.000486%`，全部低于 `0.5%` 门槛。
...
旋转和当前构形内力；压力为当前构形 follower load，分量 traction 仍是
参考构形 dead load。M4.3 已鉴定单元素、50 步、最大约 `21°` 的规定非共轴
反向路径；任意转角、任意加载路径和一般网格仍不能由这一算例外推。MOOSE
只旋转其总非弹性张量，fuelsim 分别旋转塑性和蠕变张量，因此分机制张量由
