...
三维 HEX8 小应变区域还可以显式选择 Abaqus `C3D8RT` 风格的一点减缩积分热力
耦合公式。其力学沙漏控制固定使用 Abaqus 默认的 `0.005 × 初始剪切模量` 总刚度
算法，热稳定化只由单元几何推导；没有为降低误差而设置的拟合系数。规则、两类
畸变算子、热容、热载荷、瞬态完整场、非弹性局部切线、事务和检查点均有自动
测试。当前不支持有限应变 C3D8RT，输入会明确拒绝。
