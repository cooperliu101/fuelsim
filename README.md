...
新增 `axisymmetric_1d` 几何和 `cax2t_gps` 两节点径向热力耦合单元，支持小应变
及有限应变。多切片共享端截面轴向位移，芯块与包壳分别求解轴向平衡，并通过
圆柱热机械接触与摩擦耦合。网格、完整输入卡、接触适用范围和验收说明见
[轴对称一维与 1.5 维框架](docs/axisymmetric-1d.md)。

轴对称和三维体单元、边界积分及局部接触计算现由仓库内的独立[单元库](elements/README.md)提供。该库可仅依赖
ADlite 单独构建和测试；fuelsim 负责全局装配、求解及历史状态的接受与恢复。
体单元按八种 Abaqus 型号和自有 `cax2t_gps` 型号分别维护同名头文件、实现和测试。默认 `quad4` 已删除，
每个输入区域必须显式设置 `element`，型号必须匹配网格拓扑。
...
- CAX4T/CAX4RT 的有限应变与当前测度检查、正 Jacobian 守卫，以及
...
不能外推为任意大滑移摩擦路径。
有限应变使用各型号的 Hughes-Winget 增量应变与转动、历史张量客观
...
做单次 Hughes-Winget 更新，不累计载荷步历史，主方向旋转的路径问题应使用瞬态。
