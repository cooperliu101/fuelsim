...
- STS 热流和 NTS 节点反力守恒；
- 接触投影半开区间的唯一性、参考端点夹持、越界失活和加高包壳覆盖；
- 带热源实心圆柱解析温度；
- 无应力自由热膨胀；
- 开口端 Lamé 厚壁圆筒压力解；
- 开放气隙燃料—包壳圆柱的解析热阻解；
...
表面节点均投影并接触。全部 50 个节点的温度三项误差最大为 `0.04509%`；
轴向位移三项误差最大为 `0.07657%`。径向位移的相对 L2、相对绝对峰值和
最大逐点相对误差分别为 `0.08782%`、`0.11875%` 和 `0.25233%`；后两项
的局部差异来自顶部包壳内表面节点，最大绝对差约 `6.45 nm`，因此径向
位移采用全场 L2 `<0.095%`、峰值 `<0.125%`、逐点 `<0.27%` 的回归门槛。
包壳平均等效应力误差为 `0.00431%`，平均等效塑性和蠕变应变误差分别为
`0.0489%` 和
`0.0359%`。接触压力三项误差分别为 `0.06209%`、`0.10333%` 和
`0.10333%`，总接触力误差保持远低于 `0.1%`。

包壳 8 个单元的全部 32 个 Gauss 点也与 MOOSE 一一对齐。等效应力、
等效塑性应变和等效蠕变应变的逐点相对 L2 误差分别为 `0.0124%`、
`0.13873%` 和 `0.07682%`，相对绝对峰值误差分别为 `0.03335%`、
`0.27513%` 和 `0.18805%`，最大单点相对误差分别为 `0.03335%`、
`0.29152%` 和 `0.18805%`。收紧后的径向位移峰值和逐点门槛分别为
