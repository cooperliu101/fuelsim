...
> 当前密度、固定初始质量热容与重力规则见[密度与重力说明](../docs/density-mass-and-gravity.md)。该规则取代下文历史算子识别中的当前体积热容描述，其他算子构形规则继续适用。
...
九种体单元分别拥有同名公共头文件、实现文件和测试文件：
`cax2t_gps`、`cax4t`、`cax4rt`、`cax8t`、`cax8rt`、`c3d8t`、`c3d8rt`、`c3d20t`、`c3d20rt`。
`cax2t_gps` 是自有的两节点径向广义平面应变单元，采用两个材料积分点，支持
小应变和对角有限应变；完整数值约定见[一维框架说明](../docs/axisymmetric-1d.md)。
...
五种界面类型为 `ring_gps`、`line2_rz`、`line3_rz`、`quad4_face`、`quad8_face`，同样各有
一组 `.hpp/.cpp` 和测试。每组文件同时维护本类型的边界载荷、热接触和机械接触，
以不同函数区分计算任务，不再按 `boundary/contact` 拆文件。

| 文件 | 归属 |
| --- | --- |
| `src/contact_common.hpp/.cpp` | 跨拓扑间隙导热律，以及 QUAD4/QUAD8 共用的摩擦返回映射、投影收敛判据；具体类型普通函数，几何与历史转运留在拓扑文件 |
...
局部自由度保持按场排列，分别为 6、12、20、32、68 个。径向单元的五维材料
切线直接经闭式节点链装配；二维轴对称自动微分采用宽度 6
...
独立测试包含九种体单元、五种界面、材料检查及源码边界检查。专用检查位于对应类型的
...
这些例外不允许引入通用矩阵库、材料模型模板或单元标量泛型接口。
...
3×3 矩阵的行列式、求逆、矩阵乘法分别由 `c3d_common.cpp` 内的一份私有函数
模板实现，对外仍提供具体类型的普通函数。矩阵乘法的右操作数保持为普通双精度。
材料对称张量旋转的两个标量版本共用 `material.cpp` 内的一份私有函数模板，
独立维护材料层的旋转计算，不依赖 `c3d_common`。这些模板同样只接受 `double`
和 `adlite::Scalar`，保持原有求和顺序；模板名称与位置由边界脚本逐项检查。

接触共享实现拒绝非有限的投影坐标、增量及导数，导数宽度不一致属于程序错误。
QUAD4/QUAD8 的投影牛顿迭代使用以主面节点为原点的局部坐标，避免全局平移造成
坐标相减的精度损失；达到上限仍未收敛时抛出域错误，不继续构造投影。
受压摩擦接触的切向粘着刚度必须有限且严格为正。两种间隙导热律的结果均须有限且非负；
...
QUAD8 的普通双精度和自动微分形函数入口共用 `quad8_face.cpp` 内的
`quad8_shape_impl`，仅计算固定八节点的形函数及一、二阶导数。两个具体类型入口
及结果结构声明于私有 `quad8_shape.hpp`，双精度入口不构造 ADlite 对象。
`common_math_tests.cpp` 获准读取该私有声明，验证节点插值、单位分解、导数之和为零、
两类实例的数值一致性，以及对形函数值作独立中心差分得到的一阶、二阶导数。
