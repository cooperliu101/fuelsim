...
struct Cax4tKinematicAverages final {
    double current_volume, reference_volume, hoop_new, average_trace, average_hoop;
    Cax4LocalValues volume_derivatives, hoop_shape, average_derivatives, hoop_derivatives;
};

struct Cax4tStressAverages final {
    double pressure, hoop_stress;
    Cax4LocalValues pressure_derivatives, hoop_stress_derivatives;
};

Cax4tKinematicAverages
evaluate_cax4t_kinematics(const Cax4Input& data, bool jacobian, std::array<Cax4tPointSystem, 4>& systems) {
    const auto& geometry = data.geometry;
    const auto& state = data.state;
    const auto& old_state = data.committed_state;
...
Cax4tStressAverages evaluate_cax4t_material(const Cax4Input& data,
    ElementRequest request,
    const Cax4tKinematicAverages& averages,
    ...
    for (std::size_t q = 0; q < 4; ++q) {
        ...
        const adlite::Scalar correction =
            (averages.average_trace - averages.average_hoop - k.strain_rr - k.strain_zz) / 2.0;
...
void assemble_cax4t(const Cax4Input& data,
    bool jacobian,
    const Cax4tKinematicAverages& averages,
    const Cax4tStressAverages& stresses,
    const std::array<Cax4tPointSystem, 4>& systems,
    Cax4Result& result) {
    const auto& geometry = data.geometry;
    const auto& state = data.state;
    const auto& old_state = data.committed_state;
    ...
    for (std::size_t q = 0; q < 4; ++q) {
        ...
        const double scale =
            finite ? point.weighted_measure * averages.current_volume / (averages.reference_volume * w) : 1.0;
        const std::array<double, 4> deviator = {s.stress.rr.value() - point_pressure,
            s.stress.zz.value() - point_pressure,
            0.0,
            s.stress.rz.value()};
        const std::array<double, 4> stress = {scale * deviator[0] + stresses.pressure,
        ...
        if (jacobian)
            for (std::size_t j = 0; j < 12; ++j) {
                ...
                const double ds =
                    finite ? scale * (averages.volume_derivatives[j] / averages.current_volume - wd[j] / w) : 0.0;
                for (std::size_t i = 0; i < 4; ++i)
                    sd[i][j] = i == 2 ? stresses.pressure_derivatives[j]
        ...
        const double conduction_measure =
            point.weighted_measure * (finite ? averages.current_volume / averages.reference_volume : 1.0);
        ...
        for (std::size_t n = 0; n < 4; ++n) {
            ...
            if (jacobian) {
                ...
                if (finite)
                    for (std::size_t j = 0; j < 12; ++j)
                        result.jacobian[n * 12 + j] += point.weighted_measure / averages.reference_volume
                                                       * averages.volume_derivatives[j] * conductivity.value()
                                                       * conduction.value();
                for (std::size_t j = 0; j < 4; ++j)
    ...
    const double hoop_factor = averages.current_volume / (finite_strain ? averages.hoop_new : 1.0);
    for (std::size_t n = 0; n < 4; ++n) {
        const auto row = 4 + n;
        result.residual[row] += hoop_factor * (stresses.hoop_stress - stresses.pressure) * averages.hoop_shape[row];
        if (jacobian)
            for (std::size_t j = 0; j < 12; ++j) {
                const double factor_derivative = finite_strain ? averages.volume_derivatives[j] / averages.hoop_new
                                                                     - averages.current_volume * averages.hoop_shape[j]
                                                                           / (averages.hoop_new * averages.hoop_new)
                                                               : 0.0;
                result.jacobian[row * 12 + j] +=
                    averages.hoop_shape[row]
                    * (factor_derivative * (stresses.hoop_stress - stresses.pressure)
                        + hoop_factor * (stresses.hoop_stress_derivatives[j] - stresses.pressure_derivatives[j]));
            }
    }
}
...
Cax4Result evaluate_cax4t(const Cax4Input& input, ElementRequest request) {
    ...
    validate_cax_time_input(old_history != nullptr, time_step, thermal_time);
