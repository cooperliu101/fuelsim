...
InputDocument InputParser::parse_file(const std::string& path) {
    ...
    while (std::getline(input, raw_line)) {
        ...
        if (line.front() == '[') {
            ...
            if (name.empty()) {
                ...
                if (stack.empty()) {
                    ...
                    if (current == nullptr)
                        throw std::logic_error("InputParser lost its parent section");
                }
                continue;
            }
            if (!valid_name(name))
            ...
            const bool duplicate =
                std::any_of(document._sections.begin(), document._sections.end(),
        ...
        if (duplicate)
            input_error(path, line_number, "duplicate key '" + key + "' in [" + current->_path + "]");
...
double read_optional_double(const InputDocument& document, const InputSection& section, const std::string& key,
                            double fallback) {
    const InputEntry* entry = find_entry(section, key);
...
std::size_t read_optional_size(const InputDocument& document, const InputSection& section, const std::string& key,
                               std::size_t fallback) {
    const InputEntry* entry = find_entry(section, key);
    return entry == nullptr ? fallback : parse_size(document, *entry);
}

int read_optional_int(const InputDocument& document, const InputSection& section, const std::string& key,
                      int fallback) {
    const InputEntry* entry = find_entry(section, key);
    if (entry == nullptr)
        return fallback;
...
bool read_optional_bool(const InputDocument& document, const InputSection& section, const std::string& key,
                        bool fallback) {
    const InputEntry* entry = find_entry(section, key);
    if (entry == nullptr)
        return fallback;
...
void forbid_key(const InputDocument& document, const InputSection& section, const std::string& key,
...
BoundaryConditionDefinition read_boundary_condition(const InputDocument& document, const InputSection& section) {
    ...
    if (type == "traction") {
        ...
        BoundaryConditionDefinition result =
            make_boundary_condition(document, section, BoundaryConditionType::traction, field,
                                    read_double(document, section, "value"), scale_with_load, function);
...
void read_executioner(const InputDocument& document, const std::string& path, FuelSimCaseDefinition& result) {
    ...
    if (result.problem == CaseProblem::steady) {
        ...
        validate_keys(document, executioner,
                      {"type", "end_time", "initial_time_step", "minimum_time_step", "maximum_time_step",
                       "growth_factor", "cutback_factor", "maximum_cutbacks", "load_ramp_time", "restart",
                       "target_nonlinear_iterations", "iteration_window", "time_error_relative_tolerance",
                       "temperature_time_absolute_tolerance", "displacement_time_absolute_tolerance",
                       "time_error_safety_factor", "strain_history_time_absolute_tolerance",
...
void read_solver(const InputDocument& document, const std::string& path, FuelSimCaseDefinition& result) {
    ...
    validate_keys(document, solver,
                  {"absolute_tolerance", "relative_tolerance", "step_tolerance", "maximum_iterations", "linear_solver",
                   ...
                   "mechanical_residual_absolute_tolerance", "temperature_residual_scale",
    ...
    for (const auto& output_path : output_paths) {
        ...
        for (const auto& protected_input : protected_inputs) {
            if (!protected_input.second.empty() && output_path.second == protected_input.second)
                throw std::invalid_argument(path + ": " + output_path.first + " must not overwrite the " +
                                            protected_input.first);
        }
    }
