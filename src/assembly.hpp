...
class SpatialAssembly final {
  ...
  private:
    ...
    void update_thermal_candidates(
        std::size_t contribution_begin, std::size_t contribution_end, const GlobalStateView& state) const;
    void update_mechanical_candidates(const std::vector<double>& state) const;
    void update_mechanical_candidates(
        std::size_t contribution_begin, std::size_t contribution_end, const GlobalStateView& state) const;
    std::vector<std::vector<bool>> touched_thermal_points(
    ...
    std::vector<ControlledScalar> _pressure_loads;
    std::vector<ControlledScalar> _traction_loads;
