...
void hash_region_definition(std::uint64_t& hash, const RegionDefinition& spatial, const TransientInelasticProperties& transient) {
...
std::uint64_t transient_problem_signature(const TransientProblem& problem) {
    ...
    if (cartesian) {
        hash_size(hash, cartesian3d::ProblemAccess::region_count(problem));
        const TransientProblemDefinition& definition = cartesian3d::ProblemAccess::definition(problem);
        for (std::size_t region_value = 0; region_value < cartesian3d::ProblemAccess::region_count(problem); ++region_value) {
            ...
            const Hex8RegionMesh& mesh = cartesian3d::ProblemAccess::region_mesh(problem, region_value);
    ...
    for (std::size_t region_value = 0; region_value < fuelsim::rz::ProblemAccess::region_count(problem); ++region_value) {
        const RegionDefinition& spatial = definition.spatial.regions[region_value];
        const TransientInelasticProperties& transient = definition.regions[region_value].material;
        hash_region_definition(hash, spatial, transient);
...
std::size_t checked_size(std::int64_t value, const char* description) {
    if (value < 0 || static_cast<std::uint64_t>(value) > std::numeric_limits<std::size_t>::max()) throw std::length_error(std::string(description) + " is out of range");
    return static_cast<std::size_t>(value);
}

std::int64_t checked_count(std::size_t value, const char* description) {
    if (value > static_cast<std::size_t>(std::numeric_limits<std::int64_t>::max())) throw std::length_error(std::string(description) + " is out of range");
...
std::int64_t to_exodus_id(std::size_t index, const char* description) {
    if (index >= static_cast<std::size_t>(std::numeric_limits<std::int64_t>::max())) throw std::length_error(std::string(description) + " is out of range");
    return static_cast<std::int64_t>(index) + 1;
}

void write_entity_name(int exoid, ex_entity_type type, std::int64_t id, const std::string& name, const std::string& operation) {
...
std::vector<NodeSet> read_node_sets(int exoid, std::size_t set_count, std::size_t maximum_name_length) {
    ...
    for (const std::int64_t set_id : set_ids) {
        ...
        check_exodus(ex_get_set_param(exoid, EX_NODE_SET, set_id, &entry_count, &factor_count), "Could not read Exodus node set parameters");
        std::vector<std::int64_t> entries(checked_size(entry_count, "Exodus node set size"));
        if (!entries.empty()) check_exodus(ex_get_set(exoid, EX_NODE_SET, set_id, entries.data(), nullptr), "Could not read Exodus node set");
...
std::vector<SideSet> read_side_sets(int exoid, std::size_t set_count, std::size_t maximum_name_length, std::int64_t maximum_side, const char* topology_name) {
    ...
    for (const std::int64_t set_id : set_ids) {
        std::int64_t entry_count = 0;
        std::int64_t factor_count = 0;
        check_exodus(ex_get_set_param(exoid, EX_SIDE_SET, set_id, &entry_count, &factor_count), "Could not read Exodus side set parameters");
        ...
        if (count != 0) check_exodus(ex_get_set(exoid, EX_SIDE_SET, set_id, elements.data(), sides.data()), "Could not read Exodus side set");
        SideSet set{set_id, read_entity_name(exoid, EX_SIDE_SET, set_id, maximum_name_length), {}};
        set.sides.reserve(count);
        for (std::size_t entry = 0; entry < count; ++entry) {
            if (elements[entry] <= 0 || sides[entry] <= 0 || sides[entry] > maximum_side) throw std::runtime_error(std::string("Exodus side set contains an invalid ") + topology_name + " side");
            set.sides.push_back({static_cast<std::size_t>(elements[entry] - 1), static_cast<std::size_t>(sides[entry] - 1)});
        }
        result.push_back(std::move(set));
    }
    return result;
...
ExodusMeshData read_exodus_mesh(const std::string& path, std::size_t dimension, std::size_t nodes_per_element, std::size_t maximum_side, const char* topology, const char* alternate_topology, const char* title) {
    ...
    check_exodus(ex_set_max_name_length(file.id(), static_cast<int>(maximum_name_length)), "Could not set Exodus maximum read name length");
    ...
    if (parameters.num_dim != static_cast<std::int64_t>(dimension)) throw std::runtime_error(std::string("Exodus ") + topology + " mesh has the wrong dimension");
    ...
    if (node_count == 0 || block_count == 0) throw std::runtime_error(std::string("Exodus ") + topology + " mesh must contain nodes and element blocks");

    ExodusMeshData result{dimension, nodes_per_element, maximum_side, topology, alternate_topology, title, {}, {}, {}, {}, {}, {}, {}};
    std::array<std::vector<double>, 3> coordinates = {std::vector<double>(node_count), std::vector<double>(node_count), std::vector<double>(node_count)};
    check_exodus(ex_get_coord(file.id(), coordinates[0].data(), coordinates[1].data(), dimension == 3 ? coordinates[2].data() : nullptr), "Could not read Exodus coordinates");
    result.nodes.resize(node_count);
    for (std::size_t node = 0; node < node_count; ++node) result.nodes[node] = {coordinates[0][node], coordinates[1][node], dimension == 3 ? coordinates[2][node] : 0.0};
    ...
    for (const std::int64_t block_id : block_ids) {
        ...
        if (block.num_nodes_per_entry != static_cast<std::int64_t>(nodes_per_element) || (actual_topology != topology && actual_topology != alternate_topology))
        ...
        if (block_elements > std::numeric_limits<std::size_t>::max() / nodes_per_element) throw std::length_error("Exodus connectivity is too large");
        std::vector<std::int64_t> connectivity(block_elements * nodes_per_element);
        check_exodus(ex_get_conn(file.id(), EX_ELEM_BLOCK, block_id, connectivity.data(), nullptr, nullptr), "Could not read Exodus connectivity");
        for (std::size_t element = 0; element < block_elements; ++element) {
            ...
            for (std::size_t local = 0; local < nodes_per_element; ++local) {
                ...
                if (exodus_node <= 0 || static_cast<std::uint64_t>(exodus_node) > node_count) throw std::runtime_error("Exodus connectivity contains an invalid node ID");
    ...
    if (result.elements.size() != checked_size(parameters.num_elem, "Exodus element count")) throw std::runtime_error("Exodus element count does not match its element blocks");
    result.node_sets = read_node_sets(file.id(), checked_size(parameters.num_node_sets, "Exodus node set count"), name_length);
    result.side_sets = read_side_sets(file.id(), checked_size(parameters.num_side_sets, "Exodus side set count"), name_length, static_cast<std::int64_t>(maximum_side), topology);
    file.close();
    return result;
}
...
void write_exodus_mesh(const std::string& path, const ExodusMeshData& mesh) {
    ...
    const int exoid = ex_create(path.c_str(), EX_CLOBBER | EX_ALL_INT64_DB | EX_ALL_INT64_API, &cpu_word_size, &io_word_size);
    ...
    for (std::size_t element = 0; element < mesh.elements.size(); ++element) {
        ...
        auto block = std::find_if(blocks.begin(), blocks.end(), [block_id](const BlockConnectivity& candidate) { return candidate._id == block_id; });
        ...
        for (std::size_t local = 0; local < mesh.nodes_per_element; ++local) block->_nodes.push_back(to_exodus_id(mesh.elements[element][local], "Exodus node ID"));
    }
    ex_init_params parameters{};
    ...
    check_exodus(ex_put_coord(file.id(), coordinates[0].data(), coordinates[1].data(), mesh.dimension == 3 ? coordinates[2].data() : nullptr), "Could not write Exodus coordinates");
    std::array<char*, 3> coordinate_names{};
    for (std::size_t component = 0; component < mesh.dimension; ++component) coordinate_names[component] = const_cast<char*>(mesh.coordinate_names[component]);
    ...
    for (const BlockConnectivity& block : blocks) {
        check_exodus(ex_put_block(file.id(), EX_ELEM_BLOCK, block._id, mesh.topology, checked_count(block._source_elements.size(), "Block element count"), checked_count(mesh.nodes_per_element, "Nodes per element"), 0, 0, 0),
                     "Could not write Exodus element block");
        check_exodus(ex_put_conn(file.id(), EX_ELEM_BLOCK, block._id, block._nodes.data(), nullptr, nullptr), "Could not write Exodus connectivity");
        write_entity_name(file.id(), EX_ELEM_BLOCK, block._id, block._name, "Could not write Exodus element block name");
    ...
    for (const NodeSet& set : mesh.node_sets) {
        ...
        check_exodus(ex_put_set_param(file.id(), EX_NODE_SET, set.id, checked_count(entries.size(), "Node set size"), 0), "Could not write Exodus node set parameters");
        if (!entries.empty()) check_exodus(ex_put_set(file.id(), EX_NODE_SET, set.id, entries.data(), nullptr), "Could not write Exodus node set");
    ...
    for (const SideSet& set : mesh.side_sets) {
        ...
        check_exodus(ex_put_set_param(file.id(), EX_SIDE_SET, set.id, checked_count(set.sides.size(), "Side set size"), 0), "Could not write Exodus side set parameters");
        if (!elements.empty()) check_exodus(ex_put_set(file.id(), EX_SIDE_SET, set.id, elements.data(), sides.data()), "Could not write Exodus side set");
...
UnstructuredQuad4Mesh ExodusMeshIo::read_quad4(const std::string& path) {
    ...
    return UnstructuredQuad4Mesh(std::move(nodes), std::move(elements), std::move(data.element_block_ids), std::move(data.element_blocks), std::move(data.node_sets), std::move(data.side_sets));
}

void ExodusMeshIo::write_quad4(const std::string& path, const UnstructuredQuad4Mesh& mesh) {
    ExodusMeshData data{2, 4, 4, "QUAD4", "QUAD", "fuelsim Quad4 mesh", {"r", "z", nullptr}, {}, {}, mesh.element_block_ids(), mesh.element_blocks(), mesh.node_sets(), mesh.side_sets()};
    for (const RzPoint& node : mesh.nodes()) data.nodes.push_back({node.r, node.z, 0.0});
    for (const Quad4Element& element : mesh.elements()) data.elements.push_back({element.nodes[0], element.nodes[1], element.nodes[2], element.nodes[3]});
...
UnstructuredHex8Mesh ExodusMeshIo::read_hex8(const std::string& path) {
    ...
    return UnstructuredHex8Mesh(std::move(nodes), std::move(elements), std::move(data.element_block_ids), std::move(data.element_blocks), std::move(data.node_sets), std::move(data.side_sets));
}

void ExodusMeshIo::write_hex8(const std::string& path, const UnstructuredHex8Mesh& mesh) {
    ExodusMeshData data{3, 8, 6, "HEX8", "HEX", "fuelsim HEX8 mesh", {"x", "y", "z"}, {}, {}, mesh.element_block_ids(), mesh.element_blocks(), mesh.node_sets(), mesh.side_sets()};
...
int checked_int(std::size_t value, const std::string& quantity) {
    if (value > static_cast<std::size_t>(std::numeric_limits<int>::max())) throw std::overflow_error(quantity + " exceeds the Exodus int range");
...
void define_variable_names(int exoid, ex_entity_type type, const std::vector<std::string>& names, const std::string& category) {
    std::vector<char*> pointers = variable_name_pointers(names);
    check_exodus(ex_put_variable_param(exoid, type, static_cast<int>(pointers.size())), "Could not define Exodus " + category + " variables");
    check_exodus(ex_put_variable_names(exoid, type, static_cast<int>(pointers.size()), pointers.data()), "Could not name Exodus " + category + " variables");
}
...
std::vector<std::string> cartesian_nodal_variable_names() { return {"temperature", "displacement_x", "displacement_y", "displacement_z"}; }
...
void append_component_variable_names(std::vector<std::string>& result, const char* prefix, std::size_t q) {
    for (const char* component : stress_components) result.push_back(std::string(prefix) + std::string(component) + "_q" + std::to_string(q));
}
...
std::vector<std::string> cartesian_stress_variable_names() {
    ...
    for (std::size_t q = 0; q < 8; ++q) {
        for (const char* component : cartesian_stress_components) result.push_back("stress_" + std::string(component) + "_q" + std::to_string(q));
    }
    return result;
}
...
ResultsMeshView results_mesh_view(const UnstructuredQuad4Mesh& mesh) { return {mesh.nodes().size(), mesh.elements().size(), mesh.element_blocks(), mesh.element_block_ids()}; }

ResultsMeshView results_mesh_view(const UnstructuredHex8Mesh& mesh) { return {mesh.nodes().size(), mesh.elements().size(), mesh.element_blocks(), mesh.element_block_ids()}; }

void define_result_variables(const std::string& path, const ResultsMeshView& mesh, const std::vector<std::string>& nodal_names, const std::vector<std::string>& element_names, const std::vector<std::string>& global_names) {
    ...
    check_exodus(ex_put_truth_table(file.id(), EX_ELEM_BLOCK, static_cast<int>(mesh.blocks.size()), static_cast<int>(element_names.size()), truth.data()), "Could not define Exodus element-variable truth table");
    file.close();
}

void define_variables(const std::string& path, const UnstructuredQuad4Mesh& mesh, std::vector<std::string> nodal_names, std::vector<std::string> element_names, std::vector<std::string> global_names) {
...
void define_variables(const std::string& path, const UnstructuredHex8Mesh& mesh, std::vector<std::string> nodal_names, std::vector<std::string> element_names, std::vector<std::string> global_names) {
...
void write_result_step(const std::string& path, const ResultsMeshView& mesh, std::size_t step, double time, const std::vector<std::vector<double>>& nodal_values, const std::vector<std::vector<double>>& element_values,
                       const std::vector<double>& global_values) {
    if (step == 0 || nodal_values.empty() || element_values.empty() || global_values.empty()) throw std::invalid_argument("Exodus result step is incomplete");
    ExodusFile file = open_results(path);
    ...
    check_exodus(ex_put_var(file.id(), exodus_step, EX_GLOBAL, 1, 0, static_cast<std::int64_t>(global_values.size()), global_values.data()), "Could not write Exodus global results");
    for (std::size_t variable = 0; variable < nodal_values.size(); ++variable) {
        if (nodal_values[variable].size() != mesh.node_count) throw std::invalid_argument("Exodus nodal result size does not match mesh");
        check_exodus(ex_put_var(file.id(), exodus_step, EX_NODAL, static_cast<int>(variable + 1), 1, static_cast<std::int64_t>(mesh.node_count), nodal_values[variable].data()), "Could not write Exodus nodal results");
    }
    ...
    for (std::size_t variable = 0; variable < element_values.size(); ++variable) {
        if (element_values[variable].size() != mesh.element_count) throw std::invalid_argument("Exodus element result size does not match mesh");
        for (std::size_t block = 0; block < mesh.blocks.size(); ++block) {
            ...
            check_exodus(ex_put_var(file.id(), exodus_step, EX_ELEM_BLOCK, static_cast<int>(variable + 1), mesh.blocks[block].id, static_cast<std::int64_t>(values.size()), values.data()), "Could not write Exodus element results");
        }
    }
...
void write_step(const std::string& path, const UnstructuredQuad4Mesh& mesh, std::size_t step, double time, const std::vector<std::vector<double>>& nodal_values, const std::vector<std::vector<double>>& element_values, const std::vector<double>& global_values) {
    write_result_step(path, results_mesh_view(mesh), step, time, nodal_values, element_values, global_values);
}

void write_step(const std::string& path, const UnstructuredHex8Mesh& mesh, std::size_t step, double time, const std::vector<std::vector<double>>& nodal_values, const std::vector<std::vector<double>>& element_values, const std::vector<double>& global_values) {
    write_result_step(path, results_mesh_view(mesh), step, time, nodal_values, element_values, global_values);
}

void fill_region_nodal_values(const RegionMesh& region_mesh, std::size_t region_offset, const DofMap& dof_map, const std::vector<double>& state, std::vector<bool>& present, std::vector<std::vector<double>>& values) {
...
void fill_contact_nodal_values(std::size_t contact, const std::vector<std::size_t>& nodes, const std::vector<ContactNodeSummary>& summary, std::vector<std::vector<double>>& values) {
...
void fill_steady_nodal(const UnstructuredQuad4Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state, std::vector<std::vector<double>>& values) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    values.assign(nodal_variable_names(fuelsim::rz::ProblemAccess::definition(problem).contacts).size(), std::vector<double>(mesh.nodes().size(), missing));
    ...
    for (std::size_t region = 0; region < fuelsim::rz::ProblemAccess::region_count(problem); ++region)
        fill_region_nodal_values(fuelsim::rz::ProblemAccess::region_mesh(problem, region), fuelsim::rz::ProblemAccess::region_node_offset(problem, region), fuelsim::rz::ProblemAccess::dof_map(problem), state, present, values);
    for (std::size_t contact = 0; contact < fuelsim::rz::ProblemAccess::contact_count(problem); ++contact) {
        const std::vector<std::size_t> nodes = fuelsim::rz::ProblemAccess::contact_secondary_source_nodes(problem, contact);
        const std::vector<ContactNodeSummary> summary = fuelsim::rz::ProblemAccess::summarize_contact_nodes(problem, contact, state);
        fill_contact_nodal_values(contact, nodes, summary, values);
    }
}

void fill_transient_nodal(const UnstructuredQuad4Mesh& mesh, const TransientProblem& problem, std::vector<std::vector<double>>& values) {
    const std::vector<ContactDefinition>& contact_definitions = fuelsim::rz::ProblemAccess::definition(problem).spatial.contacts;
...
void store_stress_values(std::size_t source, const std::array<AxisymmetricStressValues, 4>& stresses, std::vector<std::vector<double>>& values) {
...
std::vector<std::vector<double>> steady_elements(const UnstructuredQuad4Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(stress_variable_names().size(), std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region = 0; region < fuelsim::rz::ProblemAccess::region_count(problem); ++region) {
        ...
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) {
            const LocalValues local = fuelsim::rz::ProblemAccess::contribution_state(problem, contribution_offset + element, state);
            const auto stresses = fuelsim::rz::ProblemAccess::region_kernel(problem, region).stress_values(fuelsim::rz::ProblemAccess::region_element_geometry(problem, region, element), local);
...
std::vector<std::vector<double>> transient_elements(const UnstructuredQuad4Mesh& mesh, const TransientProblem& problem) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(transient_element_variable_names().size(), std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region = 0; region < fuelsim::rz::ProblemAccess::region_count(problem); ++region) {
        const RegionMesh& region_mesh = fuelsim::rz::ProblemAccess::region_mesh(problem, region);
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) {
            const std::size_t source = region_mesh.source_element_ids().at(element);
            const Quad4MaterialHistory& history = fuelsim::rz::ProblemAccess::material_history(problem, region, element);
...
void fill_cartesian_region_nodal(const Hex8RegionMesh& region_mesh, std::size_t region_offset, const Hex8DofMap& dof_map, const std::vector<double>& state, std::vector<bool>& present, std::vector<std::vector<double>>& values) {
    for (std::size_t local = 0; local < region_mesh.nodes().size(); ++local) {
        const std::size_t source = region_mesh.source_node_ids()[local];
...
void fill_cartesian_steady_nodal(const UnstructuredHex8Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state, std::vector<std::vector<double>>& values) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    values.assign(4, std::vector<double>(mesh.nodes().size(), missing));
    std::vector<bool> present(mesh.nodes().size(), false);
    for (std::size_t region_value = 0; region_value < cartesian3d::ProblemAccess::region_count(problem); ++region_value)
        fill_cartesian_region_nodal(cartesian3d::ProblemAccess::region_mesh(problem, region_value), cartesian3d::ProblemAccess::region_node_offset(problem, region_value), cartesian3d::ProblemAccess::dof_map(problem), state, present, values);
}

void fill_cartesian_transient_nodal(const UnstructuredHex8Mesh& mesh, const TransientProblem& problem, std::vector<std::vector<double>>& values) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    values.assign(4, std::vector<double>(mesh.nodes().size(), missing));
    std::vector<bool> present(mesh.nodes().size(), false);
    for (std::size_t region_value = 0; region_value < cartesian3d::ProblemAccess::region_count(problem); ++region_value)
        fill_cartesian_region_nodal(cartesian3d::ProblemAccess::region_mesh(problem, region_value), cartesian3d::ProblemAccess::region_node_offset(problem, region_value), cartesian3d::ProblemAccess::dof_map(problem), problem.committed_solution(), present,
                                    values);
}

void store_cartesian_stress_values(std::size_t source, const std::array<SymmetricTensor3Values, 8>& stresses, std::vector<std::vector<double>>& values) {
    for (std::size_t q = 0; q < 8; ++q) {
...
std::vector<std::vector<double>> cartesian_steady_elements(const UnstructuredHex8Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(48, std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region_value = 0; region_value < cartesian3d::ProblemAccess::region_count(problem); ++region_value) {
        const Hex8RegionMesh& region_mesh = cartesian3d::ProblemAccess::region_mesh(problem, region_value);
        const std::size_t global_offset = cartesian3d::ProblemAccess::region_node_offset(problem, region_value);
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) {
            std::array<std::size_t, 8> nodes{};
            for (std::size_t node = 0; node < 8; ++node) nodes[node] = global_offset + region_mesh.elements()[element].nodes[node];
            const Hex8LocalDofs local_dofs = cartesian3d::ProblemAccess::dof_map(problem).local_dofs(nodes);
            Hex8LocalValues local_state{};
            for (std::size_t local = 0; local < 32; ++local) local_state[local] = state.at(local_dofs[local]);
            const auto stresses = cartesian3d::ProblemAccess::region_kernel(problem, region_value).stress_values(cartesian3d::ProblemAccess::region_element_geometry(problem, region_value, element), local_state);
...
std::vector<std::vector<double>> cartesian_transient_elements(const UnstructuredHex8Mesh& mesh, const TransientProblem& problem) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(48, std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region_value = 0; region_value < cartesian3d::ProblemAccess::region_count(problem); ++region_value) {
        const Hex8RegionMesh& region_mesh = cartesian3d::ProblemAccess::region_mesh(problem, region_value);
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) store_cartesian_stress_values(region_mesh.source_element_ids()[element], cartesian3d::ProblemAccess::stress(problem, region_value, element), result);
    }
    return result;
}
...
EngineeringHistoryWriter::EngineeringHistoryWriter(std::string path, const TransientProblem& problem) : _path(std::move(path)), _problem_signature(transient_problem_signature(problem)) {
    ...
    const bool cartesian = problem.is_cartesian_3d();
    const std::size_t regions = cartesian ? cartesian3d::ProblemAccess::region_count(problem) : fuelsim::rz::ProblemAccess::region_count(problem);
    for (std::size_t region = 0; region < regions; ++region) {
        const std::string name = cartesian ? cartesian3d::ProblemAccess::region(problem, region).name : fuelsim::rz::ProblemAccess::region(problem, region).name;
        const std::string prefix = ",region_" + name;
        _stream << prefix << "_maximum_temperature" << prefix << "_maximum_equivalent_plastic_strain" << prefix << "_maximum_equivalent_creep_strain";
    }
    const std::vector<ContactDefinition>& contacts = cartesian ? cartesian3d::ProblemAccess::definition(problem).spatial.contacts : fuelsim::rz::ProblemAccess::definition(problem).spatial.contacts;
    for (const ContactDefinition& contact : contacts) {
        const std::string prefix = ",contact_" + contact.name;
        _stream << prefix << "_minimum_gap" << prefix << "_maximum_pressure" << prefix << "_total_heat_rate" << prefix << "_total_force" << prefix << "_total_tangential_force";
    }
    _stream << '\n' << std::scientific << std::setprecision(12);
}

void EngineeringHistoryWriter::append(const TransientProblem& problem, double time_step, double next_time_step, int nonlinear_iterations) {
    if (problem.time_step_active()) throw std::logic_error("Engineering history cannot be written during an active time step");
    if (transient_problem_signature(problem) != _problem_signature) throw std::invalid_argument("Engineering history problem does not match writer model");
    _stream << problem.committed_time() << ',' << time_step << ',' << next_time_step << ',' << problem.committed_load_factor() << ',' << nonlinear_iterations;
    const TransientConservationSummary& conservation = problem.last_conservation_summary();
    for (const TransientConservationField& field : transient_conservation_fields) _stream << ',' << conservation.*field.member;
    const std::vector<double>& state = problem.committed_solution();
    const bool cartesian = problem.is_cartesian_3d();
    const std::size_t regions = cartesian ? cartesian3d::ProblemAccess::region_count(problem) : fuelsim::rz::ProblemAccess::region_count(problem);
    for (std::size_t region = 0; region < regions; ++region) {
        double maximum_temperature = -std::numeric_limits<double>::infinity();
        const std::size_t offset = cartesian ? cartesian3d::ProblemAccess::region_node_offset(problem, region) : fuelsim::rz::ProblemAccess::region_node_offset(problem, region);
        const std::size_t nodes = cartesian ? cartesian3d::ProblemAccess::region_mesh(problem, region).nodes().size() : fuelsim::rz::ProblemAccess::region_mesh(problem, region).nodes().size();
        for (std::size_t local = 0; local < nodes; ++local) {
            const std::size_t dof = cartesian ? cartesian3d::ProblemAccess::dof_map(problem).temperature(offset + local) : fuelsim::rz::ProblemAccess::dof_map(problem).temperature(offset + local);
            maximum_temperature = std::max(maximum_temperature, state.at(dof));
        }
        const RegionInelasticSummary history = cartesian ? RegionInelasticSummary{0.0, 0.0} : fuelsim::rz::ProblemAccess::summarize_region_history(problem, region);
        _stream << ',' << maximum_temperature << ',' << history.maximum_equivalent_plastic_strain << ',' << history.maximum_equivalent_creep_strain;
    }
    const std::size_t contacts = cartesian ? 0 : fuelsim::rz::ProblemAccess::definition(problem).spatial.contacts.size();
    for (std::size_t contact = 0; contact < contacts; ++contact) {
        const InterfaceSummary summary = fuelsim::rz::ProblemAccess::summarize_interface(problem, contact, state);
        _stream << ',' << summary.minimum_gap << ',' << summary.maximum_contact_pressure << ',' << summary.total_heat_rate << ',' << summary.total_contact_force << ',' << summary.total_tangential_force;
    }
    _stream << '\n';
    _stream.flush();
}

void ExodusResultsIo::write_steady(const std::string& path, const UnstructuredQuad4Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state) {
    if (path.empty()) throw std::invalid_argument("Exodus result path must not be empty");
    const std::vector<std::string> nodal = nodal_variable_names(fuelsim::rz::ProblemAccess::definition(problem).contacts);
    const std::vector<std::string> element = stress_variable_names();
    const std::vector<std::string> global = global_variable_names(fuelsim::rz::ProblemAccess::definition(problem).contacts);
...
void ExodusResultsIo::write_steady(const std::string& path, const UnstructuredHex8Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state) {
    if (path.empty()) throw std::invalid_argument("Exodus result path must not be empty");
    ...
    write_step(path, mesh, 1, 1.0, nodal_values, cartesian_steady_elements(mesh, problem, state), {problem.load_factor()});
}

ExodusTransientResultsWriter::ExodusTransientResultsWriter(std::string path, UnstructuredQuad4Mesh mesh, const TransientProblem& problem)
    : _path(std::move(path)), _rz_mesh(std::make_unique<UnstructuredQuad4Mesh>(std::move(mesh))), _problem_signature(transient_problem_signature(problem)), _step_count(0) {
    if (_path.empty()) throw std::invalid_argument("Exodus result path must not be empty");
    define_variables(_path, *_rz_mesh, nodal_variable_names(fuelsim::rz::ProblemAccess::definition(problem).spatial.contacts), transient_element_variable_names(), global_variable_names(fuelsim::rz::ProblemAccess::definition(problem).spatial.contacts));
}

ExodusTransientResultsWriter::ExodusTransientResultsWriter(std::string path, UnstructuredHex8Mesh mesh, const TransientProblem& problem)
    : _path(std::move(path)), _hex_mesh(std::make_unique<UnstructuredHex8Mesh>(std::move(mesh))), _problem_signature(transient_problem_signature(problem)), _step_count(0) {
    if (_path.empty()) throw std::invalid_argument("Exodus result path must not be empty");
    define_variables(_path, *_hex_mesh, cartesian_nodal_variable_names(), cartesian_stress_variable_names(), {"load_factor"});
}

void ExodusTransientResultsWriter::append(const TransientProblem& problem) {
    if (problem.time_step_active()) throw std::logic_error("Exodus results cannot be written during an active time step");
    if (transient_problem_signature(problem) != _problem_signature) throw std::invalid_argument("Exodus result problem does not match writer model");
    ++_step_count;
    std::vector<std::vector<double>> nodal_values;
    if (_hex_mesh) {
        fill_cartesian_transient_nodal(*_hex_mesh, problem, nodal_values);
        write_step(_path, *_hex_mesh, _step_count, problem.committed_time(), nodal_values, cartesian_transient_elements(*_hex_mesh, problem), {problem.committed_load_factor()});
    } else {
        fill_transient_nodal(*_rz_mesh, problem, nodal_values);
        write_step(_path, *_rz_mesh, _step_count, problem.committed_time(), nodal_values, transient_elements(*_rz_mesh, problem), transient_globals(problem));
    }
}
...
constexpr std::array<unsigned char, 16> checkpoint_magic = {'F', 'U', 'E', 'L', 'S', 'I', 'M', '_', 'C', 'H', 'E', 'C', 'K', 'P', 'T', '\0'};
constexpr std::uint32_t checkpoint_version = 7U;
...
class BinaryCursor final {
  public:
    ...
    std::uint32_t read_u32() {
        ...
        for (std::size_t byte = 0; byte < 4; ++byte) result |= static_cast<std::uint32_t>(_bytes[_position++]) << (8U * byte);
        return result;
    }
    ...
    std::uint64_t read_u64() {
        ...
        for (std::size_t byte = 0; byte < 8; ++byte) result |= static_cast<std::uint64_t>(_bytes[_position++]) << (8U * byte);
        return result;
    }
...
// strains, stress rr/zz/hoop/rz) is the RZ part of the checkpoint version 7 byte contract;
// append_material_point and read_material_point must stay symmetric.
void append_material_point(BinaryBuffer& payload, const MaterialPointState& state, const AxisymmetricStressValues& stress) {
...
void append_conservation(BinaryBuffer& payload, const TransientConservationSummary& summary) {
    for (const TransientConservationField& field : transient_conservation_fields) payload.append_double(summary.*field.member);
}
...
TransientConservationSummary read_conservation(BinaryCursor& payload) {
    ...
    for (const TransientConservationField& field : transient_conservation_fields) {
        ...
        if (!std::isfinite(result.*field.member)) throw std::runtime_error("Checkpoint conservation summary is invalid");
    }
    return result;
}
...
BinaryBuffer state_payload(const TransientProblem& problem, double next_time_step) {
    ...
    if (cartesian) {
        const cartesian3d::TransientCommittedState state = cartesian3d::ProblemAccess::committed_state(problem);
        ...
        payload.append_u64(static_cast<std::uint64_t>(state.stresses.size()));
        for (const auto& region : state.stresses) {
            payload.append_u64(static_cast<std::uint64_t>(region.size()));
            for (const auto& element : region) {
                for (const SymmetricTensor3Values& stress : element) {
                    payload.append_double(stress.xx);
                    payload.append_double(stress.yy);
                    payload.append_double(stress.zz);
                    payload.append_double(stress.xy);
                    payload.append_double(stress.yz);
                    payload.append_double(stress.xz);
                }
            }
        }
        return payload;
    ...
    for (std::size_t region = 0; region < state.material_histories.size(); ++region) {
        ...
        for (std::size_t element = 0; element < state.material_histories[region].size(); ++element) {
            for (std::size_t q = 0; q < 4; ++q) append_material_point(payload, state.material_histories[region][element][q], state.material_stresses[region][element][q]);
        }
    }
    return payload;
}
...
std::vector<unsigned char> read_file(const std::string& path) {
    ...
    if (length < 0 || static_cast<std::uint64_t>(length) > maximum_checkpoint_bytes) throw std::runtime_error("Checkpoint size is invalid: " + path);
...
void write_atomic(const std::string& path, const std::vector<unsigned char>& bytes) {
    ...
    try {
        ...
        if (std::rename(temporary.c_str(), path.c_str()) != 0) throw std::runtime_error("Could not install checkpoint '" + path + "': " + std::strerror(errno));
    } catch (...) {
...
void TransientCheckpointIo::write(const std::string& path, const TransientProblem& problem, double next_time_step) {
    ...
    if (!std::isfinite(next_time_step) || !(next_time_step > 0.0)) throw std::invalid_argument("Checkpoint next time step must be finite and positive");
...
double TransientCheckpointIo::restore(const std::string& path, TransientProblem& problem) {
    ...
    if (checksum(payload_bytes) != expected_checksum) throw std::runtime_error("Checkpoint payload checksum does not match");

    BinaryCursor payload(payload_bytes);
    if (payload.read_u64() != transient_problem_signature(problem)) throw std::runtime_error("Checkpoint model signature does not match the current problem");
    const std::uint32_t geometry_tag = payload.read_u32();
    const bool cartesian = problem.is_cartesian_3d();
    if (cartesian) {
        if (geometry_tag != cartesian_geometry_tag) throw std::runtime_error("Checkpoint geometry does not match the current problem");
        ...
        if (!std::isfinite(next_time_step) || !(next_time_step > 0.0)) throw std::runtime_error("Checkpoint next time step is invalid");
        if (payload.read_u64() != problem.dof_count()) throw std::runtime_error("Checkpoint solution size does not match the current problem");
        state.solution.resize(problem.dof_count());
        for (double& value : state.solution) value = payload.read_double();
        const std::size_t regions = cartesian3d::ProblemAccess::region_count(problem);
        if (payload.read_u64() != regions) throw std::runtime_error("Checkpoint region count does not match the current problem");
        state.stresses.resize(regions);
        for (std::size_t region = 0; region < regions; ++region) {
            const std::size_t elements = cartesian3d::ProblemAccess::region_mesh(problem, region).elements().size();
            if (payload.read_u64() != elements) throw std::runtime_error("Checkpoint element count does not match the current problem");
            state.stresses[region].resize(elements);
            for (auto& element : state.stresses[region]) {
                for (SymmetricTensor3Values& stress : element) {
                    stress.xx = payload.read_double();
                    stress.yy = payload.read_double();
                    stress.zz = payload.read_double();
                    stress.xy = payload.read_double();
                    stress.yz = payload.read_double();
                    stress.xz = payload.read_double();
                }
            }
        }
        if (!payload.at_end()) throw std::runtime_error("Checkpoint payload contains trailing data");
        cartesian3d::ProblemAccess::restore_committed_state(problem, std::move(state));
        return next_time_step;
    }
    if (geometry_tag != rz_geometry_tag) throw std::runtime_error("Checkpoint geometry does not match the current problem");
    ...
    if (solution_size != problem.dof_count()) throw std::runtime_error("Checkpoint solution size does not match the current problem");
    state.solution.resize(problem.dof_count());
    for (double& value : state.solution) value = payload.read_double();
    ...
    if (contact_count != expected_histories.size()) throw std::runtime_error("Checkpoint contact count does not match the current problem");
    ...
    for (std::size_t contact = 0; contact < expected_histories.size(); ++contact) {
        ...
        for (ContactPointHistory& history : state.contact_histories[contact]) {
            ...
            if (!std::isfinite(history.normal_multiplier) || history.normal_multiplier < 0.0) throw std::runtime_error("Checkpoint normal contact multiplier is invalid");
        }
    }
    const std::uint64_t region_count = payload.read_u64();
    if (region_count != fuelsim::rz::ProblemAccess::region_count(problem)) throw std::runtime_error("Checkpoint region count does not match the current problem");
    ...
    for (std::size_t region = 0; region < fuelsim::rz::ProblemAccess::region_count(problem); ++region) {
        ...
        for (std::size_t element = 0; element < elements; ++element) {
            for (std::size_t q = 0; q < 4; ++q) read_material_point(payload, state.material_histories[region][element][q], state.material_stresses[region][element][q]);
        }
    }
    if (!payload.at_end()) throw std::runtime_error("Checkpoint payload contains trailing data");
