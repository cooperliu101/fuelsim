...
Cax4tSystem compute_cax4t_system(const Quad4RzData& data,
    const Quad4RzGeometry& geometry,
    const LocalValues& state,
    const LocalValues& old_state,
    const Quad4MaterialHistory* old_history,
    double time_step,
    bool jacobian,
    bool thermal_time) {
...
LocalResidual compute_quad4_rz_transient(const Quad4RzData& data,
    ...
    if (data.element_formulation == RzElementFormulation::cax4t) {
        const auto result = compute_cax4t_system(data,
            geometry,
            current_state,
            committed_state,
            &committed_material,
...
Quad4MaterialHistory compute_quad4_rz_transient_update(const Quad4RzData& data,
    ...
    if (data.element_formulation == RzElementFormulation::cax4rt)
        return rz::compute_cax4rt(data,
            ...
            .history;
    if (data.element_formulation == RzElementFormulation::cax4t)
        return compute_cax4t_system(data,
            geometry,
            converged_state,
            committed_state,
            &committed_material,
