...
namespace fuelsim {
UnstructuredQuad8Mesh::UnstructuredQuad8Mesh(std::vector<RzPoint> nodes, std::vector<Quad8Element> elements,
    std::vector<std::int64_t> ids, std::vector<ElementBlockInfo> blocks, std::vector<NodeSet> node_sets,
    std::vector<SideSet> side_sets)
    : UnstructuredMeshMetadata(nodes.size(), elements.size(), 4, std::move(ids), std::move(blocks),
          std::move(node_sets), std::move(side_sets), "UnstructuredQuad8Mesh"),
    ...
    for (const auto& element : _elements) {
        ...
        for (std::size_t n = 0; n < 8; ++n) {
            ...
            if (node >= _nodes.size()) throw std::out_of_range("QUAD8 connectivity is out of range");
...
Quad8RegionMesh Quad8RegionMesh::from_unstructured_block(const UnstructuredQuad8Mesh& source, std::int64_t block) {
    ...
    for (auto e : mesh._source_element_ids) {
        ...
        for (auto n : element.nodes) used[n] = true;
        for (std::size_t n = 0; n < 4; ++n) thermal[element.nodes[n]] = true;
    }
...
Quad8RegionBoundary Quad8RegionMesh::map_side_set(const UnstructuredQuad8Mesh& source, const std::string& name) const {
    ...
    for (const auto& side : source.side_set(name).sides) {
        ...
        if (e == invalid_index) throw std::invalid_argument("QUAD8 side is outside its region");
    ...
    result.temperature_nodes.erase(
        std::unique(result.temperature_nodes.begin(), result.temperature_nodes.end()), result.temperature_nodes.end());
