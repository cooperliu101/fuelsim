...
SpatialAssembly::SpatialAssembly(SpatialDefinition definition, const UnstructuredQuad8Mesh& source)
    ...
    for (std::size_t b = 0; b < _definition.boundary_conditions.size(); ++b) {
        ...
        if (bc.type == BoundaryConditionType::dirichlet) {
            ...
            const auto set = std::find_if(source.node_sets().begin(), source.node_sets().end(), [&](const NodeSet& s) {
                return s.name == bc.boundary;
            });
            if (set != source.node_sets().end())
                ...
                for (const auto& side : source.side_set(bc.boundary).sides) {
                    ...
                    selected.insert(selected.end(),
                        {n[side.local_side], n[(side.local_side + 1) % 4], n[4 + side.local_side]});
                }
            }
            ...
            for (std::size_t r = 0; r < region_count(); ++r)
                for (std::size_t n = 0; n < _meshes[r].nodes().size(); ++n) {
                    if (std::find(selected.begin(), selected.end(), _meshes[r].source_node_ids()[n]) == selected.end())
                        continue;
                    if (bc.field == Field::temperature && !_meshes[r].temperature_nodes()[n])
                        continue;
                    selected_dofs.push_back(dof(bc.field,
                        bc.field == Field::temperature ? global_temperature_node(r, n) : global_node(r, n)));
                }
    ...
    spatial_detail::validate_dirichlet_conditions(_dirichlet_conditions,
        "Conflicting CAX8T Dirichlet conditions",
        "Duplicate CAX8T Dirichlet conditions");
...
void SpatialAssembly::contribution_dofs(std::size_t index, std::vector<std::size_t>& dofs) const {
    ...
    if (index < volume_contribution_count()) {
        ...
        for (auto f : {Field::radial_displacement, Field::axial_displacement})
            for (auto n : nodes)
                dofs.push_back(dof(f, global_node(r, n)));
    } else {
        ...
        for (auto f : {Field::radial_displacement, Field::axial_displacement})
            for (auto n : b.edge.nodes)
                dofs.push_back(dof(f, global_node(b.region, n)));
    }
}
...
std::vector<std::size_t> SpatialAssembly::required_state_dofs(std::size_t first, std::size_t last) const {
    if (first > last || last > contribution_count())
        throw std::out_of_range("CAX8T contribution range");
...
void SpatialAssembly::validate_local_state(std::size_t first,
    ...
    if (state.size() != dof_count())
        throw std::invalid_argument("CAX8T state layout mismatch");
    const auto dofs = required_state_dofs(first, last);
    for (auto d : dofs)
        if (!std::isfinite(state[d]))
            throw std::domain_error("CAX8T state must be finite");
    for (auto d : dofs)
    ...
    for (auto index = first; index < std::min(last, volume_contribution_count()); ++index) {
        const auto [r, e] = element_location(index);
        if (region(r).strain_formulation != StrainFormulation::finite)
            continue;
...
void SpatialAssembly::compute_boundary(std::size_t index,
    ...
    if (index >= offset) {
        ...
        if (state.size() != 16)
            throw std::invalid_argument("CAX8T contact contribution layout mismatch");
        residual.assign(16, 0);
        if (jacobian)
            jacobian->assign(256, 0);
        if (_active_candidates.at(candidate.point) != c)
            return;
        std::vector<double> old(16);
        for (std::size_t i = 0; i < 16; ++i)
            old[i] = _committed_contact_solution[candidate.dofs[i]];
        const auto result = compute_line3_contact(candidate.geometry,
