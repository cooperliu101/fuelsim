...
namespace fuelsim {
void Rz4Backend::accumulate_time_error(const TransientCommittedState& full,
    const TransientCommittedState& half,
    const TransientTimeOptions& options,
    TransientTimeErrorEstimate& result) const {
    if (full.material_histories.size() != half.material_histories.size())
        throw std::logic_error("step-doubling material region layouts differ");
    MaterialTimeErrors material;
    for (std::size_t r = 0; r < full.material_histories.size(); ++r) {
        const auto& first = full.material_histories[r];
        const auto& second = half.material_histories[r];
        if (first.size() != second.size())
            throw std::logic_error("step-doubling material element layouts differ");
        for (std::size_t e = 0; e < first.size(); ++e) {
            const std::size_t count =
                _spatial.definition().regions.at(r).rz_element_formulation == RzElementFormulation::cax4rt ? 1 : 4;
