...
Quad4FaceCoordinates face_coordinates(const Hex8RegionMesh& mesh, const Quad4FaceElement& face) {
    ...
    for (std::size_t node = 0; node < 4; ++node)
        result[node] = mesh.nodes().at(face.nodes[node]);
    return result;
}
...
SurfacePullbackCovectors surface_pullback_covectors(const Quad4FaceQuadraturePoint& current,
    const Quad4FaceQuadraturePoint& reference,
    double normal_orientation,
    double tangent_orientation,
    const CartesianPoint3& current_normal,
    const CartesianPoint3& current_first) {
...
std::array<double, 2> quad8_reference_tangent_coordinates(const Quad8FaceMechanicalQuadraturePoint& point,
    ...
    const CartesianPoint3 first{axis.x - normal_component * normal.x,
        ...
        axis.z - normal_component * normal.z};
    const double g11 = dot(point.tangent_xi, point.tangent_xi), g12 = dot(point.tangent_xi, point.tangent_eta),
                 g22 = dot(point.tangent_eta, point.tangent_eta), determinant = g11 * g22 - g12 * g12;
    if (!(determinant > 0.0))
        throw std::domain_error("HEX20 reference contact tangent metric is singular");
    return {(g22 * dot(first, point.tangent_xi) - g12 * dot(first, point.tangent_eta)) / determinant,
        (g11 * dot(first, point.tangent_eta) - g12 * dot(first, point.tangent_xi)) / determinant};
}
...
const Matrix4& abaqus_quad4_averaging() {
    static const Matrix4 value = {{9.0 / 16.0,
        3.0 / 16.0,
        1.0 / 16.0,
        3.0 / 16.0,
        3.0 / 16.0,
...
Vector4 solve4(Matrix4 matrix, Vector4 right_hand_side) {
    ...
    for (std::size_t reverse = 0; reverse < 4; ++reverse) {
        ...
        for (std::size_t column = row + 1; column < 4; ++column)
            value -= matrix[row * 4 + column] * result[column];
...
const Matrix8& abaqus_quad8_pressure_recovery() {
    ...
    static const Matrix8 value = {{17.0 / 24.0,
        -3.0 / 24.0,
        1.0 / 24.0,
        -3.0 / 24.0,
        7.0 / 24.0,
...
std::array<double, 3> thermal_search_point(const Quad4ToQuad4HeatGeometry& geometry,
...
std::array<double, 3> mechanical_search_point(const NodeToQuad4ContactGeometry& geometry,
    const Quad4SurfaceContactLocalValues& state) {
...
std::array<double, 3> mechanical_search_point(const Quad4ToQuad4MechanicalGeometry& geometry,
    const Quad4SurfaceContactLocalValues& state) {
    std::array<double, 3> result{};
    for (std::size_t node = 0; node < 4; ++node) {
        result[0] += geometry.secondary_shape[node] * (geometry.secondary_coordinates[node].x + state[8 + node]);
...
std::array<double, 3> hex20_thermal_search_point(const Quad8ToQuad8HeatGeometry& geometry,
...
std::array<double, 3> hex20_mechanical_search_point(const NodeToQuad8ContactGeometry& geometry,
    const Quad8SurfaceContactLocalValues& state) {
    const std::size_t node = geometry.secondary_local_node;
    return {geometry.secondary_coordinates[node].x + state[8 + node],
...
std::array<double, 3> hex20_mechanical_search_point(const Quad8ToQuad8MechanicalGeometry& geometry,
    const Quad8SurfaceContactLocalValues& state) {
    std::array<double, 3> result{};
    for (std::size_t node = 0; node < 8; ++node) {
        result[0] +=
...
double normal_orientation(const Quad4FaceCoordinates& primary_coordinates,
    const CartesianPoint3& secondary_parent_centroid,
    const CartesianPoint3& primary_parent_centroid) {
...
double primary_material_orientation(const Quad4FaceCoordinates& primary_coordinates,
    const CartesianPoint3& primary_parent_centroid) {
...
double minimum_normal_length(const Hex8RegionMesh& mesh, const Hex8RegionBoundary& boundary, const std::string& name) {
    ...
    if (!std::isfinite(result))
        throw std::invalid_argument("Three-dimensional contact boundary is empty: " + name);
    return result;
}
...
double primary_material_orientation(const Quad8FaceCoordinates& primary_coordinates,
    const CartesianPoint3& primary_parent_centroid) {
    return normal_orientation(primary_coordinates, quad8_face_centroid(primary_coordinates), primary_parent_centroid);
}

double
minimum_normal_length(const Hex20RegionMesh& mesh, const Hex20RegionBoundary& boundary, const std::string& name) {
    double result = std::numeric_limits<double>::infinity();
    for (const Quad8FaceElement& face : boundary.faces) {
        ...
        const Quad8FaceMechanicalQuadraturePoint& center = geometry.mechanical_points[4];
        const CartesianPoint3 area{center.tangent_xi.y * center.tangent_eta.z
                                       - center.tangent_xi.z * center.tangent_eta.y,
            center.tangent_xi.z * center.tangent_eta.x - center.tangent_xi.x * center.tangent_eta.z,
            center.tangent_xi.x * center.tangent_eta.y - center.tangent_xi.y * center.tangent_eta.x};
        const double measure = std::sqrt(dot(area, area));
        if (!std::isfinite(measure) || !(measure > 0.0))
    ...
    if (!std::isfinite(result))
        throw std::invalid_argument("HEX20 contact boundary is empty: " + name);
    return result;
}

spatial_detail::ContactSearchBox quad8_search_box(const Quad8FaceCoordinates& coordinates,
    const std::vector<double>& state,
    const std::array<std::array<std::size_t, 3>, 8>& displacement_dofs) {
    ...
    for (std::size_t component = 0; component < 3; ++component) {
        ...
        for (std::size_t i = 0; i < 3; ++i) {
            const double values[3] = {x_bernstein[i][0],
                2.0 * x_bernstein[i][1] - 0.5 * (x_bernstein[i][0] + x_bernstein[i][2]),
                x_bernstein[i][2]};
...
void set_pattern_block(std::vector<unsigned char>& pattern,
    std::size_t size,
    ...
    for (std::size_t row = row_begin; row < row_end; ++row)
        std::fill(pattern.begin() + static_cast<std::ptrdiff_t>(row * size + column_begin),
            pattern.begin() + static_cast<std::ptrdiff_t>(row * size + column_end),
            1U);
}

std::pair<std::size_t, std::size_t>
offset_location(const std::vector<std::size_t>& offsets, std::size_t index, const char* message) {
...
Quad4FaceBoundaryData make_boundary_data(const BoundaryConditionDefinition& boundary, bool use_displaced_geometry) {
    if (boundary.type == BoundaryConditionType::pressure)
        return {Quad4FaceBoundaryKind::pressure,
            CartesianTractionComponent::x,
            boundary.value,
            0.0,
    ...
    if (boundary.type == BoundaryConditionType::heat_flux)
        return {Quad4FaceBoundaryKind::surface_heat_flux,
            CartesianTractionComponent::x,
            boundary.value,
            0.0,
            use_displaced_geometry};
    return {Quad4FaceBoundaryKind::convection,
        CartesianTractionComponent::x,
...
SpatialContributionType boundary_contribution_type(BoundaryConditionType type) {
    if (type == BoundaryConditionType::pressure)
        return SpatialContributionType::pressure;
    if (type == BoundaryConditionType::traction)
        return SpatialContributionType::traction;
    if (type == BoundaryConditionType::heat_flux)
...
SpatialAssembly::SpatialAssembly(SpatialDefinition definition, const UnstructuredHex8Mesh& source_mesh)
    ...
    for (std::size_t boundary_index = 0; boundary_index < _definition.boundary_conditions.size(); ++boundary_index) {
        ...
        for (const Quad4FaceElement& face : mapped.faces) {
            ...
            for (std::size_t node = 0; node < 4; ++node)
                nodes[node] = global_node(region, face.nodes[node]);
            for (std::size_t node = 0; node < 4; ++node)
    ...
    for (std::size_t region_index = 0; region_index < region_count(); ++region_index) {
        _kernel_data.push_back({IsotropicThermoelasticMaterial(region(region_index).material),
            region_heat_source(region_index),
            0.0,
...
SpatialAssembly::SpatialAssembly(SpatialDefinition definition, const UnstructuredHex20Mesh& source_mesh)
    ...
    for (std::size_t boundary_index = 0; boundary_index < _definition.boundary_conditions.size(); ++boundary_index) {
        ...
        const bool side_set = std::any_of(source_mesh.side_sets().begin(),
            source_mesh.side_sets().end(),
            [&](const SideSet& set) { return set.name == boundary.boundary; });
        if (boundary.type == BoundaryConditionType::dirichlet && !side_set) {
            const auto set = std::find_if(source_mesh.node_sets().begin(),
                source_mesh.node_sets().end(),
                [&](const NodeSet& value) { return value.name == boundary.boundary; });
            if (set != source_mesh.node_sets().end()) {
                std::vector<std::size_t> prescribed;
                for (const std::size_t source : set->nodes) {
                    bool found = false;
                    for (std::size_t region = 0; region < _hex20_meshes.size(); ++region) {
                        const auto& mesh = _hex20_meshes[region];
                        const auto& nodes = mesh.source_node_ids();
                        const auto node = std::find(nodes.begin(), nodes.end(), source);
                        if (node == nodes.end())
                            continue;
                        const auto local = static_cast<std::size_t>(node - nodes.begin());
                        found = true;
                        if (boundary.field == Field::temperature && !mesh.temperature_nodes()[local])
                            continue;
                        const auto global = boundary.field == Field::temperature
                                                ? global_temperature_node(region, local)
                                                : global_node(region, local);
                        prescribed.push_back(dof(boundary.field, global));
                    }
                    if (!found)
                        throw std::invalid_argument(
                            "HEX20 Dirichlet node set contains an inactive mesh node: " + boundary.boundary);
                }
                if (prescribed.empty())
                    throw std::invalid_argument(
                        "HEX20 Dirichlet node set contains no active field nodes: " + boundary.boundary);
                std::sort(prescribed.begin(), prescribed.end());
                prescribed.erase(std::unique(prescribed.begin(), prescribed.end()), prescribed.end());
                for (const auto index : prescribed)
                    add_dirichlet(index, boundary_index);
                continue;
            }
        }
        const std::int64_t block_id = source_mesh.side_set_block_id(boundary.boundary);
        ...
        for (const Quad8FaceElement& face : mapped.faces) {
            ...
            for (std::size_t node = 0; node < 8; ++node) {
                ...
                if (node < 4)
                    temperature_nodes[node] = global_temperature_node(region, face.nodes[node]);
            }
            _hex20_boundary_contributions.push_back({boundary_contribution_type(boundary.type),
                kernel,
                temperature_nodes,
                displacement_nodes,
                make_quad8_face_geometry(coordinates)});
        }
    }
    spatial_detail::validate_dirichlet_conditions(_dirichlet_conditions,
        "HEX20 boundary has conflicting Dirichlet values",
        "HEX20 boundary has duplicate Dirichlet values");
...
SpatialContributionType SpatialAssembly::contribution_type(std::size_t index) const {
    ...
    if (index >= ranges.end)
        throw std::out_of_range("Three-dimensional contribution index is out of range");
...
const Hex20Geometry& SpatialAssembly::hex20_region_element_geometry(std::size_t region,
...
void SpatialAssembly::validate_state(const std::vector<double>& state) const {
    ...
    for (std::size_t region_index = 0; region_index < region_count(); ++region_index) {
        if (region(region_index).strain_formulation != StrainFormulation::finite)
            continue;
...
void SpatialAssembly::contribution_dofs(std::size_t index, std::vector<std::size_t>& dofs) const {
    ...
    if (index < ranges.mechanical_begin) {
        ...
        if (_uses_hex20) {
            hex20_thermal_patch_dofs(point, dofs);
            return;
        }
...
void SpatialAssembly::contribution_jacobian_pattern(std::size_t index, std::vector<unsigned char>& pattern) const {
    ...
    if (index < ranges.mechanical_begin) {
        if (_uses_hex20) {
            ...
            hex20_thermal_patch_dofs(index - ranges.thermal_begin, dofs);
            pattern.assign(dofs.size() * dofs.size(), 0U);
            for (std::size_t row = 0; row < dofs.size(); ++row)
                if (dofs[row] < dof(Field::displacement_x, 0))
                    set_pattern_block(pattern, dofs.size(), row, row + 1, 0, dofs.size());
            return;
        }
        pattern.assign(quad4_surface_contact_local_dof_count * quad4_surface_contact_local_dof_count, 0U);
        set_pattern_block(pattern,
...
void SpatialAssembly::sparsity_contribution_dofs(std::size_t index, std::vector<std::size_t>& dofs) const {
    ...
    const std::size_t thermal_patch_begin = _sparsity_contact_offsets.back() + averaged_sparsity_contribution_count();
    if (index >= thermal_patch_begin) {
        hex20_thermal_patch_dofs(index - thermal_patch_begin, dofs, true);
        return;
    }
...
void SpatialAssembly::sparsity_contribution_jacobian_pattern(std::size_t index,
...
Hex8LocalValues SpatialAssembly::volume_state(std::size_t index, const std::vector<double>& global_state) const {
    if (_uses_hex20)
        throw std::logic_error("HEX8 volume state requested from a HEX20 problem");
    if (index >= volume_contribution_count())
        throw std::out_of_range("HEX8 volume contribution is out of range");
    const auto location = element_location(index);
    const Hex8Element& element = _meshes[location.first].elements()[location.second];
...
Hex20LocalValues SpatialAssembly::hex20_volume_state(std::size_t index, const std::vector<double>& global_state) const {
    if (!_uses_hex20)
        throw std::logic_error("HEX20 volume state requested from a HEX8 problem");
    if (index >= volume_contribution_count())
        throw std::out_of_range("HEX20 volume contribution is out of range");
    const auto location = element_location(index);
    const Hex20Element& element = _hex20_meshes[location.first].elements()[location.second];
...
void SpatialAssembly::compute_contribution(std::size_t index,
    ...
    bool include_thermal_time_term) const {
    if (index < volume_contribution_count()) {
        if (_uses_hex20) {
    ...
    if (index < ranges.mechanical_begin) {
        if (_uses_hex20) {
            const std::size_t patch = index - ranges.thermal_begin;
            std::vector<std::size_t> dofs;
            hex20_thermal_patch_dofs(patch, dofs);
            if (state.size() != dofs.size())
                throw std::invalid_argument("HEX20 thermal patch state has the wrong size");
            residual = compute_quad8_gap_heat_patch(_thermal_properties[_hex20_thermal_patches[patch].contact],
                hex20_thermal_patch_samples(patch, dofs),
                state,
                jacobian);
            return;
        }
        ...
        const Quad4SurfaceContactLocalResidual result =
            compute_quad4_to_quad4_gap_heat(_thermal_properties[entry.contact],
                entry.geometry,
                current,
    ...
    if (index < ranges.boundary_begin) {
        const std::size_t point = index - ranges.mechanical_begin,
                          point_count = _uses_hex20 ? _hex20_mechanical_points.size() : _mechanical_points.size();
        if (point >= point_count) {
            compute_averaged_constraint(_abaqus_averaged_constraints.at(point - point_count),
                state,
                residual,
                jacobian);
            return;
        ...
        if (_uses_hex20) {
            ...
            const Quad8SurfaceContactLocalResidual result =
                entry.surface_to_surface ? compute_quad8_to_quad8_contact(_mechanical_properties[entry.contact],
                                               entry.surface_geometry,
                                               current,
                                               committed,
                                               _contact_histories[entry.contact][entry.secondary],
                                               jacobian == nullptr ? nullptr : &local_jacobian)
                                         : compute_node_to_quad8_contact(_mechanical_properties[entry.contact],
                                               entry.node_geometry,
                                               current,
                                               committed,
                                               _contact_histories[entry.contact][entry.secondary],
        ...
        const Quad4SurfaceContactLocalResidual result =
            entry.surface_to_surface ? compute_quad4_to_quad4_contact(point_properties,
                                           ...
                                           _contact_histories[entry.contact][entry.secondary],
                                           jacobian == nullptr ? nullptr : &local_jacobian)
                                     : compute_node_to_quad4_contact(_mechanical_properties[entry.contact],
                                           entry.node_geometry,
                                           current,
                                           committed,
                                           _contact_histories[entry.contact][entry.secondary],
    ...
    if (_uses_hex20) {
        ...
        const Quad8FaceLocalResidual result = compute_quad8_face_boundary(_boundary_data[entry.kernel],
            entry.geometry,
            current,
            jacobian == nullptr ? nullptr : &local_jacobian);
        residual.assign(result.begin(), result.end());
    ...
    const Quad4FaceLocalResidual result = compute_quad4_face_boundary(_boundary_data[entry.kernel],
        entry.geometry,
        current,
        jacobian == nullptr ? nullptr : &local_jacobian);
    residual.assign(result.begin(), result.end());
...
SpatialAssembly::ContributionRanges SpatialAssembly::contribution_ranges() const noexcept {
    const std::size_t thermal_begin = volume_contribution_count(),
                      mechanical_begin =
                          thermal_begin
                          + (_uses_hex20 ? _hex20_thermal_patches.size() : _thermal_contact_offsets.back()),
...
std::pair<std::size_t, std::size_t> SpatialAssembly::contribution_partition(std::size_t partition,
    ...
    const auto boundary = [&](std::size_t boundary_partition) {
        if (boundary_partition == 0)
            return std::size_t{0};
        if (boundary_partition == partition_count)
            return count;
        ...
        for (std::size_t entry = 0; entry < count; ++entry) {
            ...
            if (next >= target)
                return target - accumulated < next - target ? entry : entry + 1U;
            accumulated = next;
        }
        return count;
    };
...
ResolvedBoundary SpatialAssembly::resolve_boundary(const UnstructuredHex8Mesh& source_mesh,
    ...
    if (boundary.faces.empty())
        throw std::invalid_argument("Three-dimensional contact side set is empty: " + name);
    return {region, std::move(boundary)};
}

ResolvedHex20Boundary SpatialAssembly::resolve_boundary(const UnstructuredHex20Mesh& source_mesh,
    const std::string& name) const {
    const std::int64_t block_id = source_mesh.side_set_block_id(name);
    const auto found = std::find(_block_ids.begin(), _block_ids.end(), block_id);
    if (found == _block_ids.end())
    ...
    if (boundary.faces.empty())
        throw std::invalid_argument("HEX20 contact side set is empty: " + name);
    return {region, std::move(boundary)};
}
...
Quad4SurfaceContactLocalValues SpatialAssembly::contribution_state(std::size_t index,
...
Quad4SurfaceContactLocalValues SpatialAssembly::contact_state(const std::array<std::size_t, 8>& nodes,
    const std::vector<double>& global_state) const {
    if (global_state.size() != dof_count())
...
Quad8SurfaceContactLocalDofs SpatialAssembly::hex20_contact_dofs(const Hex20MechanicalCandidate& candidate) const {
    Hex20ThermalCandidate thermal{candidate.contact,
        candidate.secondary_temperature_nodes,
        candidate.primary_temperature_nodes,
        candidate.secondary_displacement_nodes,
        candidate.primary_displacement_nodes,
        {},
        candidate.primary};
    return hex20_contact_dofs(thermal);
}

void SpatialAssembly::averaged_constraint_dofs(const AbaqusAveragedConstraint& constraint,
    std::vector<std::size_t>& dofs) const {
    const std::vector<std::size_t>& nodes =
...
std::size_t SpatialAssembly::averaged_sparsity_contribution_count() const noexcept {
    ...
    for (auto constraint = _abaqus_averaged_constraints.begin(); constraint != _abaqus_averaged_constraints.end();
        ++constraint)
        if (constraint->finite_sliding
            && std::none_of(_abaqus_averaged_constraints.begin(),
                constraint,
                [constraint](const AbaqusAveragedConstraint& previous) {
                    return previous.finite_sliding && previous.contact == constraint->contact;
                }))
...
SpatialAssembly::AbaqusAveragedConstraintValue SpatialAssembly::averaged_constraint_value(
    const AbaqusAveragedConstraint& constraint,
    const std::vector<double>& state,
    const std::vector<double>& committed_state,
    const ContactPointHistory& history) const {
    if (constraint.finite_region_normal)
        return finite_region_normal_value(constraint, constraint.active_nodes, state);
            ...
            for (std::size_t node = 0; node < node_count; ++node)
                result.gap += constraint.normal_gap_coefficients[stored_node(node)][component]
                              * (state[component * node_count + node]
                                  - (_uses_hex20 && constraint.finite_sliding ? state[component * node_count] : 0.0));
    else
        for (std::size_t component = 0; component < 3; ++component)
    ...
    if (!(result.pressure > 0.0)
        || (properties.friction_coefficient == 0.0 && !(_uses_hex20 && constraint.finite_sliding)))
        return result;
    ...
    if (constraint.friction_only) {
        ...
        result.tangential_force =
            constraint.area
            * std::hypot(result.tangential_traction_components[0], result.tangential_traction_components[1]);
        return result;
    }
    ...
    if (constraint.finite_sliding) {
        const std::array<double, 3> current_first = result.tangent_first,
                                    current_second = {normal[1] * current_first[2] - normal[2] * current_first[1],
                                        normal[2] * current_first[0] - normal[0] * current_first[2],
                                        normal[0] * current_first[1] - normal[1] * current_first[0]};
        std::array<double, 3> committed_normal = {constraint.reference_normal.x,
                                  constraint.reference_normal.y,
                                  constraint.reference_normal.z},
                              committed_first = {constraint.reference_tangent_first.x,
                                  constraint.reference_tangent_first.y,
                                  constraint.reference_tangent_first.z};
        ...
        const std::array<double, 3> committed_second = {committed_normal[1] * committed_first[2]
                                                            - committed_normal[2] * committed_first[1],
        ...
        if (_uses_hex20) {
            current_first_coordinate = 0.0;
            current_second_coordinate = 0.0;
            committed_first_coordinate = 0.0;
            committed_second_coordinate = 0.0;
            const auto& origin = constraint.reference_coordinates[stored_node(0)];
            const std::array<double, 3> reference_origin{origin.x, origin.y, origin.z};
            for (std::size_t node = 0; node < node_count; ++node) {
                const auto stored = stored_node(node);
                const auto& point = constraint.reference_coordinates[stored];
                const std::array<double, 3> coordinate{point.x, point.y, point.z};
                double old_first = 0.0, old_second = 0.0, old_normal = 0.0;
                for (std::size_t c = 0; c < 3; ++c) {
                    const double old_relative = coordinate[c] - reference_origin[c]
                                                + committed_state[c * node_count + node]
                                                - committed_state[c * node_count];
                    old_first += old_relative * committed_first[c];
                    old_second += old_relative * committed_second[c];
                    old_normal += old_relative * committed_normal[c];
                }
                // Remove the common finite rotation before integrating the
                // local virtual slip; a rigid rotation must add no slip.
                for (std::size_t c = 0; c < 3; ++c) {
                    const double increment =
                        coordinate[c] - reference_origin[c] + state[c * node_count + node] - state[c * node_count]
                        - (old_first * current_first[c] + old_second * current_second[c] + old_normal * normal[c]);
                    current_first_coordinate -= constraint.traction_first_coefficients[stored][c] * increment;
                    current_second_coordinate -= constraint.traction_second_coefficients[stored][c] * increment;
                }
            }
        }
        for (std::size_t component = 0; component < 3; ++component) {
    ...
    // Geometric slip also exists for frictionless finite sliding. It must be
    // accumulated before bypassing the elastic-slip and traction update.
    if (properties.friction_coefficient == 0.0) {
        result.elastic_tangential_slip = {};
        return result;
    }
    ...
    result.trial_tangential_magnitude = std::hypot(result.trial_tangential_traction[0],
        result.trial_tangential_traction[1],
        result.trial_tangential_traction[2]);
    if (result.trial_tangential_magnitude < sliding_limit
        || (result.trial_tangential_magnitude == sliding_limit && !history.sliding)) {
    ...
    if (_uses_hex20 && constraint.finite_sliding) {
        const auto second = cross(constraint.normal, constraint.tangent_first);
        const CartesianPoint3 traction{result.tangential_traction[0],
            result.tangential_traction[1],
            result.tangential_traction[2]};
        result.tangential_traction_components = {dot(traction, constraint.tangent_first), dot(traction, second)};
    }
    result.tangential_force =
        constraint.area
        * std::hypot(result.tangential_traction[0], result.tangential_traction[1], result.tangential_traction[2]);
    return result;
}
...
SpatialAssembly::AbaqusAveragedConstraintValue SpatialAssembly::finite_region_normal_value(
    ...
    if (state.size() != local_size)
        throw std::invalid_argument("HEX8 finite-region normal state has the wrong size");
    ...
    if (residual == nullptr && jacobian == nullptr && cached_state_matches
        ...
        if (pressure_derivative != nullptr)
            *pressure_derivative = constraint.finite_region_cached_pressure_derivative;
    ...
    if (!constraint.projected) {
        ...
        if (jacobian != nullptr)
            jacobian->assign(local_size * local_size, 0.0);
        if (pressure_derivative != nullptr)
            pressure_derivative->assign(local_size, 0.0);
        return {};
    }
    std::vector<double> unit_residual(local_size), area_derivative(local_size), gap_integral_derivative(local_size),
        unit_jacobian;
    if (jacobian != nullptr)
        unit_jacobian.resize(local_size * local_size);
    ...
    for (const AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
        ...
        for (const AbaqusAveragedConstraint::FiniteSlidingSample::NormalPoint& normal_point : sample.normal_points) {
            ...
            const Quad4FaceQuadraturePoint point = make_quad4_face_quadrature_point(secondary.coordinates,
                normal_point.xi,
                normal_point.eta,
                normal_point.weight);
            const Quad4ToQuad4MechanicalGeometry geometry = {secondary.coordinates,
                primary.coordinates,
                point.shape,
                point.derivative_xi,
                ...
                point.derivative_eta,
                normal_point.weight,
                primary_material_orientation(primary.coordinates, primary.parent_centroid),
            ...
            const Quad4FiniteRegionNormalGeometryValue value = compute_quad4_finite_region_normal_geometry(geometry,
                local_state,
                jacobian == nullptr && pressure_derivative == nullptr ? nullptr : &local_jacobian);
            if (!value.projected)
    ...
    if (pressure_derivative != nullptr)
        *pressure_derivative = local_pressure_derivative;
    if (residual != nullptr) {
        residual->resize(local_size);
        for (std::size_t row = 0; row < local_size; ++row)
            (*residual)[row] = pressure * unit_residual[row];
    }
    if (jacobian != nullptr) {
...
double SpatialAssembly::equivalent_normal_pressure(const AbaqusAveragedConstraint& constraint,
    ...
    if (finite_region != _abaqus_averaged_constraints.end()) {
        ...
        const AbaqusAveragedConstraintValue value = finite_region_normal_value(*finite_region,
            local_nodes,
            state,
            nullptr,
            nullptr,
            derivative == nullptr ? nullptr : &pressure_derivative);
        if (derivative != nullptr)
            *derivative = std::move(pressure_derivative);
        return value.pressure;
    }
    double force = 0.0;
    std::vector<double> force_derivative(state.size());
    for (const AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
        const auto point = std::find_if(_mechanical_points.begin(),
            _mechanical_points.end(),
            [&constraint, &sample](const MechanicalPoint& value) {
        ...
        const Quad4NormalForceAreaValue value = compute_quad4_to_quad4_normal_force_area(properties,
            candidate.surface_geometry,
            local_state,
            derivative == nullptr ? nullptr : &local_jacobian);
        if (!value.projected)
            throw std::domain_error("HEX8 averaged friction lost its matching normal projection");
        force += value.force;
        if (derivative != nullptr)
    ...
    for (const AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
        ...
        for (const std::size_t primary_index : sample.primary_faces) {
            ...
            const Quad4ToQuad4MechanicalGeometry geometry = {secondary.coordinates,
                ...
                1.0 / static_cast<double>(sample.primary_faces.size()),
                1.0,
                sample.normal_orientation};
            Quad4SurfaceContactLocalValues local_state{};
            ...
            const Quad4SurfaceContactLocalResidual local_residual =
                compute_quad4_to_quad4_tangential_force_geometry(geometry,
                    distribution,
                    sample.tangent_orientation,
                    traction[0],
                    traction[1],
                    local_state,
                    jacobian == nullptr ? nullptr : &local_jacobian);
...
void SpatialAssembly::compute_averaged_friction_traction_derivatives(const AbaqusAveragedConstraint& constraint,
    ...
    for (std::size_t tangent = 0; tangent < 2; ++tangent)
        increment_derivative[tangent].resize(local_size);
    const Matrix4& averaging = abaqus_quad4_averaging();
    for (const AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
        if (sample.primary_faces.empty())
            continue;
        ...
        for (const std::size_t primary_index : sample.primary_faces) {
            ...
            const Quad4AveragedFrictionGeometryValue geometry_value =
                compute_quad4_averaged_friction_geometry_value(geometry,
                    distribution,
                    sample.tangent_orientation,
                    local_state,
                    local_committed,
    ...
    if (!(area > 0.0))
        throw std::domain_error("HEX8 averaged friction derivative has a nonpositive area");
    ...
    second = {normal[1] * first[2] - normal[2] * first[1],
    ...
    for (std::size_t column = 0; column < local_size; ++column) {
        ...
        for (std::size_t component = 0; component < 3; ++component)
            first_derivative[component][column] =
                ...
                / tangent_measure;
        second_derivative[0][column] = normal_derivative[1][column] * first[2] + normal[1] * first_derivative[2][column]
                                       - normal_derivative[2][column] * first[1]
                                       - normal[2] * first_derivative[1][column];
        second_derivative[1][column] = normal_derivative[2][column] * first[0] + normal[2] * first_derivative[0][column]
                                       - normal_derivative[0][column] * first[2]
                                       - normal[0] * first_derivative[2][column];
        second_derivative[2][column] = normal_derivative[0][column] * first[1] + normal[0] * first_derivative[1][column]
                                       - normal_derivative[1][column] * first[0]
                                       - normal[1] * first_derivative[0][column];
    }
    ...
    for (std::size_t column = 0; column < local_size; ++column) {
        ...
        for (std::size_t tangent = 0; tangent < 2; ++tangent)
            derivatives[tangent][column] =
                ...
                + sliding_limit
                      * (trial_derivative[tangent][column] / trial_magnitude
...
void SpatialAssembly::compute_c3d20_finite_constraint_jacobian(const AbaqusAveragedConstraint& constraint,
    ...
    const AbaqusAveragedConstraintValue& value,
    std::vector<double>& jacobian) const {
    using ActivePoint = std::array<adlite::Scalar, 3>;
    const std::size_t nodes = constraint.active_nodes.size(), size = state.size();
    if (size != 3 * nodes || committed_state.size() != size)
        throw std::logic_error("HEX20 finite contact tangent has an inconsistent interface state");
    std::vector<adlite::Scalar> active(size), coefficients(nodes), normal_coefficients(size);
    std::vector<adlite::Scalar> first_coefficients(size), second_coefficients(size);
    adlite::seed_identity(state.data(), size, active.data());
    const auto local_node = [&constraint](std::size_t global) {
        const auto found = std::find(constraint.active_nodes.begin(), constraint.active_nodes.end(), global);
        if (found == constraint.active_nodes.end())
            throw std::logic_error("HEX20 finite contact derivative is missing an active interface node");
        return static_cast<std::size_t>(found - constraint.active_nodes.begin());
    };
    const auto active_dot = [](const ActivePoint& first, const ActivePoint& second) {
        return first[0] * second[0] + first[1] * second[1] + first[2] * second[2];
    };
    const auto active_cross = [](const ActivePoint& first, const ActivePoint& second) -> ActivePoint {
        return {first[1] * second[2] - first[2] * second[1],
            first[2] * second[0] - first[0] * second[2],
            first[0] * second[1] - first[1] * second[0]};
    };
    const auto active_norm = [](const ActivePoint& vector) {
        return adlite::hypot(adlite::hypot(vector[0], vector[1]), vector[2]);
    };
    const auto normalized = [&active_norm](const ActivePoint& vector) -> ActivePoint {
        const auto length = active_norm(vector);
        if (!(length.value() > 0.0))
            throw std::domain_error("HEX20 finite contact has a degenerate active frame");
        return {vector[0] / length, vector[1] / length, vector[2] / length};
    };
    adlite::Scalar area = 0.0;
    ActivePoint normal_sum{}, tangent_sum{};
    for (const auto& sample : constraint.finite_sliding_samples) {
        const auto& secondary = _hex20_secondary_contact_faces[constraint.contact][sample.secondary_face];
        std::array<ActivePoint, 8> current{};
        std::array<std::size_t, 8> secondary_nodes{};
        for (std::size_t node = 0; node < 8; ++node) {
            secondary_nodes[node] = local_node(secondary.displacement_nodes[node]);
            const auto& coordinate = secondary.coordinates[node];
            current[node] = {coordinate.x + active[secondary_nodes[node]],
                coordinate.y + active[nodes + secondary_nodes[node]],
                coordinate.z + active[2 * nodes + secondary_nodes[node]]};
        }
        const auto tangents = [&current](const Quad8FaceMechanicalQuadraturePoint& point) {
            std::array<ActivePoint, 2> result{};
            for (std::size_t node = 0; node < 8; ++node)
                for (std::size_t component = 0; component < 3; ++component) {
                    result[0][component] += point.derivative_xi[node] * current[node][component];
                    result[1][component] += point.derivative_eta[node] * current[node][component];
                }
            return result;
        };
        const double fraction = sample.secondary_local_point < 4 ? 1.0 / 24.0 : 5.0 / 24.0;
        const auto transfers = abaqus_quad8_primary_transfer_rule(sample.secondary_local_point);
        adlite::Scalar local_area = 0.0;
        for (const auto& transfer : transfers) {
            const auto point = make_quad8_face_mechanical_point(secondary.coordinates,
                2.0 * transfer.first - 1.0,
                2.0 * transfer.second - 1.0,
                1.0);
            const auto basis = tangents(point);
            const auto area_vector = active_cross(basis[0], basis[1]);
            local_area += 4.0 * fraction * transfer.weight * active_norm(area_vector);
            ...
            point_first = normalized(point_first);
            const auto point_second = active_cross(point_normal, point_first);
            const auto weight = 4.0 * fraction * transfer.weight * active_norm(area_vector);
            for (std::size_t c = 0; c < 3; ++c) {
                tangent_sum[c] += weight * point_first[c];
                normal_sum[c] += weight * point_second[c];
            }
        }
        area += local_area;
        if (transfers.size() != sample.primary_transfer_faces.size())
            throw std::logic_error("HEX20 finite contact has an incomplete selected transfer map");
        for (std::size_t entry = 0; entry < transfers.size(); ++entry) {
            const auto& transfer = transfers[entry];
            const auto point = make_quad8_face_mechanical_point(secondary.coordinates,
                2.0 * transfer.first - 1.0,
                2.0 * transfer.second - 1.0,
                1.0);
            const auto point_basis = tangents(point);
            const auto point_vector = active_cross(point_basis[0], point_basis[1]);
            auto point_normal = normalized(point_vector);
            for (auto& component : point_normal)
                component *= sample.normal_orientation;
            const auto point_area = 4.0 * fraction * transfer.weight * active_norm(point_vector);
            for (std::size_t node = 0; node < 8; ++node) {
                const auto coefficient = point_area * point.displacement_shape[node];
                coefficients[secondary_nodes[node]] -= coefficient;
                for (std::size_t component = 0; component < 3; ++component)
                    normal_coefficients[component * nodes + secondary_nodes[node]] -=
                        coefficient * point_normal[component];
            }
            std::vector<std::array<std::size_t, 8>> support_nodes;
            std::vector<std::array<adlite::Scalar, 8>> support_shapes;
            adlite::Scalar total_fraction = 0.0;
            for (const std::size_t primary_face : sample.primary_transfer_faces[entry]) {
                const auto& primary = _hex20_primary_contact_faces[constraint.contact].at(primary_face);
                const Quad8ToQuad8HeatGeometry geometry{secondary.coordinates,
                    primary.coordinates,
                    {},
                ...
                Quad8SurfaceContactLocalAdValues inputs{};
                std::array<std::size_t, 8> primary_nodes{};
                for (std::size_t node = 0; node < 8; ++node) {
                    primary_nodes[node] = local_node(primary.displacement_nodes[node]);
                    for (std::size_t component = 0; component < 3; ++component) {
                        const std::size_t offset = 8 + 16 * component;
                        inputs[offset + node] = active[component * nodes + secondary_nodes[node]];
                        inputs[offset + 8 + node] = active[component * nodes + primary_nodes[node]];
                        local[offset + node] = inputs[offset + node].value();
                        local[offset + 8 + node] = inputs[offset + 8 + node].value();
                    }
                }
                // Projection is differentiated only on one 56-DOF interface pair.
                // Compose those partial derivatives onto this constraint's node map.
                std::array<adlite::Scalar, 8> primary_shape{};
                (void)compute_quad8_disk_transfer(geometry, local, &primary_shape);
                std::array<adlite::Scalar, 8> composed_shapes{};
                for (std::size_t node = 0; node < 8; ++node) {
                    std::array<double, quad8_surface_contact_local_dof_count> partials{};
                    primary_shape[node].copy_derivatives(partials.data(), partials.size());
                    const auto shape =
                        adlite::compose(primary_shape[node].value(), inputs.data(), partials.data(), inputs.size());
                    composed_shapes[node] = shape;
                    total_fraction += shape;
                }
                support_nodes.push_back(primary_nodes);
                support_shapes.push_back(std::move(composed_shapes));
            }
            ...
            point_first = normalized(point_first);
            const auto point_second = active_cross(point_normal, point_first);
            ActivePoint point_separation{};
            for (std::size_t node = 0; node < 8; ++node)
                for (std::size_t c = 0; c < 3; ++c)
                    point_separation[c] -= point.displacement_shape[node] * current[node][c];
            for (std::size_t face = 0; face < support_nodes.size(); ++face)
                for (std::size_t node = 0; node < 8; ++node) {
                    const auto local = support_nodes[face][node];
                    const auto& reference = constraint.reference_coordinates[constraint.active_node_indices[local]];
                    const std::array<double, 3> x{reference.x, reference.y, reference.z};
                    for (std::size_t c = 0; c < 3; ++c)
                        point_separation[c] +=
                            support_shapes[face][node] / total_fraction * (x[c] + active[c * nodes + local]);
                }
            const auto point_gap = active_dot(point_separation, point_normal);
            const auto g11 = active_dot(point_basis[0], point_basis[0]),
                       g12 = active_dot(point_basis[0], point_basis[1]),
                       g22 = active_dot(point_basis[1], point_basis[1]), det = g11 * g22 - g12 * g12;
            if (!(det.value() > 0.0))
                throw std::domain_error("HEX20 friction surface metric is singular");
            for (std::size_t node = 0; node < 8; ++node) {
                const auto a = (g22 * point.derivative_xi[node] - g12 * point.derivative_eta[node]) / det;
                const auto b = (g11 * point.derivative_eta[node] - g12 * point.derivative_xi[node]) / det;
                ActivePoint gradient{};
                for (std::size_t c = 0; c < 3; ++c)
                    gradient[c] = a * point_basis[0][c] + b * point_basis[1][c];
                const auto dfirst = active_dot(gradient, point_first), dsecond = active_dot(gradient, point_second);
                for (std::size_t c = 0; c < 3; ++c) {
                    const auto row = c * nodes + secondary_nodes[node];
                    first_coefficients[row] -=
                        point_area
                        * (point.displacement_shape[node] * point_first[c] - point_gap * dfirst * point_normal[c]);
                    second_coefficients[row] -=
                        point_area
                        * (point.displacement_shape[node] * point_second[c] - point_gap * dsecond * point_normal[c]);
                }
            }
            for (std::size_t face_index = 0; face_index < support_nodes.size(); ++face_index)
                for (std::size_t node = 0; node < 8; ++node) {
                    const auto coefficient = point_area * support_shapes[face_index][node] / total_fraction;
                    const auto primary_node = support_nodes[face_index][node];
                    coefficients[primary_node] += coefficient;
                    for (std::size_t c = 0; c < 3; ++c) {
                        first_coefficients[c * nodes + primary_node] += coefficient * point_first[c];
                        second_coefficients[c * nodes + primary_node] += coefficient * point_second[c];
                    }
                    for (std::size_t component = 0; component < 3; ++component)
                        normal_coefficients[component * nodes + primary_node] += coefficient * point_normal[component];
                }
        }
    }
    auto first = normalized(tangent_sum);
    auto normal = normalized(active_cross(first, normal_sum));
    for (auto& coefficient : coefficients)
        coefficient /= area;
    for (auto& coefficient : normal_coefficients)
        coefficient /= area;
    for (auto& coefficient : first_coefficients)
        coefficient /= area;
    for (auto& coefficient : second_coefficients)
        coefficient /= area;
    // Use exactly the ordinary residual's geometry values while retaining the
    // independently assembled geometric derivatives (only roundoff differs).
    area = adlite::compose(constraint.area, area, 1.0);
    for (std::size_t component = 0; component < 3; ++component) {
        const std::array<double, 3> n{constraint.normal.x, constraint.normal.y, constraint.normal.z};
        const std::array<double, 3> t{constraint.tangent_first.x,
            constraint.tangent_first.y,
            constraint.tangent_first.z};
        normal[component] = adlite::compose(n[component], normal[component], 1.0);
        first[component] = adlite::compose(t[component], first[component], 1.0);
    }
    const auto second = active_cross(normal, first);
    ActivePoint separation{}, old_separation{};
    adlite::Scalar gap = 0.0;
    const auto& gap_origin = constraint.reference_coordinates.front();
    const std::array<double, 3> gap_reference_origin{gap_origin.x, gap_origin.y, gap_origin.z};
    for (std::size_t node = 0; node < nodes; ++node) {
        const std::size_t stored = constraint.active_node_indices[node];
        coefficients[node] = adlite::compose(constraint.gap_coefficients[stored], coefficients[node], 1.0);
        const auto& reference = constraint.reference_coordinates[stored];
        const std::array<double, 3> coordinate{reference.x, reference.y, reference.z};
        for (std::size_t component = 0; component < 3; ++component) {
            const std::size_t index = component * nodes + node;
            first_coefficients[index] = adlite::compose(constraint.traction_first_coefficients[stored][component],
                first_coefficients[index],
                1.0);
            second_coefficients[index] = adlite::compose(constraint.traction_second_coefficients[stored][component],
                second_coefficients[index],
                1.0);
            normal_coefficients[index] =
                adlite::compose(constraint.normal_gap_coefficients[stored][component], normal_coefficients[index], 1.0);
            gap +=
                normal_coefficients[index]
                * (coordinate[component] - gap_reference_origin[component] + active[index] - active[component * nodes]);
            separation[component] += coefficients[node] * (coordinate[component] + active[index]);
            old_separation[component] += coefficients[node] * (coordinate[component] + committed_state[index]);
        }
    }
    gap = adlite::compose(value.gap, gap, 1.0);
    const auto& properties = _mechanical_properties[constraint.contact];
    const adlite::Scalar pressure = -properties.penalty * gap;
    ActivePoint traction{};
    if (properties.friction_coefficient > 0.0) {
        std::array<double, 3> old_normal{constraint.reference_normal.x,
            constraint.reference_normal.y,
            constraint.reference_normal.z};
        std::array<double, 3> old_first{constraint.reference_tangent_first.x,
            constraint.reference_tangent_first.y,
            constraint.reference_tangent_first.z};
        if (history.cartesian_tangent_basis_initialized) {
            old_normal = history.cartesian_contact_normal;
            old_first = history.cartesian_contact_tangent_first;
        }
        const std::array<double, 3> old_second{old_normal[1] * old_first[2] - old_normal[2] * old_first[1],
            old_normal[2] * old_first[0] - old_normal[0] * old_first[2],
            old_normal[0] * old_first[1] - old_normal[1] * old_first[0]};
        adlite::Scalar first_increment = -active_dot(separation, first),
                       second_increment = -active_dot(separation, second);
        double history_first = 0.0, history_second = 0.0;
        for (std::size_t component = 0; component < 3; ++component) {
            first_increment += old_separation[component] * old_first[component];
            second_increment += old_separation[component] * old_second[component];
            history_first += history.cartesian_elastic_tangential_slip[component] * old_first[component];
            history_second += history.cartesian_elastic_tangential_slip[component] * old_second[component];
        }
        first_increment = 0.0;
        second_increment = 0.0;
        const auto& origin = constraint.reference_coordinates[constraint.active_node_indices[0]];
        const std::array<double, 3> reference_origin{origin.x, origin.y, origin.z};
        for (std::size_t node = 0; node < nodes; ++node) {
            const auto& point = constraint.reference_coordinates[constraint.active_node_indices[node]];
            const std::array<double, 3> coordinate{point.x, point.y, point.z};
            double old_first_coordinate = 0.0, old_second_coordinate = 0.0, old_normal_coordinate = 0.0;
            for (std::size_t component = 0; component < 3; ++component) {
                const double old_relative = coordinate[component] - reference_origin[component]
                                            + committed_state[component * nodes + node]
                                            - committed_state[component * nodes];
                old_first_coordinate += old_relative * old_first[component];
                old_second_coordinate += old_relative * old_second[component];
                old_normal_coordinate += old_relative * old_normal[component];
            }
            for (std::size_t component = 0; component < 3; ++component) {
                const std::size_t index = component * nodes + node;
                const auto increment =
                    coordinate[component] - reference_origin[component] + active[index] - active[component * nodes]
                    - (old_first_coordinate * first[component] + old_second_coordinate * second[component]
                        + old_normal_coordinate * normal[component]);
                first_increment -= first_coefficients[index] * increment;
                second_increment -= second_coefficients[index] * increment;
            }
        }
        ActivePoint increment{}, elastic{};
        for (std::size_t component = 0; component < 3; ++component) {
            increment[component] = first_increment * first[component] + second_increment * second[component];
            elastic[component] =
                history_first * first[component] + history_second * second[component] + increment[component];
        }
        const auto normal_increment = active_dot(normal, increment);
        for (std::size_t component = 0; component < 3; ++component)
            elastic[component] -= normal_increment * normal[component];
        const auto history_normal = active_dot(normal, elastic);
        for (std::size_t component = 0; component < 3; ++component)
            elastic[component] -= history_normal * normal[component];
        const adlite::Scalar limit = properties.friction_coefficient * pressure;
        const adlite::Scalar stiffness = properties.maximum_elastic_slip > 0.0 ? limit / properties.maximum_elastic_slip
                                                                               : adlite::Scalar(properties.penalty);
        for (std::size_t component = 0; component < 3; ++component)
            traction[component] = stiffness * elastic[component];
        if (value.sliding) {
            const auto magnitude = active_norm(traction);
            for (auto& component : traction)
                component *= limit / magnitude;
        }
    }
    const auto first_traction = active_dot(traction, first), second_traction = active_dot(traction, second);
    std::vector<adlite::Scalar> result(size);
    for (std::size_t component = 0; component < 3; ++component)
        for (std::size_t node = 0; node < nodes; ++node) {
            const std::size_t row = component * nodes + node;
            result[row] = -area
                          * (pressure * normal_coefficients[row] + first_coefficients[row] * first_traction
                              + second_coefficients[row] * second_traction);
        }
    std::vector<double> residual(size);
    jacobian.resize(size * size);
    adlite::extract_jacobian(result.data(), size, size, residual.data(), jacobian.data());
}
...
void SpatialAssembly::compute_averaged_constraint(const AbaqusAveragedConstraint& constraint,
    ...
    const std::array<double, 3> normal = {constraint.normal.x, constraint.normal.y, constraint.normal.z};
    const bool local_normal_gradients = !constraint.normal_gap_coefficients.empty();
    const auto gap_gradient = [&constraint, &normal, &stored_node, local_normal_gradients](std::size_t node,
    ...
    if (constraint.friction_only) {
        ...
        compute_averaged_friction_geometry(constraint,
            state,
            value.tangential_traction_components,
            geometric_residual,
            jacobian);
        std::array<std::vector<double>, 2> traction_derivatives;
        compute_averaged_friction_traction_derivatives(constraint,
            state,
            committed_state,
            history,
            value,
    ...
    for (std::size_t component = 0; component < 3; ++component) {
        for (std::size_t node = 0; node < node_count; ++node) {
            ...
            const double friction =
                _uses_hex20 && constraint.finite_sliding
                    ? constraint.traction_first_coefficients[stored_node(node)][component]
                              * value.tangential_traction_components[0]
                          + constraint.traction_second_coefficients[stored_node(node)][component]
                                * value.tangential_traction_components[1]
                    : constraint.gap_coefficients[stored_node(node)] * value.tangential_traction[component];
            residual[local] = -constraint.area * (value.pressure * gap_gradient(node, component) + friction);
        }
    }
    if (jacobian == nullptr)
        return;
...
void SpatialAssembly::refresh_hex20_finite_averaged_constraints(const std::vector<double>& state) const {
    const auto current_face = [this, &state](const std::array<std::size_t, 8>& nodes,
                                  const Quad8FaceCoordinates& reference) {
    ...
    for (AbaqusAveragedConstraint& constraint : _abaqus_averaged_constraints) {
        if (!constraint.finite_sliding)
            continue;
        if (_touched_mechanical_nodes[mechanical_node_index(constraint.contact, constraint.history)] == 0U)
            continue;
        ...
        std::fill(constraint.normal_gap_coefficients.begin(),
            constraint.normal_gap_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.secondary_normal_coefficients.begin(),
            constraint.secondary_normal_coefficients.end(),
            std::array<double, 3>{});
        constraint.traction_first_coefficients.assign(constraint.nodes.size(), {});
        constraint.traction_second_coefficients.assign(constraint.nodes.size(), {});
        constraint.secondary_tangent_first_coefficients.assign(constraint.secondary_output_nodes.size(), {});
        constraint.secondary_tangent_second_coefficients.assign(constraint.secondary_output_nodes.size(), {});
        constraint.normal = {};
        ...
        for (AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
            ...
            sample.primary_transfer_faces.clear();
            const Hex20SecondaryContactFace& secondary =
                _hex20_secondary_contact_faces.at(constraint.contact).at(sample.secondary_face);
            required_nodes.insert(required_nodes.end(),
                secondary.displacement_nodes.begin(),
                secondary.displacement_nodes.end());
            const Quad8FaceCoordinates secondary_current =
                current_face(secondary.displacement_nodes, secondary.coordinates);
            const double fraction = sample.secondary_local_point < 4 ? 1.0 / 24.0 : 5.0 / 24.0;
            double local_area = 0.0;
            for (const auto& transfer : abaqus_quad8_primary_transfer_rule(sample.secondary_local_point)) {
                const auto point = make_quad8_face_mechanical_point(secondary_current,
                    2.0 * transfer.first - 1.0,
                    2.0 * transfer.second - 1.0,
                    1.0);
                local_area += 4.0 * fraction * transfer.weight * reference_measure(point);
                const auto area_vector = cross(point.tangent_xi, point.tangent_eta);
                const double measure = reference_measure(point);
                const CartesianPoint3 normal{sample.normal_orientation * area_vector.x / measure,
                    sample.normal_orientation * area_vector.y / measure,
                    sample.normal_orientation * area_vector.z / measure};
                const auto reference = make_quad8_face_mechanical_point(secondary.coordinates,
                    2.0 * transfer.first - 1.0,
                    2.0 * transfer.second - 1.0,
                    1.0);
                ...
                const double length = std::sqrt(dot(first, first));
                if (!(length > 0.0))
                    throw std::domain_error("HEX20 current contact tangent is singular");
                first = {first.x / length, first.y / length, first.z / length};
                const auto second = cross(normal, first);
                const double weight = 4.0 * fraction * transfer.weight * measure;
                constraint.tangent_first.x += weight * first.x;
                constraint.tangent_first.y += weight * first.y;
                constraint.tangent_first.z += weight * first.z;
                constraint.normal.x += weight * second.x;
                constraint.normal.y += weight * second.y;
                constraint.normal.z += weight * second.z;
            }
            ...
            for (const AbaqusQuad8TransferSample& transfer :
                ...
                const auto point_vector = cross(secondary_point.tangent_xi, secondary_point.tangent_eta);
                const double point_measure = reference_measure(secondary_point);
                const double point_area = 4.0 * fraction * transfer.weight * point_measure;
                const CartesianPoint3 point_normal{sample.normal_orientation * point_vector.x / point_measure,
                    sample.normal_orientation * point_vector.y / point_measure,
                    sample.normal_orientation * point_vector.z / point_measure};
                for (std::size_t local_node = 0; local_node < 8; ++local_node) {
                    const double coefficient = point_area * secondary_point.displacement_shape[local_node];
                    const auto node = std::find(constraint.nodes.begin(),
                        constraint.nodes.end(),
                        secondary.displacement_nodes[local_node]);
                    ...
                    constraint.normal_gap_coefficients[stored_node][0] -= coefficient * point_normal.x;
                    constraint.normal_gap_coefficients[stored_node][1] -= coefficient * point_normal.y;
                    constraint.normal_gap_coefficients[stored_node][2] -= coefficient * point_normal.z;
                    const auto output = std::find(
                        _hex20_secondary_boundaries.at(constraint.contact).boundary.displacement_nodes.begin(),
                    ...
                    const std::size_t output_index = static_cast<std::size_t>(
                        output
                        - _hex20_secondary_boundaries.at(constraint.contact).boundary.displacement_nodes.begin());
                    const auto stored = std::find(constraint.secondary_output_nodes.begin(),
                        constraint.secondary_output_nodes.end(),
                        output_index);
                    ...
                    constraint.secondary_normal_coefficients[stored_output][0] += coefficient * point_normal.x;
                    constraint.secondary_normal_coefficients[stored_output][1] += coefficient * point_normal.y;
                    constraint.secondary_normal_coefficients[stored_output][2] += coefficient * point_normal.z;
                }
                ...
                if (primary_faces.size() > spatial_detail::contact_search_tree_minimum_items) {
                    ...
                    while (_contact_search_trees[constraint.contact].next_candidate(_contact_search_query,
                        minimum_distance,
                        primary_index))
                ...
                sample.primary_transfer_faces.emplace_back();
                ...
                for (std::size_t node = 0; node < 8; ++node) {
                    separation.x -= secondary_point.displacement_shape[node] * secondary_current[node].x;
                    separation.y -= secondary_point.displacement_shape[node] * secondary_current[node].y;
                    separation.z -= secondary_point.displacement_shape[node] * secondary_current[node].z;
                }
                for (const auto& entry : support) {
                    const auto& primary = primary_faces[entry.first];
                    const auto coordinates = current_face(primary.displacement_nodes, primary.coordinates);
                    for (std::size_t node = 0; node < 8; ++node) {
                        const double weight = entry.second[node] / total_fraction;
                        separation.x += weight * coordinates[node].x;
                        separation.y += weight * coordinates[node].y;
                        separation.z += weight * coordinates[node].z;
                    }
                }
                const double point_gap = dot(separation, point_normal);
                ...
                const double first_length = std::sqrt(dot(point_first, point_first));
                if (!(first_length > 0.0))
                    throw std::domain_error("HEX20 current contact tangent is singular");
                point_first = {point_first.x / first_length,
                    point_first.y / first_length,
                    point_first.z / first_length};
                const auto point_second = cross(point_normal, point_first);
                const double g11 = dot(tx, tx), g12 = dot(tx, ty), g22 = dot(ty, ty), det = g11 * g22 - g12 * g12;
                if (!(det > 0.0))
                    throw std::domain_error("HEX20 friction surface metric is singular");
                for (std::size_t node = 0; node < 8; ++node) {
                    const double a =
                        (g22 * secondary_point.derivative_xi[node] - g12 * secondary_point.derivative_eta[node]) / det;
                    const double b =
                        (g11 * secondary_point.derivative_eta[node] - g12 * secondary_point.derivative_xi[node]) / det;
                    const CartesianPoint3 gradient{a * tx.x + b * ty.x, a * tx.y + b * ty.y, a * tx.z + b * ty.z};
                    const auto stored = static_cast<std::size_t>(
                        std::find(constraint.nodes.begin(), constraint.nodes.end(), secondary.displacement_nodes[node])
                        - constraint.nodes.begin());
                    const auto output_node = static_cast<std::size_t>(
                        std::find(_hex20_secondary_boundaries[constraint.contact].boundary.displacement_nodes.begin(),
                            _hex20_secondary_boundaries[constraint.contact].boundary.displacement_nodes.end(),
                            _hex20_secondary_boundaries[constraint.contact]
                                .boundary.faces[sample.secondary_face]
                                .nodes[node])
                        - _hex20_secondary_boundaries[constraint.contact].boundary.displacement_nodes.begin());
                    const auto output = static_cast<std::size_t>(std::find(constraint.secondary_output_nodes.begin(),
                                                                     constraint.secondary_output_nodes.end(),
                                                                     output_node)
                                                                 - constraint.secondary_output_nodes.begin());
                    const std::array<double, 3> n{point_normal.x, point_normal.y, point_normal.z},
                        first{point_first.x, point_first.y, point_first.z},
                        second{point_second.x, point_second.y, point_second.z};
                    for (std::size_t c = 0; c < 3; ++c) {
                        // The local gap term is required by rotation of the
                        // secondary surface in the tangential virtual motion.
                        const double first_weight = point_area
                                                    * (secondary_point.displacement_shape[node] * first[c]
                                                        - point_gap * dot(gradient, point_first) * n[c]);
                        const double second_weight = point_area
                                                     * (secondary_point.displacement_shape[node] * second[c]
                                                         - point_gap * dot(gradient, point_second) * n[c]);
                        constraint.traction_first_coefficients[stored][c] -= first_weight;
                        constraint.traction_second_coefficients[stored][c] -= second_weight;
                        constraint.secondary_tangent_first_coefficients[output][c] += first_weight;
                        constraint.secondary_tangent_second_coefficients[output][c] += second_weight;
                    }
                }
                for (const auto& entry : support) {
                    const std::size_t primary_index = entry.first;
                    sample.primary_transfer_faces.back().push_back(primary_index);
                    if (std::find(sample.primary_faces.begin(), sample.primary_faces.end(), primary_index)
                        == sample.primary_faces.end())
                        sample.primary_faces.push_back(primary_index);
                    const auto& primary = primary_faces[primary_index];
                    const double fraction =
                        std::accumulate(entry.second.begin(), entry.second.end(), 0.0) / total_fraction;
                    primary_face_weight[primary_index] += point_area * fraction;
                    required_nodes.insert(required_nodes.end(),
                        primary.displacement_nodes.begin(),
                        primary.displacement_nodes.end());
                    for (std::size_t local_node = 0; local_node < 8; ++local_node) {
                        const auto node = std::find(constraint.nodes.begin(),
                            constraint.nodes.end(),
                            primary.displacement_nodes[local_node]);
                        if (node == constraint.nodes.end())
                            throw std::logic_error("HEX20 disk transfer lost a primary node");
                        const std::size_t stored_node = static_cast<std::size_t>(node - constraint.nodes.begin());
                        const double coefficient = point_area * entry.second[local_node] / total_fraction;
                        constraint.gap_coefficients[stored_node] += coefficient;
                        const std::array<double, 3> first{point_first.x, point_first.y, point_first.z},
                            second{point_second.x, point_second.y, point_second.z};
                        for (std::size_t c = 0; c < 3; ++c) {
                            constraint.traction_first_coefficients[stored_node][c] += coefficient * first[c];
                            constraint.traction_second_coefficients[stored_node][c] += coefficient * second[c];
                        }
                        constraint.normal_gap_coefficients[stored_node][0] += coefficient * point_normal.x;
                        constraint.normal_gap_coefficients[stored_node][1] += coefficient * point_normal.y;
                        constraint.normal_gap_coefficients[stored_node][2] += coefficient * point_normal.z;
        ...
        if (!(tangent_measure > 0.0))
            throw std::domain_error("HEX20 averaged first contact tangent is singular");
        constraint.tangent_first = {constraint.tangent_first.x / tangent_measure,
            constraint.tangent_first.y / tangent_measure,
            constraint.tangent_first.z / tangent_measure};
        constraint.normal = cross(constraint.tangent_first, constraint.normal);
        const double normal_measure = std::sqrt(dot(constraint.normal, constraint.normal));
        if (!(normal_measure > 0.0))
            throw std::domain_error("HEX20 averaged contact normal is singular");
        constraint.normal = {constraint.normal.x / normal_measure,
            constraint.normal.y / normal_measure,
            constraint.normal.z / normal_measure};
        for (double& coefficient : constraint.gap_coefficients)
            coefficient /= constraint.area;
        for (double& coefficient : constraint.secondary_coefficients)
            coefficient /= constraint.area;
        for (std::array<double, 3>& coefficient : constraint.normal_gap_coefficients)
            for (double& component : coefficient)
                component /= constraint.area;
        for (std::array<double, 3>& coefficient : constraint.secondary_normal_coefficients)
            for (double& component : coefficient)
                component /= constraint.area;
        for (auto* coefficients : {&constraint.traction_first_coefficients,
                 &constraint.traction_second_coefficients,
                 &constraint.secondary_tangent_first_coefficients,
                 &constraint.secondary_tangent_second_coefficients})
            for (auto& coefficient : *coefficients)
                for (double& component : coefficient)
                    component /= constraint.area;
        ...
        for (std::size_t node = 0; node < constraint.nodes.size(); ++node) {
            constraint.reference_gap +=
                constraint.normal_gap_coefficients[node][0] * (constraint.reference_coordinates[node].x - gap_origin.x)
                + constraint.normal_gap_coefficients[node][1]
                      * (constraint.reference_coordinates[node].y - gap_origin.y)
                + constraint.normal_gap_coefficients[node][2]
                      * (constraint.reference_coordinates[node].z - gap_origin.z);
        }
...
void SpatialAssembly::refresh_finite_averaged_constraints(const std::vector<double>& state) const {
    ...
    for (AbaqusAveragedConstraint& constraint : _abaqus_averaged_constraints) {
        ...
        std::fill(constraint.gap_coefficients.begin(), constraint.gap_coefficients.end(), 0.0);
        std::fill(constraint.secondary_coefficients.begin(), constraint.secondary_coefficients.end(), 0.0);
        std::fill(constraint.tangent_first_coefficients.begin(),
            constraint.tangent_first_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.tangent_second_coefficients.begin(),
            constraint.tangent_second_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.traction_first_coefficients.begin(),
            constraint.traction_first_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.traction_second_coefficients.begin(),
            constraint.traction_second_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.secondary_tangent_first_coefficients.begin(),
            constraint.secondary_tangent_first_coefficients.end(),
            std::array<double, 3>{});
        std::fill(constraint.secondary_tangent_second_coefficients.begin(),
            constraint.secondary_tangent_second_coefficients.end(),
            std::array<double, 3>{});
        constraint.normal = {};
        constraint.tangent_first = {};
        constraint.area = 0.0;
        ...
        for (AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
            ...
            const Quad4FaceQuadraturePoint normal_point = make_quad4_face_quadrature_point(secondary_current,
                (4.0 / 3.0) * location[0],
                (4.0 / 3.0) * location[1],
                1.0);
            ...
            const SurfacePullbackCovectors pullback = surface_pullback_covectors(point,
                reference_point,
                sample.normal_orientation,
                sample.tangent_orientation,
                local_normal,
            ...
            const auto consider_projection = [&](std::size_t primary) {
                if (considered[primary] != 0U)
                    return;
                considered[primary] = 1U;
                const Quad4ReferenceProjectionValue value = compute_quad4_reference_projection(secondary_current,
                    primary_current_faces[primary],
                    point.shape,
                    1.0);
                if (!value.projected)
                    return;
            ...
            const bool has_interior = std::any_of(projections.begin(), projections.end(), [](const Projection& value) {
                return value.interior;
            });
            if (has_interior && projections.size() > 1) {
                const auto closest = std::min_element(projections.begin(),
                    projections.end(),
            ...
            if (constraint.finite_region_normal) {
                ...
                const auto owner = [&](double xi, double eta, std::size_t cached_primary) {
                    ...
                    const auto consider = [&](std::size_t primary) {
                        const Quad4ReferenceProjectionValue unbounded =
                            compute_quad4_reference_projection(secondary_current,
                                primary_current_faces[primary],
                                integration_point.shape,
                                1.0);
                        ...
                        if (!value.projected) {
                            ...
                            if (!xi_supported || !eta_supported)
                                return;
                            value = compute_quad4_reference_closest_projection(secondary_current,
                                primary_current_faces[primary],
                                integration_point.shape,
                                1.0);
                        }
                    ...
                    if (cached_primary < primary_faces.size())
                        consider(cached_primary);
                    if (primary_faces.size() > spatial_detail::contact_search_tree_minimum_items) {
                        std::array<double, 3> search_point{};
                        for (std::size_t node = 0; node < 4; ++node) {
        ...
        if (constraint.finite_region_normal) {
            ...
            for (const AbaqusAveragedConstraint::FiniteSlidingSample& sample : constraint.finite_sliding_samples) {
                ...
                for (const AbaqusAveragedConstraint::FiniteSlidingSample::NormalPoint& normal_point :
                    sample.normal_points)
                    constraint.area += make_quad4_face_quadrature_point(secondary_current,
                        normal_point.xi,
                        normal_point.eta,
                        normal_point.weight)
                                           .weighted_measure;
    ...
    for (AbaqusAveragedConstraint& constraint : _abaqus_averaged_constraints) {
        ...
        required_nodes.insert(required_nodes.end(),
            finite_region->active_nodes.begin(),
            finite_region->active_nodes.end());
        set_active_support(constraint, std::move(required_nodes));
    }
}

bool SpatialAssembly::summarize_averaged_contact(std::size_t contact_value,
    const std::vector<double>& state,
    ...
    for (const AbaqusAveragedConstraint& constraint : _abaqus_averaged_constraints) {
        if (constraint.contact != contact_value)
            continue;
        ...
        const AbaqusAveragedConstraintValue value =
            constraint.finite_region_normal
                ...
                : averaged_constraint_value(constraint,
                      local_state,
                      committed_state,
        ...
        for (std::size_t entry = 0; entry < constraint.secondary_output_nodes.size(); ++entry) {
            ...
            for (std::size_t component = 0; component < 3; ++component) {
                ...
                output.tangential_contact_force[component] +=
                    (constraint.friction_only || (_uses_hex20 && constraint.finite_sliding))
                        ? constraint.area
                              * (constraint.secondary_tangent_first_coefficients[entry][component]
                                      * value.tangential_traction_components[0]
    ...
    if (_uses_hex20 && !finite_region_normals) {
        ...
        for (const Quad8FaceElement& face : secondary.boundary.faces) {
            ...
            for (std::size_t local_node = 1; local_node < output_nodes.size(); ++local_node) {
                ...
                if (first_root != next_root)
                    component_parent[next_root] = first_root;
            }
        ...
        for (std::size_t node = 0; node < summaries.size(); ++node)
            if (recovery_count[node] != 0) {
                ...
                summaries[node].pressure = recover_finite_sliding_nodal_tractions
                                               ? std::clamp(average, component_minimum[root], component_maximum[root])
                                               : average;
            }
    }
    if (finite_region_normals)
        for (std::size_t node = 0; node < summaries.size(); ++node) {
            ...
            summary.tributary_area = std::hypot(finite_region_normal_area[node][0],
                finite_region_normal_area[node][1],
    ...
    return std::any_of(_abaqus_averaged_constraints.begin(),
        _abaqus_averaged_constraints.end(),
        [contact_value](const AbaqusAveragedConstraint& constraint) {
            return constraint.contact == contact_value && !constraint.friction_only;
        });
...
SpatialAssembly::FiniteRegionPartitionSummary
SpatialAssembly::finite_region_partition_summary(std::size_t contact_index, const std::vector<double>& state) const {
    refresh_finite_averaged_constraints(state);
    FiniteRegionPartitionSummary result{};
    std::set<std::size_t> active_primary_faces;
    for (const AbaqusAveragedConstraint& constraint : _abaqus_averaged_constraints) {
        if (constraint.contact != contact_index || !constraint.finite_region_normal)
            continue;
        ...
        if (constraint_primary_faces.size() > 1)
            ++result.cross_face_constraint_count;
    }
    result.active_primary_face_count = active_primary_faces.size();
    return result;
}

Quad8SurfaceContactLocalValues SpatialAssembly::hex20_contact_state(const Hex20ThermalCandidate& candidate,
    const std::vector<double>& global_state) const {
    if (global_state.size() != dof_count())
        throw std::invalid_argument("HEX20 contact state has the wrong size");
...
Quad8SurfaceContactLocalValues SpatialAssembly::hex20_contact_state(const Hex20MechanicalCandidate& candidate,
    const std::vector<double>& global_state) const {
    if (global_state.size() != dof_count())
        throw std::invalid_argument("HEX20 contact state has the wrong size");
    const Quad8SurfaceContactLocalDofs dofs = hex20_contact_dofs(candidate);
    ...
    for (std::size_t contact_value = 0; contact_value < _definition.contacts.size(); ++contact_value) {
        ...
        if (secondary.boundary.faces.size() > 1) {
            const Quad4FaceQuadraturePoint reference_normal_point =
                make_quad4_face_quadrature_point(face_coordinates(secondary_mesh, secondary.boundary.faces.front()),
                    0.0,
                    0.0,
                    1.0);
            ...
            for (std::size_t face = 1; face < secondary.boundary.faces.size(); ++face) {
                const Quad4FaceQuadraturePoint normal_point =
                    make_quad4_face_quadrature_point(face_coordinates(secondary_mesh, secondary.boundary.faces[face]),
                        0.0,
                        0.0,
                        1.0);
        ...
        if (finite_region_normal)
            finite_averaged = true;
        ...
        for (const std::size_t secondary_local : secondary.boundary.nodes) {
            ...
            if (std::any_of(primary.boundary.nodes.begin(),
                    primary.boundary.nodes.end(),
        ...
        const double maximum_elastic_slip =
            slip_tolerance > 0.0
                ? slip_tolerance
                      * std::sqrt(reference_secondary_area / static_cast<double>(secondary.boundary.faces.size()))
                : 0.0;
        ...
        _mechanical_properties.push_back({definition.mechanical ? definition.penalty : 1.0,
            definition.mechanical ? definition.friction_coefficient : 0.0,
            false,
        ...
        for (const Quad4FaceElement& secondary_face : secondary.boundary.faces) {
            ...
            if (definition.thermal)
                thermal_point_count += secondary_geometry.points.size();
        ...
        if (finite_averaged || finite_averaged_friction) {
            ...
            for (std::size_t secondary_face_index = 0; secondary_face_index < secondary_faces.size();
                ...
                for (std::size_t local_constraint = 0; local_constraint < 4; ++local_constraint) {
                    ...
                    for (std::size_t node = 0; node < 4; ++node) {
                        ...
                        const auto output_node = std::find(secondary.boundary.nodes.begin(),
                            secondary.boundary.nodes.end(),
                            source_face.nodes[node]);
                    ...
                    for (std::size_t primary_index = 0; primary_index < primary_faces.size(); ++primary_index) {
                        ...
                        const Quad4ReferenceProjectionValue projection =
                            compute_quad4_reference_projection(face.coordinates,
                                primary_face.coordinates,
                                point.shape,
                    ...
                    const double orientation = normal_orientation(face.coordinates,
                        face.parent_centroid,
                        primary_faces[selected].parent_centroid);
                    const Quad4FaceQuadraturePoint normal_point = make_quad4_face_quadrature_point(face.coordinates,
                        (4.0 / 3.0) * location[0],
                        (4.0 / 3.0) * location[1],
                        1.0);
            ...
            for (std::size_t output = 0; output < builders.size(); ++output) {
                ...
                constraint.normal = {builder.normal.x / normal_measure,
                    builder.normal.y / normal_measure,
                ...
                constraint.tangent_first = {tangent.x / tangent_measure,
                    tangent.y / tangent_measure,
                    tangent.z / tangent_measure};
        ...
        if (definition.mechanical && surface_to_surface && !finite_sliding) {
            ...
            for (std::size_t secondary_face_index = 0; secondary_face_index < secondary_faces.size();
                ...
                const auto projection = [&](double xi, double eta, std::size_t primary_index) {
                    ...
                    return compute_quad4_reference_projection(face.coordinates,
                        primary_face.coordinates,
                        point.shape,
                        normal_orientation(primary_face.coordinates,
                            face.parent_centroid,
                            primary_face.parent_centroid));
                };
                ...
                while (!pending.empty()) {
                    ...
                    const std::array<std::size_t, 5> owners = {owner(xi_left, eta_lower),
                        owner(xi_right, eta_lower),
                        owner(xi_right, eta_upper),
                        owner(xi_left, eta_upper),
                        owner(xi_center, eta_center)};
                    if (std::find(owners.begin(), owners.end(), std::numeric_limits<std::size_t>::max())
                        != owners.end())
                        throw std::invalid_argument("HEX8 small-sliding surface contact '" + definition.name
                                                    + "' has an unprojected reference averaging region");
                    const bool uniform = std::all_of(owners.begin() + 1, owners.end(), [&](std::size_t value) {
                        return value == owners.front();
                    });
...
void SpatialAssembly::build_hex20_contacts(const UnstructuredHex20Mesh& source_mesh) {
    ...
    for (std::size_t contact_value = 0; contact_value < _definition.contacts.size(); ++contact_value) {
        ...
        for (const std::size_t secondary_local : secondary.boundary.displacement_nodes) {
            const std::size_t source_node = secondary_mesh.source_node_ids().at(secondary_local);
            if (std::any_of(primary.boundary.displacement_nodes.begin(),
                    primary.boundary.displacement_nodes.end(),
                    [&](std::size_t primary_local) {
                        return primary_mesh.source_node_ids().at(primary_local) == source_node;
                    }))
        ...
        const double slip_tolerance =
            definition.mechanical && definition.friction_coefficient > 0.0 && surface_to_surface
                ? (definition.friction_slip_tolerance > 0.0 ? definition.friction_slip_tolerance : 5.0e-3)
                : 0.0;
        double reference_secondary_length = 0.0;
        if (slip_tolerance > 0.0)
            for (const Quad8FaceElement& face : secondary.boundary.faces) {
                double area = 0.0;
                for (const Quad8FaceMechanicalQuadraturePoint& point :
                    ...
                    area += point.quadrature_weight * reference_measure(point);
                reference_secondary_length += std::sqrt(area);
            }
        const double maximum_elastic_slip =
            slip_tolerance > 0.0
                ? slip_tolerance * reference_secondary_length / static_cast<double>(secondary.boundary.faces.size())
                : 0.0;
        if (!std::isfinite(maximum_elastic_slip) || maximum_elastic_slip < 0.0)
            throw std::invalid_argument(
        ...
        for (const Quad8FaceElement& secondary_face : secondary.boundary.faces) {
            ...
            const std::size_t secondary_face_index = secondary_faces.size();
            secondary_faces.push_back({temperature_nodes,
                displacement_nodes,
                coordinates,
                geometry,
            ...
            if (definition.thermal) {
                ...
                for (std::size_t local_constraint = 0; local_constraint < 8; ++local_constraint) {
                    ...
                    for (const auto& sample : abaqus_quad8_primary_transfer_rule(local_constraint)) {
                        ...
                        const double weight = 4.0 * fraction * sample.weight;
                        const auto point = make_quad8_face_mechanical_point(coordinates, xi, eta, weight);
                        Quad8FaceThermalQuadraturePoint thermal{};
                        thermal.temperature_shape = {0.25 * (1.0 - xi) * (1.0 - eta),
                            0.25 * (1.0 + xi) * (1.0 - eta),
                            0.25 * (1.0 + xi) * (1.0 + eta),
                            0.25 * (1.0 - xi) * (1.0 + eta)};
                        thermal.displacement_shape = point.displacement_shape;
                        thermal.derivative_xi = point.derivative_xi;
            ...
            if (definition.mechanical && !surface_to_surface)
                for (std::size_t secondary_local_node = 0; secondary_local_node < 8; ++secondary_local_node) {
                    ...
                    _hex20_mechanical_points.push_back({contact_value,
                        static_cast<std::size_t>(found - secondary.boundary.displacement_nodes.begin()),
                        secondary_face_index,
        ...
        if (definition.mechanical && surface_to_surface && !finite_averaged) {
            ...
            for (std::size_t secondary_face_index = 0; secondary_face_index < secondary_faces.size();
                ...
                const auto projection = [&](double xi, double eta, std::size_t primary_index) {
                    ...
                    return compute_quad8_reference_projection(face.coordinates,
                        primary_face.coordinates,
                        point.displacement_shape,
                        normal_orientation(primary_face.coordinates,
                            face.parent_centroid,
                ...
                const auto owner = [&](double xi, double eta) {
                    ...
                    for (std::size_t primary_index = 0; primary_index < primary_faces.size(); ++primary_index) {
                        ...
                        const Quad8ReferenceProjectionValue reference =
                            compute_quad8_reference_projection(face.coordinates,
                                primary_face.coordinates,
                                point.displacement_shape,
                                normal_orientation(primary_face.coordinates,
                                    face.parent_centroid,
                ...
                while (!pending.empty()) {
                    ...
                    const bool corner_transition = owners[0] != owners[1] || owners[3] != owners[2]
                               ...
                               split_xi = owners[0] != owners[1] || owners[3] != owners[2] || !corner_transition,
                               split_eta = owners[0] != owners[3] || owners[1] != owners[2] || !corner_transition;
                    // Straight primary edges can be integrated exactly without a long chain of dyadic cells.
                    // Curved or oblique transitions retain the adaptive midpoint subdivision.
                    double xi_split = xi_center, eta_split = eta_center;
                    const auto transition =
                        [&](bool along_xi, double lower, double upper, double transverse, std::size_t initial_owner) {
                            for (std::size_t iteration = 0; iteration < 48; ++iteration) {
                                const double middle = 0.5 * (lower + upper);
                                const std::size_t selected =
                                    along_xi ? owner(middle, transverse) : owner(transverse, middle);
                                if (selected == initial_owner)
                                    lower = middle;
                                else
                                    upper = middle;
                            }
                    ...
                    if (split_xi && !split_eta && owners[0] == owners[3] && owners[1] == owners[2]) {
                        const double lower = transition(true, xi_left, xi_right, eta_lower, owners[0]);
                        const double middle = transition(true, xi_left, xi_right, eta_center, owners[0]);
                        const double upper = transition(true, xi_left, xi_right, eta_upper, owners[0]);
                        if (std::max({lower, middle, upper}) - std::min({lower, middle, upper}) < 1e-10 * xi_size)
                            xi_split = middle;
                    } else if (split_eta && !split_xi && owners[0] == owners[1] && owners[3] == owners[2]) {
                        const double lower = transition(false, eta_lower, eta_upper, xi_left, owners[0]);
                        const double middle = transition(false, eta_lower, eta_upper, xi_center, owners[0]);
                        const double upper = transition(false, eta_lower, eta_upper, xi_right, owners[0]);
                        if (std::max({lower, middle, upper}) - std::min({lower, middle, upper}) < 1e-10 * eta_size)
                            eta_split = middle;
                    }
                    if (split_xi && split_eta) {
                        pending.push_back({cell.xi_lower, xi_split, cell.eta_lower, eta_split, cell.depth + 1});
                        pending.push_back({xi_split, cell.xi_upper, cell.eta_lower, eta_split, cell.depth + 1});
                        pending.push_back({xi_split, cell.xi_upper, eta_split, cell.eta_upper, cell.depth + 1});
                        pending.push_back({cell.xi_lower, xi_split, eta_split, cell.eta_upper, cell.depth + 1});
                    } else if (split_xi) {
                        pending.push_back({cell.xi_lower, xi_split, cell.eta_lower, cell.eta_upper, cell.depth + 1});
                        pending.push_back({xi_split, cell.xi_upper, cell.eta_lower, cell.eta_upper, cell.depth + 1});
                    } else {
                        pending.push_back({cell.xi_lower, cell.xi_upper, cell.eta_lower, eta_split, cell.depth + 1});
                        pending.push_back({cell.xi_lower, cell.xi_upper, eta_split, cell.eta_upper, cell.depth + 1});
                    }
                }
                for (const Cell& cell : leaves) {
                    const double xi_half = 0.5 * (cell.xi_upper - cell.xi_lower),
                    ...
                    for (std::size_t eta_point = 0; eta_point < points.size(); ++eta_point)
                        for (std::size_t xi_point = 0; xi_point < points.size(); ++xi_point) {
                            ...
                            if (primary_face == std::numeric_limits<std::size_t>::max())
                                throw std::invalid_argument("HEX20 small-sliding surface contact '" + definition.name
                                                            + "' has an unprojected reference integration point");
                            const auto point = make_quad8_face_mechanical_point(face.coordinates, xi, eta, weight);
                            face.contact_points.push_back(point);
                            ...
                            face.contact_normal_orientations.push_back(
                                normal_orientation(primary_faces[primary_face].coordinates,
                                    face.parent_centroid,
        ...
        if (definition.mechanical && abaqus_averaged) {
                ...
                if (finite_averaged) {
                    ...
                    for (std::size_t primary_index = 0; primary_index < primary_faces.size(); ++primary_index) {
                        ...
                        const Quad8ReferenceProjectionValue reference =
                            compute_quad8_reference_projection(face.coordinates,
                                primary_face.coordinates,
                                face_center.displacement_shape,
                                normal_orientation(primary_face.coordinates,
                                    face.parent_centroid,
                                    primary_face.parent_centroid));
                        if (!reference.projected)
            ...
            if (finite_averaged) {
                ...
                finite_primary_nodes.erase(std::unique(finite_primary_nodes.begin(), finite_primary_nodes.end()),
                    finite_primary_nodes.end());
            }
            for (std::size_t output = 0; output < builders.size(); ++output) {
                const ConstraintBuilder& builder = builders[output];
                if (!std::isfinite(builder.area) || !(builder.area > 0.0))
                ...
                if (finite_averaged) {
                    ...
                    for (const auto& sample : builder.samples) {
                        ...
                        const double fraction = sample.secondary_local_point < 4 ? 1.0 / 24.0 : 5.0 / 24.0;
                        for (const auto& transfer : abaqus_quad8_primary_transfer_rule(sample.secondary_local_point)) {
                            const auto point = make_quad8_face_mechanical_point(face.coordinates,
                                2.0 * transfer.first - 1.0,
                                2.0 * transfer.second - 1.0,
                                1.0);
                            const auto weights =
                            ...
                            CartesianPoint3 first{weights[0] * point.tangent_xi.x + weights[1] * point.tangent_eta.x,
                                weights[0] * point.tangent_xi.y + weights[1] * point.tangent_eta.y,
                                weights[0] * point.tangent_xi.z + weights[1] * point.tangent_eta.z};
                            const double length = std::sqrt(dot(first, first)), measure = reference_measure(point);
                            if (!(length > 0.0))
                                throw std::invalid_argument("HEX20 reference contact tangent is singular");
                            first = {first.x / length, first.y / length, first.z / length};
                            const auto area = cross(point.tangent_xi, point.tangent_eta);
                            const CartesianPoint3 normal{sample.normal_orientation * area.x / measure,
                                sample.normal_orientation * area.y / measure,
                                sample.normal_orientation * area.z / measure};
                            const auto point_second = cross(normal, first);
                            const double weight = 4.0 * fraction * transfer.weight * measure;
                            tangent.x += weight * first.x;
                            tangent.y += weight * first.y;
                            tangent.z += weight * first.z;
                            second.x += weight * point_second.x;
                            second.y += weight * point_second.y;
                            second.z += weight * point_second.z;
                        }
                    }
                    const double tangent_measure = std::sqrt(dot(tangent, tangent));
                    if (!std::isfinite(tangent_measure) || !(tangent_measure > 0.0))
                    ...
                    constraint.tangent_first = {tangent.x / tangent_measure,
                        tangent.y / tangent_measure,
                        tangent.z / tangent_measure};
                    const auto normal = cross(constraint.tangent_first, second);
                    const double measure = std::sqrt(dot(normal, normal));
                    if (!(measure > 0.0))
                        throw std::invalid_argument("HEX20 reference averaged contact normal is singular");
                    constraint.normal = {normal.x / measure, normal.y / measure, normal.z / measure};
                    constraint.reference_normal = constraint.normal;
                    constraint.reference_tangent_first = constraint.tangent_first;
                    constraint.finite_sliding = true;
                    constraint.finite_sliding_samples = builder.samples;
                ...
                for (const auto& entry : builder.secondary) {
                    const double coefficient = -entry.second / builder.area;
                    if (!finite_averaged && std::abs(coefficient) <= 1.0e-14)
                        continue;
                    constraint.nodes.push_back(entry.first);
                    constraint.gap_coefficients.push_back(coefficient);
                    if (finite_averaged)
                ...
                CartesianPoint3 separation{};
                const CartesianPoint3& origin = finite_averaged ? constraint.reference_coordinates.front()
                                                                : builder.coordinates.at(constraint.nodes.front());
                for (std::size_t node = 0; node < constraint.nodes.size(); ++node) {
                    ...
                    separation.x += constraint.gap_coefficients[node] * (coordinate.x - origin.x);
                    separation.y += constraint.gap_coefficients[node] * (coordinate.y - origin.y);
                    separation.z += constraint.gap_coefficients[node] * (coordinate.z - origin.z);
                }
                constraint.reference_gap = dot(separation, constraint.normal);
                _abaqus_averaged_constraints.push_back(std::move(constraint));
            }
    ...
    std::map<std::pair<std::size_t, std::size_t>, std::size_t> patch_indices;
    for (std::size_t point = 0; point < _hex20_thermal_points.size(); ++point) {
        const auto& metadata = _hex20_thermal_points[point];
        const auto& shape = metadata.quadrature.temperature_shape;
        const double first = shape[1] + shape[2], second = shape[2] + shape[3];
        std::vector<std::size_t> corners;
        constexpr double boundary_tolerance = 1e-12;
        for (std::size_t corner = 0; corner < 4; ++corner) {
            const bool high_first = corner == 1 || corner == 2, high_second = corner >= 2;
            if ((high_first ? first >= 0.5 - boundary_tolerance : first <= 0.5 + boundary_tolerance)
                && (high_second ? second >= 0.5 - boundary_tolerance : second <= 0.5 + boundary_tolerance))
                corners.push_back(corner);
        }
        if (corners.empty())
            throw std::logic_error("HEX20 thermal sample has no corner patch");
        for (std::size_t corner : corners) {
            const auto node =
                _hex20_secondary_contact_faces[metadata.contact][metadata.secondary_face].temperature_nodes[corner];
            const auto key = std::make_pair(metadata.contact, node);
            auto found = patch_indices.find(key);
            if (found == patch_indices.end()) {
                found = patch_indices.emplace(key, _hex20_thermal_patches.size()).first;
                _hex20_thermal_patches.push_back({metadata.contact, {}, {}});
            }
            auto& patch = _hex20_thermal_patches[found->second];
            patch.points.push_back(point);
            patch.fractions.push_back(1.0 / static_cast<double>(corners.size()));
        }
    }
    _thermal_contact_offsets.assign(_definition.contacts.size() + 1, 0);
    _mechanical_contact_offsets.assign(_definition.contacts.size() + 1, 0);
...
SpatialAssembly::MechanicalCandidate SpatialAssembly::mechanical_candidate(std::size_t point,
    ...
    if (surface_to_surface) {
        ...
        const Quad4FaceQuadraturePoint normal_point = make_quad4_face_quadrature_point(secondary.coordinates,
            (4.0 / 3.0) * constraint_location[0],
            (4.0 / 3.0) * constraint_location[1],
            1.0);
        result.surface_geometry = {secondary.coordinates,
            primary_face.coordinates,
            contact_point.shape,
            contact_point.derivative_xi,
            contact_point.derivative_eta,
            normal_point.derivative_xi,
            normal_point.derivative_eta,
            1.0,
            primary_material_orientation(primary_face.coordinates, primary_face.parent_centroid),
    ...
    result.node_geometry = {secondary.coordinates,
        primary_face.coordinates,
        shapes,
        derivatives_xi,
        derivatives_eta,
...
SpatialAssembly::Hex20ThermalCandidate SpatialAssembly::hex20_thermal_candidate(std::size_t point,
    ...
    const std::size_t contact = metadata.contact;
    const Hex20SecondaryContactFace& secondary = _hex20_secondary_contact_faces.at(contact).at(metadata.secondary_face);
    const Hex20PrimaryContactFace& primary_face = _hex20_primary_contact_faces.at(contact).at(primary);
    const Quad8FaceThermalQuadraturePoint& quadrature = metadata.quadrature;
    return {contact,
        secondary.temperature_nodes,
        primary_face.temperature_nodes,
        secondary.displacement_nodes,
        primary_face.displacement_nodes,
        {secondary.coordinates,
            primary_face.coordinates,
            quadrature.temperature_shape,
...
std::vector<Quad8HeatPatchSample> SpatialAssembly::hex20_thermal_patch_samples(std::size_t patch_index,
    const std::vector<std::size_t>& dofs) const {
    const auto& patch = _hex20_thermal_patches.at(patch_index);
    const bool disk = _definition.contacts[patch.contact].mechanical_sliding == MechanicalContactSliding::finite;
    std::vector<Quad8HeatPatchSample> samples;
    ...
    for (std::size_t entry = 0; entry < patch.points.size(); ++entry) {
        ...
        for (std::size_t candidate_index = 0; candidate_index < count; ++candidate_index) {
            ...
            for (std::size_t local = 0; local < global.size(); ++local) {
                ...
                if (found == dofs.end() || *found != global[local])
                    throw std::logic_error("HEX20 thermal patch local DOF mapping is incomplete");
                sample.local_dofs[local] = static_cast<std::size_t>(found - dofs.begin());
            }
...
SpatialAssembly::Hex20MechanicalCandidate SpatialAssembly::hex20_mechanical_candidate(std::size_t point,
    std::size_t primary) const {
    ...
    if (result.surface_to_surface) {
        ...
        result.surface_geometry = {secondary.coordinates,
            ...
            secondary.contact_primary_derivatives_eta.at(metadata.secondary_local_point),
            quadrature.quadrature_weight,
...
SpatialAssembly::SparsityContact SpatialAssembly::sparsity_contact(std::size_t index) const {
    const auto location = offset_location(_sparsity_contact_offsets,
        index,
        "Three-dimensional sparsity contribution index is out of range");
...
SpatialAssembly::Hex20SparsityContact SpatialAssembly::hex20_sparsity_contact(std::size_t index) const {
    ...
    return {_hex20_secondary_contact_faces.at(contact).at(secondary_face).temperature_nodes,
        ...
        _hex20_primary_contact_faces.at(contact).at(primary).displacement_nodes,
        _definition.contacts[contact].thermal,
...
void SpatialAssembly::update_contact_search_trees(const std::vector<double>& state) const {
    ...
    for (std::size_t contact = 0; contact < _definition.contacts.size(); ++contact) {
        ...
        if (!thermal_touched && !mechanical_touched)
            continue;
        if (_uses_hex20) {
            const ResolvedHex20Boundary& primary = _hex20_primary_boundaries[contact];
            if (primary.boundary.faces.size() <= spatial_detail::contact_search_tree_minimum_items)
                continue;
            ...
            for (std::size_t face_index = 0; face_index < primary.boundary.faces.size(); ++face_index) {
                ...
                for (std::size_t node = 0; node < 8; ++node) {
                    ...
                    displacement_dofs[node] = {dof(Field::displacement_x, global),
                        dof(Field::displacement_y, global),
...
bool SpatialAssembly::mark_touched_thermal_points(std::size_t first, std::size_t last) const {
    ...
    for (std::size_t entry = begin; entry < end; ++entry) {
        if (_uses_hex20) {
            for (std::size_t point : _hex20_thermal_patches[entry - ranges.thermal_begin].points)
                _touched_thermal_points[point] = 1U;
        } else
            _touched_thermal_points[entry - ranges.thermal_begin] = 1U;
    }
    return true;
}
...
void SpatialAssembly::update_thermal_candidates(std::size_t first,
    std::size_t last,
    const std::vector<double>& state) const {
    if (state.size() != dof_count())
    ...
    for (std::size_t point = 0; point < _thermal_active_primary.size(); ++point) {
        if (_touched_thermal_points[point] == 0U)
            continue;
        const auto location = offset_location(_thermal_contact_offsets,
            point,
            _uses_hex20 ? "HEX20 thermal-contact point is out of range"
                        : "Three-dimensional thermal-contact point is out of range");
        const std::size_t contact = location.first;
        const std::size_t primary_count =
            _uses_hex20 ? _hex20_primary_contact_faces[contact].size() : _primary_contact_faces[contact].size();
        ...
        if (cached_primary < primary_count)
            consider(cached_primary);
        if (primary_count > spatial_detail::contact_search_tree_minimum_items) {
            ...
            if (_uses_hex20) {
                ...
                search_point = thermal_search_point(representative.geometry, representative_state);
            }
            _contact_search_trees[contact].begin_query(search_point, _contact_search_query);
            std::size_t primary = 0;
            while (_contact_search_trees[contact].next_candidate(_contact_search_query,
...
void SpatialAssembly::update_mechanical_candidates(std::size_t first,
    std::size_t last,
    const std::vector<double>& state) const {
    if (state.size() != dof_count())
    ...
    if (_uses_hex20) {
        for (std::size_t point = 0; point < _hex20_mechanical_points.size(); ++point) {
            ...
            if (surface_to_surface && !finite_sliding) {
                ...
                const ContactProjectionValue value =
                    compute_quad8_to_quad8_contact_projection(candidate.surface_geometry,
                        hex20_contact_state(candidate, state));
                if (value.projected) {
            ...
            const auto consider = [this, point, node, &state](std::size_t primary) {
                ...
                if (distance < _mechanical_minimum_distance[node]
                    || (distance == _mechanical_minimum_distance[node]
                        && primary < _mechanical_selected_primary[node])) {
            ...
            if (primary_count > spatial_detail::contact_search_tree_minimum_items) {
                ...
                std::size_t primary = 0;
                while (_contact_search_trees[metadata.contact].next_candidate(_contact_search_query,
        ...
        for (std::size_t point = 0; point < _hex20_mechanical_points.size(); ++point) {
            ...
            if (_touched_mechanical_nodes[node] == 0U || primary == std::numeric_limits<std::size_t>::max())
                continue;
            const Hex20MechanicalCandidate candidate = hex20_mechanical_candidate(point, primary);
            const Quad8SurfaceContactLocalValues candidate_state = hex20_contact_state(candidate, state);
            const ContactProjectionValue value =
                candidate.surface_to_surface
                    ...
                    : compute_node_to_quad8_contact_projection(candidate.node_geometry, candidate_state);
            if (value.projected)
                _mechanical_active_primary[point] = primary;
        }
    } else {
...
std::vector<std::size_t> SpatialAssembly::required_state_dofs(std::size_t first, std::size_t last) const {
    ...
    const auto append_face = [this, &result](const std::array<std::size_t, 4>& nodes) {
        for (const Field field :
            ...
            for (const std::size_t node : nodes)
                result.push_back(dof(field, node));
    };
    const auto append_hex20_face = [this, &result](const std::array<std::size_t, 4>& temperature_nodes,
        ...
        for (const std::size_t node : temperature_nodes)
            result.push_back(dof(Field::temperature, node));
        for (const Field field : {Field::displacement_x, Field::displacement_y, Field::displacement_z})
            for (const std::size_t node : displacement_nodes)
                result.push_back(dof(field, node));
    };
        ...
        for (std::size_t point = 0; point < _touched_thermal_points.size(); ++point) {
            ...
            if (_uses_hex20) {
                const Hex20SecondaryContactFace& face =
                    _hex20_secondary_contact_faces[location.first][_hex20_thermal_points.at(point).secondary_face];
    ...
    if (mark_touched_mechanical_nodes(first, last)) {
        if (_uses_hex20) {
            for (const Hex20MechanicalPoint& point : _hex20_mechanical_points) {
                if (_touched_mechanical_nodes[mechanical_node_index(point.contact, point.secondary)] == 0U)
                    continue;
    ...
    for (std::size_t contact = 0; contact < _definition.contacts.size(); ++contact) {
        ...
        if (_definition.contacts[contact].mechanical) {
            ...
            for (std::size_t node = _mechanical_contact_offsets[contact];
                node < _mechanical_contact_offsets[contact + 1];
                ++node)
                if (_touched_mechanical_nodes[node] != 0U
                    && _mechanical_selected_primary[node] == std::numeric_limits<std::size_t>::max())
                    ++unprojected;
...
double SpatialAssembly::commit_contact_state(const std::vector<double>& state) {
    ...
    const bool validated_state_matches =
        _fully_validated_contact_state_current && _fully_validated_contact_state.size() == state.size()
    ...
    for (std::size_t point = 0; point < _mechanical_active_primary.size(); ++point) {
        ...
        if (updated[contact][secondary]) {
            ...
            for (std::size_t component = 0; component < 3; ++component) {
                ...
                equal = equal && prior.cartesian_contact_normal[component] == trial.cartesian_contact_normal[component]
...
std::vector<CartesianContactNodeSummary> SpatialAssembly::summarize_contact_nodes(std::size_t contact_value,
    const std::vector<double>& state) const {
    ...
    if (_uses_hex20) {
        ...
        if (summarize_averaged_contact(contact_value, state, result))
            return result;
        if (_definition.contacts[contact_value].mechanical_discretization
            ...
            for (std::size_t point = 0; point < _hex20_mechanical_points.size(); ++point) {
                ...
                if (candidate.primary != _mechanical_selected_primary[history])
                    continue;
                const Quad8SurfaceContactLocalValues current = hex20_contact_state(candidate, state),
                                                     committed =
                                                         hex20_contact_state(candidate, _committed_contact_solution);
            ...
            for (std::size_t secondary_face = 0; secondary_face < secondary.boundary.faces.size(); ++secondary_face) {
                ...
                for (const Quad8FaceMechanicalQuadraturePoint& quadrature :
                    ...
                    for (std::size_t primary_face = 0;
                        primary_face < _hex20_primary_contact_faces[contact_value].size();
                        ++primary_face) {
                        ...
                        if (finite_sliding) {
                            ...
                            trial.surface_geometry = {secondary_geometry.coordinates,
                                primary_geometry.coordinates,
                                quadrature.displacement_shape,
                                quadrature.derivative_xi,
                                quadrature.derivative_eta,
                                {},
                                ...
                                {},
                                quadrature.quadrature_weight,
                                primary_material_orientation(primary_geometry.coordinates,
                                    primary_geometry.parent_centroid),
                                true};
                            const ContactProjectionValue projection =
                                compute_quad8_to_quad8_contact_projection(trial.surface_geometry,
                                    hex20_contact_state(trial, state));
                            if (!projection.projected || std::abs(projection.gap) >= minimum_distance)
                                continue;
                            minimum_distance = std::abs(projection.gap);
                        ...
                        const Quad8ReferenceProjectionValue projection =
                            compute_quad8_reference_projection(secondary_geometry.coordinates,
                                primary_geometry.coordinates,
                                quadrature.displacement_shape,
                                normal_orientation(primary_geometry.coordinates,
                                    secondary_geometry.parent_centroid,
                                    primary_geometry.parent_centroid));
                        if (!projection.projected || std::abs(projection.gap) >= minimum_distance)
                            continue;
                        minimum_distance = std::abs(projection.gap);
                    ...
                    recovery.surface_geometry = {secondary_geometry.coordinates,
                        primary_geometry.coordinates,
                        quadrature.displacement_shape,
                        quadrature.derivative_xi,
                        quadrature.derivative_eta,
                        selected_projection.primary_shape,
                        selected_projection.primary_derivative_xi,
                        selected_projection.primary_derivative_eta,
                        quadrature.quadrature_weight,
                        finite_sliding ? primary_material_orientation(primary_geometry.coordinates,
                                             primary_geometry.parent_centroid)
                                       : normal_orientation(primary_geometry.coordinates,
                                             secondary_geometry.parent_centroid,
                                             primary_geometry.parent_centroid),
                    ...
                    const CartesianContactPointValue value = compute_quad8_to_quad8_contact_value(recovery_properties,
                        recovery.surface_geometry,
                        hex20_contact_state(recovery, state),
            ...
            for (std::size_t node = 0; node < result.size(); ++node) {
                ...
                if (summary.tributary_area == 0.0)
                    continue;
                if (recovery_area[node] != 0.0)
                    summary.pressure = recovery_force[node] / recovery_area[node];
                summary.constraint_pressure = summary.pressure;
                summary.tangential_traction = summary.tangential_force / summary.tributary_area;
                for (std::size_t component = 0; component < 3; ++component) {
        ...
        for (std::size_t point = 0; point < _hex20_mechanical_points.size(); ++point) {
            ...
            if (candidate.primary != _mechanical_selected_primary[node])
                continue;
            const CartesianContactPointValue value =
                compute_node_to_quad8_contact_value(_mechanical_properties[candidate.contact],
                    candidate.node_geometry,
...
InterfaceSummary SpatialAssembly::summarize_interface(std::size_t contact_value,
    const std::vector<double>& state) const {
    if (contact_value >= _definition.contacts.size())
        throw std::out_of_range("Three-dimensional contact index is out of range");
    ...
    if (_uses_hex20) {
        for (std::size_t patch = 0; patch < _hex20_thermal_patches.size(); ++patch) {
            if (_hex20_thermal_patches[patch].contact != contact_value)
                continue;
            std::vector<std::size_t> dofs;
            hex20_thermal_patch_dofs(patch, dofs);
            std::vector<double> local;
            local.reserve(dofs.size());
            for (std::size_t entry : dofs)
                local.push_back(state.at(entry));
            CartesianHeatQuadratureValue value{};
            compute_quad8_gap_heat_patch(_thermal_properties[contact_value],
                hex20_thermal_patch_samples(patch, dofs),
                local,
                nullptr,
                &value);
    ...
    for (std::size_t point = _thermal_contact_offsets[contact_value];
        !_uses_hex20 && point < _thermal_contact_offsets[contact_value + 1];
        ++point) {
        const std::size_t primary = _thermal_active_primary[point];
        if (primary == std::numeric_limits<std::size_t>::max())
            continue;
        const ThermalCandidate candidate = thermal_candidate(point, primary);
        const CartesianHeatQuadratureValue value =
            compute_quad4_to_quad4_gap_heat_value(_thermal_properties[contact_value],
                candidate.geometry,
                contact_state(candidate.nodes, state));
        if (!value.projected)
    ...
    if (_definition.contacts[contact_value].mechanical) {
        for (const CartesianContactNodeSummary& node : summarize_contact_nodes(contact_value, state)) {
            ...
            if (node.constraint_pressure > 0.0)
...
void SpatialAssembly::refresh_controls() {
    ...
    for (std::size_t kernel = 0; kernel < _boundary_data.size(); ++kernel) {
        ...
        if (boundary.type != BoundaryConditionType::convection) {
            _boundary_data[kernel].load = spatial_detail::controlled_value(_definition,
                _time,
                _load_factor,
                boundary.value,
