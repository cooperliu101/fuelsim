...
UnstructuredMeshMetadata::UnstructuredMeshMetadata(std::size_t node_count,
    ...
    if (_element_blocks.empty())
        throw std::invalid_argument(geometry_name + " requires element block metadata");
    for (std::size_t block = 0; block < _element_blocks.size(); ++block) {
        ...
        for (std::size_t previous = 0; previous < block; ++previous)
            if (_element_blocks[previous].id == _element_blocks[block].id
                || (!_element_blocks[block].name.empty()
    ...
    for (const std::int64_t id : _element_block_ids)
        if (std::none_of(_element_blocks.begin(), _element_blocks.end(), [id](const ElementBlockInfo& block) {
                return block.id == id;
            }))
    ...
    for (std::size_t set = 0; set < _node_sets.size(); ++set) {
        ...
        if (current.id < 0)
            throw std::invalid_argument(geometry_name + " node set IDs must be nonnegative");
        ...
        for (const std::size_t node : current.nodes)
            if (node >= node_count)
                throw std::out_of_range(geometry_name + " node set is out of range");
    }
    for (std::size_t set = 0; set < _side_sets.size(); ++set) {
        const SideSet& current = _side_sets[set];
        if (current.id < 0)
            throw std::invalid_argument(geometry_name + " side set IDs must be nonnegative");
        for (std::size_t previous = 0; previous < set; ++previous)
...
const ElementBlockInfo& UnstructuredMeshMetadata::element_block(const std::string& name) const {
    const auto block = std::find_if(_element_blocks.begin(),
        _element_blocks.end(),
...
const SideSet& UnstructuredMeshMetadata::side_set(const std::string& name) const {
    const auto set = std::find_if(_side_sets.begin(), _side_sets.end(), [&name](const SideSet& candidate) {
...
UnstructuredQuad4Mesh::UnstructuredQuad4Mesh(std::vector<RzPoint> nodes,
...
UnstructuredHex8Mesh::UnstructuredHex8Mesh(std::vector<CartesianPoint3> nodes,
...
UnstructuredHex20Mesh::UnstructuredHex20Mesh(std::vector<CartesianPoint3> nodes,
    ...
    for (const Hex20Element& element : _elements)
        for (const std::size_t node : element.nodes)
            if (node >= _nodes.size())
                throw std::out_of_range("UnstructuredHex20Mesh connectivity is out of range");
}

RegionMeshMapping::RegionMeshMapping(const UnstructuredMeshMetadata& source,
    ...
    const bool known_block = std::any_of(source.element_blocks().begin(),
        source.element_blocks().end(),
...
Hex8RegionMesh Hex8RegionMesh::from_unstructured_block(const UnstructuredHex8Mesh& source, std::int64_t block_id) {
    ...
    for (const std::size_t source_node : mesh._source_node_ids)
...
Hex8RegionBoundary Hex8RegionMesh::map_side_set(const UnstructuredHex8Mesh& source,
    ...
    std::sort(result.nodes.begin(), result.nodes.end(), [&](std::size_t lhs, std::size_t rhs) {
...
Hex20RegionMesh Hex20RegionMesh::from_unstructured_block(const UnstructuredHex20Mesh& source, std::int64_t block_id) {
    ...
    for (const std::size_t source_element : mesh._source_element_ids) {
        ...
        for (std::size_t node : element.nodes)
            used_nodes[node] = true;
        for (std::size_t node = 0; node < 8; ++node)
            temperature_source_nodes[element.nodes[node]] = true;
    }
    mesh.select_nodes(used_nodes);
    ...
    for (std::size_t source_element : mesh._source_element_ids) {
        ...
        for (std::size_t node = 0; node < element.nodes.size(); ++node) {
            const std::size_t local = mesh._source_node_to_local.at(source.elements()[source_element].nodes[node]);
            if (local == invalid_index)
                throw std::logic_error("Hex20RegionMesh connectivity crosses element blocks");
            element.nodes[node] = local;
        }
        mesh._elements.push_back(element);
    }
...
Hex20RegionBoundary Hex20RegionMesh::map_side_set(const UnstructuredHex20Mesh& source,
    const std::string& side_set_name) const {
    if (source.side_set_block_id(side_set_name) != _block_id)
        throw std::invalid_argument("Side set belongs to an unexpected block: " + side_set_name);
    ...
    for (const ElementSide& side : source.side_set(side_set_name).sides) {
        ...
        for (std::size_t node = 0; node < face.nodes.size(); ++node) {
            const std::size_t source_node = source.elements()[side.element].nodes[face_nodes[side.local_side][node]],
                              local_node = _source_node_to_local.at(source_node);
            if (local_node == invalid_index)
                throw std::logic_error("Hex20RegionMesh side-set node mapping failed");
            face.nodes[node] = local_node;
            result.displacement_nodes.push_back(local_node);
            if (node < 4)
                result.temperature_nodes.push_back(local_node);
        }
        result.faces.push_back(face);
    }
