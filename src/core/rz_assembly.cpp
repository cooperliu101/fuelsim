...
std::vector<std::int64_t> resolve_block_ids(const SpatialDefinition& definition,
    ...
    for (const RegionDefinition& region : definition.regions) {
        ...
        const bool known = std::any_of(source_mesh.element_blocks().begin(),
...
void validate_dirichlet_conditions(std::vector<DirichletCondition>& conditions,
    ...
    std::sort(conditions.begin(), conditions.end(), [](const DirichletCondition& lhs, const DirichletCondition& rhs) {
        return lhs.dof < rhs.dof;
    });
    for (std::size_t index = 1; index < conditions.size(); ++index) {
        if (conditions[index - 1].dof != conditions[index].dof)
            continue;
...
SpatialLayout::SpatialLayout(SpatialDefinition definition, std::vector<std::int64_t> block_ids, DofLayout layout)
    ...
    for (const auto& contact : _definition.contacts)
        if (_layout != DofLayout::axisymmetric_rz
            && contact.thermal_discretization == ThermalContactDiscretization::node_to_surface)
            throw std::invalid_argument("node_to_surface thermal contact requires axisymmetric_rz");
}

void SpatialLayout::initialize_counts(const std::vector<std::size_t>& node_counts,
    ...
    for (std::size_t region = 0; region < region_count(); ++region) {
        ...
        for (std::size_t local = 0; local < end - begin; ++local)
            _region_global_nodes[region][local] = begin + local;
    }
...
void SpatialLayout::initialize_field_layout() {
    ...
    if (_layout == DofLayout::axisymmetric_rz) {
        ...
        _field_layout.push_back({"axial",
            _temperature_node_count + _node_count,
            _temperature_node_count + 2 * _node_count,
            FieldCategory::mechanical});
    } else {
        _field_layout.push_back({"displacement_x",
            _temperature_node_count,
            _temperature_node_count + _node_count,
            FieldCategory::mechanical});
        _field_layout.push_back({"displacement_y",
            _temperature_node_count + _node_count,
            _temperature_node_count + 2 * _node_count,
            FieldCategory::mechanical});
        _field_layout.push_back({"displacement_z",
            _temperature_node_count + 2 * _node_count,
...
void SpatialLayout::initialize_shared_nodes(const std::vector<std::vector<std::size_t>>& region_source_node_ids) {
    ...
    if (next_global == 0)
        throw std::invalid_argument("Shared-node spatial layout must contain at least one node");
...
void SpatialLayout::initialize_mixed_shared_nodes(const std::vector<std::vector<std::size_t>>& region_source_node_ids,
    ...
    for (std::size_t region = 0; region < region_count(); ++region) {
        ...
        for (std::size_t local = 0; local < mapping.size(); ++local) {
            if (!region_temperature_nodes[region][local])
                continue;
...
std::size_t SpatialLayout::region_node_offset(std::size_t index) const {
    ...
    if (nodes.empty())
        throw std::logic_error("Spatial layout region has no nodes");
    return nodes.front();
}
...
std::pair<std::size_t, std::size_t> SpatialLayout::element_location(std::size_t index) const {
    if (index >= volume_contribution_count())
        throw std::out_of_range("Volume contribution is out of range");
...
double SpatialLayout::region_heat_source(std::size_t index) const {
    ...
    return controlled_value(_definition,
        _time,
        _load_factor,
        value.volumetric_heat_source,
        true,
        value.heat_source_function);
}
...
std::size_t SpatialLayout::dof(Field field, std::size_t node) const {
    ...
    if (_layout == DofLayout::axisymmetric_rz) {
        ...
        if (field == Field::displacement_z)
            return temperature_node_count() + 2 * node_count() + node;
    }
...
std::size_t SpatialLayout::global_temperature_node(std::size_t region_index, std::size_t local_node) const {
    ...
    if (result == invalid_node)
        throw std::invalid_argument("Spatial layout node does not carry temperature");
    return result;
}
...
void SpatialLayout::add_dirichlet(std::size_t dof, std::size_t boundary_index) {
    ...
    _dirichlet_conditions.push_back({dof,
        controlled_value(_definition,
            _time,
            _load_factor,
            boundary.value,
...
void SpatialLayout::set_load_factor_value(double value) {
    if (!std::isfinite(value))
        throw std::invalid_argument("Spatial layout load factor must be finite");
    _load_factor = value;
}

void SpatialLayout::set_time_value(double value) {
    if (!std::isfinite(value))
        throw std::invalid_argument("Spatial layout time must be finite");
    _time = value;
}
...
void SpatialLayout::refresh_dirichlet_values() {
    for (const ControlledDirichlet& controlled : _controlled_dirichlet_conditions) {
        ...
        condition->value = controlled_value(_definition,
            _time,
            _load_factor,
            boundary.value,
            boundary.scale_with_load,
...
bool SpatialLayout::boundary_uses_displaced_geometry(const BoundaryConditionDefinition& boundary,
...
void SpatialLayout::record_configuration_warning(const BoundaryConditionDefinition& boundary,
    ...
    if (!boundary.configuration_explicit)
        return;
    if (boundary.use_displaced_geometry == recommended_current_configuration)
        return;
...
ResolvedBoundary SpatialAssembly::resolve_boundary(const UnstructuredQuad4Mesh& source_mesh,
...
std::pair<std::size_t, std::array<std::size_t, 2>> SpatialAssembly::edge_parent(std::size_t region,
...
SpatialAssembly::ContributionLocation SpatialAssembly::locate_contribution(std::size_t index) const {
    ...
    if (index < ranges.thermal_begin)
        return {SpatialContributionType::volume, index};
...
void SpatialAssembly::build_boundaries(const UnstructuredQuad4Mesh& source_mesh) {
    for (std::size_t boundary_index = 0; boundary_index < _definition.boundary_conditions.size(); ++boundary_index) {
        ...
        if (definition.type == BoundaryConditionType::pressure) {
            ...
            _boundary_data.push_back({Line2RzBoundaryKind::pressure,
                TractionComponent::radial,
                spatial_detail::controlled_value(_definition,
                    _time,
                    _load_factor,
                    definition.value,
                    ...
                    definition.function),
                0.0,
                boundary_uses_displaced_geometry(definition, region(resolved.region))});
            ...
            _boundary_data.push_back({Line2RzBoundaryKind::convection,
                TractionComponent::radial,
...
void SpatialAssembly::refresh_controlled_values() {
    ...
    for (std::size_t load = 0; load < _boundary_definition_indices.size(); ++load) {
        ...
        if (boundary.type == BoundaryConditionType::convection) {
            ...
            _boundary_data[load].load = spatial_detail::controlled_value(_definition,
                _time,
                _load_factor,
                boundary.value,
                boundary.scale_with_load,
...
void SpatialAssembly::commit_contact_state(const std::vector<double>& state) {
    ...
    for (std::size_t point = 0; point < _mechanical_active_candidates.size(); ++point) {
        ...
        if (!value.projected)
            continue;
        ContactPointHistory trial = {value.elastic_tangential_slip,
            value.sliding,
...
void SpatialAssembly::update_thermal_candidates(std::size_t first,
    ...
    if (!_uses_contact_search_tree) {
        std::fill(_thermal_minimum_distance.begin(),
            _thermal_minimum_distance.end(),
            std::numeric_limits<double>::infinity());
        std::fill(_thermal_active_candidates.begin(),
            _thermal_active_candidates.end(),
...
void SpatialAssembly::update_large_thermal_candidates(const std::vector<double>& state) const {
    for (std::size_t point = 0; point < _thermal_active_candidates.size(); ++point) {
        ...
        if (end - begin > spatial_detail::contact_search_tree_minimum_items) {
            ...
            while (_contact_search_trees[representative.contact].next_candidate(_contact_search_query,
...
void SpatialAssembly::update_mechanical_candidates(std::size_t first,
    ...
    if (!mark_touched_mechanical_nodes(first, last))
        return;
    if (!_uses_contact_search_tree) {
        std::fill(_mechanical_minimum_distance.begin(),
            _mechanical_minimum_distance.end(),
            std::numeric_limits<double>::infinity());
        std::fill(_mechanical_selected_primary.begin(),
            _mechanical_selected_primary.end(),
            std::numeric_limits<std::size_t>::max());
        std::fill(_projected_mechanical_candidates.begin(), _projected_mechanical_candidates.end(), 0U);
        std::fill(_mechanical_active_candidates.begin(),
            _mechanical_active_candidates.end(),
            std::numeric_limits<std::size_t>::max());
...
void SpatialAssembly::update_large_mechanical_candidates(const std::vector<double>& state) const {
    ...
    for (std::size_t point = 0; point < _mechanical_points.size(); ++point) {
        ...
        const ContactProjectionValue value =
            compute_node_to_line_rz_contact_projection(candidate.geometry, contact_state(candidate.nodes, state));
        if (value.projected)
            _mechanical_active_candidates[point] = entry;
    }
    for (std::size_t node = 0; node < _mechanical_selected_primary.size(); ++node)
        if (_touched_mechanical_nodes[node] != 0U
            && _mechanical_selected_primary[node] != std::numeric_limits<std::size_t>::max())
...
bool SpatialAssembly::mark_touched_thermal_points(std::size_t first, std::size_t last) const {
    ...
    for (std::size_t full = begin; full < end; ++full)
        _touched_thermal_points[full - ranges.thermal_begin] = 1U;
    return true;
}
...
std::vector<ContactNodeSummary> SpatialAssembly::summarize_contact_nodes(std::size_t contact_value,
    ...
    for (std::size_t node : secondary.boundary.nodes) {
        result.push_back({mesh.nodes().at(node).r,
            mesh.nodes().at(node).z,
            false,
            std::numeric_limits<std::size_t>::max(),
            std::numeric_limits<double>::infinity(),
            0.0,
    ...
    for (std::size_t contact_value = 0; contact_value < _definition.contacts.size(); ++contact_value) {
        if (_definition.contacts[contact_value].thermal) {
            ...
            for (std::size_t point = _thermal_contact_offsets[contact_value];
                point < _thermal_contact_offsets[contact_value + 1];
                ++point)
                if (_touched_thermal_points[point] != 0U
                    && _thermal_active_candidates[point] == std::numeric_limits<std::size_t>::max())
                    ++unprojected;
        ...
        if (_definition.contacts[contact_value].mechanical) {
            ...
            for (std::size_t node = _mechanical_contact_offsets[contact_value];
                node < _mechanical_contact_offsets[contact_value + 1];
                ++node)
                if (_touched_mechanical_nodes[node] != 0U
                    && _mechanical_selected_primary[node] == std::numeric_limits<std::size_t>::max())
                    ++unprojected;
...
double
minimum_boundary_normal_length(const SpatialAssembly& layout, std::size_t region, const RegionBoundary& boundary) {
...
std::vector<RzPoint>
boundary_parent_centroids(const SpatialAssembly& layout, std::size_t region, const RegionBoundary& boundary) {
    const RegionMesh& mesh = layout.region_mesh(region);
...
std::array<std::size_t, 4> interface_nodes(const SpatialAssembly& layout,
    std::size_t secondary_region,
    const Line2BoundaryElement& secondary_edge,
    std::size_t primary_region,
...
double zero_gap_orientation_hint(const RzPoint& secondary_centroid,
    const RzPoint& primary_centroid,
...
void SpatialAssembly::initialize_contact_search_workspace() {
    ...
    std::partial_sum(_thermal_candidate_offsets.begin(),
        _thermal_candidate_offsets.end(),
        _thermal_candidate_offsets.begin());
    ...
    std::partial_sum(_mechanical_candidate_offsets.begin(),
        _mechanical_candidate_offsets.end(),
    ...
    for (const ResolvedBoundary& primary : _primary_boundaries)
        _uses_contact_search_tree =
            _uses_contact_search_tree
            || primary.boundary.elements.size() > spatial_detail::contact_search_tree_minimum_items;
}
...
void SpatialAssembly::update_contact_search_trees(const std::vector<double>& state) const {
    ...
    for (std::size_t contact = 0; contact < _definition.contacts.size(); ++contact) {
        ...
        if (primary.boundary.elements.size() <= spatial_detail::contact_search_tree_minimum_items)
            continue;
        ...
        for (std::size_t edge = 0; edge < primary.boundary.elements.size(); ++edge) {
            ...
            for (std::size_t local_node : element.nodes) {
                ...
                const std::array<double, 3> current = {reference.r + state[dof(Field::radial_displacement, node)],
                    reference.z + state[dof(Field::axial_displacement, node)],
                    0.0};
...
Cax4LocalDofs SpatialAssembly::contribution_dofs(std::size_t index) const {
    ...
    case SpatialContributionType::convection:
        return local_dofs(_boundary_contributions.at(entry.local_index).nodes);
    }
...
Cax4LocalDofs SpatialAssembly::sparsity_contribution_dofs(std::size_t index) const {
    ...
    if (index < _thermal_contributions.size())
        return local_dofs(_thermal_contributions[index].nodes);
    index -= _thermal_contributions.size();
    if (index < _mechanical_contributions.size())
        return local_dofs(_mechanical_contributions[index].nodes);
    index -= _mechanical_contributions.size();
    if (index < _boundary_contributions.size())
        return local_dofs(_boundary_contributions[index].nodes);
...
Cax4LocalResidual SpatialAssembly::compute_contribution(std::size_t index,
    ...
    case SpatialContributionType::volume:
        throw std::logic_error("SpatialAssembly does not own volume physics");
    case SpatialContributionType::thermal_contact: {
    ...
    case SpatialContributionType::mechanical_contact: {
        ...
        return compute_node_to_line_rz_contact(_mechanical_properties[entry.contact],
            entry.geometry,
            state,
            contribution_state(index, _committed_contact_solution),
    ...
    for (std::size_t contact_value = 0; contact_value < _definition.contacts.size(); ++contact_value) {
        ...
        if (contact_definition.thermal) {
            for (std::size_t edge_index = 0; edge_index < secondary.boundary.elements.size(); ++edge_index) {
                ...
                if (contact_definition.thermal_discretization == ThermalContactDiscretization::node_to_surface) {
                    for (std::size_t node = 0; node < 2; ++node) {
                        ...
                        for (std::size_t candidate = 0; candidate < primary.boundary.elements.size(); ++candidate) {
                            const auto& candidate_edge = primary.boundary.elements[candidate];
                            const auto coordinates = edge_coordinates(primary_mesh, candidate_edge);
                            auto geometry = make_line2_rz_heat_point_geometry(secondary_coordinates,
                                coordinates,
                                shape,
                                1.0,
                            ...
                            geometry.primary_segment_is_first = candidate == 0;
                            _thermal_contributions.push_back({contact_value,
                                interface_nodes(*this,
                                    secondary.region,
                                    secondary_edge,
                                    primary.region,
                                    candidate_edge),
                        ...
                        for (std::size_t candidate = 0; candidate < primary.boundary.elements.size(); ++candidate) {
                            ...
                            _thermal_contributions.push_back({contact_value,
                                interface_nodes(*this,
                                    ...
                                    primary.region,
                                    candidate_edge),
                                make_line2_rz_heat_point_geometry(secondary_coordinates,
                                    candidate_coordinates,
                                    point.secondary_shape,
                ...
                for (std::size_t secondary_node = 0; secondary_node < line2_interface_side_node_count;
                    ...
                    for (std::size_t candidate = 0; candidate < primary.boundary.elements.size(); ++candidate) {
                        ...
                        _mechanical_contributions.push_back({contact_value,
                            ...
                            make_node_to_line_rz_contact_geometry(secondary_coordinates,
                                primary_coordinates,
                                secondary_node,
                                candidate == 0,
                                candidate + 1 == primary.boundary.elements.size(),
