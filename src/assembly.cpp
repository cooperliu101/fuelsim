...
SpatialLayout::SpatialLayout(SpatialDefinition definition, std::vector<std::int64_t> block_ids,
...
const Quad4RzGeometry& SpatialLayout::element_geometry(std::size_t region_value, std::size_t element) const {
    return _region_geometries.at(region_value).at(element);
}

SpatialLayout::ResolvedBoundary SpatialLayout::resolve_boundary(const UnstructuredQuad4Mesh& source_mesh,
...
std::pair<std::size_t, std::array<std::size_t, 2>> SpatialLayout::edge_parent(std::size_t region,
...
SpatialAssembly::ContributionRanges SpatialAssembly::contribution_ranges() const noexcept {
    ...
    return {thermal_begin,  mechanical_begin, pressure_begin,
            traction_begin, convection_begin, convection_begin + _boundary.convection_contribution_count()};
}
...
const Quad4RzGeometry& SpatialAssembly::region_element_geometry(std::size_t region_value,
...
BoundaryEdgeData boundary_edge_data(const SpatialLayout& layout, const SpatialLayout::ResolvedBoundary& boundary,
        ...
        if (definition.type == BoundaryConditionType::dirichlet) {
            for (const std::size_t local_node : resolved.boundary.nodes) {
                const std::size_t dof =
                    layout.dof_map().dof(definition.field, layout.global_node(resolved.region, local_node));
                _dirichlet_conditions.push_back(
                    {dof, load_multiplier(definition.scale_with_load, definition.function, layout) * definition.value});
                if (definition.scale_with_load || !definition.function.empty())
                    _controlled_dirichlet_conditions.push_back(
                        {dof, definition.value, definition.scale_with_load, definition.function});
            }
            ...
            _pressure_kernels.emplace_back(PressureProperties{
                load_multiplier(definition.scale_with_load, definition.function, layout) * definition.value,
                displaced});
            ...
            if (definition.use_displaced_geometry &&
                layout.region(resolved.region).strain_formulation != StrainFormulation::finite)
                throw std::invalid_argument("Current-configuration traction requires finite strain: " +
                                            definition.name);
            ...
            _convection_loads.push_back({definition.heat_transfer_coefficient, definition.ambient_temperature,
                                         definition.coefficient_function, definition.ambient_temperature_function});
            _convection_kernels.emplace_back(
                ConvectionProperties{definition.heat_transfer_coefficient, definition.ambient_temperature});
            for (const Line2BoundaryElement& edge : resolved.boundary.elements) {
                const BoundaryEdgeData data = boundary_edge_data(layout, resolved, edge);
...
double BoundaryAssembly::function_value(const std::string& name, const SpatialLayout& layout) const {
    const auto found = std::find_if(layout.definition().time_tables.begin(), layout.definition().time_tables.end(),
...
double BoundaryAssembly::load_multiplier(bool scale_with_load, const std::string& function,
...
void BoundaryAssembly::refresh_controlled_values(const SpatialLayout& layout) {
    for (const ControlledDirichlet& controlled : _controlled_dirichlet_conditions) {
        const auto condition =
            std::lower_bound(_dirichlet_conditions.begin(), _dirichlet_conditions.end(), controlled.dof,
        ...
        if (condition == _dirichlet_conditions.end() || condition->dof != controlled.dof)
            throw std::logic_error("SpatialAssembly controlled Dirichlet mapping is invalid");
    ...
    for (std::size_t load = 0; load < _convection_loads.size(); ++load) {
        ...
        const double coefficient_multiplier =
            convection.coefficient_function.empty() ? 1.0 : function_value(convection.coefficient_function, layout);
        const double ambient_multiplier = convection.ambient_temperature_function.empty()
                                              ? 1.0
                                              : function_value(convection.ambient_temperature_function, layout);
...
double BoundaryAssembly::region_heat_source(std::size_t region_index, const SpatialLayout& layout) const {
    ...
    const double multiplier =
        region.heat_source_function.empty() ? _load_factor : function_value(region.heat_source_function, layout);
...
LocalDofs BoundaryAssembly::contribution_dofs(SpatialContributionType type, std::size_t index,
                                              const SpatialLayout& layout) const {
    switch (type) {
    ...
    case SpatialContributionType::convection:
        return layout.dof_map().local_dofs(_convection_contributions.at(index).nodes);
    default: throw std::invalid_argument("BoundaryAssembly requires a boundary contribution type");
    }
}

LocalResidual BoundaryAssembly::contribution_residual(SpatialContributionType type, std::size_t index,
...
LocalSystem BoundaryAssembly::linearize_contribution(SpatialContributionType type, std::size_t index,
                                                     const LocalValues& state) const {
    switch (type) {
    case SpatialContributionType::pressure: {
        const PressureContribution& contribution = _pressure_contributions.at(index);
...
const std::vector<DirichletCondition>& BoundaryAssembly::dirichlet_conditions() const noexcept {
...
AugmentedContactUpdate SpatialAssembly::update_augmented_contact_multipliers(const std::vector<double>& state,
    ...
    return _contact.update_augmented_multipliers(*this, state, completed_updates);
}
...
std::size_t ContactAssembly::contact_count(const SpatialLayout& layout) const noexcept {
    return layout.definition().contacts.size();
}

const ContactDefinition& ContactAssembly::contact(std::size_t index, const SpatialLayout& layout) const {
    return layout.definition().contacts.at(index);
}

const std::vector<std::vector<ContactPointHistory>>& ContactAssembly::committed_histories() const noexcept {
    return _contact_histories;
}

bool ContactAssembly::uses_augmented_contact(const SpatialLayout& layout) const noexcept {
    return std::any_of(layout.definition().contacts.begin(), layout.definition().contacts.end(),
...
AugmentedContactUpdate ContactAssembly::update_augmented_multipliers(SpatialAssembly& assembly,
...
void ContactAssembly::commit_state(SpatialAssembly& assembly, const std::vector<double>& state) {
    ...
    for (std::size_t contribution = 0; contribution < _mechanical_contributions.size(); ++contribution) {
        ...
        const LocalValues committed_state =
            assembly.contribution_state(first_mechanical + contribution, _committed_contact_solution);
        const ContactPointValue value =
            _mechanical_kernels[candidate.contact].value(candidate.geometry, local_state, committed_state,
                                                         _contact_histories[candidate.contact][candidate.secondary]);
        if (!value.projected) continue;
        const ContactPointHistory trial = _mechanical_kernels[candidate.contact].trial_history(
...
void ContactAssembly::restore_state(const SpatialAssembly& assembly, const std::vector<double>& state,
                                    std::vector<std::vector<ContactPointHistory>> histories) {
...
void SpatialAssembly::update_thermal_candidates(std::size_t contribution_begin, std::size_t contribution_end,
    ...
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value) {
        minimum_distance[contact_value].assign(_contact._thermal_point_counts[contact_value],
                                               std::numeric_limits<double>::infinity());
        selected_primary[contact_value].assign(_contact._thermal_point_counts[contact_value],
    ...
    for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
        ...
        const HeatQuadratureValue value =
            _contact._thermal_kernels[candidate.contact].quadrature_value(candidate.geometry, local_state);
        if (!value.projected) continue;
        ...
        if (distance < minimum_distance[candidate.contact][candidate.integration_point] ||
            (distance == minimum_distance[candidate.contact][candidate.integration_point] &&
             candidate.primary < selected_primary[candidate.contact][candidate.integration_point])) {
...
void SpatialAssembly::update_mechanical_candidates(std::size_t contribution_begin, std::size_t contribution_end,
                                                   const GlobalStateView& state) const {
    ...
    for (std::size_t contribution = 0; contribution < _contact._mechanical_contributions.size(); ++contribution) {
        ...
        const LocalValues committed_state =
            contribution_state(ranges.mechanical_begin + contribution, _contact._committed_contact_solution);
        const ContactPointValue value = _contact._mechanical_kernels[candidate.contact].value(
            candidate.geometry, local_state, committed_state,
        ...
        if (distance < minimum_distance[candidate.contact][candidate.secondary] ||
            (distance == minimum_distance[candidate.contact][candidate.secondary] &&
             candidate.primary < selected_primary[candidate.contact][candidate.secondary])) {
...
std::vector<std::vector<bool>> SpatialAssembly::touched_thermal_points(std::size_t contribution_begin,
    ...
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value)
        result[contact_value].resize(_contact._thermal_point_counts[contact_value], false);
...
std::vector<std::vector<bool>> SpatialAssembly::touched_mechanical_nodes(std::size_t contribution_begin,
                                                                         std::size_t contribution_end) const {
    const ContributionRanges ranges = contribution_ranges();
    ...
    std::vector<std::vector<bool>> result(contact_count());
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value)
        result[contact_value].resize(_contact._contact_histories[contact_value].size(), false);
    for (std::size_t full = begin; full < end; ++full) {
...
std::vector<ContactNodeSummary> SpatialAssembly::summarize_contact_nodes(std::size_t contact_value,
    ...
    for (std::size_t node : secondary.boundary.nodes) {
        result.push_back({mesh.nodes().at(node).r, mesh.nodes().at(node).z, false,
                          std::numeric_limits<std::size_t>::max(), std::numeric_limits<double>::infinity(), 0.0, 0.0,
    ...
    for (std::size_t contribution = 0; contribution < _contact._mechanical_contributions.size(); ++contribution) {
        ...
        const LocalValues committed_state =
            contribution_state(first_mechanical + contribution, _contact._committed_contact_solution);
        const std::size_t secondary_index = candidate.secondary;
        const ContactPointValue value = _contact._mechanical_kernels[contact_value].value(
            candidate.geometry, local_state, committed_state,
...
void SpatialAssembly::validate_state(const std::vector<double>& state) const {
    ...
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value) {
        if (_layout._definition.contacts[contact_value].thermal) {
            const std::size_t unprojected =
                static_cast<std::size_t>(std::count(_contact._projected_thermal_points[contact_value].begin(),
                                                    _contact._projected_thermal_points[contact_value].end(), false));
            if (unprojected != 0)
                throw std::domain_error("Thermal contact '" + _layout._definition.contacts[contact_value].name +
        ...
        const std::size_t unprojected =
            static_cast<std::size_t>(std::count(_contact._projected_mechanical_nodes[contact_value].begin(),
                                                _contact._projected_mechanical_nodes[contact_value].end(), false));
        if (unprojected != 0)
            throw std::domain_error("Mechanical contact '" + _layout._definition.contacts[contact_value].name +
                                    "' lost projection for " + std::to_string(unprojected) +
                                    " secondary nodes after searching the complete primary chain");
    }
}

void SpatialAssembly::validate_local_state(std::size_t contribution_begin, std::size_t contribution_end,
                                           const GlobalStateView& state) const {
    ...
    if (!touched_thermal.empty()) {
        for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value) {
            ...
            for (std::size_t point = 0; point < touched_thermal[contact_value].size(); ++point) {
                if (touched_thermal[contact_value][point] && !_contact._projected_thermal_points[contact_value][point])
                    ++unprojected;
            }
            if (unprojected != 0)
                throw std::domain_error("Thermal contact '" + _layout._definition.contacts[contact_value].name +
    ...
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value) {
        ...
        for (std::size_t node = 0; node < touched[contact_value].size(); ++node) {
            if (touched[contact_value][node] && !_contact._projected_mechanical_nodes[contact_value][node])
                ++unprojected;
        }
        if (unprojected != 0)
            throw std::domain_error("Mechanical contact '" + _layout._definition.contacts[contact_value].name +
...
InterfaceSummary SpatialAssembly::summarize_interface(std::size_t contact_value,
    ...
    for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
        ...
        const HeatQuadratureValue value =
            _contact._thermal_kernels[contact_value].quadrature_value(candidate.geometry, local_state);
...
std::vector<RzPoint> boundary_parent_centroids(const SpatialLayout& layout, std::size_t region,
...
std::array<std::size_t, 4> interface_nodes(const SpatialLayout& layout, std::size_t secondary_region,
        ...
        for (std::size_t previous = 0; previous < region; ++previous) {
            ...
            if ((!value.block.empty() && definition.regions[previous].block == value.block) ||
                ...
                throw std::invalid_argument("Duplicate region block: " +
                                            (value.block.empty() ? std::to_string(value.block_id) : value.block));
        }
    }
...
std::vector<std::int64_t> SpatialAssembly::resolve_block_ids(const SpatialDefinition& definition,
...
std::vector<RegionMesh> SpatialAssembly::build_meshes(const SpatialDefinition& definition,
                                                      const UnstructuredQuad4Mesh& source_mesh) {
...
SpatialAssembly::SpatialAssembly(SpatialDefinition definition, const UnstructuredQuad4Mesh& source_mesh)
    : SpatialAssembly(definition, source_mesh, resolve_block_ids(definition, source_mesh),
...
std::vector<double> SpatialAssembly::initial_state() const {
    ...
    for (std::size_t region_value = 0; region_value < region_count(); ++region_value) {
        ...
        for (std::size_t node = 0; node < _layout._meshes[region_value].nodes().size(); ++node) {
            result[_layout._dof_map.temperature(offset + node)] =
                _layout._definition.regions[region_value].initial_temperature;
        }
    }
    for (const DirichletCondition& condition : _boundary.dirichlet_conditions())
...
std::vector<std::size_t> SpatialAssembly::required_state_dofs(std::size_t contribution_begin,
                                                              std::size_t contribution_end) const {
    if (contribution_begin > contribution_end || contribution_end > contribution_count())
        throw std::out_of_range("SpatialAssembly contribution range is out of bounds");
    std::vector<std::size_t> result;
...
std::pair<std::size_t, std::size_t> SpatialAssembly::element_location(std::size_t contribution_index) const {
    ...
    const auto upper =
        std::upper_bound(_layout._element_offsets.begin(), _layout._element_offsets.end(), contribution_index);
...
LocalDofs SpatialAssembly::contribution_dofs(std::size_t contribution_index) const {
    ...
    case SpatialContributionType::volume: {
        ...
        std::array<std::size_t, 4> nodes{};
        for (std::size_t node = 0; node < nodes.size(); ++node)
            nodes[node] = _layout.global_node(location.first, element.nodes[node]);
        return _layout._dof_map.local_dofs(nodes);
    }
    case SpatialContributionType::thermal_contact:
        return _layout._dof_map.local_dofs(_contact._thermal_contributions.at(contribution.local_index).nodes);
    case SpatialContributionType::mechanical_contact:
        return _layout._dof_map.local_dofs(_contact._mechanical_contributions.at(contribution.local_index).nodes);
    case SpatialContributionType::pressure:
    case SpatialContributionType::traction:
    case SpatialContributionType::convection:
        return _boundary.contribution_dofs(contribution.type, contribution.local_index, _layout);
    }
    throw std::logic_error("SpatialAssembly contribution type is invalid");
}

LocalValues SpatialAssembly::contribution_state(std::size_t contribution_index,
...
LocalResidual SpatialAssembly::contribution_residual(std::size_t contribution_index, const LocalValues& state) const {
    ...
    case SpatialContributionType::mechanical_contact: {
        ...
        return _contact._mechanical_kernels[contribution.contact].residual(
            contribution.geometry, state, contribution_state(contribution_index, _contact._committed_contact_solution),
            _contact._contact_histories[contribution.contact][contribution.secondary]);
    }
    case SpatialContributionType::pressure:
    case SpatialContributionType::traction:
    case SpatialContributionType::convection:
        return _boundary.contribution_residual(location.type, location.local_index, state);
    }
    throw std::logic_error("SpatialAssembly contribution type is invalid");
}
...
LocalSystem SpatialAssembly::linearize_contribution(std::size_t contribution_index, const LocalValues& state) const {
    ...
    case SpatialContributionType::traction:
    case SpatialContributionType::convection:
        return _boundary.linearize_contribution(location.type, location.local_index, state);
    }
    throw std::logic_error("SpatialAssembly contribution type is invalid");
}
...
void SpatialAssembly::build_contacts(const UnstructuredQuad4Mesh& source_mesh) {
    ...
    for (std::size_t contact_value = 0; contact_value < contact_count(); ++contact_value) {
        ...
        const std::vector<RzPoint> primary_parent_centroids =
            boundary_parent_centroids(_layout, primary.region, primary.boundary);
        const std::vector<RzPoint> secondary_parent_centroids =
            boundary_parent_centroids(_layout, secondary.region, secondary.boundary);
        ...
        if (contact_definition.mechanical && contact_definition.automatic_penalty) {
            ...
            const double secondary_length =
                minimum_boundary_normal_length(_layout, secondary.region, secondary.boundary);
            ...
            if (!std::isfinite(contact_definition.penalty) || !(contact_definition.penalty > 0.0))
                throw std::overflow_error("Automatic contact penalty is not finite and positive: " +
                                          contact_definition.name);
        }

        _contact._thermal_kernels.emplace_back(
            ...
            for (std::size_t edge_index = 0; edge_index < secondary.boundary.elements.size(); ++edge_index) {
                ...
                for (std::size_t primary_edge = 0; primary_edge < primary.boundary.elements.size(); ++primary_edge) {
                    ...
                    if (projection_interval(secondary_coordinates,
                                            edge_coordinates(primary_mesh, primary.boundary.elements[primary_edge]),
                                            lower, upper))
                ...
                for (const ThermalProjection& projection : projections) {
                    ...
                    const Line2RzHeatGeometry integration_geometry = make_line2_rz_heat_geometry(
                        secondary_coordinates, primary_coordinates, projection.lower, projection.upper,
                    ...
                    for (const Line2RzHeatQuadraturePoint& point : integration_geometry.points) {
                        for (std::size_t candidate = 0; candidate < primary.boundary.elements.size(); ++candidate) {
                            ...
                            _contact._thermal_contributions.push_back(
                                {contact_value,
                                 interface_nodes(_layout, secondary.region, secondary_edge, primary.region,
                                                 candidate_edge),
                                 make_line2_rz_heat_point_geometry(
                                     secondary_coordinates, candidate_coordinates, point.secondary_shape,
                                     point.integration_weight, candidate + 1 == primary.boundary.elements.size(),
                                     zero_gap_orientation_hint(secondary_parent_centroids[edge_index],
                ...
                for (std::size_t secondary_node = 0; secondary_node < line2_interface_side_node_count;
                    ...
                    for (std::size_t candidate = 0; candidate < primary.boundary.elements.size(); ++candidate) {
                        ...
                        _contact._mechanical_contributions.push_back(
                            {contact_value,
                             interface_nodes(_layout, secondary.region, secondary_edge, primary.region, primary_edge),
                             make_node_to_line_rz_contact_geometry(
                                 secondary_coordinates, primary_coordinates, secondary_node, candidate == 0,
