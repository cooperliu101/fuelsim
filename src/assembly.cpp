...
SpatialLayout::ResolvedBoundary SpatialLayout::resolve_boundary(const UnstructuredQuad4Mesh& source_mesh,
    ...
    if (found == _block_ids.end())
        throw std::invalid_argument("Boundary belongs to an undeclared block: " + name);
...
std::pair<std::size_t, std::array<std::size_t, 2>> SpatialLayout::edge_parent(std::size_t region,
    ...
    if (parent == mesh.elements().size())
        throw std::invalid_argument("Boundary edge has no adjacent region element");
...
SpatialAssembly::ContributionLocation SpatialAssembly::locate_contribution(std::size_t contribution_index) const {
    ...
    if (contribution_index >= ranges.end)
        throw std::out_of_range("SpatialAssembly contribution index is out of range");
...
LocalDofs BoundaryAssembly::contribution_dofs(SpatialContributionType type, std::size_t index,
    ...
    case SpatialContributionType::pressure:
        return layout.dof_map().local_dofs(_pressure_contributions.at(index).nodes);
    case SpatialContributionType::traction:
        return layout.dof_map().local_dofs(_traction_contributions.at(index).nodes);
    case SpatialContributionType::convection:
        return layout.dof_map().local_dofs(_convection_contributions.at(index).nodes);
    default:
        throw std::invalid_argument("BoundaryAssembly requires a boundary contribution type");
    }
}
...
void SpatialAssembly::update_thermal_candidates(std::size_t contribution_begin, std::size_t contribution_end,
    ...
    for (const ThermalContribution& contribution : _contact._thermal_contributions)
        contribution.active = false;
    for (std::vector<bool>& points : _contact._projected_thermal_points)
        std::fill(points.begin(), points.end(), false);
    ...
    for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
        ...
        if (!touched[candidate.contact][candidate.integration_point])
            continue;
    ...
    for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
        if (!projected[contribution])
            continue;
        const ThermalContribution& candidate = _contact._thermal_contributions[contribution];
        if (candidate.primary != selected_primary[candidate.contact][candidate.integration_point])
            continue;
...
void SpatialAssembly::update_mechanical_candidates(std::size_t contribution_begin, std::size_t contribution_end,
    ...
    for (std::size_t contribution = 0; contribution < _contact._mechanical_contributions.size(); ++contribution) {
        if (!projected[contribution])
            continue;
        const MechanicalContribution& candidate = _contact._mechanical_contributions[contribution];
        if (candidate.primary != selected_primary[candidate.contact][candidate.secondary])
            continue;
        candidate.active = true;
...
InterfaceSummary SpatialAssembly::summarize_interface(std::size_t contact_value,
    ...
    for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
        ...
        if (!value.projected)
            throw std::logic_error("Active thermal-contact candidate is not projected");
...
RegionBoundary ordered_connected_boundary(const RegionMesh& mesh, RegionBoundary boundary, const std::string& name) {
    ...
    for (const Line2BoundaryElement& edge : boundary.elements) {
        if (edge.nodes[0] == edge.nodes[1])
            throw std::invalid_argument("Contact side set has a zero edge: " + name);
        for (std::size_t node : edge.nodes) {
            if (++degree.at(node) > 2)
                throw std::invalid_argument("Contact side set must not branch: " + name);
        }
    }
...
std::vector<std::int64_t> SpatialAssembly::resolve_block_ids(const SpatialDefinition& definition,
    ...
    for (const RegionDefinition& region : definition.regions) {
        ...
        if (!known)
            throw std::invalid_argument("Unknown element block ID: " + std::to_string(id));
...
std::vector<std::size_t> SpatialAssembly::required_state_dofs(std::size_t contribution_begin,
    ...
    if (!touched_thermal.empty()) {
        ...
        for (std::size_t contribution = 0; contribution < _contact._thermal_contributions.size(); ++contribution) {
            const ThermalContribution& candidate = _contact._thermal_contributions[contribution];
            if (!touched_thermal[candidate.contact][candidate.integration_point])
                continue;
