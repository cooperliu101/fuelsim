...
UnstructuredQuad4Mesh::UnstructuredQuad4Mesh(std::vector<RzPoint> nodes, std::vector<Quad4Element> elements,
    ...
    if (_nodes.empty())
        throw std::invalid_argument("UnstructuredQuad4Mesh requires at least one node");
    if (_elements.empty())
        throw std::invalid_argument("UnstructuredQuad4Mesh requires at least one element");
    ...
    if (_element_blocks.empty())
        throw std::invalid_argument("UnstructuredQuad4Mesh requires element block metadata");
    ...
    for (std::size_t index = 0; index < _elements.size(); ++index) {
        ...
        if (!known_block)
            throw std::invalid_argument("UnstructuredQuad4Mesh element references an unknown block");
        for (const std::size_t node : _elements[index].nodes) {
            if (node >= _nodes.size())
                throw std::out_of_range("UnstructuredQuad4Mesh connectivity is out of range");
        }
    }
    ...
    for (std::size_t set = 0; set < _node_sets.size(); ++set) {
        ...
        if (node_set.id < 0)
            throw std::invalid_argument("UnstructuredQuad4Mesh node set IDs must be nonnegative");
        ...
        for (const std::size_t node : node_set.nodes) {
            if (node >= _nodes.size())
                throw std::out_of_range("UnstructuredQuad4Mesh node set is out of range");
        }
    }
    for (std::size_t set = 0; set < _side_sets.size(); ++set) {
        const SideSet& side_set = _side_sets[set];
        if (side_set.id < 0)
            throw std::invalid_argument("UnstructuredQuad4Mesh side set IDs must be nonnegative");
        for (std::size_t previous_index = 0; previous_index < set; ++previous_index) {
...
const std::vector<Quad4Element>& UnstructuredQuad4Mesh::elements() const noexcept {
    return _elements;
}
...
const std::vector<ElementBlockInfo>& UnstructuredQuad4Mesh::element_blocks() const noexcept {
...
UnstructuredHex8Mesh::UnstructuredHex8Mesh(std::vector<CartesianPoint3> nodes, std::vector<Hex8Element> elements,
    ...
    : _nodes(std::move(nodes)), _elements(std::move(elements)), _element_block_ids(std::move(element_block_ids)),
      _element_blocks(std::move(element_blocks)), _node_sets(std::move(node_sets)), _side_sets(std::move(side_sets)) {
    if (_nodes.empty())
        throw std::invalid_argument("UnstructuredHex8Mesh requires at least one node");
    if (_elements.empty())
        throw std::invalid_argument("UnstructuredHex8Mesh requires at least one element");
    if (_elements.size() != _element_block_ids.size())
    ...
    if (_element_blocks.empty())
        throw std::invalid_argument("UnstructuredHex8Mesh requires element block metadata");
    for (std::size_t block = 0; block < _element_blocks.size(); ++block) {
        if (_element_blocks[block].id < 0)
    ...
    for (std::size_t index = 0; index < _elements.size(); ++index) {
        const bool known_block =
            std::any_of(_element_blocks.begin(), _element_blocks.end(),
                        [id = _element_block_ids[index]](const ElementBlockInfo& block) { return block.id == id; });
        if (!known_block)
            throw std::invalid_argument("UnstructuredHex8Mesh element references an unknown block");
        for (const std::size_t node : _elements[index].nodes) {
            if (node >= _nodes.size())
                throw std::out_of_range("UnstructuredHex8Mesh connectivity is out of range");
        }
    }
    for (std::size_t set = 0; set < _node_sets.size(); ++set) {
        const NodeSet& node_set_value = _node_sets[set];
        if (node_set_value.id < 0)
            throw std::invalid_argument("UnstructuredHex8Mesh node set IDs must be nonnegative");
        ...
        for (const std::size_t node : node_set_value.nodes) {
            if (node >= _nodes.size())
                throw std::out_of_range("UnstructuredHex8Mesh node set is out of range");
        }
    }
    for (std::size_t set = 0; set < _side_sets.size(); ++set) {
        const SideSet& side_set_value = _side_sets[set];
        if (side_set_value.id < 0)
            throw std::invalid_argument("UnstructuredHex8Mesh side set IDs must be nonnegative");
        for (std::size_t previous = 0; previous < set; ++previous) {
...
const std::vector<CartesianPoint3>& UnstructuredHex8Mesh::nodes() const noexcept {
    return _nodes;
}
const std::vector<Hex8Element>& UnstructuredHex8Mesh::elements() const noexcept {
    return _elements;
}
const std::vector<std::int64_t>& UnstructuredHex8Mesh::element_block_ids() const noexcept {
    return _element_block_ids;
}
const std::vector<ElementBlockInfo>& UnstructuredHex8Mesh::element_blocks() const noexcept {
    return _element_blocks;
}
const std::vector<NodeSet>& UnstructuredHex8Mesh::node_sets() const noexcept {
    return _node_sets;
}
const std::vector<SideSet>& UnstructuredHex8Mesh::side_sets() const noexcept {
    return _side_sets;
}
...
Hex8RegionMesh Hex8RegionMesh::from_unstructured_block(const UnstructuredHex8Mesh& source, std::int64_t block_id) {
    ...
    if (!known_block)
        throw std::invalid_argument("Unknown element block ID: " + std::to_string(block_id));
    ...
    for (std::size_t source_element : mesh._source_element_ids) {
        ...
        for (std::size_t node = 0; node < element.nodes.size(); ++node) {
            ...
            if (local == invalid)
                throw std::logic_error("Hex8RegionMesh connectivity crosses element blocks");
...
const std::vector<CartesianPoint3>& Hex8RegionMesh::nodes() const noexcept {
    return _nodes;
}
const std::vector<Hex8Element>& Hex8RegionMesh::elements() const noexcept {
    return _elements;
}
const std::vector<std::size_t>& Hex8RegionMesh::source_node_ids() const noexcept {
    return _source_node_ids;
}
const std::vector<std::size_t>& Hex8RegionMesh::source_element_ids() const noexcept {
    return _source_element_ids;
}
std::int64_t Hex8RegionMesh::block_id() const noexcept {
    return _block_id;
}
    ...
    for (const ElementSide& side : source.side_set(side_set_name).sides) {
        ...
        for (std::size_t node = 0; node < face.nodes.size(); ++node) {
            ...
            if (local_node == invalid)
                throw std::logic_error("Hex8RegionMesh side-set node mapping failed");
...
RegionMesh RegionMesh::from_unstructured_block(const UnstructuredQuad4Mesh& source, std::int64_t block_id) {
    ...
    for (std::size_t source_element : mesh._source_element_ids) {
        ...
        for (std::size_t node = 0; node < element.nodes.size(); ++node) {
            const std::size_t local = mesh._source_node_to_local.at(source.elements()[source_element].nodes[node]);
            if (local == invalid)
                throw std::logic_error("RegionMesh connectivity crosses element blocks");
            element.nodes[node] = local;
        }
        mesh._elements.push_back(element);
    }
...
const std::vector<std::size_t>& RegionMesh::source_element_ids() const noexcept {
    return _source_element_ids;
}
...
RegionBoundary RegionMesh::map_side_set(const UnstructuredQuad4Mesh& source, const std::string& side_set_name) const {
    ...
    std::sort(nodes.begin(), nodes.end(), [&](std::size_t lhs, std::size_t rhs) {
        ...
        if (kind == RegionBoundaryKind::bottom || kind == RegionBoundaryKind::top)
...
Hex8DofMap::Hex8DofMap(std::size_t node_count) : _node_count(node_count) {
    if (node_count == 0)
        throw std::invalid_argument("Hex8DofMap node_count must be positive");
...
std::size_t Hex8DofMap::node_count() const noexcept {
    return _node_count;
}
std::size_t Hex8DofMap::dof_count() const noexcept {
    return 4 * _node_count;
}
const std::vector<FieldDescriptor>& Hex8DofMap::field_layout() const noexcept {
    return _field_layout;
}

std::size_t Hex8DofMap::dof(Field field, std::size_t node) const {
    if (node >= _node_count)
        throw std::out_of_range("Hex8DofMap node index is out of range");
    switch (field) {
    case Field::temperature:
        return node;
    case Field::displacement_x:
...
std::size_t Hex8DofMap::temperature(std::size_t node) const {
    return dof(Field::temperature, node);
}
std::size_t Hex8DofMap::displacement_x(std::size_t node) const {
    return dof(Field::displacement_x, node);
}
std::size_t Hex8DofMap::displacement_y(std::size_t node) const {
    return dof(Field::displacement_y, node);
}
std::size_t Hex8DofMap::displacement_z(std::size_t node) const {
    return dof(Field::displacement_z, node);
}
