...
UnstructuredQuad4Mesh::UnstructuredQuad4Mesh(std::vector<RzPoint> nodes, std::vector<Quad4Element> elements, std::vector<std::int64_t> element_block_ids, std::vector<ElementBlockInfo> element_blocks, std::vector<NodeSet> node_sets,
                                             std::vector<SideSet> side_sets)
    : _nodes(std::move(nodes)), _elements(std::move(elements)), _element_block_ids(std::move(element_block_ids)), _element_blocks(std::move(element_blocks)), _node_sets(std::move(node_sets)), _side_sets(std::move(side_sets)) {
    ...
    if (_elements.size() != _element_block_ids.size()) throw std::invalid_argument("UnstructuredQuad4Mesh block ID count must match element count");
    ...
    for (std::size_t block = 0; block < _element_blocks.size(); ++block) {
        if (_element_blocks[block].id < 0) throw std::invalid_argument("UnstructuredQuad4Mesh block IDs must be nonnegative");
        for (std::size_t previous = 0; previous < block; ++previous) {
            if (_element_blocks[previous].id == _element_blocks[block].id) throw std::invalid_argument("UnstructuredQuad4Mesh block IDs must be unique");
            if (!_element_blocks[block].name.empty() && _element_blocks[previous].name == _element_blocks[block].name) throw std::invalid_argument("UnstructuredQuad4Mesh block names must be unique");
        }
    }
    for (std::size_t index = 0; index < _elements.size(); ++index) {
        const bool known_block = std::any_of(_element_blocks.begin(), _element_blocks.end(), [id = _element_block_ids[index]](const ElementBlockInfo& block) { return block.id == id; });
    ...
    for (std::size_t set = 0; set < _side_sets.size(); ++set) {
        ...
        for (const ElementSide& side : side_set.sides) {
            if (side.element >= _elements.size() || side.local_side >= 4U) throw std::out_of_range("UnstructuredQuad4Mesh side set is out of range");
        }
    }
}
...
const std::vector<std::int64_t>& UnstructuredQuad4Mesh::element_block_ids() const noexcept { return _element_block_ids; }
...
const ElementBlockInfo& UnstructuredQuad4Mesh::element_block(const std::string& name) const {
    const auto block = std::find_if(_element_blocks.begin(), _element_blocks.end(), [&name](const ElementBlockInfo& candidate) { return candidate.name == name; });
...
const NodeSet& UnstructuredQuad4Mesh::node_set(const std::string& name) const {
    const auto set = std::find_if(_node_sets.begin(), _node_sets.end(), [&name](const NodeSet& candidate) { return candidate.name == name; });
...
const SideSet& UnstructuredQuad4Mesh::side_set(const std::string& name) const {
    const auto set = std::find_if(_side_sets.begin(), _side_sets.end(), [&name](const SideSet& candidate) { return candidate.name == name; });
...
std::int64_t UnstructuredQuad4Mesh::side_set_block_id(const std::string& name) const {
    ...
    for (const ElementSide& side : set.sides) {
        if (_element_block_ids.at(side.element) != block_id) throw std::invalid_argument("Side set crosses element blocks: " + name);
    }
    return block_id;
}

UnstructuredHex8Mesh::UnstructuredHex8Mesh(std::vector<CartesianPoint3> nodes, std::vector<Hex8Element> elements, std::vector<std::int64_t> element_block_ids, std::vector<ElementBlockInfo> element_blocks, std::vector<NodeSet> node_sets,
                                           std::vector<SideSet> side_sets)
    : _nodes(std::move(nodes)), _elements(std::move(elements)), _element_block_ids(std::move(element_block_ids)), _element_blocks(std::move(element_blocks)), _node_sets(std::move(node_sets)), _side_sets(std::move(side_sets)) {
    ...
    if (_elements.size() != _element_block_ids.size()) throw std::invalid_argument("UnstructuredHex8Mesh block ID count must match element count");
    for (const CartesianPoint3& node : _nodes) {
        if (!std::isfinite(node.x) || !std::isfinite(node.y) || !std::isfinite(node.z)) throw std::invalid_argument("UnstructuredHex8Mesh requires finite Cartesian coordinates");
    }
    if (_element_blocks.empty()) throw std::invalid_argument("UnstructuredHex8Mesh requires element block metadata");
    for (std::size_t block = 0; block < _element_blocks.size(); ++block) {
        if (_element_blocks[block].id < 0) throw std::invalid_argument("UnstructuredHex8Mesh block IDs must be nonnegative");
        for (std::size_t previous = 0; previous < block; ++previous) {
            if (_element_blocks[previous].id == _element_blocks[block].id) throw std::invalid_argument("UnstructuredHex8Mesh block IDs must be unique");
            if (!_element_blocks[block].name.empty() && _element_blocks[previous].name == _element_blocks[block].name) throw std::invalid_argument("UnstructuredHex8Mesh block names must be unique");
        }
    }
    for (std::size_t index = 0; index < _elements.size(); ++index) {
        const bool known_block = std::any_of(_element_blocks.begin(), _element_blocks.end(), [id = _element_block_ids[index]](const ElementBlockInfo& block) { return block.id == id; });
    ...
    for (std::size_t set = 0; set < _node_sets.size(); ++set) {
        ...
        for (std::size_t previous = 0; previous < set; ++previous) {
            if (_node_sets[previous].id == node_set_value.id || (!node_set_value.name.empty() && _node_sets[previous].name == node_set_value.name)) throw std::invalid_argument("UnstructuredHex8Mesh node set IDs and names must be unique");
        }
    ...
    for (std::size_t set = 0; set < _side_sets.size(); ++set) {
        ...
        for (std::size_t previous = 0; previous < set; ++previous) {
            if (_side_sets[previous].id == side_set_value.id || (!side_set_value.name.empty() && _side_sets[previous].name == side_set_value.name)) throw std::invalid_argument("UnstructuredHex8Mesh side set IDs and names must be unique");
        }
        for (const ElementSide& side : side_set_value.sides) {
            if (side.element >= _elements.size() || side.local_side >= 6U) throw std::out_of_range("UnstructuredHex8Mesh side set is out of range");
        }
    }
}
...
const ElementBlockInfo& UnstructuredHex8Mesh::element_block(const std::string& name) const {
    const auto found = std::find_if(_element_blocks.begin(), _element_blocks.end(), [&name](const ElementBlockInfo& candidate) { return candidate.name == name; });
...
const NodeSet& UnstructuredHex8Mesh::node_set(const std::string& name) const {
    const auto found = std::find_if(_node_sets.begin(), _node_sets.end(), [&name](const NodeSet& candidate) { return candidate.name == name; });
...
const SideSet& UnstructuredHex8Mesh::side_set(const std::string& name) const {
    const auto found = std::find_if(_side_sets.begin(), _side_sets.end(), [&name](const SideSet& candidate) { return candidate.name == name; });
...
Hex8RegionMesh Hex8RegionMesh::from_unstructured_block(const UnstructuredHex8Mesh& source, const std::string& block_name) { return from_unstructured_block(source, source.element_block(block_name).id); }

Hex8RegionMesh Hex8RegionMesh::from_unstructured_block(const UnstructuredHex8Mesh& source, std::int64_t block_id) {
    const bool known_block = std::any_of(source.element_blocks().begin(), source.element_blocks().end(), [block_id](const ElementBlockInfo& block) { return block.id == block_id; });
    ...
    if (mesh._source_element_ids.empty()) throw std::invalid_argument("Element block is empty: " + std::to_string(block_id));
...
Hex8RegionBoundary Hex8RegionMesh::map_side_set(const UnstructuredHex8Mesh& source, const std::string& side_set_name) const {
    if (source.side_set_block_id(side_set_name) != _block_id) throw std::invalid_argument("Side set belongs to an unexpected block: " + side_set_name);
    ...
    std::sort(result.nodes.begin(), result.nodes.end(), [&](std::size_t lhs, std::size_t rhs) { return _source_node_ids[lhs] < _source_node_ids[rhs]; });
    result.nodes.erase(std::unique(result.nodes.begin(), result.nodes.end()), result.nodes.end());
    return result;
}

RegionMesh RegionMesh::from_unstructured_block(const UnstructuredQuad4Mesh& source, const std::string& block_name) { return from_unstructured_block(source, source.element_block(block_name).id); }
...
RegionBoundary RegionMesh::map_side_set(const UnstructuredQuad4Mesh& source, const std::string& side_set_name) const {
    ...
    for (const ElementSide& side : side_set.sides) {
        if (_source_element_to_local.at(side.element) == invalid) throw std::invalid_argument("Side set is outside its region: " + side_set_name);
    ...
    if (kind != RegionBoundaryKind::general) {
        for (Line2BoundaryElement& edge : elements) {
            ...
            if ((sort_by_axial && first.z > second.z) || (!sort_by_axial && first.r > second.r)) std::swap(edge.nodes[0], edge.nodes[1]);
        }
        std::sort(elements.begin(), elements.end(), [&](const Line2BoundaryElement& lhs, const Line2BoundaryElement& rhs) {
    ...
    std::sort(nodes.begin(), nodes.end(), [&](std::size_t lhs, std::size_t rhs) {
        if (kind == RegionBoundaryKind::radial_inner || kind == RegionBoundaryKind::radial_outer) return _nodes[lhs].z < _nodes[rhs].z;
...
DofMap::DofMap(std::size_t node_count) : _node_count(node_count) {
    ...
    _field_layout = {{"temperature", 0, node_count, FieldCategory::thermal}, {"radial", node_count, 2 * node_count, FieldCategory::mechanical}, {"axial", 2 * node_count, 3 * node_count, FieldCategory::mechanical}};
}
...
Hex8DofMap::Hex8DofMap(std::size_t node_count) : _node_count(node_count) {
    ...
    if (node_count > std::numeric_limits<std::size_t>::max() / 4) throw std::length_error("Hex8DofMap DOF count overflows");
