...
void write_entity_name(int exoid,
    ex_entity_type type,
    std::int64_t id,
...
std::vector<SideSet> read_side_sets(int exoid,
    std::size_t set_count,
    std::size_t maximum_name_length,
    std::int64_t maximum_side,
...
ExodusMeshData read_exodus_mesh(const std::string& path,
    ...
    ExodusMeshData result{dimension,
        nodes_per_element,
        maximum_side,
        topology,
        alternate_topology,
        ...
        {},
        {}};
    std::array<std::vector<double>, 3> coordinates = {std::vector<double>(node_count),
        std::vector<double>(node_count),
        std::vector<double>(node_count)};
    ...
    result.side_sets = read_side_sets(file.id(),
        checked_size(parameters.num_side_sets, "Exodus side set count"),
        name_length,
        static_cast<std::int64_t>(maximum_side),
        topology);
    file.close();
    return result;
}
...
void write_exodus_mesh(const std::string& path, const ExodusMeshData& mesh) {
    ...
    for (std::size_t element = 0; element < mesh.elements.size(); ++element) {
        ...
        auto block = std::find_if(blocks.begin(), blocks.end(), [block_id](const BlockConnectivity& candidate) {
    ...
    for (std::size_t component = 0; component < mesh.dimension; ++component) {
        ...
        for (const auto& node : mesh.nodes)
            coordinates[component].push_back(node[component]);
    }
    check_exodus(ex_put_coord(file.id(),
                     coordinates[0].data(),
                     coordinates[1].data(),
    ...
    for (const BlockConnectivity& block : blocks) {
        check_exodus(ex_put_block(file.id(),
                         EX_ELEM_BLOCK,
                         block._id,
                         mesh.topology,
        ...
        for (const std::size_t source : block._source_elements)
            written_element_ids[source] = next_element_id++;
    }
...
UnstructuredQuad4Mesh read_exodus_quad4(const std::string& path) {
    ...
    for (const auto& value : data.elements)
        elements.push_back({{{value[0], value[1], value[2], value[3]}}});
...
UnstructuredQuad8Mesh read_exodus_quad8(const std::string& path) {
    ...
    return UnstructuredQuad8Mesh(std::move(nodes),
        std::move(elements),
        std::move(data.element_block_ids),
        std::move(data.element_blocks),
        std::move(data.node_sets),
...
void write_exodus_quad8(const std::string& path, const UnstructuredQuad8Mesh& mesh) {
    ExodusMeshData data{2,
        ...
        mesh.side_sets()};
    for (const auto& node : mesh.nodes())
        data.nodes.push_back({node.r, node.z, 0});
...
UnstructuredHex8Mesh read_exodus_hex8(const std::string& path) {
    ...
    for (const auto& value : data.elements)
        elements.push_back({{{value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7]}}});
    return UnstructuredHex8Mesh(std::move(nodes),
        std::move(elements),
        std::move(data.element_block_ids),
        std::move(data.element_blocks),
        std::move(data.node_sets),
...
UnstructuredHex20Mesh read_exodus_hex20(const std::string& path) {
    ...
    return UnstructuredHex20Mesh(std::move(nodes),
        std::move(elements),
        std::move(data.element_block_ids),
        std::move(data.element_blocks),
        std::move(data.node_sets),
...
void write_exodus_hex20(const std::string& path, const UnstructuredHex20Mesh& mesh) {
    ...
    for (const CartesianPoint3& node : mesh.nodes())
        data.nodes.push_back({node.x, node.y, node.z});
    for (const Hex20Element& element : mesh.elements())
        data.elements.push_back(element.nodes);
    write_exodus_mesh(path, data);
}
...
void define_variable_names(int exoid,
    ex_entity_type type,
...
constexpr std::array<const char*, 26> cartesian_contact_fields = {"gap",
    "pressure",
    "tangential_traction",
    "elastic_tangential_slip",
    "sliding",
...
std::vector<std::string> transient_element_variable_names() {
    ...
    for (std::size_t q = 0; q < 4; ++q)
        append_component_variable_names(result, "elastic_", q);
    for (std::size_t q = 0; q < 4; ++q)
...
void define_result_variables(const std::string& path,
    const ResultsMeshView& mesh,
    const std::vector<std::string>& nodal_names,
...
void write_result_step(const std::string& path,
    const ResultsMeshView& mesh,
    std::size_t step,
    double time,
    ...
    for (const ElementBlockInfo& block : mesh.blocks)
...
void fill_region_nodal_values(const std::vector<std::size_t>& source_nodes,
    std::size_t region,
    bool allow_shared_source_nodes,
    const spatial_detail::SpatialLayout& layout,
...
void fill_contact_nodal_values(std::size_t contact,
...
void fill_rz_nodal(const UnstructuredQuad4Mesh& mesh,
    const rz::SpatialAssembly& spatial,
    const std::vector<double>& state,
    std::vector<std::vector<double>>& values) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    values.assign(nodal_variable_names(spatial.definition().contacts).size(),
        std::vector<double>(mesh.nodes().size(), missing));
    std::vector<bool> present(mesh.nodes().size(), false);
    for (std::size_t region = 0; region < spatial.region_count(); ++region)
        fill_region_nodal_values(spatial.region_mesh(region).source_node_ids(),
            region,
            true,
            spatial,
            state,
...
std::vector<std::vector<double>> quad8_nodal(const UnstructuredQuad8Mesh& source,
    ...
    std::vector<std::vector<double>> values(count,
        std::vector<double>(source.nodes().size(), std::numeric_limits<double>::quiet_NaN()));
        ...
        for (const auto& e : mesh.elements())
            for (std::size_t s = 0; s < 4; ++s)
                values[0][mesh.source_node_ids()[e.nodes[4 + s]]] =
                    .5
                    * (state[spatial.dof(Field::temperature, spatial.global_temperature_node(r, e.nodes[s]))]
                        + state[spatial.dof(Field::temperature,
                            spatial.global_temperature_node(r, e.nodes[(s + 1) % 4]))]);
    }
...
std::vector<double> quad8_globals(const rz8::SpatialAssembly& spatial, const std::vector<double>& state, double load) {
    ...
    for (std::size_t c = 0; c < spatial.definition().contacts.size(); ++c) {
        ...
        result.insert(result.end(),
            {summary.total_heat_rate, summary.total_contact_force, summary.total_tangential_force});
    }
    return result;
}

std::vector<double>
rz_globals(const rz::SpatialAssembly& spatial, const std::vector<double>& state, double load_factor) {
...
void store_stress_values(std::size_t source,
    const std::array<AxisymmetricStressValues, 4>& stresses,
    std::vector<std::vector<double>>& values,
    std::size_t point_count = 4) {
    for (std::size_t q = 0; q < point_count; ++q) {
...
steady_elements(const UnstructuredQuad4Mesh& mesh, const SteadyProblem& problem, const std::vector<double>& state) {
                ...
                if (reduced) {
                    ...
                    for (const auto& p : geometry.points) {
                        ...
                        for (std::size_t n = 0; n < 4; ++n)
                            point.shape[n] += p.weighted_measure * p.shape[n];
                    }
                    point.radius /= point.weighted_measure;
                    for (double& value : point.shape)
                        value /= point.weighted_measure;
                }
...
void fill_cartesian_contact_values(std::size_t contact,
    const std::vector<std::size_t>& nodes,
    const std::vector<CartesianContactNodeSummary>& summary,
    std::vector<std::vector<double>>& values) {
    if (nodes.size() != summary.size())
        throw std::logic_error("Cartesian contact result mapping size mismatch");
    const double missing = std::numeric_limits<double>::quiet_NaN();
    ...
    for (std::size_t node = 0; node < nodes.size(); ++node) {
        ...
        const std::array<double, 26> fields = {s.projected ? s.gap : missing,
            s.projected ? s.pressure : missing,
            s.projected ? s.tangential_traction : missing,
            s.projected ? std::hypot(slip[0], slip[1], slip[2]) : missing,
            s.projected ? (s.sliding ? 1.0 : 0.0) : missing,
            s.projected ? 1.0 : 0.0,
            s.projected ? static_cast<double>(s.primary_face) : missing,
            s.tributary_area,
            s.contact_force,
            s.constraint_pressure,
            s.x + values[1].at(nodes[node]),
            s.y + values[2].at(nodes[node]),
            s.z + values[3].at(nodes[node]),
            s.normal_contact_force[0],
            s.normal_contact_force[1],
            s.normal_contact_force[2],
            s.tangential_contact_force[0],
            s.tangential_contact_force[1],
            s.tangential_contact_force[2],
            s.tangential_slip[0],
            s.tangential_slip[1],
            s.tangential_slip[2],
            slip[0],
...
void fill_cartesian_nodal(const UnstructuredHex8Mesh& mesh,
...
void fill_cartesian_nodal(const UnstructuredHex20Mesh& mesh,
    const cartesian::SpatialAssembly& spatial,
    const std::vector<double>& state,
    std::vector<std::vector<double>>& values) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    ...
    for (const Hex20Element& element : mesh.elements())
        for (std::size_t edge = 0; edge < edge_corners.size(); ++edge) {
            ...
            const double interpolated =
                0.5
                * (values[0][element.nodes[edge_corners[edge][0]]] + values[0][element.nodes[edge_corners[edge][1]]]);
    ...
    for (std::size_t contact = 0; contact < spatial.definition().contacts.size(); ++contact) {
        ...
        const std::vector<CartesianContactNodeSummary> summary = spatial.summarize_contact_nodes(contact, state);
        if (nodes.size() != summary.size())
            throw std::logic_error("HEX20 contact result mapping size mismatch");
        fill_cartesian_contact_values(contact, nodes, summary, values);
    }
}

void append_reaction_values(const spatial_detail::SpatialLayout& spatial,
    const std::vector<std::vector<std::size_t>>& source_nodes,
    bool quadratic,
    const std::vector<std::vector<bool>>& temperature_nodes,
    ...
    for (std::size_t region = 0; region < source_nodes.size(); ++region)
        for (std::size_t local = 0; local < source_nodes[region].size(); ++local)
            for (std::size_t field = 0; field < fields.size(); ++field) {
                if (quadratic && field == 0 && !temperature_nodes[region][local])
                    continue;
...
std::vector<double>
cartesian_globals(const cartesian::SpatialAssembly& spatial, const std::vector<double>& state, double load_factor) {
    std::vector<double> result = {load_factor};
    for (std::size_t contact = 0; contact < spatial.definition().contacts.size(); ++contact) {
        const InterfaceSummary summary = spatial.summarize_interface(contact, state);
        result.push_back(summary.total_heat_rate);
...
void store_cartesian_stress_values(std::size_t source,
    const SymmetricTensor3Values* stresses,
...
std::vector<std::vector<double>> cartesian_elements(const UnstructuredHex8Mesh& mesh,
    ...
    double time = 0.0) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(histories == nullptr ? 48 : 400,
        std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region = 0; region < spatial.region_count(); ++region) {
        ...
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) {
            ...
            if (histories == nullptr)
                ...
                for (std::size_t q = 0; q < 8; ++q)
                    stresses[q] = histories->at(region).at(element)[q].stress;
            ...
            const auto derived = io_detail::hex8_derived_results(spatial.region_element_geometry(region, element),
                local_state,
                material,
                spatial.region(region).strain_formulation,
                spatial.region_material_point_count(region) == 1,
                time);
            for (std::size_t q = 0; q < 8; ++q)
...
std::vector<std::vector<double>> cartesian_elements(const UnstructuredHex20Mesh& mesh,
    ...
    const std::vector<std::vector<CartesianMaterialHistory>>* histories = nullptr) {
    const double missing = std::numeric_limits<double>::quiet_NaN();
    std::vector<std::vector<double>> result(histories == nullptr ? 163 : 784,
        std::vector<double>(mesh.elements().size(), missing));
    for (std::size_t region = 0; region < spatial.region_count(); ++region) {
        const Hex20RegionMesh& region_mesh = spatial.hex20_region_mesh(region);
        for (std::size_t element = 0; element < region_mesh.elements().size(); ++element) {
            const std::size_t source = region_mesh.source_element_ids()[element];
            std::vector<SymmetricTensor3Values> stresses(spatial.region_material_point_count(region));
            result.back()[source] = static_cast<double>(stresses.size());
            if (histories == nullptr)
...
void write_steady_results(const std::string& path,
    ...
    write_result_step(path,
        ...
        1.0,
        nodal_values,
        cartesian_elements(mesh, spatial, &state),
...
ExodusTransientResultsWriter::ExodusTransientResultsWriter(std::string path,
    ...
    define_result_variables(_path,
        ...
        global_variable_names(contacts, true));
}

ExodusTransientResultsWriter::ExodusTransientResultsWriter(std::string path,
    UnstructuredHex8Mesh mesh,
    const TransientProblem& problem)
    ...
    define_result_variables(_path,
        ...
        cartesian_transient_variable_names(),
        global_variable_names(contacts, true));
}

ExodusTransientResultsWriter::ExodusTransientResultsWriter(std::string path,
    UnstructuredHex20Mesh mesh,
    const TransientProblem& problem)
    ...
    define_result_variables(
        ...
        cartesian_nodal_variable_names(contacts, true),
        [&] {
            auto names = cartesian_transient_variable_names(27);
            names.push_back("material_point_count");
            return names;
        }(),
        global_variable_names(contacts, true));
...
void ExodusTransientResultsWriter::append(const TransientProblem& problem) {
    ...
    if (_bar2_mesh) {
        ...
        write_result_step(_path,
            ...
            transient_globals(rz_globals(spatial, problem.committed_solution(), problem.committed_load_factor()),
                problem));
    }
}
