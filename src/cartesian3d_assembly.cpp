...
std::vector<std::int64_t> resolve_block_ids(const SpatialDefinition& definition,
                                            const UnstructuredHex8Mesh& source_mesh) {
...
Hex8Coordinates element_coordinates(const Hex8RegionMesh& mesh, const Hex8Element& element) {
    ...
    for (std::size_t node = 0; node < 8; ++node)
        result[node] = mesh.nodes().at(element.nodes[node]);
    return result;
}
...
SpatialAssembly::SpatialAssembly(SpatialDefinition definition, const UnstructuredHex8Mesh& source_mesh)
    : _definition(std::move(definition)), _block_ids(resolve_block_ids(_definition, source_mesh)),
      _meshes(build_meshes(_definition, source_mesh)), _dof_map(total_nodes(_meshes)), _load_factor(0.0), _time(0.0) {
    ...
    for (std::size_t region_value = 0; region_value < _meshes.size(); ++region_value) {
        for (const Hex8Element& element : _meshes[region_value].elements())
            _geometries[region_value].push_back(
                make_hex8_geometry(element_coordinates(_meshes[region_value], element)));
    }
    ...
    for (const BoundaryConditionDefinition& boundary : _definition.boundary_conditions) {
        ...
        if (boundary.scale_with_load && !boundary.function.empty())
            throw std::invalid_argument("Boundary condition cannot combine scale_with_load and a time function: " +
                                        boundary.name);
        ...
        if (found == _block_ids.end())
            throw std::invalid_argument("Boundary belongs to an undeclared three-dimensional block: " +
                                        boundary.boundary);
        ...
        if (boundary.type == BoundaryConditionType::pressure) {
            ...
            _convection_controls.push_back({boundary.heat_transfer_coefficient, boundary.ambient_temperature,
                                            boundary.coefficient_function, boundary.ambient_temperature_function});
        ...
        for (const Quad4FaceElement& face : mapped.faces) {
            ...
            for (std::size_t node = 0; node < 4; ++node)
                nodes[node] = global_node(region_value, face.nodes[node]);
...
const Hex8DofMap& SpatialAssembly::dof_map() const noexcept {
    return _dof_map;
}
...
std::size_t SpatialAssembly::region_index(const std::string& name) const {
    for (std::size_t region_value = 0; region_value < _definition.regions.size(); ++region_value) {
        if (_definition.regions[region_value].name == name)
            return region_value;
    }
...
const Hex8RegionMesh& SpatialAssembly::region_mesh(std::size_t index) const {
    return _meshes.at(index);
}
std::size_t SpatialAssembly::region_node_offset(std::size_t index) const {
    if (index >= region_count())
        throw std::out_of_range("Three-dimensional region index is out of range");
...
const Hex8Geometry& SpatialAssembly::region_element_geometry(std::size_t region_value,
                                                             std::size_t element_index) const {
    return _geometries.at(region_value).at(element_index);
}
...
void SpatialAssembly::set_load_factor(double value) {
    if (!std::isfinite(value))
        throw std::invalid_argument("Three-dimensional load factor must be finite");
    _load_factor = value;
    refresh_controls();
}
...
void SpatialAssembly::set_time(double value) {
    if (!std::isfinite(value))
        throw std::invalid_argument("Three-dimensional time must be finite");
    _time = value;
    refresh_controls();
}
...
std::vector<double> SpatialAssembly::initial_state() const {
    ...
    for (std::size_t region_value = 0; region_value < region_count(); ++region_value) {
        for (std::size_t local_node = 0; local_node < _meshes[region_value].nodes().size(); ++local_node)
            result[_dof_map.temperature(global_node(region_value, local_node))] =
                region(region_value).initial_temperature;
    }
    return result;
}
...
void SpatialAssembly::fill_contribution_dofs(std::size_t contribution_index, std::vector<std::size_t>& dofs) const {
    ...
    const BoundaryContribution& contribution =
        _boundary_contributions.at(contribution_index - volume_contribution_count());
    const Quad4FaceLocalDofs fixed = _dof_map.face_local_dofs(contribution.nodes);
    dofs.assign(fixed.begin(), fixed.end());
}

Quad4FaceLocalResidual SpatialAssembly::boundary_residual(std::size_t contribution_index,
                                                          const Quad4FaceLocalValues& state) const {
    const BoundaryContribution& contribution =
        _boundary_contributions.at(contribution_index - volume_contribution_count());
    if (contribution.type == SpatialContributionType::pressure)
        return _pressure_kernels[contribution.kernel].residual(contribution.geometry, state);
    if (contribution.type == SpatialContributionType::traction)
        return _traction_kernels[contribution.kernel].residual(contribution.geometry, state);
    return _convection_kernels[contribution.kernel].residual(contribution.geometry, state);
}

Quad4FaceLocalSystem SpatialAssembly::boundary_system(std::size_t contribution_index,
                                                      const Quad4FaceLocalValues& state) const {
    const BoundaryContribution& contribution =
        _boundary_contributions.at(contribution_index - volume_contribution_count());
    if (contribution.type == SpatialContributionType::pressure)
        return _pressure_kernels[contribution.kernel].linearize(contribution.geometry, state);
    if (contribution.type == SpatialContributionType::traction)
        return _traction_kernels[contribution.kernel].linearize(contribution.geometry, state);
...
double SpatialAssembly::controlled_value(const ControlledScalar& control) const {
    if (!control.function.empty())
        return control.base_value * function_value(control.function);
...
void SpatialAssembly::refresh_controls() {
    ...
    for (std::size_t kernel = 0; kernel < _pressure_kernels.size(); ++kernel)
        _pressure_kernels[kernel].set_pressure(controlled_value(_pressure_controls[kernel]));
    for (std::size_t kernel = 0; kernel < _traction_kernels.size(); ++kernel)
        _traction_kernels[kernel].set_traction(controlled_value(_traction_controls[kernel]));
    for (std::size_t kernel = 0; kernel < _convection_kernels.size(); ++kernel) {
        ...
        const double coefficient = control.coefficient_function.empty()
                                       ? control.coefficient
                                       : control.coefficient * function_value(control.coefficient_function);
        const double ambient = control.ambient_function.empty()
                                   ? control.ambient
                                   : control.ambient * function_value(control.ambient_function);
