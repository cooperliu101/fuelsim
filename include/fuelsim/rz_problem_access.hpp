...
class ProblemAccess final {
  public:
    ...
    static const Quad4RzGeometry& region_element_geometry(const SteadyProblem& problem, std::size_t region_index, std::size_t element_index);
    ...
    static const std::vector<std::vector<ContactPointHistory>>& committed_contact_histories(const SteadyProblem& problem) noexcept;
    static void commit_contact_state(SteadyProblem& problem, const std::vector<double>& state);
    static void restore_contact_state(SteadyProblem& problem, const std::vector<double>& state, std::vector<std::vector<ContactPointHistory>> histories);
    static std::vector<ContactNodeSummary> summarize_contact_nodes(const SteadyProblem& problem, std::size_t contact_index, const std::vector<double>& state);
    static std::vector<std::size_t> contact_secondary_source_nodes(const SteadyProblem& problem, std::size_t contact_index);
    static InterfaceSummary summarize_interface(const SteadyProblem& problem, std::size_t contact_index, const std::vector<double>& state);
    static LocalDofs contribution_dofs(const SteadyProblem& problem, std::size_t contribution_index);
    static LocalValues contribution_state(const SteadyProblem& problem, std::size_t contribution_index, const std::vector<double>& global_state);
    static LocalValues contribution_state(const SteadyProblem& problem, std::size_t contribution_index, const GlobalStateView& global_state);
    static LocalResidual contribution_residual(const SteadyProblem& problem, std::size_t contribution_index, const LocalValues& state);
    static LocalSystem linearize_contribution(const SteadyProblem& problem, std::size_t contribution_index, const LocalValues& state);
    ...
    static const Quad4RzGeometry& region_element_geometry(const TransientProblem& problem, std::size_t region_index, std::size_t element_index);
    ...
    static const Quad4MaterialHistory& material_history(const TransientProblem& problem, std::size_t region_index, std::size_t element_index);
    static const std::array<AxisymmetricStressValues, 4>& material_stress(const TransientProblem& problem, std::size_t region_index, std::size_t element_index);
    static RegionInelasticSummary summarize_region_history(const TransientProblem& problem, std::size_t region_index);
    static InterfaceSummary summarize_interface(const TransientProblem& problem, std::size_t contact_index, const std::vector<double>& state);
    static std::vector<ContactNodeSummary> summarize_contact_nodes(const TransientProblem& problem, std::size_t contact_index, const std::vector<double>& state);
    static std::vector<std::size_t> contact_secondary_source_nodes(const TransientProblem& problem, std::size_t contact_index);
    static LocalDofs contribution_dofs(const TransientProblem& problem, std::size_t contribution_index);
    static LocalValues contribution_state(const TransientProblem& problem, std::size_t contribution_index, const std::vector<double>& global_state);
    static LocalValues contribution_state(const TransientProblem& problem, std::size_t contribution_index, const GlobalStateView& global_state);
    static LocalResidual contribution_residual(const TransientProblem& problem, std::size_t contribution_index, const LocalValues& state);
    static LocalSystem linearize_contribution(const TransientProblem& problem, std::size_t contribution_index, const LocalValues& state);
};
