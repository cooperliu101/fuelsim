...
constexpr std::array<PathStep, 6> path = {{{"FULL_CLOSE", -14.0 * gap_quantum, 0.0, 0.0, false},
    {"PARTIAL_ORIGIN", -6.0 * gap_quantum, 0.0, 0.0, true},
    {"PARTIAL_LOW", -6.0 * gap_quantum, 0.125, 0.125, true},
    {"CROSS_PARTIAL", -6.0 * gap_quantum, 0.25, 0.25, true},
    {"CROSS_VERTEX", -6.0 * gap_quantum, 0.375, 0.375, true},
...
fuelsim::UnstructuredHex8Mesh generate_mesh() {
    ...
    for (std::size_t z = 0; z + 1 < primary_z.size(); ++z)
        for (std::size_t y = 0; y + 1 < primary_y.size(); ++y) {
            ...
            elements.push_back(append_cuboid(nodes,
                primary_nodes,
                0.0,
                1.0,
                primary_y[y],
    ...
    for (std::size_t z = 0; z + 1 < secondary_z.size(); ++z)
        for (std::size_t y = 0; y + 1 < secondary_y.size(); ++y) {
            ...
            for (const auto& entry : tile_nodes)
    ...
    primary_patch_nodes.erase(std::unique(primary_patch_nodes.begin(), primary_patch_nodes.end()),
        primary_patch_nodes.end());
    return fuelsim::UnstructuredHex8Mesh(std::move(nodes),
        ...
        {{40, "primary_contact", std::move(primary_contact)},
            {50, "secondary_contact", std::move(secondary_contact)},
...
void write_abaqus_input(const std::string& path_name, const fuelsim::UnstructuredHex8Mesh& mesh, bool swapped) {
    ...
    if (!output)
        throw std::runtime_error("Could not write B4.4 Abaqus input: " + path_name);
    ...
    for (const fuelsim::ElementSide& side : mesh.side_sets()[2].sides)
        output << side.element + 1 << ",\n";
...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& file) {
    if (column >= values.size())
        throw std::invalid_argument("Incomplete B4.4 CSV row: " + file);
    std::size_t parsed = 0;
...
std::vector<NodeReference> read_nodes(const std::string& file) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B4.4 node reference: " + file);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        result.push_back({index_value(values, 0, file),
            index_value(values, 1, file),
...
std::vector<ContactReference> read_contact(const std::string& file) {
    std::ifstream input(file);
    if (!input)
        throw std::runtime_error("Could not read B4.4 contact reference: " + file);
    std::string line;
    if (!std::getline(input, line)
        || line
               != "step,id,x,y,z,normal_x,normal_y,normal_z,tangential_x,tangential_y,tangential_z,slip_1,"
                  "slip_2,gap,pressure")
    ...
    while (std::getline(input, line)) {
        ...
        result.push_back({index_value(values, 0, file),
            ...
            {number(values, 2, file), number(values, 3, file), number(values, 4, file)},
            {number(values, 5, file), number(values, 6, file), number(values, 7, file)},
            {number(values, 8, file), number(values, 9, file), number(values, 10, file)},
            number(values, 11, file),
            number(values, 12, file),
            number(values, 13, file),
            number(values, 14, file)});
    }
    return result;
}
...
std::vector<double>
state_for_step(const fuelsim::SteadyProblem& problem, const fuelsim::UnstructuredHex8Mesh& mesh, const PathStep& step) {
        ...
        for (std::size_t row = 0; row < dofs.size(); ++row) {
            ...
            for (std::size_t component = 0; component < 3; ++component) {
                ...
                if (global >= field.begin && global < field.end)
                    result.action_reaction[component] += residual[row];
            }
        }
        double residual_scale = 0.0;
        for (const double value : residual)
            residual_scale = std::max(residual_scale, std::abs(value));
...
bool run(const std::string& mesh_path,
    const std::string& node_path,
    const std::string& contact_path,
    double& partial_resultant,
    double& partial_reference_resultant) {
    ...
    for (std::size_t step = 0; step < path.size(); ++step) {
        ...
        if (actual.size() != contact_sources.size())
            throw std::logic_error("B4.4 contact-node count changed");
        std::size_t active = 0;
        ...
        for (std::size_t region = 0; region < spatial.region_count(); ++region) {
            ...
            for (std::size_t local = 0; local < region_mesh.nodes().size(); ++local) {
                ...
                const auto found = std::find_if(node_reference.begin(), node_reference.end(), [&](const auto& value) {
                    return value.step == step + 1 && value.source_node == source;
                });
                if (found == node_reference.end())
                    throw std::logic_error("B4.4 node mapping is incomplete");
                const fuelsim::CartesianPoint3& reference = mesh.nodes()[source];
                maximum_reference_coordinate_difference = std::max({maximum_reference_coordinate_difference,
                    std::abs(reference.x - found->reference.x),
                    std::abs(reference.y - found->reference.y),
                    std::abs(reference.z - found->reference.z)});
        ...
        for (std::size_t node = 0; node < actual.size(); ++node) {
            ...
            if (found == contact_reference.end())
                throw std::logic_error("B4.4 contact mapping is incomplete");
            ...
            four_face_vertex_visited =
                four_face_vertex_visited
                || (std::abs(actual_point[1] - 0.5) < 1.0e-14 && std::abs(actual_point[2] - 0.5) < 1.0e-14);
        ...
        if (step == 1)
            derivative_check = contact_checks(spatial, state);
    ...
    for (const auto& entry : std::array<std::pair<const char*, const fuelsim::test::FieldErrorMetrics*>, 19>{
             {{"displacement_x", &displacement_x},
                 ...
                 {"normal_force_x", &normal_x},
                 {"normal_force_y", &normal_y},
                 {"normal_force_z", &normal_z},
                 {"tangent_force_y", &tangent_y},
                 {"tangent_force_z", &tangent_z},
                 {"gap", &gap},
                 {"pressure", &pressure},
    ...
    return check(node_reference.size() == path.size() * mesh.nodes().size(),
           ...
           && check(maximum_reference_coordinate_difference < 1.0e-14
                        && maximum_current_coordinate_difference < 1.0e-14,
...
bool run_swapped(const fuelsim::UnstructuredHex8Mesh& mesh,
    const std::string& node_path,
    const std::string& contact_path,
    double base_resultant,
    double base_reference_resultant) {
    const std::vector<NodeReference> node_reference = read_nodes(node_path);
    const std::vector<ContactReference> contact_reference = read_contact(contact_path);
    ...
    for (std::size_t region = 0; region < spatial.region_count(); ++region) {
        ...
        for (std::size_t local = 0; local < region_mesh.nodes().size(); ++local) {
            const std::size_t source = region_mesh.source_node_ids()[local];
            const auto found = std::find_if(node_reference.begin(), node_reference.end(), [&](const auto& value) {
                return value.step == 1 && value.source_node == source;
            });
            if (found == node_reference.end())
                throw std::logic_error("B4.4 swapped node mapping is incomplete");
            const fuelsim::CartesianPoint3& reference = mesh.nodes()[source];
            maximum_reference_coordinate_difference = std::max({maximum_reference_coordinate_difference,
                std::abs(reference.x - found->reference.x),
                std::abs(reference.y - found->reference.y),
    ...
    for (std::size_t node = 0; node < actual.size(); ++node) {
        const auto found = std::find_if(contact_reference.begin(), contact_reference.end(), [&](const auto& value) {
            return value.step == 1 && value.source_node == contact_sources[node];
        });
        if (found == contact_reference.end())
            throw std::logic_error("B4.4 swapped contact mapping is incomplete");
        const auto& value = actual[node];
        all_projected = all_projected && value.projected;
        visited_primary_faces.insert(value.primary_face);
        const bool actual_active = value.pressure > 1.0e-8, reference_active = found->pressure > 1.0e-8;
        ...
        const std::size_t secondary_local =
            static_cast<std::size_t>(std::find(spatial.region_mesh(0).source_node_ids().begin(),
                                         spatial.region_mesh(0).source_node_ids().end(),
                                         contact_sources[node])
        ...
        if (value.gap > 0.0 && value.pressure > 0.0)
            ++fuelsim_positive_gap_pressure;
        if (found->gap > 0.0 && found->pressure > 0.0)
            ++abaqus_positive_gap_pressure;
    ...
    return check(node_reference.size() == mesh.nodes().size() && contact_reference.size() == contact_sources.size(),
           ...
           && check(fuelsim_penalty_law_difference > 1.0e4 && abaqus_penalty_law_difference > 1.0e4
                        && fuelsim_positive_gap_pressure == abaqus_positive_gap_pressure
                        && abaqus_positive_gap_pressure > 0,
           ...
           && check(passes(normal_x, force_zero_tolerance) && passes(gap, displacement_zero_tolerance)
                        ...
                        && passes(moment_y, force_zero_tolerance) && passes(moment_z, force_zero_tolerance)
                        && passes(center_x, displacement_zero_tolerance)
                        && passes(center_y, displacement_zero_tolerance)
                        && passes(center_z, displacement_zero_tolerance),
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (passed)
            std::cout << "[PASS] B4.4 HEX8 nonmatching finite-sliding partial-contact comparison\n";
        return passed ? 0 : 1;
    } catch (const std::exception& error) {
