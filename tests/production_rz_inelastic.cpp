...
double average(const fuelsim::test::ExodusResults& result, const std::string& name) {
    double value = 0.0;
    for (std::size_t q = 0; q < 4; ++q) value += result.element(name + "_q" + std::to_string(q)).at(0) / 4.0;
    return value;
}

double top_displacement(const fuelsim::test::ExodusResults& result) {
    double maximum_z = -std::numeric_limits<double>::infinity();
    for (const auto& point : result.nodes) maximum_z = std::max(maximum_z, point[1]);
    double value = 0.0;
    std::size_t count = 0;
    ...
    if (count == 0) throw std::runtime_error("RZ single-element output has no top nodes");
...
double maximum_trace(const fuelsim::test::ExodusResults& result, const std::string& prefix) {
    ...
    for (std::size_t q = 0; q < 4; ++q) {
        ...
        value = std::max(
            value, std::abs(result.element(prefix + "_rr" + suffix)[0] + result.element(prefix + "_zz" + suffix)[0] +
...
bool check_unload_history(const std::string& output_path, const std::string& reference_path) {
    ...
    for (std::size_t step = 1; step <= final.step_count; ++step) {
        ...
        actual.push_back(
            {frame.time, top_displacement(frame), average(frame, "plastic_zz"), average(frame, "stress_zz"),
                average(frame, "equiv_plastic"), average(frame, "plastic_hoop"), average(frame, "plastic_rr")});
    }
    bool passed = true;
...
bool fuelsim::test::check_rz_inelastic(
    const std::string& output_path, const std::string& branch, const std::string& history_path) {
    const auto result = read_final_exodus_results(output_path);
    bool passed = check(
        result.block_element_counts.size() == 1 && result.block_element_counts[0] == 1 && result.nodes.size() == 4,
    ...
    if (branch == "j2_unload_reload") return check_unload_history(output_path, history_path) && passed;
    ...
    const std::array<std::string, 4> names = {
        "axial_stress", "equivalent_plastic", "equivalent_creep", "top_displacement"};
    for (std::size_t f = 0; f < 4; ++f)
        if (expected[f] != 0.0)
            passed = check_scalar_metrics(
                         "m22_" + branch + "_" + names[f], values[f], expected[f], moose_relative_tolerance) &&
                     passed;
