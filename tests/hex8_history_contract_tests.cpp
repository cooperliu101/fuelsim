...
void verify(
    const std::string& card, const std::string& checkpoint, const std::string& roundtrip, const std::string& output) {
    ...
    const std::array<std::string, 4> reaction_names = {
    ...
    if (!active) throw std::runtime_error("HEX8 checkpoint history is not active");
    ...
    if (std::remove(roundtrip.c_str()) != 0) throw std::runtime_error("Could not remove test checkpoint roundtrip");
}
} // namespace
