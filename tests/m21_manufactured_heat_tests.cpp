        ...
        for (std::size_t i = 0; i < points.size(); ++i) {
            ...
            for (std::size_t j = 0; j < points.size(); ++j) {
                ...
                const std::array<double, 4> shape = {0.25 * (1.0 - xi) * (1.0 - eta),
                    ...
                    0.25 * (1.0 + xi) * (1.0 + eta),
                    0.25 * (1.0 - xi) * (1.0 + eta)};
                const std::array<double, 4> dshape_dxi = {-0.25 * (1.0 - eta),
                    0.25 * (1.0 - eta),
                    0.25 * (1.0 + eta),
                    -0.25 * (1.0 + eta)};
                const std::array<double, 4> dshape_deta = {-0.25 * (1.0 - xi),
                    -0.25 * (1.0 + xi),
                    0.25 * (1.0 + xi),
                    0.25 * (1.0 - xi)};
                double r = 0.0;
...
ManufacturedResult solve_manufactured(std::size_t radial_elements,
    ...
    for (std::size_t node = 0; node < node_count; ++node) {
        result.maximum_displacement = std::max(result.maximum_displacement,
            std::abs(problem.committed_solution()[fuelsim::rz::ProblemAccess::dof_map(problem)
                    .dof(fuelsim::Field::radial_displacement, node)]));
        result.maximum_displacement = std::max(result.maximum_displacement,
            std::abs(problem.committed_solution()[fuelsim::rz::ProblemAccess::dof_map(problem)
                    .dof(fuelsim::Field::axial_displacement, node)]));
    }
    return result;
}
