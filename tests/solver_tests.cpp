...
bool test_m1_closed_gap_end_to_end() {
    ...
    const double expected_heat_rate =
        ...
        parameters.fuel_radius * parameters.length;
    const double heat_balance_error =
        metric_relative_error(interface.total_heat_rate, expected_heat_rate);

    // Generated by verification/moose/m1_fuel_cladding_gap_rz.i with
    // node-face penalty contact normalized by nodal area. Peak contact
    // pressure is intentionally diagnostic-only because its endpoint value
    // is sensitive to the different interface discretizations.
    constexpr double expected_temperature_center = 751.1094257211;
    constexpr double expected_temperature_fuel_surface = 614.7574993153;
    constexpr double expected_temperature_cladding_inner = 613.7290989887;
    constexpr double expected_radial_fuel_surface = 3.213733472694e-6;
    constexpr double expected_radial_cladding_inner = 1.189595954743e-6;
    constexpr double expected_axial_fuel_top = 1.053503902300e-5;
