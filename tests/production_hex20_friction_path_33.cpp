...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("H20.33 CSV row is too short: " + path);
    std::size_t parsed = 0;
...
std::vector<std::vector<DisplacementReference>> read_displacement_reference(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read H20.33 displacement reference: " + path);
    std::string line;
...
std::vector<std::vector<ContactReference>> read_contact_reference(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read H20.33 contact reference: " + path);
    std::string line;
    std::getline(input, line);
    if (line
...
std::vector<ReactionReference> read_reaction_reference(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read H20.33 reaction reference: " + path);
    std::string line;
    std::getline(input, line);
    ...
    for (std::size_t step = 0; step < states.size(); ++step) {
        ...
        for (std::size_t node = 0; node < contact_nodes.size(); ++node) {
            ...
            if (found == contact[step].end())
                throw std::invalid_argument("H20.33 contact-node mapping is incomplete");
        ...
        passed =
            check(maximum_coordinate_difference < 1.0e-7,
            ...
            && check(fuelsim::test::grouped_relative_metrics_below(displacement_vector, tolerance)
                ...
                "H20.33 step " + std::to_string(step + 1) + " complete displacement-vector metrics are below 1 percent")
...
double tangential_resultant_y(const StepState& state) {
    double result = 0.0;
    for (const fuelsim::test::OutputContact& node : state.contact)
        result += node.tangential_contact_force[1];
    return result;
}
