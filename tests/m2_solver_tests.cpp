...
bool test_moose_mesh_backward_euler_heat_source(const std::string& input_path,
    ...
    bool passed =
        check(fuelsim::rz::ProblemAccess::dof_map(problem).node_count() == 15 && problem.contribution_count() == 8,
    ...
    passed = check(fields.node_count == imported.nodes().size() && fields.maximum_coordinate_difference < 1.0e-12,
    ...
    passed = check(fuelsim::test::absolute_metrics_below(fields.radial_displacement, 1.0e-12),
             ...
             passed;
    passed = check(fuelsim::test::absolute_metrics_below(fields.axial_displacement, 1.0e-12),
                   "M2.1 zero axial field three absolute errors pass") &&
             passed;
    passed =
        check(result.aggregate_timing.workspace_setups == 1, "ten backward-Euler steps create one PETSc workspace") &&
        passed;
    passed =
        check(result.aggregate_timing.solve_calls == 10, "one PETSc solve is issued per backward-Euler step") && passed;

    fuelsim::test::print_relative_metrics("m21_temperature", fields.temperature);
    fuelsim::test::print_absolute_metrics("m21_radial_displacement", fields.radial_displacement);
    fuelsim::test::print_absolute_metrics("m21_axial_displacement", fields.axial_displacement);
    return passed;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(argc, argv, "fuelsim input-card M2.1 MOOSE comparison test\n");
