...
fuelsim::FuelSimCaseDefinition augmented_pcmi_input(const std::string& input_path, double penetration_tolerance,
...
bool test_transient_augmented_time_error(const std::string& input_path) {
    ...
    const fuelsim::UnstructuredQuad4Mesh mesh = fuelsim::ExodusMeshIo::read_quad4(input.mesh_file);
    fuelsim::TransientProblem problem(input.transient_definition(), mesh);
    fuelsim::TransientTimeOptions options = {input.transient_execution.end_time,      4.0, 0.125, 4.0, 1.0, 0.5, 12,
                                             input.transient_execution.load_ramp_time};
