...
fuelsim::FuelSimCaseDefinition
augmented_pcmi_input(const std::string& input_path,
...
time_options(const fuelsim::FuelSimCaseDefinition& input) {
...
double maximum_committed_multiplier(const fuelsim::TransientProblem& problem) {
    ...
    for (const std::vector<fuelsim::ContactPointHistory>& histories :
         problem.committed_state().contact_histories)
  ...
  public:
    void accepted_step(const fuelsim::TransientProblem& problem,
        ...
        const fuelsim::InterfaceSummary interface =
            problem.summarize_interface(0, problem.committed_solution());
        _maximum_penetration =
            std::max(_maximum_penetration,
                     std::max(-interface.minimum_contact_gap, 0.0));
        _maximum_multiplier = std::max(_maximum_multiplier,
                                       maximum_committed_multiplier(problem));
        _final_active_contact_nodes = interface.active_contact_nodes;
        _final_total_contact_force = interface.total_contact_force;
    }

    double maximum_penetration() const noexcept {
        return _maximum_penetration;
    }
    double maximum_multiplier() const noexcept {
        return _maximum_multiplier;
    }
    std::size_t final_active_contact_nodes() const noexcept {
        return _final_active_contact_nodes;
    }
    double final_total_contact_force() const noexcept {
        return _final_total_contact_force;
    }
...
bool test_transient_augmented_contact(const std::string& input_path) {
    const fuelsim::FuelSimCaseDefinition input =
        augmented_pcmi_input(input_path, 1.0e-9, 50);
    ...
    const fuelsim::TransientResult result =
        fuelsim::solve_transient(problem, time_options(input),
                                 solver_options(input), &observer);
    std::cout << "transient_augmented_completed=" << result.completed << '\n'
              << "transient_augmented_accepted_steps="
              << result.accepted_steps.size() << '\n'
              << "transient_augmented_rejected_steps="
              << result.rejected_steps.size() << '\n'
              << "transient_augmented_solve_calls="
              << result.aggregate_timing.solve_calls << '\n'
              << "transient_augmented_last_step_multiplier_updates="
              << result.last_attempt.augmented_lagrangian_iterations << '\n'
              << "transient_augmented_maximum_committed_multiplier="
              << observer.maximum_multiplier() << '\n'
              << "transient_augmented_maximum_penetration="
              << observer.maximum_penetration() << '\n'
              << "transient_augmented_penetration_tolerance="
              << input.contacts.at(0).penetration_tolerance << '\n'
              << "transient_augmented_final_active_contact_nodes="
              << observer.final_active_contact_nodes() << '\n'
              << "transient_augmented_final_total_contact_force="
              << observer.final_total_contact_force() << '\n';
    return check(problem.uses_augmented_contact(),
                 "transient fixture enables the augmented contact "
                 "formulation") &&
           check(result.completed &&
                     result.termination_reason ==
                         fuelsim::TransientTerminationReason::completed &&
           ...
           check(observer.maximum_multiplier() > 0.0,
                 ...
                 "the transient solve") &&
           check(observer.maximum_penetration() <=
                     input.contacts.at(0).penetration_tolerance,
                 "every accepted transient step satisfies the 1 nm "
                 "penetration tolerance") &&
           check(observer.final_active_contact_nodes() > 0 &&
                     observer.final_total_contact_force() > 0.0,
...
bool test_transient_augmented_failure_rollback(
    const std::string& input_path) {
    const fuelsim::FuelSimCaseDefinition input =
        augmented_pcmi_input(input_path, 1.0e-20, 1);
    const fuelsim::UnstructuredQuad4Mesh mesh =
        fuelsim::ExodusMeshIo::read_quad4(input.mesh_file);
    fuelsim::TransientProblem problem(input.transient_definition(), mesh);
    const fuelsim::TransientResult result = fuelsim::solve_transient(
        problem, time_options(input), solver_options(input));
    std::cout << "transient_augmented_failure_committed_time="
              << result.committed_time << '\n'
              << "transient_augmented_failure_termination="
              << fuelsim::transient_termination_reason_name(
                     result.termination_reason)
              << '\n'
              << "transient_augmented_failure_rejected_steps="
              << result.rejected_steps.size() << '\n'
              << "transient_augmented_failure_committed_multiplier="
              << maximum_committed_multiplier(problem) << '\n';
    ...
    return check(!result.completed && cutback_terminated &&
                     !result.accepted_steps.empty() &&
                     result.committed_time > 0.0 &&
                     result.committed_time <
                         input.transient_execution.end_time,
                 "unreachable penetration tolerance stops the transient "
                 "solve at the cutback or minimum-step limit") &&
           check(result.last_attempt.failure_category ==
...
bool test_transient_augmented_time_error(const std::string& input_path) {
    ...
    fuelsim::TransientTimeOptions options = {
        input.transient_execution.end_time, 4.0, 0.125, 4.0,
        1.0, 0.5, 12, input.transient_execution.load_ramp_time};
    ...
    AugmentedContactObserver observer;
    const fuelsim::TransientResult result = fuelsim::solve_transient(
        problem, options, solver_options(input), &observer);
    ...
    for (const fuelsim::TransientAcceptedStep& step : result.accepted_steps)
        maximum_accepted_multiplier_error =
            std::max(maximum_accepted_multiplier_error,
                     step.time_error_components.contact_normal_multiplier);
    ...
    for (const fuelsim::TransientRejectedStep& step : result.rejected_steps) {
        ...
        const fuelsim::TransientTimeErrorEstimate& components =
            step.time_error_components;
        maximum_rejected_multiplier_error =
            std::max(maximum_rejected_multiplier_error,
                     components.contact_normal_multiplier);
    ...
    std::cout << "transient_augmented_time_error_completed="
              << result.completed << '\n'
              << "transient_augmented_time_error_accepted_steps="
              << result.accepted_steps.size() << '\n'
              << "transient_augmented_time_error_rejections="
              << result.time_error_rejections << '\n'
              << "transient_augmented_time_error_maximum_accepted_multiplier="
              << maximum_accepted_multiplier_error << '\n'
              << "transient_augmented_time_error_maximum_rejected_multiplier="
              << maximum_rejected_multiplier_error << '\n'
              << "transient_augmented_time_error_multiplier_dominated="
              << multiplier_dominated_rejection << '\n';
    return check(result.completed &&
                     result.termination_reason ==
                         fuelsim::TransientTerminationReason::completed,
           ...
           check(observer.maximum_penetration() <=
                 ...
                 "tolerance") &&
           check(observer.maximum_multiplier() > 0.0 &&
                     observer.final_active_contact_nodes() > 0,
                 "error-controlled solve commits active augmented "
                 "multipliers") &&
           check(result.aggregate_timing.workspace_setups == 1,
                 "step-doubling retries reuse one PETSc workspace");
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(
            argc, argv,
            "fuelsim M5.4 transient augmented-contact tests\n");
        bool passed =
            test_transient_augmented_contact(argv[1]);
        passed =
            test_transient_augmented_failure_rollback(argv[1]) && passed;
        ...
        std::cerr << "[FAIL] M5.4 transient augmented tests raised: "
                  << error.what() << '\n';
        return 1;
    }
}
