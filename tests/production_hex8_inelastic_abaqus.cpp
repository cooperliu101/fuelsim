...
const char* case_id(Branch branch) {
    ...
    case Branch::coupled:
        return "B5.12";
    case Branch::noncoaxial:
        return "B5.13";
    case Branch::finite_plastic:
        return "B5.15";
    case Branch::finite_coupled:
        return "B5.17";
    case Branch::finite_noncoaxial:
        return "B5.18";
    case Branch::reduced_plastic:
        return "B5.41";
    case Branch::reduced_coupled:
        return "B5.43";
    case Branch::finite_reduced_plastic:
        return "B5.35";
    case Branch::finite_reduced_coupled:
        return "B5.37";
    case Branch::finite_reduced_noncoaxial:
        return "B5.45";
    }
...
bool long_noncoaxial(Branch branch) {
    return branch == Branch::finite_reduced_noncoaxial;
}
...
std::size_t stage_count(Branch branch) {
    return long_noncoaxial(branch) ? 100 : noncoaxial(branch) ? 20 : 10;
}

double time_step(Branch branch) {
    return noncoaxial(branch) ? 0.001 : standard_time_step;
}

Branch parse_branch(const std::string& value) {
    if (value == "plastic")
        return Branch::plastic;
    if (value == "coupled")
        return Branch::coupled;
    if (value == "noncoaxial")
        return Branch::noncoaxial;
    if (value == "finite_plastic")
        return Branch::finite_plastic;
    if (value == "finite_coupled")
        return Branch::finite_coupled;
    if (value == "finite_noncoaxial")
        return Branch::finite_noncoaxial;
    if (value == "reduced_plastic")
        return Branch::reduced_plastic;
    if (value == "reduced_coupled")
        return Branch::reduced_coupled;
    if (value == "finite_reduced_plastic")
        return Branch::finite_reduced_plastic;
    if (value == "finite_reduced_coupled")
        return Branch::finite_reduced_coupled;
    if (value == "finite_reduced_noncoaxial")
        return Branch::finite_reduced_noncoaxial;
...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete Abaqus B5.10 row in " + path);
...
tensor(const std::vector<std::string>& values, std::size_t first, const std::string& path, double zero_threshold) {
    Tensor result = {number(values, first, path),
...
std::vector<NodeReference> read_nodes(const std::string& path, Branch branch) {
    ...
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.10 nodes: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 11)
            throw std::invalid_argument("Unexpected Abaqus B5.10 nodal column count in " + path);
        std::array<double, 8> fields{};
...
std::vector<IntegrationReference> read_integration(const std::string& path, Branch branch) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.10 integration points: " + path);
    std::string line;
    std::getline(input, line);
    ...
    while (std::getline(input, line)) {
        ...
        Tensor stress = {number(values, 8, path),
        ...
        if (noncoaxial(branch))
            maximum_raw_free_z_stress = std::max(maximum_raw_free_z_stress, std::abs(stress.zz));
...
std::vector<EnergyReference> read_energy(const std::string& path, Branch branch) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.10 energy: " + path);
    std::string line;
    std::getline(input, line);
    const std::string expected_header =
        plastic_only(branch)
    ...
    if (line != expected_header)
        throw std::invalid_argument("Unexpected Abaqus B5.10 energy header in " + path);
    ...
    while (std::getline(input, line)) {
        ...
        result.push_back({positive_integer(number(values, 0, path), path),
            ...
            plastic_only(branch) ? 0.0 : number(values, 4, path),
...
std::array<double, 6>
output_tensor(const fuelsim::test::ExodusResults& output, const std::string& prefix, std::size_t q) {
...
bool check_hex8_inelastic_abaqus(const std::string& output_path,
    ...
    const std::array<std::string, 34> integration_names = {"stress_xx",
        "stress_yy",
        "stress_zz",
        "stress_xy",
        "stress_yz",
    ...
    for (std::size_t field = 0; field < integration_metrics.size(); ++field) {
        ...
        passed = check(metrics_pass(integration_metrics[field],
                           1.0e-3,
                           zero_tolerance,
    ...
    const std::array<std::string, 4> energy_names = {"internal_energy",
        "plastic_dissipation",
        "creep_dissipation",
        "external_work"};
        ...
        passed =
            check(snapshots.at(1)->element("equiv_plastic_q0").at(0) > 0.0
                      ...
                      && snapshots.at(9)->element("plastic_xy_q0").at(0)
                             ...
                             < 0.0,
                std::string(case_id(branch)) + " accumulates both mechanisms and reverses the plastic shear direction")
            && passed;
