...
using IntegrationVolumes = std::array<double, 1>;
...
std::map<std::string, StepValues> read_reference(const std::string& path) {
    ...
    if (line != "step,element,local_node,temperature_k,reaction_heat_flux_w")
        throw std::invalid_argument("Unexpected Abaqus C3D8RT thermal-load header in " + path);
    std::map<std::string, StepValues> result;
    std::map<std::string, std::array<std::array<bool, 8>, 3>> present;
    while (std::getline(input, line)) {
        const std::vector<std::string> values = split_csv(line);
        if (values.size() != 5)
            throw std::invalid_argument("Unexpected Abaqus C3D8RT thermal-load column count in " + path);
        ...
        if (element < 1 || element > 3 || node < 1 || node > 8)
            throw std::invalid_argument("Abaqus C3D8RT thermal-load label lies outside the probe");
        if (present[values[0]][element - 1][node - 1])
            throw std::invalid_argument("Duplicate Abaqus C3D8RT thermal-load row");
    ...
    for (const char* step : {"BODY", "SURFACE", "FILM_BASE", "FILM_ACTIVE"}) {
        ...
        for (const auto& element : present[step])
            if (std::find(element.begin(), element.end(), false) != element.end())
                throw std::invalid_argument(std::string("Incomplete Abaqus C3D8RT thermal-load step ") + step);
    }
    return result;
}
...
IntegrationVolumes read_integration_volumes(const std::string& path) {
    ...
    if (line != "step,element,integration_point,temperature_k,integration_volume_m3")
        throw std::invalid_argument("Unexpected Abaqus C3D8RT integration-volume header in " + path);
    IntegrationVolumes result{};
    std::array<bool, 1> present{};
    while (std::getline(input, line)) {
        const std::vector<std::string> values = split_csv(line);
        if (values.size() != 5)
    ...
    if (std::find(present.begin(), present.end(), false) != present.end())
        throw std::invalid_argument("Incomplete Abaqus C3D8RT integration-volume reference");
    return result;
}
...
SurfaceGeometryValues read_surface_geometry(const std::string& path) {
    ...
    if (line != "element,local_node,reaction_heat_flux_w")
        throw std::invalid_argument("Unexpected Abaqus C3D8RT surface-geometry header in " + path);
    SurfaceGeometryValues result;
    std::map<std::size_t, std::array<bool, 8>> present;
    while (std::getline(input, line)) {
        const std::vector<std::string> values = split_csv(line);
        if (values.size() != 3)
            throw std::invalid_argument("Unexpected Abaqus C3D8RT surface-geometry column count in " + path);
        ...
        if (node < 1 || node > 8 || present[element][node - 1])
            throw std::invalid_argument("Invalid Abaqus C3D8RT surface-geometry row");
...
NodalValues full_gauss_body_load(const fuelsim::Hex8Geometry& geometry, double source) {
    NodalValues result{};
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: fuelsim_b530_hex8_c3d8rt_thermal_load_abaqus_tests <nodal.csv> <integration.csv> "
    ...
    try {
        ...
        const double full_gauss_body_error =
            relative_error(full_gauss_body_load(geometry, 80.0), reference.at("BODY")[0]);
        ...
        std::cout << "b530_body_source_relative_error=" << body_error << '\n'
                  << "b530_surface_heat_flux_relative_error=" << surface_error << '\n'
                  << "b530_convection_relative_error=" << film_error << '\n'
                  << "b530_integration_volume_maximum_absolute_error=" << integration_volume_maximum_error << '\n'
                  << "b530_surface_geometry_maximum_relative_error=" << surface_geometry_maximum_error << '\n'
                  << "b530_identical_face_maximum_absolute_difference=" << identical_face_maximum_difference << '\n'
                  << "b530_full_gauss_body_relative_error=" << full_gauss_body_error << '\n'
