...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("B4.0 CSV row is too short: " + path);
    std::size_t parsed = 0;
...
std::vector<ContactReference> read_contact(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B4.0 contact reference: " + path);
    std::string line;
    ...
    if (result.size() != 16)
        throw std::invalid_argument("B4.0 contact reference must contain sixteen rows");
    return result;
}
...
std::vector<ReactionReference> read_reaction(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B4.0 reaction reference: " + path);
    std::string line;
    ...
    if (result.size() != 4)
        throw std::invalid_argument("B4.0 reaction reference must contain four rows");
    return result;
}
...
bool compare(const std::string& output_path, const std::string& contact_path, const std::string& reaction_path) {
    ...
    for (std::size_t step = 0; step < 4; ++step) {
        ...
        if (sources.size() != 4)
            throw std::invalid_argument("B4.0 requires four contact constraints");
        ...
        for (std::size_t node = 0; node < sources.size(); ++node) {
            const auto found = std::find_if(reference.begin(), reference.end(), [&](const ContactReference& value) {
                return value.step == step + 1 && value.id == sources[node];
            });
            if (found == reference.end())
                throw std::invalid_argument("B4.0 contact-node mapping is incomplete");
            ...
            const auto scalar = [&](const std::string& field) {
                ...
                if (!std::isfinite(value))
                    throw std::invalid_argument("B4.0 contact output is not finite");
                return value;
            };
            const std::array<double, 3> normal = {scalar("normal_force_x"),
                scalar("normal_force_y"),
                scalar("normal_force_z")};
            const std::array<double, 3> tangent = {scalar("tangential_force_x"),
                scalar("tangential_force_y"),
                scalar("tangential_force_z")};
            ...
            maximum_coordinate_error = std::max({maximum_coordinate_error,
                std::abs(frame.nodes.at(source)[0] - found->point[0]),
            ...
            maximum_zero_force = std::max({maximum_zero_force,
                std::abs(normal[1]),
                std::abs(normal[2]),
                std::abs(tangent[0]),
        ...
        if (step > 0)
            later_path_reaches_sliding = later_path_reaches_sliding && sliding_count > 0;
    }
...
bool check_hex8_biaxial_friction(const std::string& output_path,
