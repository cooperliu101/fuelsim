...
std::array<fuelsim::test::FieldErrorMetrics, 15> compare_integration(const fuelsim::TransientProblem& problem,
    ...
    for (const IntegrationReference& expected : reference) {
        if (expected.element < 1 || expected.element > 2)
            throw std::invalid_argument("Abaqus B5.5 element label lies outside mesh");
        ...
        for (std::size_t point = 0; point < 8; ++point) {
            const fuelsim::CartesianPoint3& position = geometry.points[point].position;
            const double distance_squared = std::pow(position.x - expected.position.x, 2) +
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: fuelsim_b55_hex8_c3d8t_transient_abaqus_tests <nodes.csv> <integration.csv>\n";
        return 2;
    }
    try {
        std::cout << std::scientific << std::setprecision(12);
        fuelsim::PetscSession session(argc, argv, "fuelsim B5.5 Abaqus transient C3D8T comparison\n");
        ...
        passed = check(coordinate_error < 1.0e-7, "B5.5 maps all sixteen integration points uniquely") && passed;
        if (passed && session.rank() == 0) std::cout << "[PASS] B5.5 Abaqus C3D8T transient full-field comparison\n";
        return passed ? 0 : 1;
    } catch (const std::exception& error) {
        std::cerr << "[FAIL] B5.5 Abaqus C3D8T transient comparison raised: " << error.what() << '\n';
        return 1;
    }
}
