...
std::vector<MortarReference> read_mortar_nodes(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.24 MOOSE mortar nodal reference: " + path);
    std::string line;
    std::getline(input, line);
...
double read_mortar_reaction(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.24 MOOSE mortar reaction reference: " + path);
    std::string line;
    std::getline(input, line);
...
bool fuelsim::test::check_hex20_nonmatching(const std::string& output, const std::string& displacement_path,
    ...
    for (std::size_t source = 0; source < result.nodes.size(); ++source) {
        ...
        const auto found_mortar = std::find_if(
            mortar.begin(), mortar.end(), [&](const MortarReference& value) { return value.id == source; });
        ...
        maximum_mortar_coordinate_difference = std::max(
            maximum_mortar_coordinate_difference, coordinate_difference(result.nodes[source], found_mortar->point));
    ...
    for (std::size_t node = 0; node < projected.size(); ++node) {
        ...
        if (projected[node] == 1.0 && actual_pressure[node] > 0.0) ++active_nodes;
        if (projected[node] != 1.0) ++unprojected_nodes;
    }
    ...
    for (std::size_t node = 0; node < secondary_sources.size(); ++node) {
        ...
        if (found == pressure.end()) throw std::invalid_argument("H20.24 pressure source-node mapping is incomplete");
        const auto found_mortar = std::find_if(mortar.begin(), mortar.end(),
            [&](const MortarReference& value) { return value.id == secondary_sources.at(node); });
        if (found_mortar == mortar.end())
        ...
        maximum_pressure_coordinate_difference = std::max(
            maximum_pressure_coordinate_difference, coordinate_difference(result.nodes[found->id], found->point));
    ...
    passed =
        ...
        check(fuelsim::test::relative_metrics_below(mortar_contact_pressure_abaqus, mortar_abaqus_pressure_tolerance),
