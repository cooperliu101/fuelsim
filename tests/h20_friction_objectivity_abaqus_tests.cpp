...
std::array<AbaqusStep, 2> read_reference(const std::string& path) {
    ...
    if (!input) throw std::runtime_error("Could not read H20.38 Abaqus reference: " + path);
    std::string line;
    ...
    for (std::size_t step = 0; step < result.size(); ++step) {
        if (!std::getline(input, line)) throw std::invalid_argument("Incomplete H20.38 CSV: " + path);
    ...
    if (std::getline(input, line)) throw std::invalid_argument("H20.38 CSV has extra rows: " + path);
    return result;
}

std::vector<fuelsim::CartesianPoint3> read_input_coordinates(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.38 Abaqus input: " + path);
    ...
    while (std::getline(input, line)) {
        ...
        if (!reading_nodes || line.empty()) continue;
        const std::vector<std::string> values = split(line);
...
fuelsim::Quad8FaceCoordinates face_coordinates(
    const fuelsim::UnstructuredHex20Mesh& mesh, const std::string& side_set) {
    ...
    if (set.sides.size() != 1) throw std::invalid_argument("H20.38 requires one face in " + side_set);
...
fuelsim::Quad8SurfaceContactLocalValues rotated_state(
    const fuelsim::Quad8FaceCoordinates& secondary, const fuelsim::Quad8FaceCoordinates& primary) {
...
FuelsimStep evaluate(const fuelsim::NormalContactProperties& properties, const fuelsim::Quad8FaceCoordinates& secondary,
    const fuelsim::Quad8FaceCoordinates& primary, const fuelsim::Quad8SurfaceContactLocalValues& state,
    ...
    for (std::size_t point = 0; point < geometry.mechanical_points.size(); ++point) {
        ...
        if (!reference.projected) throw std::invalid_argument("H20.38 reference point did not project");
        const fuelsim::Quad8ToQuad8MechanicalGeometry contact_geometry{secondary, primary,
            quadrature.displacement_shape, quadrature.derivative_xi, quadrature.derivative_eta, reference.primary_shape,
        ...
        const fuelsim::CartesianContactPointValue value = fuelsim::compute_quad8_to_quad8_contact_value(
            properties, contact_geometry, state, committed_state, history);
        ...
        for (std::size_t component = 0; component < 3; ++component) {
            ...
            for (std::size_t node = 0; node < 16; ++node) balance += residual[offset + node];
        ...
        if (value.sliding) ++result.sliding_points;
...
std::array<double, 3> rotate(const std::array<double, 3>& value) {
    ...
    return {std::cos(angle) * value[0] - std::sin(angle) * value[1],
        std::sin(angle) * value[0] + std::cos(angle) * value[1], value[2]};
}
...
bool compare(const std::string& mesh_path, const std::string& input_path, const std::string& reference_path) {
    ...
    for (std::size_t node = 0; node < mesh.nodes().size(); ++node)
        maximum_coordinate_difference =
            std::max({maximum_coordinate_difference, std::abs(mesh.nodes()[node].x - input_coordinates[node].x),
    ...
    for (std::size_t component = 0; component < 3; ++component) {
        fuelsim_rotation_error =
            std::max({fuelsim_rotation_error, std::abs(second.normal[component] - rotated_fuelsim_normal[component]),
    ...
    for (std::size_t point = 0; point < first.histories.size(); ++point) {
        ...
        biaxial_history = biaxial_history &&
                          std::abs(first.histories[point].cartesian_elastic_tangential_slip[1]) > 0.0 &&
    ...
    const double abaqus_slip_change = std::max(
        std::abs(reference[1].slip_1 - reference[0].slip_1), std::abs(reference[1].slip_2 - reference[0].slip_2));
    ...
    bool passed = check(maximum_coordinate_difference < 1.0e-14,
                      "H20.38 Abaqus input uses the tracked Exodus node coordinates") &&
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (passed) std::cout << "[PASS] H20.38 HEX20 friction objectivity Abaqus comparison\n";
        return passed ? 0 : 1;
