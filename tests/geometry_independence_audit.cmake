...
string(FIND "${spatial_problem}" "TransientTimeErrorEstimate TransientProblem::step_doubling_error" error_begin)
string(FIND "${spatial_problem}" "void TransientProblem::combine_last_half_step_conservation" error_end)
if(error_begin EQUAL -1 OR error_end LESS error_begin)
    message(FATAL_ERROR "Cannot locate the public time-error coordinator")
endif()
math(EXPR error_length "${error_end} - ${error_begin}")
string(SUBSTRING "${spatial_problem}" ${error_begin} ${error_length} error_coordinator)
foreach(forbidden "material_histories" "contact_histories" "dynamic_cast" "BackendAccess::uses")
    string(FIND "${error_coordinator}" "${forbidden}" location)
    if(NOT location EQUAL -1)
        message(FATAL_ERROR "Time-error coordination must delegate backend history layouts: ${forbidden}")
    endif()
endforeach()
string(FIND "${error_coordinator}" "_backend->accumulate_time_error" error_dispatch)
if(error_dispatch EQUAL -1)
    message(FATAL_ERROR "Time-error coordination must retain the private backend protocol")
endif()
