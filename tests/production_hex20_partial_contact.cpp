...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("Incomplete H20.41 reference row: " + path);
    std::size_t parsed = 0;
...
std::vector<Reference> read_reference(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read H20.41 contact reference: " + path);
    std::string line;
    ...
    if (result.size() != 29)
        throw std::invalid_argument("H20.41 requires 29 secondary contact nodes");
    return result;
}
...
bool run(const std::string& output_path, const std::string& reference_path) {
    ...
    for (std::size_t node = 0; node < source_nodes.size(); ++node) {
        ...
        for (std::size_t component = 0; component < 3; ++component)
            coordinate_error = std::max(coordinate_error,
                std::abs(output.nodes.at(reference.node)[component] - reference.point[component]));
        const auto scalar = [&](const std::string& field) {
            ...
            if (!std::isfinite(value))
                throw std::invalid_argument("H20.41 contact output is not finite");
            return value;
        };
    ...
    if (side == output.side_set_names.end())
        throw std::invalid_argument("H20.41 output lacks secondary contact sides");
    ...
    for (const auto& face : faces) {
        std::size_t active = 0;
        if (face.size() != 8)
            throw std::invalid_argument("H20.41 contact faces must contain eight nodes");
        for (const std::size_t source : face)
    ...
    passed = check(fuelsim_constraint_active == 11 && fuelsim_recovered_positive == 16 && abaqus_constraint_active == 11
