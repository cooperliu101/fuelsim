...
bool check_hex20_curved(const std::string& output_path, const std::string& name, const std::string& displacement_path,
    ...
    const std::array<std::string, 3> forces = {
        "contact_normal_force_x_interface", "contact_normal_force_y_interface", "contact_normal_force_z_interface"};
    ...
    for (const auto& row : displacement) {
        if (row.id >= present.size() || present[row.id]) throw std::runtime_error("H20.30 repeated or invalid node ID");
        present[row.id] = true;
    ...
    for (const auto& row : force) {
        ...
        if (!(radius > 0.0)) throw std::runtime_error("H20.30 radial force has zero radius");
        const double actual =
    ...
    if (!(reference_resultants[0] > 0.0)) throw std::runtime_error("H20.30 reference resultant is not positive");
