...
bool compare_committed_states(const fuelsim::TransientCommittedState& left,
    ...
    const std::array<double, 18> left_conservation = {left.conservation.generated_heat_rate,
                                                      ...
                                                      left.conservation.interface_heat_imbalance,
                                                      left.conservation.dirichlet_heat_input_rate,
                                                      left.conservation.global_thermal_balance,
                                                      left.conservation.relative_thermal_balance,
                                                      left.conservation.unconstrained_thermal_residual_l2,
                                                      left.conservation.internal_mechanical_work_increment,
                                                      left.conservation.pressure_traction_work_increment,
                                                      left.conservation.dirichlet_reaction_work_increment,
                                                      left.conservation.contact_work_increment,
                                                      left.conservation.mechanical_work_balance,
                                                      left.conservation.relative_mechanical_work_balance,
                                                      left.conservation.unconstrained_mechanical_residual_l2,
                                                      left.conservation.elastic_energy_change,
                                                      left.conservation.plastic_dissipation_increment,
    ...
    const std::array<double, 18> right_conservation = {right.conservation.generated_heat_rate,
                                                       right.conservation.stored_heat_rate,
                                                       right.conservation.convection_heat_rate,
                                                       right.conservation.interface_heat_imbalance,
                                                       right.conservation.dirichlet_heat_input_rate,
                                                       right.conservation.global_thermal_balance,
                                                       right.conservation.relative_thermal_balance,
                                                       right.conservation.unconstrained_thermal_residual_l2,
                                                       right.conservation.internal_mechanical_work_increment,
                                                       right.conservation.pressure_traction_work_increment,
                                                       right.conservation.dirichlet_reaction_work_increment,
                                                       right.conservation.contact_work_increment,
                                                       right.conservation.mechanical_work_balance,
                                                       right.conservation.relative_mechanical_work_balance,
                                                       right.conservation.unconstrained_mechanical_residual_l2,
                                                       right.conservation.elastic_energy_change,
                                                       right.conservation.plastic_dissipation_increment,
...
bool test_friction_history_checkpoint(const std::string& input_path, const std::string& checkpoint_path) {
    ...
    passed =
        expect_failure([&]() { (void)fuelsim::TransientCheckpointIo::restore(checkpoint_path, old_version_target); },
...
bool run_tests(const std::string& steady_input_path, const std::string& transient_input_path,
               const std::string& finite_strain_input_path, const std::string& checkpoint_path,
               const std::string& results_path) {
    const fuelsim::FuelSimCaseDefinition steady_input = fuelsim::CaseInputReader::read(steady_input_path);
    ...
    const fuelsim::UnstructuredQuad4Mesh steady_mesh = fuelsim::ExodusMeshIo::read_quad4(steady_input.mesh_file);
    ...
    const fuelsim::FuelSimCaseDefinition input = fuelsim::CaseInputReader::read(transient_input_path);
    const fuelsim::UnstructuredQuad4Mesh mesh = fuelsim::ExodusMeshIo::read_quad4(input.mesh_file);
    const fuelsim::SolverOptions solver{input.solver.absolute_tolerance, input.solver.relative_tolerance,
                                        input.solver.step_tolerance, input.solver.maximum_iterations};
    ...
    passed = expect_failure([&]() { (void)fuelsim::TransientCheckpointIo::restore(checkpoint_path, changed_problem); },
                            "signature", "checkpoint rejects a changed registered material function version") &&
             passed;

    mismatch.begin_time_step({1.0, 0.05});
    passed = expect_failure([&]() { fuelsim::TransientCheckpointIo::write(checkpoint_path, mismatch, 1.0); },
                            "active time step", "checkpoint cannot capture uncommitted trial state") &&
             passed;
    mismatch.rollback_time_step();
    passed = expect_failure([&]() { fuelsim::TransientCheckpointIo::write(checkpoint_path, mismatch, 0.0); },
    ...
    passed = expect_failure([&]() { (void)fuelsim::TransientCheckpointIo::restore(checkpoint_path, mismatch); },
