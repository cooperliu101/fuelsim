...
const std::vector<PathStep> path = [] {
    std::vector<PathStep> result = {{"STICK", 0.0, {-0.01, 3.0e-5, 4.0e-5}, false},
        {"SLIDE", 0.0, {-0.01, 1.2e-3, 1.6e-3}, true}};
...
fuelsim::CartesianPoint3
current_point(const fuelsim::CartesianPoint3& reference, const PathStep& step, bool secondary) {
...
void write_abaqus_input(const std::string& output_path, const fuelsim::UnstructuredHex8Mesh& mesh) {
    ...
    if (!output)
        throw std::runtime_error("Could not write B4.2 Abaqus input: " + output_path);
    ...
    for (const std::size_t node : mesh.elements().at(0).nodes)
        output << ", " << node + 1;
    output << "\n*Element, type=C3D8, elset=SECONDARY\n2";
    for (const std::size_t node : mesh.elements().at(1).nodes)
        output << ", " << node + 1;
...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& input_path) {
    if (column >= values.size())
        throw std::invalid_argument("Incomplete B4.2 CSV row: " + input_path);
    std::size_t parsed = 0;
...
std::vector<std::vector<ReferenceNode>> read_reference(const std::string& input_path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B4.2 Abaqus reference: " + input_path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (step == 0 || step > path.size())
            throw std::invalid_argument("Invalid B4.2 CSV step: " + input_path);
    ...
    for (const auto& values : result)
        if (values.size() != 4)
            throw std::invalid_argument("Each B4.2 step must contain four contact nodes");
    return result;
}
...
bool compare_case(const fuelsim::UnstructuredHex8Mesh& mesh,
    const std::vector<std::vector<ReferenceNode>>& reference,
    ...
    for (std::size_t step = 0; step < path.size(); ++step) {
        ...
        if (actual.size() != source_nodes.size())
            throw std::invalid_argument("B4.2 contact-node count differs");
        ...
        for (std::size_t node = 0; node < actual.size(); ++node) {
            ...
            if (found == reference[step].end())
                throw std::invalid_argument("B4.2 source-node mapping is incomplete");
            ...
            maximum_coordinate_difference = std::max({maximum_coordinate_difference,
                std::abs(expected_point.x - found->current_point.x),
            ...
            physical_state_sequence_matches = physical_state_sequence_matches
                                              && (path[step].sliding ? std::abs(actual_ratio - 1.0) < 2.0e-3
                                                                           && std::abs(reference_ratio - 1.0) < 2.0e-3
        ...
        for (const auto& history : histories) {
            ...
            if (step == 1)
                sliding_history_before_rotation = value;
    ...
    return check(maximum_coordinate_difference < 3.0e-8,
           ...
           && check(fuelsim::test::relative_metrics_below(tangent_first, tolerance)
               ...
               prefix + "both tangent-plane nodal-force metrics and zero references agree with Abaqus below 1 percent")
...
int main(int argc, char** argv) {
    try {
        ...
        if (small && finite)
            std::cout << "[PASS] B4.2 HEX8 friction-objectivity Abaqus comparison\n";
