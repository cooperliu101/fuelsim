...
std::vector<Row> read_rows(const std::string& path) {
    ...
    if (!input) throw std::runtime_error("Could not read H20.28 operator reference: " + path);
    std::string line;
    std::getline(input, line);
    if (!line.empty() && line.back() == '\r') line.pop_back();
    if (line != "step,input_local_node,input_label,side,output_local_node,output_label,closure_delta_m,"
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 11) throw std::invalid_argument("Incomplete H20.28 operator row in " + path);
        result.push_back({values[0], values[3], static_cast<std::size_t>(std::stoull(values[2])),
            static_cast<std::size_t>(std::stoull(values[5])), values[7].empty() ? 0.0 : std::stod(values[7]),
...
Row find_row(const std::vector<Row>& rows, const std::string& step, const std::string& side, std::size_t output_label) {
    const auto found = std::find_if(rows.begin(), rows.end(),
        [&](const Row& row) { return row.step == step && row.side == side && row.output_label == output_label; });
    if (found == rows.end()) throw std::invalid_argument("Missing H20.28 perturbation row");
    return *found;
}
...
Matrix8 inverse8(const Matrix8& matrix) {
    ...
    for (std::size_t column = 0; column < secondary_count; ++column) {
        ...
        for (std::size_t row = 0; row < secondary_count; ++row) result[row * secondary_count + column] = solution[row];
    }
    return result;
}
...
IdentifiedOperator identify(const std::string& path) {
    ...
    for (std::size_t input_node = 0; input_node < secondary_count; ++input_node) {
        ...
        if (input_found == secondary_labels.end()) throw std::invalid_argument("Invalid H20.28 input label");
    ...
    for (double& value : normalized_tangent) value /= penalty;
...
double relative_difference(const std::vector<double>& first, const std::vector<double>& second) {
    if (first.size() != second.size()) throw std::logic_error("H20.28 comparison size mismatch");
...
std::vector<double> values(const Matrix8& matrix) { return {matrix.begin(), matrix.end()}; }
} // namespace
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const double default_quadratic_averaging =
                     ...
                     default_quadratic_area = relative_difference(
                         values(default_operator.area_matrix), values(quadratic_operator.area_matrix)),
        ...
        const std::array<std::array<double, 2>, secondary_count> expected_centers{{{0.125, 0.125}, {0.875, 0.125},
            {0.125, 0.875}, {0.875, 0.875}, {0.5, 0.25}, {0.5, 0.75}, {0.25, 0.5}, {0.75, 0.5}}};
        for (std::size_t row = 0; row < secondary_count; ++row) {
            ...
            for (std::size_t primary = 0; primary < primary_count; ++primary) {
                ...
                if (std::abs(default_value) > 1.0e-8) ++default_support[row];
                if (std::abs(linear_value) > 1.0e-8) ++linear_support[row];
            }
            ...
            center_error = std::max({center_error, std::abs(center_y - expected_centers[row][0]),
        ...
        passed = check(row_sum_error < 1.0e-9 && transfer_sum_error < 1.0e-9 && center_error < 1.0e-10,
                 ...
                 passed;
        passed =
            check(default_support == std::array<std::size_t, secondary_count>{45, 45, 45, 45, 109, 109, 109, 109} &&
