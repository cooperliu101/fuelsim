...
std::vector<RegionAverages::Point> read_cladding_points(const std::string& coordinate_path,
    ...
    for (const std::vector<std::string>& value_row : values.rows) {
        ...
        for (const std::size_t qp : moose_qp_from_fuelsim) {
            const auto coordinate = std::find_if(coordinates.rows.begin(),
                coordinates.rows.end(),
                [&coordinates, element, qp](const auto& row) {
            ...
            result.push_back({csv_value(coordinates, *coordinate, "x"),
...
RegionAverages region_averages(const fuelsim::test::ExodusResults& result) {
    ...
    if (found == result.block_names.end())
        throw std::invalid_argument("M5.7 output has no clad block");
    ...
    for (std::size_t e = offset; e < offset + result.block_element_counts[block]; ++e) {
        for (std::size_t q = 0; q < 4; ++q) {
            ...
            const double stress = std::sqrt(1.5
                                            * ((rr - mean) * (rr - mean) + (zz - mean) * (zz - mean)
            ...
            averages.points.push_back({result.element("reference_r" + suffix)[e],
                result.element("reference_z" + suffix)[e],
                stress,
                plastic,
                creep});
        }
    ...
    if (!(measure > 0.0))
        throw std::runtime_error("M5.7 clad integration measure is empty");
...
bool check_cladding_fields(const fuelsim::test::ExodusResults& result,
    ...
    for (const auto& entry : {std::pair<const char*, double>{"average_effective_plastic", cladding.plastic},
             {"average_effective_creep", cladding.creep},
             {"average_vonmises_stress", cladding.stress}}) {
...
bool fuelsim::test::check_rz_integrated(const std::string& output_path,
    const std::string& coordinate_path,
    const std::string& value_path,
    const std::string& scalar_path) {
...
bool fuelsim::test::check_rz_pcmi(const std::string& output_path,
    const std::string& coordinate_path,
    const std::string& value_path,
    const std::string& scalar_path,
    ...
    const auto side_count = [&](const std::string& name) {
        ...
        if (found == result.side_set_names.end())
            throw std::runtime_error("PCMI output is missing side set " + name);
