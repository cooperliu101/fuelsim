...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete Abaqus B5.11 row in " + path);
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.11 nodes: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 11)
            throw std::invalid_argument("Unexpected Abaqus B5.11 nodal column count in " + path);
        std::array<double, 8> fields{};
    ...
    if (result.size() != 80)
        throw std::invalid_argument("Abaqus B5.11 nodal reference must contain 80 rows");
...
std::vector<IntegrationReference>
read_integration(const std::string& path, bool finite_strain, bool reduced_integration) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.11 integration points: " + path);
    std::string line;
    std::getline(input, line);
...
std::vector<EnergyReference> read_energy(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.11 energy: " + path);
    std::string line;
    std::getline(input, line);
    ...
    while (std::getline(input, line)) {
        const std::vector<std::string> values = split_csv(line);
        if (values.size() != 5)
            throw std::invalid_argument("Unexpected Abaqus B5.11 energy column count in " + path);
        result.push_back({positive_integer(number(values, 0, path), path),
            number(values, 1, path),
    ...
    if (result.size() != 10)
        throw std::invalid_argument("Abaqus B5.11 energy reference must contain ten rows");
    return result;
}
...
bool check_hex8_norton_abaqus(const std::string& output_path,
    ...
    for (const auto& reference : node_reference) {
        ...
        if (reference.node < 1 || reference.node > 8 || std::abs(reference.time - (frame.time - preload_time)) > 1e-12
    ...
    const std::array<std::string, 27> integration_names = {"stress_xx",
        "stress_yy",
        "stress_zz",
        "stress_xy",
        "stress_yz",
