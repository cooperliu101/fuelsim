...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete three-dimensional contact row in " + path);
...
std::vector<NodeReference> read_nodes(const std::string& thermal_path, const std::string& mechanical_path) {
    ...
    if (!thermal)
        throw std::runtime_error("Could not read three-dimensional thermal-contact nodes: " + thermal_path);
    std::string line;
    std::getline(thermal, line);
    if (line != "T,id,x,y,z")
        throw std::invalid_argument("Unexpected thermal-contact header in " + thermal_path);
    ...
    while (std::getline(mechanical, line)) {
        ...
        const std::array<double, 3> point = {number(values, 4, mechanical_path),
            number(values, 5, mechanical_path),
            number(values, 6, mechanical_path)};
        if (std::max({std::abs(point[0] - result[row].point[0]),
                std::abs(point[1] - result[row].point[1]),
                std::abs(point[2] - result[row].point[2])})
            > 1.0e-12)
    ...
    if (row != result.size())
        throw std::invalid_argument("Thermal and mechanical MOOSE node counts differ");
    return result;
}
...
std::vector<ContactReference> read_contact(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read three-dimensional contact pressure: " + path);
    std::string line;
...
bool check_hex8_sticking(const std::string& output_path,
    const std::string& thermal_path,
    const std::string& mechanical_path,
    ...
    for (std::size_t n = 0; n < nodes.size(); ++n) {
        if (nodes[n].id != n)
            throw std::runtime_error("B3.3 node IDs are not contiguous");
        ...
        for (std::size_t f = 0; f < 4; ++f)
            fields[f].add(output.nodal(names[f]).at(n), nodes[n].fields[f]);
    }
    bool passed = true;
    for (std::size_t f = 0; f < 4; ++f) {
    ...
    for (const auto& row : contact) {
        if (row.id >= seen.size() || seen[row.id])
            throw std::runtime_error("B3.3 contact reference ID is invalid");
        seen[row.id] = true;
        for (std::size_t c = 0; c < 3; ++c)
    ...
    passed = check(output.global("contact_heat_rate_interface") > 0.0 && output.global("contact_force_interface") > 0.0
