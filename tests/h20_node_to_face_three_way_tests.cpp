...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size()) throw std::invalid_argument("Incomplete H20.25 reference row in " + path);
    return std::stod(values[index]);
}

std::vector<DisplacementReference> read_displacement(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.25 displacement reference: " + path);
    std::string line;
...
std::vector<PressureReference> read_abaqus_pressure(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.25 Abaqus pressure reference: " + path);
    std::string line;
    std::getline(input, line);
    if (line != "pressure,id,x,y,z") throw std::invalid_argument("Unexpected H20.25 pressure header in " + path);
...
std::vector<MooseContactReference> read_moose_contact(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.25 MOOSE contact reference: " + path);
    std::string line;
    std::getline(input, line);
...
double read_normal_reaction(const std::string& path) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read H20.25 reaction reference: " + path);
    std::string line;
    std::getline(input, line);
...
double relative_error(double first, double second) {
    if (!(second > 0.0)) throw std::invalid_argument("H20.25 relative-error reference must be positive");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        bool passed =
            ...
            check(solve.aggregate_timing.workspace_setups == 1, "H20.25 Fuelsim solve constructs one PETSc workspace");
        ...
        for (std::size_t region = 0; region < spatial.region_count(); ++region) {
            ...
            for (std::size_t local = 0; local < region_mesh.nodes().size(); ++local) {
                ...
                const auto moose = std::find_if(moose_displacement.begin(), moose_displacement.end(),
                    [&](const DisplacementReference& value) { return value.id == source; });
                const auto abaqus = std::find_if(abaqus_displacement.begin(), abaqus_displacement.end(),
                    [&](const DisplacementReference& value) { return value.id == source; });
                ...
                present[source] = true;
                maximum_moose_coordinate_difference = std::max(
                    maximum_moose_coordinate_difference, coordinate_difference(mesh.nodes().at(source), moose->point));
                ...
                for (std::size_t component = 0; component < 3; ++component) {
                    ...
                    moose_abaqus_displacement[component].add(
                        moose->displacement[component], abaqus->displacement[component]);
                }
            }
        }
        ...
        for (std::size_t index = 0; index < contact.size(); ++index) {
            ...
            const auto moose = std::find_if(moose_contact.begin(), moose_contact.end(),
                [&](const MooseContactReference& value) { return value.id == source; });
            const auto abaqus = std::find_if(abaqus_pressure.begin(), abaqus_pressure.end(),
            ...
            if (contact[index].pressure > 0.0) ++active;
        ...
        for (std::size_t component = 0; component < 3; ++component) {
            fuelsim::test::print_relative_metrics(
                "h20_25_fuelsim_moose_displacement_" + names[component], fuelsim_moose_displacement[component]);
            fuelsim::test::print_relative_metrics(
                "h20_25_fuelsim_abaqus_displacement_" + names[component], fuelsim_abaqus_displacement[component]);
            fuelsim::test::print_relative_metrics(
                "h20_25_moose_abaqus_displacement_" + names[component], moose_abaqus_displacement[component]);
        }
        ...
        passed =
            ...
            passed;
        if (passed && session.rank() == 0) std::cout << "[PASS] H20.25 Hex20 node-to-face three-way comparison\n";
        return passed ? 0 : 1;
