...
constexpr std::array<PathStep, step_count> path = {
    {{"CLOSE", {-0.01, 0.05, 0.04}, true}, {"SLIDE", {-0.01, 0.95, 0.20}, true}, {"OPEN", {0.02, 0.95, 0.20}, false},
        {"OPEN_CROSS", {0.02, 0.05, 0.40}, false}, {"RECONTACT", {-0.01, 0.05, 0.40}, true}}};
...
Reference read_reference(const std::string& path_name) {
    ...
    if (!input) throw std::runtime_error("Could not read B5.21 Abaqus nodal reference: " + path_name);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 10) throw std::invalid_argument("Unexpected B5.21 Abaqus column count");
        ...
        result[step - 1][node - 1] = {std::stod(values[2]), std::stod(values[3]),
...
std::vector<double> flatten_committed_state(const fuelsim::TransientProblem& problem) {
    ...
    for (const auto& region : state.cartesian_material_histories)
        for (const fuelsim::CartesianMaterialHistory& element : region)
            for (const fuelsim::CartesianMaterialPointState& point : element) {
                ...
                result.insert(result.end(), {point.stress.xx, point.stress.yy, point.stress.zz, point.stress.xy,
                                                point.stress.yz, point.stress.xz});
            }
    for (const auto& contact : state.contact_histories)
        for (const fuelsim::ContactPointHistory& history : contact) {
            ...
            result.insert(result.end(), history.cartesian_contact_tangent_first.begin(),
...
bool compare(const Reference& reference) {
    ...
    for (std::size_t step = 0; step < step_count; ++step) {
        ...
        const std::vector<double> thermal = assembled_contact_residual(
            spatial, state, fuelsim::SpatialContributionType::thermal_contact, &conservation_error);
        ...
        if (step == 0 && !nodes.empty()) first_primary_face = nodes.front().primary_face;
        ...
        for (std::size_t source = 0; source < node_count; ++source) {
            ...
            for (std::size_t component = 0; component < 3; ++component)
                state_difference =
                    std::max(state_difference, std::abs(state[spatial.dof(fields[component], global)] -
        ...
        if (path[step].active) {
            ...
            zero_reference_normal_force_difference = std::max(
                {zero_reference_normal_force_difference, std::abs(actual_normal_force - reference_normal_force),
...
bool check_transaction_and_restart(const std::string& checkpoint_path) {
    ...
    const std::vector<double> trial_heat = assembled_contact_residual(
        fuelsim::cartesian::ProblemAccess::view(problem), closed, fuelsim::SpatialContributionType::thermal_contact);
    ...
    const std::vector<double> open_heat = assembled_contact_residual(
        fuelsim::cartesian::ProblemAccess::view(restored), open, fuelsim::SpatialContributionType::thermal_contact);
    double open_heat_maximum = 0.0;
    for (double value : open_heat) open_heat_maximum = std::max(open_heat_maximum, std::abs(value));
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (passed) std::cout << "[PASS] B5.21 Abaqus C3D8T thermal-contact path\n";
        return passed ? 0 : 1;
