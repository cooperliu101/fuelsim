...
bool run_comparison(const std::string& input_path, const std::string& nodal_reference_path,
    ...
    bool passed =
        ...
        check(result.aggregate_timing.workspace_setups == 1, "M5.1 frictional path reuses one PETSc workspace");
    ...
    for (const fuelsim::ContactNodeSummary& node : contact_nodes) {
        ...
        maximum_capacity_excess =
            std::max(maximum_capacity_excess,
    ...
    passed = check(active == contact_nodes.size() && sliding > 0 &&
                       maximum_capacity_excess <= 1.0e-12 * interface.maximum_contact_pressure &&
    ...
    const double friction_effect = std::sqrt(axial_difference_squared / axial_scale_squared);
    ...
    fuelsim::test::print_relative_metrics("m51_temperature", fields.temperature);
    fuelsim::test::print_relative_metrics("m51_radial_displacement", fields.radial_displacement);
    fuelsim::test::print_relative_metrics("m51_axial_displacement", fields.axial_displacement);
    ...
    std::cout << "m51_active_contact_nodes=" << active << '\n'
              ...
              << "m51_total_tangential_force=" << interface.total_tangential_force << '\n'
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(argc, argv, "fuelsim M5.1 Coulomb friction MOOSE comparison\n");
