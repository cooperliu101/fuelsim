...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("Incomplete B5.49 CSV row: " + path);
    std::size_t parsed = 0;
...
std::vector<TemperatureReference> read_temperature(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B5.49 temperature reference: " + path);
    std::string line;
    ...
    if (result.size() != 40)
        throw std::invalid_argument("B5.49 requires 40 corner-temperature rows");
    return result;
}
...
std::vector<DisplacementReference> read_displacement(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B5.49 displacement reference: " + path);
    std::string line;
    ...
    if (result.size() != 112)
        throw std::invalid_argument("B5.49 requires 112 displacement rows");
    return result;
}
...
std::vector<MaterialReference> read_material(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B5.49 material reference: " + path);
    std::string line;
    ...
    if (result.size() != 216)
        throw std::invalid_argument("B5.49 requires 216 material-point rows");
    return result;
}
...
std::vector<ContactReference> read_contact(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read C3D20T contact reference: " + path);
    std::string line;
    const std::string expected = "id,x,y,z,gap,pressure,normal_x,normal_y,normal_z,shear_x,shear_y,shear_z";
    const std::string friction_expected = expected
                                          + ",slip_1,slip_2,tangent_1_x,tangent_1_y,tangent_1_z,tangent_2_x,"
    ...
    if (result.size() != 8)
        throw std::invalid_argument("C3D20T contact reference requires eight secondary nodes");
    return result;
}
...
bool run(const std::string& output_path,
    ...
    for (const auto& reference : material_reference)
        references_by_element[reference.element].push_back(reference);
    ...
    for (std::size_t element = 0; element < 8; ++element) {
        ...
        for (std::size_t q = 0; q < 27; ++q) {
            ...
            positions[q] = {output.element("current_x" + suffix).at(element),
                output.element("current_y" + suffix).at(element),
                output.element("current_z" + suffix).at(element)};
        ...
        for (const auto& candidate : candidates) {
            ...
            if (actual_used[actual] || reference_used[reference])
                continue;
            ...
            const Stress stress = {output.element("stress_xx" + suffix).at(element),
                output.element("stress_yy" + suffix).at(element),
                output.element("stress_zz" + suffix).at(element),
                output.element("stress_xy" + suffix).at(element),
                output.element("stress_yz" + suffix).at(element),
            ...
            plastic_metrics.add(output.element("equiv_plastic" + suffix).at(element),
                references[reference].equivalent_plastic_strain);
            creep_metrics.add(output.element("equiv_creep" + suffix).at(element),
                references[reference].equivalent_creep_strain);
        }
        if (pair_count != 27)
            throw std::logic_error("B5.49 material-point association is incomplete");
    }
    ...
    if (!contact_reference.empty()) {
        ...
        for (std::size_t node = 0; node < projected.size(); ++node)
            if (std::isfinite(projected[node]))
        ...
        for (std::size_t node = 0; node < source_nodes.size(); ++node) {
            const auto scalar = [&](const std::string& field) {
                ...
                if (!std::isfinite(value))
                    throw std::invalid_argument("C3D20T contact output is not finite");
                return value;
            };
            ...
            constraint_contact_pressure_metrics.add(std::max(-penalty * scalar("gap"), 0.0),
                std::max(-penalty * reference.gap, 0.0));
            ...
            contact_normal_force_metrics.add(fuelsim_secondary_force.data(),
                reference.normal_force.data(),
                reference.normal_force.size());
            if (frictional) {
                contact_tangential_force_metrics.add(fuelsim_secondary_tangential_force.data(),
                    reference.tangential_force.data(),
                    reference.tangential_force.size());
                contact_tangential_slip_metrics.add(slip.data(),
                    reference.tangential_slip.data(),
                    reference.tangential_slip.size());
            }
        }
        ...
        passed = check(abaqus_contact_force > 0.0
                           && std::abs(fuelsim_contact_force - abaqus_contact_force) / abaqus_contact_force
                                  < contact_tolerance,
        ...
        if (frictional) {
            passed = check(abaqus_nodal_tangential_force_magnitude_sum > 0.0
                     ...
                     && passed;
            passed = check(fuelsim::test::relative_metrics_below(contact_gap_metrics, contact_tolerance)
                               && fuelsim::test::grouped_relative_metrics_below(contact_tangential_force_metrics,
                                   contact_tolerance)
                               && fuelsim::test::grouped_relative_metrics_below(contact_tangential_slip_metrics,
                                   contact_tolerance)
                               && fuelsim::test::grouped_relative_metrics_below(contact_tangential_resultant_metrics,
                                   contact_tolerance),
        ...
        fuelsim::test::print_relative_metrics(prefix + "constraint_contact_pressure",
            constraint_contact_pressure_metrics);
        fuelsim::test::print_relative_metrics(prefix + "recovered_contact_pressure",
            recovered_contact_pressure_metrics);
        fuelsim::test::print_grouped_relative_metrics(prefix + "contact_normal_force", contact_normal_force_metrics);
        if (frictional) {
            fuelsim::test::print_grouped_relative_metrics(prefix + "contact_tangential_force",
                contact_tangential_force_metrics);
            fuelsim::test::print_grouped_relative_metrics(prefix + "contact_tangential_slip",
                contact_tangential_slip_metrics);
            fuelsim::test::print_grouped_relative_metrics(prefix + "contact_tangential_resultant",
                contact_tangential_resultant_metrics);
        }
    }
...
bool check_hex20_integrated(const std::string& output_path,
    const std::string& temperature_path,
    const std::string& displacement_path,
    const std::string& material_path,
    const std::string& contact_path,
