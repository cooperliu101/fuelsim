...
VariantConfig variant_config(const std::string& name) {
    ...
    if (name == "material_oracle")
        return {name,   ExpectedBehavior::coupled,
                1,      100,
                5.0e-6, 1.0e-6,
                1.0e-6, 1.0e-6,
                2.0e-6, 1.0e-7};
  ...
  public:
    ...
    void accepted_step(const fuelsim::TransientProblem& problem,
        ...
        if (problem.region_count() != 1 ||
            problem.region_mesh(0).elements().size() != _expected_element_count)
        ...
        for (std::size_t node = 0; node < mesh.nodes().size(); ++node) {
            if (std::abs(mesh.nodes()[node].z - maximum_z) > time_tolerance)
                continue;
            radial += solution.at(problem.dof_map().radial_displacement(node));
...
std::vector<ReferenceSnapshot> read_reference_history(const std::string& path) {
    ...
    const std::array<std::size_t, 4> elastic = {
        column("elastic_rr"), column("elastic_zz"), column("elastic_hoop"),
        column("elastic_rz")};
    const std::array<std::size_t, 4> combined_inelastic = {
        column("combined_rr"), column("combined_zz"), column("combined_hoop"),
        column("combined_rz")};
    ...
    std::sort(
        ...
        [](const ReferenceSnapshot& left, const ReferenceSnapshot& right) {
...
std::vector<NodalHistorySnapshot> read_nodal_history(const std::string& path,
                                                     std::size_t node_count) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read M4.3 MOOSE nodal history: " +
                                 path);
    std::string line;
    if (!std::getline(input, line))
        throw std::invalid_argument("M4.3 MOOSE nodal history is empty: " +
                                    path);
    const std::vector<std::string> header = split_csv(line);
    const auto column = [&header](const std::string& name) {
        return column_index(header, name);
    ...
    if (entries.empty())
        throw std::invalid_argument(
            "M4.3 MOOSE nodal history has no transient rows: " + path);
    std::sort(
        entries.begin(), entries.end(),
        [](const NodalHistoryEntry& left, const NodalHistoryEntry& right) {
            if (left.time != right.time)
                return left.time < right.time;
    ...
    for (const NodalHistoryEntry& entry : entries) {
        if (result.empty() || entry.time != result.back().time) {
            if (!result.empty() &&
                ...
                throw std::invalid_argument(
                    "M4.3 MOOSE nodal history misses a node: " + path);
            result.push_back(
                {entry.time,
                 std::vector<fuelsim::test::NodalFieldReference>(node_count)});
            present.assign(node_count, false);
        }
        if (entry.node_id >= node_count || present[entry.node_id])
            throw std::invalid_argument(
                "M4.3 MOOSE nodal history has an invalid or duplicate node "
                "ID: " +
                path);
        result.back().nodes[entry.node_id] = entry.field;
        present[entry.node_id] = true;
    }
    if (std::any_of(present.begin(), present.end(),
                    [](bool value) { return !value; }))
        throw std::invalid_argument("M4.3 MOOSE nodal history misses a node: " +
                                    path);
    return result;
}
...
bool check_metrics(const std::string& name,
                   ...
                   double zero_reference_tolerance, double aggregate_tolerance,
    ...
    std::cout << name << "_relative_l2_tolerance=" << aggregate_tolerance
              << '\n';
    std::cout << name
              << "_relative_absolute_peak_tolerance=" << aggregate_tolerance
              << '\n';
    std::cout << name
              << "_maximum_pointwise_relative_tolerance=" << pointwise_tolerance
              << '\n';
...
void print_tensor_metric_locations(
    const std::string& name, const fuelsim::test::FieldErrorMetrics& metrics,
    const std::vector<ReferenceSnapshot>& reference) {
    constexpr std::array<const char*, 4> components = {"rr", "zz", "hoop",
                                                       "rz"};
    const auto print_location = [&name, &reference,
                                 &components](const std::string& metric,
...
bool check_load_path(const std::vector<HistorySnapshot>& snapshots,
    ...
    const double axial_stretch = 1.0 + positive_shear.top_axial_displacement /
                                           positive_shear.reference_height;
    ...
    for (const ElementSnapshot& element : positive_shear.elements) {
        maximum_plastic_shear = std::max(
            maximum_plastic_shear, std::abs(element.state.plastic_strain[3]));
        maximum_creep_shear = std::max(maximum_creep_shear,
...
bool audit_creep_shared_state(const fuelsim::FuelSimCaseDefinition& definition,
    ...
    if (reference.size() != nodal_history.size() * mesh.elements().size())
        throw std::invalid_argument(
            "M4.3 creep shared-state node and element histories differ in "
            "length");
    ...
    for (const NodalHistorySnapshot& snapshot : nodal_history) {
        ...
        for (std::size_t local = 0; local < mesh.nodes().size(); ++local) {
            ...
            const fuelsim::test::NodalFieldReference& field =
                snapshot.nodes.at(source_node);
            ...
            state[problem.dof_map().temperature(global_node)] =
                field.temperature;
        ...
        const double load_factor =
            ramp_time > 0.0 ? std::min(snapshot.time / ramp_time, 1.0) : 1.0;
        ...
        for (const fuelsim::DirichletCondition& condition :
            ...
            maximum_boundary_value_difference =
                std::max(maximum_boundary_value_difference,
                         std::abs(state.at(condition.dof) - condition.value));
        }
        ...
        if (!(local_force_l2 > 0.0) || !(local_force_infinity > 0.0))
            throw std::domain_error(
                "M4.3 creep shared-state force scale is zero");
        const double l2_ratio = free_residual_l2 / local_force_l2;
        const double infinity_ratio =
            free_residual_infinity / local_force_infinity;
        if (l2_ratio > maximum_free_residual_l2_ratio)
            maximum_free_residual_ratio_time = snapshot.time;
        maximum_free_residual_l2 =
            std::max(maximum_free_residual_l2, free_residual_l2);
        maximum_free_residual_l2_ratio =
            std::max(maximum_free_residual_l2_ratio, l2_ratio);
        maximum_free_residual_infinity =
            std::max(maximum_free_residual_infinity, free_residual_infinity);
        maximum_free_residual_infinity_ratio =
            std::max(maximum_free_residual_infinity_ratio, infinity_ratio);
        ...
        for (std::size_t element = 0; element < mesh.elements().size();
            ...
            if (std::abs(expected.time - snapshot.time) >= time_tolerance ||
                ...
                throw std::invalid_argument(
                    "M4.3 creep shared-state history ordering differs");
            const fuelsim::Quad4RzGeometry& geometry =
                problem.region_element_geometry(0, element);
            const std::array<fuelsim::AxisymmetricStressValues, 4>& stresses =
                problem.material_stress(0, element);
            ...
            for (std::size_t point = 0; point < geometry.points.size();
                ...
                for (std::size_t component = 0; component < 4; ++component) {
                    average_state.elastic_strain[component] +=
                        weight * history[point].elastic_strain[component];
                    average_state.plastic_strain[component] +=
                        weight * history[point].plastic_strain[component];
                    average_state.creep_strain[component] +=
                        weight * history[point].creep_strain[component];
                }
                average_state.equivalent_creep_strain +=
                    weight * history[point].equivalent_creep_strain;
            }
            if (!(total_weight > 0.0))
                throw std::domain_error(
                    "M4.3 creep shared-state element volume is zero");
            std::array<double, 4> actual_stress =
                stress_components(average_stress);
            for (std::size_t component = 0; component < 4; ++component) {
                ...
                elastic.add(average_state.elastic_strain[component],
            ...
            equivalent_creep.add(average_state.equivalent_creep_strain,
    ...
    bool passed =
        check_metrics(prefix + "stress", stress, 1.0e-3, 1.0e-8, 1.0e-5);
    passed = check_metrics(prefix + "elastic_strain", elastic, 1.0e-12, 1.0e-8,
                           1.0e-5) &&
             passed;
    passed = check_metrics(prefix + "combined_inelastic_strain",
                           combined_inelastic, 1.0e-12, 1.0e-8, 1.0e-5) &&
             passed;
    passed = check_metrics(prefix + "equivalent_creep", equivalent_creep,
                           1.0e-12, 1.0e-8, 1.0e-5) &&
             passed;
    print_tensor_metric_locations(prefix + "stress", stress, reference);
    std::cout << prefix << "compared_node_time_rows="
              << nodal_history.size() * source.nodes().size() << '\n';
    std::cout << prefix << "maximum_coordinate_difference="
              << maximum_coordinate_difference << '\n';
    std::cout << prefix << "maximum_boundary_value_difference="
              << maximum_boundary_value_difference << '\n';
    std::cout << prefix
              << "maximum_free_residual_l2=" << maximum_free_residual_l2
              << '\n';
    std::cout << prefix << "maximum_free_residual_l2_ratio="
              << maximum_free_residual_l2_ratio << '\n';
    std::cout << prefix << "maximum_free_residual_infinity="
              << maximum_free_residual_infinity << '\n';
    std::cout << prefix << "maximum_free_residual_infinity_ratio="
              << maximum_free_residual_infinity_ratio << '\n';
    std::cout << prefix << "maximum_free_residual_ratio_time="
              << maximum_free_residual_ratio_time << '\n';

    passed =
        check(nodal_history.size() == 100 && reference_row == reference.size(),
              "M4.3 creep shared-state audit covers all nodes, elements, "
              "and time steps") &&
        passed;
    passed = check(maximum_coordinate_difference < 1.0e-12 &&
                       maximum_boundary_value_difference < 1.0e-12,
                   "M4.3 creep shared-state coordinates and prescribed values "
                   "match") &&
             passed;
    passed = check(maximum_free_residual_l2_ratio < 2.0e-5 &&
                       maximum_free_residual_infinity_ratio < 2.0e-5,
...
bool run_test(const VariantConfig& variant, const std::string& input_path,
    ...
    bool passed =
        check(result.completed &&
                  result.accepted_steps.size() == variant.step_count &&
    ...
    passed = check_metrics("m43_temperature", nodal.temperature, 1.0e-12,
                           ...
                           variant.comparison_tolerance) &&
             passed;
    passed = check_metrics("m43_radial_displacement", nodal.radial_displacement,
                           1.0e-14, variant.comparison_tolerance,
                           variant.comparison_tolerance) &&
             passed;
    passed = check_metrics("m43_axial_displacement", nodal.axial_displacement,
                           1.0e-14, variant.comparison_tolerance,
                           variant.comparison_tolerance) &&
             passed;
    ...
    for (std::size_t step = 0; step < actual.size(); ++step) {
        ...
        for (const ElementSnapshot& element : actual[step].elements) {
            ...
            maximum_coordinate_difference =
                std::max({maximum_coordinate_difference,
            ...
            equivalent_plastic.add(element.state.equivalent_plastic_strain,
    ...
    print_tensor_metric_locations(prefix + "elastic_strain", elastic,
                                  reference);
    print_tensor_metric_locations(prefix + "combined_inelastic_strain",
    ...
    if (variant.behavior == ExpectedBehavior::elastic) {
        fuelsim::test::print_absolute_metrics(
            prefix + "combined_inelastic_strain", combined_inelastic);
        passed =
    ...
    if (plastic_active(variant.behavior))
        passed =
            check_metrics(prefix + "equivalent_plastic", equivalent_plastic,
    ...
    if (variant.behavior == ExpectedBehavior::creep)
        passed = audit_creep_shared_state(definition, source, reference,
                                          nodal_history_path) &&
                 passed;
    return passed;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const VariantConfig variant = variant_config(argv[1]);
        if ((variant.behavior == ExpectedBehavior::creep) != (argc == 6))
            throw std::invalid_argument(
                "M4.3 only the creep displacement variant requires a nodal "
                "history reference");
        return run_test(variant, argv[2], argv[3], argv[4],
                        argc == 6 ? argv[5] : "")
                   ? 0
                   : 1;
