...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete Abaqus B5.9 row in " + path);
...
double canonical_zero(double value, double tolerance) {
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.9 nodes: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 12)
            throw std::invalid_argument("Unexpected Abaqus B5.9 nodal column count in " + path);
        std::array<double, 8> fields{};
        for (std::size_t field = 0; field < fields.size(); ++field) {
            fields[field] = number(values, field + 4, path);
            const double tolerance = field >= 5 ? 1.0e-5 : (field == 4 ? 1.0e-8 : 1.0e-16);
            if (field != 0)
                fields[field] = canonical_zero(fields[field], tolerance);
        }
        result.push_back({values[0],
            positive_integer(number(values, 1, path), path),
            positive_integer(number(values, 3, path), path),
            number(values, 2, path),
            fields});
    }
    if (result.size() != 672)
        throw std::invalid_argument("Abaqus B5.9 nodal reference must contain 672 rows");
    return result;
}
...
std::vector<IntegrationReference> read_integration(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.9 integration points: " + path);
    std::string line;
    std::getline(input, line);
    if (line
        != "case,stage,time_s,element,integration_point,x_m,y_m,z_m,temperature_k,hfl_x_w_m2,hfl_y_w_m2,"
...
BendingSummary run_case(const CaseSpec& spec,
    const std::string& output_path,
    const std::vector<NodeReference>& node_reference,
    ...
    if (history.size() != 5)
        throw std::invalid_argument("B5.9 requires four output increments");
    ...
    const std::array<std::string, 8> fields = {"temperature",
        "displacement_x",
        "displacement_y",
        "displacement_z",
        "reaction_heat_flux",
    ...
    for (std::size_t stage = 1; stage <= 4; ++stage) {
        ...
        for (const NodeReference& reference : node_reference) {
            if (reference.case_name != spec.name || reference.stage != stage)
                continue;
            if (std::abs(reference.time - snapshot.time) > 1.0e-6 || reference.node < 1 || reference.node > node_count
                || !seen_nodes.emplace(stage, reference.node).second)
        ...
        passed = check(thermal_balance_passed,
    ...
    for (const IntegrationReference& reference : integration_reference) {
        if (reference.case_name != spec.name)
            continue;
        ...
        std::array<double, 3> actual_heat_flux = {snapshot.element("heat_flux_x" + suffix).at(element),
            snapshot.element("heat_flux_y" + suffix).at(element),
            snapshot.element("heat_flux_z" + suffix).at(element)};
        ...
        const std::array<double, 6> actual_stress_components = {actual_stress.xx,
            actual_stress.yy,
            actual_stress.zz,
            actual_stress.xy,
            actual_stress.yz,
            actual_stress.xz};
        const std::array<double, 6> reference_stress_components = {reference.stress.xx,
            reference.stress.yy,
            reference.stress.zz,
            reference.stress.xy,
            reference.stress.yz,
            reference.stress.xz};
        const std::array<double, 6> actual_strain_components = {actual_strain.xx,
            actual_strain.yy,
            actual_strain.zz,
            actual_strain.xy,
            actual_strain.yz,
            actual_strain.xz};
        const std::array<double, 6> reference_strain_components = {reference.strain.xx,
            reference.strain.yy,
            reference.strain.zz,
            reference.strain.xy,
            reference.strain.yz,
        ...
        for (std::size_t component = 0; component < 6; ++component) {
            integration_metrics[3 + component].add(actual_stress_components[component],
                reference_stress_components[component]);
            integration_metrics[9 + component].add(actual_strain_components[component],
                reference_strain_components[component]);
        }
        integration_metrics[15].add(material_temperature, reference.temperature);
        integration_metrics[16].add(snapshot.element("integration_measure" + suffix).at(element),
    ...
    passed = check(compared_points == 32 * element_count,
             ...
             && passed;
    const std::array<std::string, 17> integration_names = {"heat_flux_x",
        "heat_flux_y",
        "heat_flux_z",
        "stress_xx",
        "stress_yy",
        ...
        "integration_volume"};
    const std::array<double, 17> integration_zero_tolerances = {1.0e-8,
        1.0e-8,
        1.0e-8,
        1.0e-3,
        1.0e-3,
    ...
    passed = check(std::abs(result.peak) > 1.0e-4,
...
bool check_hex8_multimaterial(const std::string& coarse,
    const std::string& distorted,
    ...
    for (std::size_t i = 0; i < paths.size(); ++i)
        (void)run_case(regression_cases[i], paths[i], nodes, points, passed);
    return passed;
}
} // namespace fuelsim::test
