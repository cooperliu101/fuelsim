...
constexpr std::array<PathStep, 4> path = {{{"CLOSE", {-0.01, 0.05, 0.04}}, {"CROSS", {-0.01, 1.0, 0.4}},
    {"STRADDLE", {-0.01, 0.6, -0.3}}, {"RETURN", {-0.01, 0.0, 0.1}}}};
...
void write_abaqus_input(const std::string& output_path, const fuelsim::UnstructuredHex8Mesh& mesh, bool nlgeom) {
    ...
    if (!output) throw std::runtime_error("Could not write B4.1 Abaqus input: " + output_path);
...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& input_path) {
    if (column >= values.size()) throw std::invalid_argument("Incomplete B4.1 CSV row: " + input_path);
    std::size_t parsed = 0;
...
std::vector<std::vector<ReferenceNode>> read_reference(const std::string& input_path) {
    ...
    if (!input) throw std::runtime_error("Could not read B4.1 Abaqus reference: " + input_path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (step == 0 || step > path.size()) throw std::invalid_argument("Invalid B4.1 CSV step: " + input_path);
...
bool compare_case(const fuelsim::UnstructuredHex8Mesh& mesh, const std::string& reference_path,
...
int main(int argc, char** argv) {
    try {
        ...
        if (small && finite) std::cout << "[PASS] B4.1 HEX8 finite-sliding Abaqus comparison\n";
