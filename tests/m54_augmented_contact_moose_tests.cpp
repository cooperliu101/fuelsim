...
bool run_comparison(const std::string& input_path,
    ...
    bool passed =
        check(result.completed && result.solve.converged,
              "M5.4 augmented-contact load path converges") &&
        check(result.solve.augmented_lagrangian_iterations > 0,
              "M5.4 final load step performs a multiplier update") &&
    ...
    passed =
        ...
        check(fuelsim::test::relative_metrics_below(
                  fields.radial_displacement, field_tolerance),
              "M5.4 radial-displacement three full-field errors pass") &&
        check(fuelsim::test::relative_metrics_below(
                  fields.axial_displacement, field_tolerance),
              "M5.4 axial-displacement three full-field errors pass") &&
        check(fuelsim::test::relative_metrics_below(pressure,
                                                     field_tolerance),
    ...
    passed =
        check(interface.projected_contact_nodes == contact_nodes.size() &&
                  ...
                  maximum_penetration <=
                      definition.contacts.at(0).penetration_tolerance,
              "M5.4 activates every contact node and satisfies the 1 nm "
              "penetration tolerance") &&
        passed;

    fuelsim::test::print_relative_metrics("m54_temperature",
                                          fields.temperature);
    fuelsim::test::print_relative_metrics("m54_radial_displacement",
                                          fields.radial_displacement);
    fuelsim::test::print_relative_metrics("m54_axial_displacement",
    ...
    std::cout << "m54_maximum_penetration=" << maximum_penetration << '\n'
              << "m54_penetration_tolerance="
              << definition.contacts.at(0).penetration_tolerance << '\n'
              << "m54_multiplier_updates="
              << result.solve.augmented_lagrangian_iterations << '\n';
    return passed;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(
            argc, argv, "fuelsim M5.4 augmented-contact MOOSE comparison\n");
