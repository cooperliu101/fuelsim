...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete Abaqus B5.5 row in " + path);
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.5 nodes: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 9)
            throw std::invalid_argument("Unexpected Abaqus B5.5 nodal column count in " + path);
        std::array<double, 8> fields{};
    ...
    if (result.size() != 12)
        throw std::invalid_argument("Abaqus B5.5 nodal reference must contain twelve nodes");
    return result;
}
...
std::vector<IntegrationReference> read_integration(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.5 integration points: " + path);
    std::string line;
    std::getline(input, line);
    if (line
...
bool check_hex8_b55(const std::string& output_path,
