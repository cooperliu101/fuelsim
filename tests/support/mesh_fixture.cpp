...
std::size_t annular_node_id(std::size_t radial_elements,
    ...
    if (axial_index >
        (std::numeric_limits<std::size_t>::max() - radial_index) / radial_nodes)
...
make_disconnected_annular_mesh(const std::vector<AnnularBlockSpec>& blocks) {
    if (blocks.empty())
        throw std::invalid_argument(
            "annular mesh fixture requires at least one block");
    ...
    for (const AnnularBlockSpec& block : blocks) {
        if (block.id < 0 || block.name.empty())
            throw std::invalid_argument(
                "annular mesh fixture requires a named nonnegative block");
        if (!(block.inner_radius >= 0.0) ||
            !(block.outer_radius > block.inner_radius) ||
            !(block.length > 0.0) || block.radial_elements == 0 ||
            block.axial_elements == 0)
            throw std::invalid_argument(
                "annular mesh fixture has invalid geometry or element count");

        const std::size_t radial_nodes = block.radial_elements + 1;
        const std::size_t axial_nodes = block.axial_elements + 1;
        if (radial_nodes >
            std::numeric_limits<std::size_t>::max() / axial_nodes)
        ...
        for (std::size_t axial = 0; axial < axial_nodes; ++axial) {
            const double z = block.length * static_cast<double>(axial) /
                             static_cast<double>(block.axial_elements);
            for (std::size_t radial = 0; radial < radial_nodes; ++radial) {
                const double fraction =
                    static_cast<double>(radial) /
                    static_cast<double>(block.radial_elements);
                nodes.push_back(
                    {block.inner_radius +
                         (block.outer_radius - block.inner_radius) * fraction,
                     z});
            }
        }

        for (std::size_t axial = 0; axial < block.axial_elements; ++axial) {
            for (std::size_t radial = 0; radial < block.radial_elements;
                 ++radial) {
                const std::size_t lower_left =
                    node_offset +
                    annular_node_id(block.radial_elements, radial, axial);
        ...
        for (std::size_t axial = 0; axial < block.axial_elements; ++axial) {
            const std::size_t row =
                element_offset + axial * block.radial_elements;
        ...
        for (std::size_t radial = 0; radial < block.radial_elements; ++radial) {
            ...
            top.push_back(
                {element_offset +
                     (block.axial_elements - 1) * block.radial_elements +
                     radial,
                 2});
        }
        ...
        side_sets.push_back(
            {next_side_set_id++, block.name + "_inner", std::move(inner)});
        side_sets.push_back(
            {next_side_set_id++, block.name + "_outer", std::move(outer)});
        side_sets.push_back(
            {next_side_set_id++, block.name + "_bottom", std::move(bottom)});
        side_sets.push_back(
            {next_side_set_id++, block.name + "_top", std::move(top)});
    }
