...
fuelsim::UnstructuredHex8Mesh two_element_mesh() {
    ...
    const std::vector<fuelsim::ElementSide> all_faces = {{0, 3}, {1, 1}, {0, 0}, {1, 0}, {0, 2},
                                                         {1, 2}, {0, 4}, {1, 4}, {0, 5}, {1, 5}};
    return fuelsim::UnstructuredHex8Mesh(
        std::move(nodes), std::move(elements), {1, 1}, {{1, "solid"}}, {},
...
fuelsim::ThermoelasticProperties material() {
    return {0.0, 10.0, 1.0e9, 0.25, 1.0e-5, 300.0};
}
...
bool test_steady(const fuelsim::PetscSession& session, const fuelsim::UnstructuredHex8Mesh& mesh,
...
bool test_transient(const fuelsim::PetscSession& session, const fuelsim::UnstructuredHex8Mesh& mesh,
    ...
    bool passed = check(result.completed && result.accepted_steps.size() == 1,
                  ...
                  check_uniform_solution(fuelsim::cartesian3d::ProblemAccess::dof_map(problem), mesh,
                                         problem.committed_solution(), 301.0);
    session.collective_root_action([&]() {
    ...
    passed = check(next_time_step == 0.25 && restored.committed_time() == 1.0 &&
             ...
             passed;
    const auto before = fuelsim::cartesian3d::ProblemAccess::committed_state(problem);
    const auto after = fuelsim::cartesian3d::ProblemAccess::committed_state(restored);
    bool stresses_match = before.stresses.size() == after.stresses.size();
    for (std::size_t region = 0; stresses_match && region < before.stresses.size(); ++region) {
        stresses_match = before.stresses[region].size() == after.stresses[region].size();
        for (std::size_t element = 0; stresses_match && element < before.stresses[region].size(); ++element) {
            for (std::size_t q = 0; stresses_match && q < 8; ++q) {
                const fuelsim::SymmetricTensor3Values& first = before.stresses[region][element][q];
                const fuelsim::SymmetricTensor3Values& second = after.stresses[region][element][q];
