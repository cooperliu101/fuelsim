...
std::size_t column(const std::vector<std::string>& header, const std::string& name, const std::string& path) {
    ...
    if (found == header.end())
        throw std::invalid_argument("MOOSE CSV is missing '" + name + "': " + path);
...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("MOOSE CSV row is incomplete: " + path);
    const double value = std::stod(values[index]);
    if (!std::isfinite(value))
        throw std::invalid_argument("MOOSE CSV contains a non-finite value: " + path);
    return value;
}
...
std::vector<NodeReference> read_nodes(const std::string& path, std::size_t node_count) {
    ...
    const std::array<std::size_t, 8> columns = {column(header, "id", path),
        column(header, "x", path),
    ...
    while (std::getline(input, line)) {
        ...
        if (id >= node_count)
            throw std::invalid_argument("MOOSE node ID is outside the Exodus mesh: " + path);
        ...
        if (present[id]) {
            ...
            if (std::abs(existing.point[0] - candidate.point[0]) > 1.0e-12
                ...
                || !std::equal(existing.fields.begin(),
                    existing.fields.end(),
...
std::vector<ElementReference> read_elements(const std::string& path, std::size_t element_count) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read MOOSE element reference: " + path);
    std::string line;
    if (!std::getline(input, line))
        throw std::invalid_argument("MOOSE element reference is empty: " + path);
    const auto header = split_csv(line);
    const std::array<std::size_t, 4> columns = {column(header, "id", path),
        column(header, "stress_xx", path),
    ...
    while (std::getline(input, line)) {
        ...
        const auto values = split_csv(line);
        const std::size_t id = static_cast<std::size_t>(number(values, columns[0], path));
        if (id >= element_count || present[id])
            throw std::invalid_argument("Invalid MOOSE element ID: " + path);
...
bool check_hex8_shared_plate(const std::string& output_path,
    ...
    if (!transient) {
        ...
        for (std::size_t plane = 0; plane < 3; ++plane) {
            if (count[plane] == 0)
                throw std::runtime_error("B3.5 temperature plane is empty");
