...
bool run_comparison(const std::string& input_path, const std::string& nodal_reference_path,
    ...
    const fuelsim::SteadyResult result = fuelsim::solve_steady(problem,
        ...
        options);
    passed = check(result.completed && result.solve.converged, "non-tensor MOOSE mesh solve converged") && passed;
    passed = check(result.aggregate_timing.workspace_setups == 1, "non-tensor load path reuses one PETSc workspace") &&
             passed;
    const std::vector<fuelsim::test::NodalFieldReference> reference =
        fuelsim::test::read_moose_nodal_reference(nodal_reference_path);
    passed = check(reference.size() == source.nodes().size(), "MOOSE reference covers every Exodus node") && passed;
    ...
    const fuelsim::test::FieldErrorMetrics pressure = fuelsim::test::compare_moose_contact_pressure(
    ...
    passed = check(fuelsim::test::relative_metrics_below(fields.temperature, tolerance) &&
                       fuelsim::test::relative_metrics_below(fields.radial_displacement, tolerance) &&
                       fuelsim::test::relative_metrics_below(fields.axial_displacement, tolerance) &&
                       fuelsim::test::relative_metrics_below(pressure, tolerance),
                 "all MOOSE full-field three-metric errors pass 1%") &&
             passed;
    fuelsim::test::print_relative_metrics("unstructured_temperature", fields.temperature);
    fuelsim::test::print_relative_metrics("unstructured_radial_displacement", fields.radial_displacement);
    fuelsim::test::print_relative_metrics("unstructured_axial_displacement", fields.axial_displacement);
    fuelsim::test::print_relative_metrics("unstructured_contact_pressure", pressure);
    return passed;
}
} // namespace
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(argc, argv, "fuelsim non-tensor Quad4 MOOSE comparison\n");
