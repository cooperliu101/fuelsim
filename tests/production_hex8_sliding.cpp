...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("B3.4 CSV row is too short: " + path);
    std::size_t parsed = 0;
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B3.4 Abaqus nodal reference: " + path);
    std::string line;
    ...
    if (result.size() != 16)
        throw std::invalid_argument("B3.4 Abaqus reference must contain sixteen nodes");
    return result;
}
...
std::vector<ContactReference> read_contact(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B3.4 Abaqus contact reference: " + path);
    std::string line;
    ...
    if (result.size() != 4)
        throw std::invalid_argument("B3.4 Abaqus reference must contain four contact nodes");
    return result;
}
...
ReactionReference read_reaction(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B3.4 Abaqus reaction reference: " + path);
    std::string line;
...
bool check_hex8_sliding(const std::string& output_path,
    ...
    if (output.nodes.size() != nodes.size())
        throw std::runtime_error("B3.4 output node count differs");
    ...
    for (const auto& row : nodes) {
        if (row.id >= present.size() || present[row.id])
            throw std::runtime_error("B3.4 invalid node ID");
        present[row.id] = true;
        for (std::size_t component = 0; component < 3; ++component) {
            maximum_coordinate_difference = std::max(maximum_coordinate_difference,
                std::abs(output.nodes[row.id][component] - row.point[component]));
    ...
    for (std::size_t node = 0; node < projected.size(); ++node) {
        ...
        if (projected[node] != 1.0)
            throw std::runtime_error("B3.4 lost a contact projection");
    ...
    if (source_nodes.size() != reference.size())
        throw std::runtime_error("B3.4 contact node counts differ");
    ...
    for (std::size_t node = 0; node < source_nodes.size(); ++node) {
        ...
        if (found == reference.end())
            throw std::invalid_argument("B3.4 Abaqus contact-node mapping is incomplete");
        maximum_coordinate_difference = std::max(maximum_coordinate_difference,
            std::max({std::abs(output.nodes[source_nodes[node]][0] - found->point[0]),
        ...
        tangential_force_y.add(-output.nodal("contact_tangential_force_y_interface")[source_nodes[node]],
            found->tangential_force[1]);
        ...
        maximum_theoretical_zero_force = std::max(maximum_theoretical_zero_force,
            std::max({std::abs(output.nodal("contact_normal_force_y_interface")[source_nodes[node]]),
                ...
                std::abs(found->tangential_force[2])}));
        if (output.nodal("contact_pressure_interface")[source_nodes[node]] > 0.0)
            ++active;
        if (output.nodal("contact_sliding_interface")[source_nodes[node]])
            ++sliding;
    }
