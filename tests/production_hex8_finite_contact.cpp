...
double number(const std::vector<std::string>& values, std::size_t column, const std::string& path) {
    if (column >= values.size())
        throw std::invalid_argument("Incomplete B4.3 CSV row: " + path);
    std::size_t parsed = 0;
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    ...
    if (!input)
        throw std::runtime_error("Could not read B4.3 node reference: " + path);
    std::string line;
...
std::vector<ContactReference> read_contact(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read B4.3 contact reference: " + path);
    std::string line;
...
bool check_hex8_finite_contact(const std::string& output_path,
    ...
    bool passed = check(seen_nodes.size() == 320 && seen_contacts.size() == 80,
        "B4.3 covers all nodes and constraints at every step");
