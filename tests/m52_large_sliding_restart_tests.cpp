...
fuelsim::TransientTimeOptions time_options(double end_time) {
    return {end_time, 0.05, 0.05, 0.05, 1.0, 0.5, 2, 1.0};
}
...
bool run_test(const std::string& input_path,
    ...
    const fuelsim::TransientResult full = fuelsim::solve_transient(
        uninterrupted, time_options(1.0), solver);
    bool passed = check(full.completed,
                        "M5.2 uninterrupted dynamic-search path completes");
    ...
    const fuelsim::TransientResult first = fuelsim::solve_transient(
        split, time_options(0.5), solver);
    passed = check(first.completed,
                   "M5.2 pre-checkpoint dynamic-search path completes") &&
             passed;
    fuelsim::TransientCheckpointIo::write(checkpoint_path, split, 0.05);

    fuelsim::TransientProblem restarted(input.transient_definition(), mesh);
    const double restored_step = fuelsim::TransientCheckpointIo::restore(
        checkpoint_path, restarted);
    const fuelsim::TransientResult second = fuelsim::solve_transient(
        restarted, time_options(1.0), solver);
    passed = check(restored_step == 0.05 && second.completed,
                   "M5.2 checkpoint restores its controller step and "
                   "continues") &&
             passed;
    ...
    for (std::size_t dof = 0; dof < expected.size(); ++dof) {
        ...
        maximum_scaled =
            std::max(maximum_scaled,
                     difference / (1.0 + std::abs(expected[dof])));
    }
    const std::vector<fuelsim::ContactNodeSummary> expected_contact =
        uninterrupted.summarize_contact_nodes(0, expected);
    const std::vector<fuelsim::ContactNodeSummary> actual_contact =
        restarted.summarize_contact_nodes(0, actual);
    bool contact_equal = expected_contact.size() == actual_contact.size();
    for (std::size_t node = 0;
         contact_equal && node < expected_contact.size(); ++node) {
        contact_equal =
            expected_contact[node].projected == actual_contact[node].projected &&
            expected_contact[node].primary_segment ==
                actual_contact[node].primary_segment &&
            expected_contact[node].pressure == actual_contact[node].pressure;
    }
    passed = check(maximum_scaled < 1.0e-13 && contact_equal,
                   "M5.2 restarted nodal state, active primary segments, and "
    ...
    return check(std::remove(checkpoint_path.c_str()) == 0,
                 "M5.2 checkpoint artifact is removed") &&
           passed;
}
...
int main(int argc, char** argv) {
    ...
    try {
        fuelsim::PetscSession session(
            argc, argv, "fuelsim M5.2 large-sliding restart test\n");
