...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete Abaqus B5.31/B5.34 row in " + path);
...
std::vector<NodeReference> read_nodes(const std::string& path, bool finite_strain) {
    ...
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.31 nodes: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 9)
            throw std::invalid_argument("Unexpected Abaqus B5.31 nodal column count in " + path);
        std::array<double, 8> fields{};
        ...
        if (!z_constrained)
            fields[7] = 0.0;
        if (!finite_strain && (node == 4 || node == 7))
            fields[5] = 0.0;
        result.push_back({node, fields});
    }
    if (result.size() != 12)
        throw std::invalid_argument("Abaqus B5.31 nodal reference must contain twelve nodes");
    return result;
}
...
std::vector<IntegrationReference> read_integration(const std::string& path, bool finite_strain) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read Abaqus B5.31 integration points: " + path);
    std::string line;
    std::getline(input, line);
    if (line
...
bool check_hex8_b531(const std::string& output_path,
    ...
    if (output.time != 1.0 || output.step_count != 2 || output.nodes.size() != 12)
        throw std::invalid_argument(
            "B5.31/B5.34 requires twelve nodes, the initial frame, and one one-second increment");
    bool passed = true;
    ...
    for (const auto& reference : read_nodes(nodal_path, finite_strain)) {
        if (reference.node < 1 || reference.node > 12 || !seen.insert(reference.node).second)
            throw std::invalid_argument("B5.31/B5.34 reference node mapping is invalid");
        for (std::size_t field = 0; field < 8; ++field) {
            // RF/RFL reference values apply only on constrained equations.
            if (field >= 4 && output.nodal("dirichlet_" + nodal_names[field - 4]).at(reference.node - 1) != 1.0)
                continue;
    ...
    for (std::size_t field = 0; field < 8; ++field) {
        print_relative_metrics("b531_" + nodal_names[field], nodal[field]);
        passed = check(relative_metrics_below(nodal[field], 1e-3)
                     ...
                     "B5.31/B5.34 nodal field " + nodal_names[field] + " passes all three 0.1 percent metrics")
                 && passed;
    }
    ...
    for (const auto& reference : read_integration(integration_path, finite_strain)) {
        if (reference.element < 1 || reference.element > 2)
            throw std::invalid_argument("B5.31/B5.34 reference element is invalid");
        ...
        for (std::size_t q = 0; q < 1; ++q) {
            ...
            const double difference =
                std::hypot(output.element(std::string(finite_strain ? "current_x" : "reference_x") + suffix).at(element)
                               - reference.position.x,
                    output.element(std::string(finite_strain ? "current_y" : "reference_y") + suffix).at(element)
                        - reference.position.y,
                    output.element(std::string(finite_strain ? "current_z" : "reference_z") + suffix).at(element)
        ...
        if (!std::isfinite(distance) || !matched.emplace(element, closest).second)
            throw std::invalid_argument("B5.31/B5.34 integration point association is not unique");
    ...
    for (std::size_t field = 0; field < 15; ++field) {
        if (finite_strain && field >= 9)
            continue; // The original B5.34 contract does not qualify strain.
        if (integration[field].has_relative_norm())
            print_relative_metrics("b531_" + names[field], integration[field]);
        else
            print_absolute_metrics("b531_" + names[field], integration[field]);
        const bool relative = integration[field].has_relative_norm()
                                  ? relative_metrics_below(integration[field], 1e-3)
                                  : integration[field].zero_reference_count == integration[field].value_count;
        passed = check(relative
                           && integration[field].maximum_zero_reference_difference
                                  < (field < 3 ? 1e-8 : (field < 9 ? 1e-2 : 1e-14)),
                     "C3D8RT integration field " + names[field] + " retains its original acceptance gate")
                 && passed;
    }
    return check(matched.size() == 2 && coordinate_error < 1e-7,
               "B5.31/B5.34 matches both reduced material integration points uniquely")
           && passed;
}
} // namespace fuelsim::test
