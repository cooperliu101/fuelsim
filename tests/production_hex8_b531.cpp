...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size()) throw std::invalid_argument("Incomplete Abaqus B5.31/B5.34 row in " + path);
...
bool check_hex8_b531(const std::string& output_path, const std::string& nodal_path, const std::string& integration_path,
    ...
    for (const auto& reference : read_integration(integration_path, finite_strain)) {
        ...
        for (std::size_t q = 0; q < 1; ++q) {
            ...
            const double difference = std::hypot(
                output.element(std::string(finite_strain ? "current_x" : "reference_x") + suffix).at(element) -
    ...
    for (std::size_t field = 0; field < 15; ++field) {
        if (finite_strain && field >= 9) continue; // The original B5.34 contract does not qualify strain.
        ...
        passed = check(relative && integration[field].maximum_zero_reference_difference <
