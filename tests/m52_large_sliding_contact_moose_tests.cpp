...
bool run_comparison(const std::string& input_path, const std::string& nodal_reference_path,
    ...
    bool passed =
        ...
        check(result.aggregate_timing.workspace_setups == 1, "M5.2 keeps one PETSc workspace and locked sparsity");
    ...
    passed = check(fields.node_count == 402 && fields.maximum_coordinate_difference < 1.0e-12,
    ...
    for (std::size_t node = 0; node < contact_nodes.size(); ++node) {
        ...
        if (contact.pressure > 0.0) ++active;
        const std::size_t initial_segment = initial_contact_nodes.at(node).primary_segment;
        if (contact.primary_segment >= initial_segment)
            maximum_segment_change = std::max(maximum_segment_change, contact.primary_segment - initial_segment);
        const std::size_t source_node = secondary_sources[node];
        std::size_t global_node = fuelsim::rz::ProblemAccess::dof_map(problem).node_count();
        for (std::size_t region = 0; region < fuelsim::rz::ProblemAccess::region_count(problem); ++region) {
            ...
            global_node = fuelsim::rz::ProblemAccess::region_node_offset(problem, region) +
    ...
    passed = check(active == contact_nodes.size() && maximum_segment_change >= 2 && maximum_current_radius > 4.2e-3 &&
                       maximum_current_radius < 8.0e-3 && interface.total_contact_force > 0.0,
                 "M5.2 keeps all five nodes active while crossing at least "
                 "two primary segments inside the full chain") &&
             passed;
    fuelsim::test::print_relative_metrics("m52_temperature", fields.temperature);
    fuelsim::test::print_relative_metrics("m52_radial_displacement", fields.radial_displacement);
    fuelsim::test::print_relative_metrics("m52_axial_displacement", fields.axial_displacement);
    fuelsim::test::print_relative_metrics("m52_contact_pressure", pressure);
    std::cout << "m52_active_contact_nodes=" << active << '\n'
              << "m52_maximum_primary_segment_change=" << maximum_segment_change << '\n'
              << "m52_maximum_secondary_current_radius=" << maximum_current_radius << '\n'
              << "m52_total_contact_force=" << interface.total_contact_force << '\n';
    return passed;
}
} // namespace
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fuelsim::PetscSession session(argc, argv, "fuelsim M5.2 large-sliding MOOSE comparison\n");
