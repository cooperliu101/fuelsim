...
int main(int argc, char** argv) {
    if (argc != 7) {
        std::cerr << "Usage: fuelsim_h20_24_hex20_nonmatching_abaqus_tests <case.fsi> <displacement.csv> "
                     "<reaction.csv> <pressure.csv> <mortar_nodes.csv> <mortar_reaction.csv>\n";
        return 2;
    }
    try {
        ...
        const auto mortar = read_mortar_nodes(argv[5]);
        ...
        for (std::size_t region = 0; region < spatial.region_count(); ++region) {
            ...
            for (std::size_t local = 0; local < region_mesh.nodes().size(); ++local) {
                ...
                maximum_mortar_coordinate_difference = std::max(maximum_mortar_coordinate_difference,
                    coordinate_difference(mesh.nodes()[source], found_mortar->point));
                const std::size_t global = spatial.global_node(region, local);
                const double actual = solve.solve.state[fields[1].begin + global];
        ...
        const double mortar_force = read_mortar_reaction(argv[6]);
        const double force_error = std::abs(interface.total_contact_force - reference_force) / reference_force;
        const double mortar_force_error = std::abs(interface.total_contact_force - mortar_force) / mortar_force;
        ...
        passed =
            ...
            check(mortar.size() == mesh.nodes().size() && displacement_x_mortar.value_count == mesh.nodes().size(),
        ...
        if (passed && session.rank() == 0)
            std::cout << "[PASS] H20.24 nonmatching HEX20 Abaqus and MOOSE mortar comparison\n";
        return passed ? 0 : 1;
    } catch (const std::exception& error) {
        std::cerr << "[FAIL] H20.24 Abaqus and MOOSE mortar comparison raised: " << error.what() << '\n';
        return 1;
    }
}
