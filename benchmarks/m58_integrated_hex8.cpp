...
void write_dof_reference(const std::string& path, const std::vector<double>& state) {
    ...
    if (!output)
        throw std::runtime_error("Could not write M5.8 degree-of-freedom reference: " + path);
    output << state.size() << '\n' << std::setprecision(17);
    for (double value : state)
        output << value << '\n';
    if (!output)
        throw std::runtime_error("Could not complete M5.8 degree-of-freedom reference: " + path);
}

bool compare_dof_reference(const std::string& path, const std::vector<double>& state) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read M5.8 degree-of-freedom reference: " + path);
    std::size_t count = 0;
    input >> count;
    ...
    if (state.size() % 4U != 0U)
        throw std::runtime_error("M5.8 state does not contain four complete fields");
...
std::size_t column(const std::vector<std::string>& header, const std::string& name, const std::string& path) {
    ...
    if (found == header.end())
        throw std::invalid_argument("Missing column '" + name + "' in " + path);
...
double number(const std::vector<std::string>& values, std::size_t index, const std::string& path) {
    if (index >= values.size())
        throw std::invalid_argument("Incomplete M5.8 MOOSE row in " + path);
...
std::vector<NodeReference> read_nodes(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read M5.8 MOOSE nodes: " + path);
    std::string line;
    std::getline(input, line);
    const auto header = split_csv(line);
    const std::array<std::size_t, 7> columns = {column(header, "x", path),
        column(header, "y", path),
        column(header, "z", path),
        column(header, "T", path),
        column(header, "disp_x", path),
        column(header, "disp_y", path),
        column(header, "disp_z", path)};
        ...
        result.push_back(
            {{number(values, columns[0], path), number(values, columns[1], path), number(values, columns[2], path)},
                {number(values, columns[3], path),
                    number(values, columns[4], path),
                    number(values, columns[5], path),
...
std::vector<ContactReference> read_contact(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read M5.8 MOOSE contact pressure: " + path);
    std::string line;
    std::getline(input, line);
    const auto header = split_csv(line);
    const std::array<std::size_t, 4> columns = {column(header, "x", path),
        column(header, "y", path),
        column(header, "z", path),
        column(header, "contact_pressure", path)};
...
std::vector<ElementReference> read_elements(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Could not read M5.8 MOOSE element states: " + path);
    std::string line;
    std::getline(input, line);
    const auto header = split_csv(line);
    const std::array<std::size_t, 6> columns = {column(header, "x", path),
        column(header, "y", path),
        column(header, "z", path),
        column(header, "vonmises_stress", path),
        column(header, "effective_plastic_strain", path),
        column(header, "effective_creep_strain", path)};
    std::vector<ElementReference> result;
    while (std::getline(input, line)) {
        const auto values = split_csv(line);
        result.push_back({{number(values, columns[0], path),
                              number(values, columns[1], path),
...
bool below_with_pointwise_tolerance(const std::string& name,
    ...
    if (output)
        fuelsim::test::print_relative_metrics(name, metrics);
    return check(fuelsim::test::relative_metrics_below_with_pointwise_tolerance(metrics,
                     aggregate_tolerance,
                     pointwise_tolerance),
        name
            + " relative L2 and relative absolute-peak errors satisfy the aggregate tolerance, and the maximum "
...
bool run(const std::string& input_path,
    ...
    if (node_reference.size() != source.nodes().size())
        throw std::invalid_argument("M5.8 node counts differ");
    ...
    for (std::size_t region = 0; region < fuelsim::cartesian::ProblemAccess::region_count(problem); ++region) {
        ...
        for (std::size_t local = 0; local < mesh.nodes().size(); ++local) {
            ...
            const auto reference = std::find_if(node_reference.begin(),
                node_reference.end(),
            ...
            maximum_coordinate_difference = std::max(maximum_coordinate_difference,
                std::max({std::abs(point.x - expected.point.x),
                    std::abs(point.y - expected.point.y),
                    std::abs(point.z - expected.point.z)}));
            std::array<double, 4> actual{};
            for (std::size_t field = 0; field < nodal.size(); ++field) {
                actual[field] =
                    problem.committed_solution()[dofs.dof(std::array<fuelsim::Field, 4>{fuelsim::Field::temperature,
                                                              fuelsim::Field::displacement_x,
                                                              fuelsim::Field::displacement_y,
                                                              fuelsim::Field::displacement_z}[field],
                        offset + local)];
    ...
    const std::array<std::string, 4> nodal_names = {"temperature",
        "displacement_x",
        "displacement_y",
        "displacement_z"};
    ...
    for (std::size_t field = 0; field < nodal.size(); ++field) {
        if (abaqus_reference && (field == 1 || field == 2)) {
            if (output)
                fuelsim::test::print_relative_metrics("m58_" + nodal_names[field], nodal[field]);
        } else {
            passed = below("m58_" + nodal_names[field], nodal[field], output) && passed;
        }
    }
    if (abaqus_reference) {
        passed = below_with_pointwise_tolerance("m58_radial_displacement", radial_displacement, 5.0e-3, 4.0e-2, output)
                 && passed;
        if (output) {
    ...
    for (std::size_t node = 0; node < contact.size(); ++node) {
        ...
        if (found == contact_reference.end())
            throw std::invalid_argument("M5.8 contact-node coordinate is missing");
        pressure.add(contact[node].pressure, found->pressure);
        if (contact[node].sliding)
            ++sliding;
        if (contact[node].primary_face != initial_contact[node].primary_face)
            ++crossed_faces;
    }
    ...
    for (std::size_t element = 0; element < cladding_mesh.elements().size(); ++element) {
        ...
        const auto found = std::find_if(element_reference.begin(),
            element_reference.end(),
            [&](const ElementReference& value) { return same_point(value.point, centroid); });
        if (found == element_reference.end())
            throw std::invalid_argument("M5.8 cladding element is missing");
        maximum_element_coordinate_difference = std::max(maximum_element_coordinate_difference,
            std::max({std::abs(centroid.x - found->point.x),
                std::abs(centroid.y - found->point.y),
    ...
    const std::array<std::string, 3> material_names = {"equivalent_stress",
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (!run(argv[1], argv[2], argv[3], argv[4], reference_mode, reference_path, session.rank() == 0))
            return 1;
        if (session.rank() == 0)
