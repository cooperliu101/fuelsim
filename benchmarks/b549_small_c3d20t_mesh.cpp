...
fuelsim::UnstructuredHex20Mesh make_mesh() {
    ...
    const auto append_block =
        [&](double x_lower, double x_upper, double transverse_lower, double transverse_upper, std::int64_t block) {
            ...
            for (std::size_t ix = 0; ix < normal_elements; ++ix) {
                ...
                if (block == 1 && ix == 0)
                    faces[0].push_back({element, 3});
                if (block == 1 && ix + 1 == normal_elements)
                    faces[1].push_back({element, 1});
                if (block == 2 && ix == 0)
                    faces[2].push_back({element, 3});
                if (block == 2 && ix + 1 == normal_elements)
                    faces[3].push_back({element, 1});
            }
        };
    append_block(0.0, 1.0, -0.1, 1.1, 1);
    append_block(1.0, 2.0, 0.0, 1.0, 2);

    static constexpr std::array<Edge, 12> edge_corners = {{{{0, 1}},
        {{1, 2}},
        {{2, 3}},
        {{3, 0}},
        {{0, 4}},
    ...
    return fuelsim::UnstructuredHex20Mesh(std::move(nodes),
        ...
        {},
        {{21, "primary_outer", std::move(faces[0])},
            {22, "primary_contact", std::move(faces[1])},
            {23, "secondary_contact", std::move(faces[2])},
            {24, "secondary_outer", std::move(faces[3])}});
}
} // namespace
