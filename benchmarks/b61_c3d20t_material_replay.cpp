...
std::vector<SourceDofs> source_dofs(
    const fuelsim::UnstructuredHex20Mesh& mesh, const fuelsim::cartesian::SpatialAssembly& spatial) {
...
std::vector<double> read_state(const std::string& path, const fuelsim::UnstructuredHex20Mesh& mesh,
    const fuelsim::cartesian::SpatialAssembly& spatial, const std::vector<SourceDofs>& mapping,
    const std::vector<double>& committed) {
    std::ifstream input(path);
    if (!input) throw std::runtime_error("Could not read B6.1 replay nodal file: " + path);
    std::string line;
    ...
    while (std::getline(input, line)) {
        ...
        if (values.size() != 8) throw std::runtime_error("Unexpected B6.1 replay nodal row: " + path);
        ...
        if (visited[source]) throw std::runtime_error("Duplicate B6.1 replay node label: " + path);
        visited[source] = true;
        ...
        if (!dofs.present) throw std::runtime_error("B6.1 replay node is not active in the problem: " + path);
    ...
    for (std::size_t source = 0; source < visited.size(); ++source)
        if (!visited[source]) throw std::runtime_error("B6.1 replay nodal file omits a source node: " + path);
    return result;
}
    ...
    for (std::size_t region = 0; region < spatial.region_count(); ++region) {
        ...
        for (std::size_t element = 0; element < mesh.elements().size(); ++element) {
            ...
            for (std::size_t point = 0; point < history.size(); ++point) {
                ...
                const std::array<const std::array<double, 6>*, 3> histories = {
                    &state.elastic_strain, &state.plastic_strain, &state.creep_strain};
...
void write_residual_step(std::ofstream& output, std::size_t step, const fuelsim::TransientProblem& problem,
    const fuelsim::cartesian::SpatialAssembly& spatial, const std::vector<SourceDofs>& mapping,
    ...
    for (std::size_t source = 0; source < mapping.size(); ++source) {
        ...
        for (std::size_t dof : global) output << ',' << residual[dof];
        for (std::size_t dof : global) output << ',' << (constrained[dof] ? 1 : 0);
        if (dofs.temperature_active) {
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (!output) throw std::runtime_error("Could not write B6.1 replay material output");
        output << std::scientific << std::setprecision(17);
        write_header(output);
        std::ofstream residual_output;
        if (argc == 9) {
            residual_output.open(argv[8]);
            if (!residual_output) throw std::runtime_error("Could not write B6.1 replay residual output");
        ...
        for (std::size_t step = 1; step <= 5; ++step) {
            ...
            if (residual_output) write_residual_step(residual_output, step, problem, spatial, mapping, state);
